...
通过 **channel**，你可以从外部聊天应用——Telegram、Lark / 飞书、钉钉、微信——操作 OpenHub 的智能体，而不必坐在桌面客户端前面。你启用一个插件，粘贴它的凭证，用一次性验证码授权一个聊天用户，从此发到你机器人的消息就会被分发到智能体，智能体的回复也会回到同一个会话。
...
> 每个平台插件都是 `openhub-channel` 上的一个 Cargo feature（`telegram`、`lark`、`dingtalk`、`weixin`）。OpenHub 的默认构建把它们全部打开；如果你用非默认 feature 集合自行构建后端，对应的 tab 就直接消失。

![Channels 设置总览](../images/channels-01-overview.png)

## 在哪里找

打开 OpenHub 页面（`/openhub`），选择一只伙伴，然后进入 **Remote** tab（`/openhub?companion=<id>&tab=remote`）。这个 tab 会列出该伙伴可用的远程连接器——内置的（Telegram、Lark、DingTalk、WeChat、WeCom、Slack、Discord、扩展）。每个插件你能看到：
...
2. 在 **OpenHub → Remote → Telegram** 粘入 token。
3. 点 **Test**——后端会调 `getMe`，成功后显示 bot 用户名。
4. 点 **Enable**。插件开始长轮询（25 s 超时，指数退避，最多 10 次重连）。

为了把 Telegram 用户与桌面端配对：用户给你的 bot 发消息；bot 用一个 6 位验证码（10 分钟 TTL）回复。在桌面端的 **OpenHub → Remote → Pending pairings** 中粘入或键入该验证码并点 **Approve**。从此该 Telegram 用户即可与智能体对话。
...
2. 你可以在 **OpenHub → Remote → Pending pairings** 中批准 / 拒绝待处理请求，或以编程方式调用 `POST /api/channel/pairings/approve` 与 `POST /api/channel/pairings/reject`。
...
OpenHub 伙伴本尊接待。会话继承伙伴的人格与记忆，并且 agent 接上了
**Desktop Gateway** 工具——所以你在手机上对话的不是一个孤立的聊天
bot，而是那个掌管你整个桌面的 agent。

网关工具（统一前缀 `openhub_*`，目前共 32 个）能替你做的事：

- **会话**——列出所有会话及其运行态，查看单个会话（状态 + 最近消息，
  含进行中的流式回复），向任意会话注入消息或任务 prompt，新建会话，
  修改与删除旧会话（`openhub_list_conversations`、`openhub_conversation_status`、
...
- **定时任务**——列出 / 创建 / 修改 / 删除 cron 任务
  ...
  `openhub_cron_delete`）。
- **长期记忆**——读写伙伴的全局记忆库（`openhub_memory_list`、
  `openhub_memory_save`、`openhub_memory_update`、`openhub_memory_delete`）。
- **需求平台**——浏览与管理需求平台（`openhub_requirement_list`、
  `openhub_requirement_create`、`openhub_requirement_update`、
  `openhub_requirement_delete`）。
- **终端与监督**——列出终端会话、创建新终端（可经 `knowledge_base_ids`
  顺带绑定知识库），以及读取 / 切换某个终端的 AutoWork 绑定与 IDMM
  监督（`openhub_list_terminals`、`openhub_create_terminal`、
  `openhub_get_autowork`、`openhub_set_autowork`、`openhub_get_idmm`、
  `openhub_set_idmm`）。
- **知识库**——浏览知识库与绑定关系，改绑会话 / 终端 / 伙伴，新建
  ...
  `openhub_knowledge_fetch_url`）。`openhub_knowledge_create_base` 带
  `urls` 时抓取为后台异步——工具立即返回，等待期间勿重复建库；
  库描述（description）出现即代表抓取与梗概流水线已完成。
- **Provider**——列出已配置的 LLM provider（`openhub_list_providers`）。
...
OpenHub 引擎上。主 Agent 模式下的模型解析顺序：平台 **默认模型**（若已
