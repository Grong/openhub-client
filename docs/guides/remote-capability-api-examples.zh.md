...
> 配套 `remote-capability-api.zh.md`。所有示例用同一枚**伙伴访问令牌**（绑定到某个具体伙伴，调用即以该伙伴身份运行）；端点 = WebUI/LAN 端口（默认 `25808`）或 `openhub-web` 的服务端口。下文用 `$HOST`/`$TOKEN` 占位。
...
- **令牌（运维侧一次性发放，绑定到一个伙伴）**：
  ...
  - 无头服务器：启动时 `OPENHUB_COMPANION_TOKEN=$(openssl rand -hex 32) openhub-web --host 127.0.0.1 --port 8787`，绑定到默认伙伴，把这串 hex 当令牌。
...
- **以伙伴身份运行**：调用继承所绑定伙伴的模型/人格/知识库；`openhub_agent_run` 不带 `model` 时用该伙伴的 profile 模型，**所以该伙伴要先配置好可用模型**（否则铸造响应里会带 `warning`）。
- **能力发现**：`GET /v1/tools`（或 `/v1/tools?profile=agent` 精瘦集）列出所有工具名 + 描述 + JSON Schema；下文工具名以此为准（`openhub_agent_run` 一定有）。

---

## 1. MCP 客户端（Claude Code / Cursor / 任意 MCP Agent）—— 旗舰

最省事：把 OpenHub 作为一个 Streamable-HTTP MCP server 配进去。Claude Code / Cursor 的 `mcpServers`：
...
- 连上后 `tools/list` 即见 `openhub_*` 工具，`tools/call` 驱动。委派整件事就调 `openhub_agent_run`。
...
async def main():
    ...
    async with streamablehttp_client("http://%s:25808/mcp" % HOST, headers=headers) as (r, w, _):
        async with ClientSession(r, w) as s:
            ...
            res = await s.call_tool("openhub_agent_run", {"goal": "调研 X 并写 notes.md"})
            print(res)
```
...
curl -s -X POST "http://$HOST:25808/v1/tools/openhub_agent_result" \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
...
```bash
curl -N -X POST "http://$HOST:25808/v1/tools/openhub_agent_run/stream" \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"goal":"..."}'
...
# 调用
r = requests.post(f"{BASE}/v1/tools/openhub_agent_run", headers=H, json={"goal": "..."})
print(r.json())   # {"result": {...}} / {"error":...} / {"needs_confirmation":...}

# 流式
with requests.post(f"{BASE}/v1/tools/openhub_agent_run/stream", headers=H,
...
```bash
export OPENHUB_URL=http://$HOST:25808
export OPENHUB_COMPANION_TOKEN=$TOKEN

nomicore tools                              # 离线列出 Remote 能力（无需运行实例）
nomicore call openhub_agent_run '{"goal":"..."}'
nomicore agent "调研竞品定价并总结"           # openhub_agent_run 的便捷包装
```
...
`GET /v1/tools` 每个工具自带 `name` + `description` + `input_schema`（标准 JSON Schema）。把它们直接注册成你框架的工具列表；模型决定调用某工具时，转一发 `POST /v1/tools/{name}`（带 `confirm` 处理 409）。等于让 OpenHub 全平台能力即插即用地成为你 agent 的工具集。
...
- **MCP vs REST 选择**：agent/MCP 客户端用 `/mcp`(-agent)；脚本/自动化/其它语言用 `/v1`；要实时进度用 `/v1/tools/{name}/stream`(SSE) 或（MCP 端长任务）`openhub_agent_run` 的 `{status:running}` 句柄 + `openhub_agent_result` 轮询。
