...
```bash
OPENHUB_COMPUTER_USE=1   # 所有 nomi 会话默认启用 Computer
OPENHUB_BROWSER_USE=1    # 所有 nomi 会话默认启用 Browser（进程内 native CDP 引擎）
```
...
| nomi CLI | ✅ 当前 `openhub-cli` manifest 启用 | ❌ 当前 `openhub-cli` manifest 未启用 |
