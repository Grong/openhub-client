...
OpenHub agent 内置/接入两项可选的系统级能力：

- **Computer**（computer use，进程内 Rust）：截屏、鼠标键盘合成输入、窗口枚举/聚焦——让 agent 看到并操作本机桌面。crate：`openhub-computer`（xcap + enigo）。
- **Browser**（browser use，进程内自研 CDP 引擎）：通过内置浏览器引擎驱动 Chrome 完成导航、读取、点击、填表等，以单工具 `Browser` 暴露。crate：`openhub-browser-engine`（自研 Rust CDP）+ `openhub-browser`（facade）。首次启用时引擎按需自动获取 Chrome（`acquire.rs` 内置 CfT 下载/解压），无需 Node/npm。由 `openhub-agent::bootstrap` 在启用且 `browser-use` feature 开启时注册 `BrowserTool`。
...
```bash
OPENHUB_COMPUTER_USE=1   # 所有 openhub 会话默认启用 Computer
OPENHUB_BROWSER_USE=1    # 所有 openhub 会话默认启用 Browser（进程内 native CDP 引擎）
```

### 4. openhub CLI / 配置文件

`~/.openhub/config.toml` 或项目 `.openhub/config.toml`：
...
|---|---|---|
| 桌面应用（openhub-desktop） | ✅ 默认编译（`computer-use` feature） | ✅（`browser-use` feature；首次自动获取 Chrome） |
| openhub CLI | ✅ 当前 `openhub-cli` manifest 启用 | ❌ 当前 `openhub-cli` manifest 未启用 |
| Web/服务器（openhub-web、Docker） | ❌ 不编译（无显示器；xcap/enigo 不进二进制） | ❌ 当前 headless web host 未启用 `browser-use` feature |

`computer-use` feature 链：`apps/desktop` → `openhub-app` → `openhub-ai-agent` → `openhub-agent` → `openhub-computer`。Web 构建若配置中误开 computer，仅记录 warning，不报错。Browser 由 `browser-use` feature 门控（`openhub-browser` / `openhub-browser-engine`）。
