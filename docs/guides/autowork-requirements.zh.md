...
| **回合完成** | 一轮如何宣告"完成"。对智能体目标来说，是该智能体结束本轮回复（或调用 OpenHub 专属工具）；对终端目标来说，是终端输出静默下来（干净收尾）。                                       |
...
2. 它构造一段注入 prompt，点名该需求并告知智能体如何上报完成状态。具体协议是 **engine-aware** 的：
   - 仅在 OpenHub-engine 会话上，智能体会注册 `requirement_complete` / `requirement_update_status` 工具，并由 prompt 要求模型调用它们。
