...
`openhub-web` is the **headless, self-host** way to run OpenHub. It is the same Rust backend that the [desktop app](./desktop-app.md) embeds, but built as a standalone binary that also serves the SPA (`ui/dist`) on the same port. There is no GUI, no WebView, no `DISPLAY` requirement — it runs anywhere a Linux/macOS/Windows server will run a static binary.

Unlike the desktop shell, **`openhub-web` requires authentication by default**. The first browser visitor either creates the admin account interactively (first-run setup), or you pre-seed credentials with `OPENHUB_ADMIN_PASSWORD`.
...
```text
  Browser / phone / LAN              openhub-web  (one process, one port)
  ┌──────────────────┐               ┌───────────────────────────────────────┐
...
./target/release/openhub-web --host 127.0.0.1 --port 8787 \
  --data-dir ./data --dist ./ui/dist
```

Then open `http://127.0.0.1:8787` and the first visit lets you create the admin account. After that, the setup endpoint returns `409 Conflict` and the only way in is via the login form (or `OPENHUB_ADMIN_PASSWORD`).
...
cargo run -p openhub-web      # picks up the default --dist=../../ui/dist
```
...
|---|---|---|---|
| `--host` | `OPENHUB_WEB_HOST` | `127.0.0.1` | IP to bind on. `0.0.0.0` accepts LAN/VPN/public traffic; pre-seed the admin or complete first-run setup before broad exposure. |
| `--port` | `OPENHUB_WEB_PORT` | `8787` | TCP port. Serves the API, the WebSocket at `/ws`, and the SPA. |
| `--data-dir` | `OPENHUB_DATA_DIR` | per-user dir | Backend data dir (SQLite database, agent state, logs, Bun cache). Defaults to the per-user location shared with the desktop app (`%LOCALAPPDATA%\OpenHub\OpenHub`, `~/Library/Application Support/OpenHub/OpenHub`, `$XDG_DATA_HOME/OpenHub/OpenHub`). **Still set an explicit absolute path in production.** |
| `--dist` | `OPENHUB_WEB_DIST` | `../../ui/dist` | Directory containing the built SPA. **Set this explicitly when deploying.** |
| `--admin-user` | `OPENHUB_ADMIN_USERNAME` | `admin` | Username used when pre-seeding the first admin. Ignored once an admin exists. |
| `--admin-password` | `OPENHUB_ADMIN_PASSWORD` | — | Pre-seed the first admin password at boot, skipping interactive setup. Ignored once an admin exists. |
...
| — | `OPENHUB_HTTPS` | `false` | When `true`, session and CSRF cookies are flagged `Secure`. Set this whenever the app is reached over HTTPS (e.g. behind a TLS reverse proxy). |
...
At startup the backend takes an OS-level exclusive lock on `{data_dir}/server.lock` — **one backend instance per data dir**. A second process pointed at the same directory fails fast with an error naming the current holder (pid + exe); to deploy multiple instances, give each its own `OPENHUB_DATA_DIR` / `--data-dir`. The OS releases the lock on exit or crash, so a leftover `server.lock` file is harmless.
...
A weak `OPENHUB_ADMIN_PASSWORD` will refuse to boot. A weak interactively-typed password will return `400` with the validation message.
...
Leave `OPENHUB_ADMIN_PASSWORD` unset. On a fresh data dir the install is "uninitialised": `GET /api/auth/status` reports `needs_setup: true`, the SPA shows the first-run form, and the **first browser visitor's chosen username + password become the admin** via an atomic `POST /api/auth/setup`. The write is a conditional UPDATE — even two concurrent first-run requests cannot both win; the loser receives `409 Conflict`.
...
Provide `OPENHUB_ADMIN_PASSWORD` (and optionally `OPENHUB_ADMIN_USERNAME`, default `admin`) before first boot. The bootstrap routine hashes and stores the credentials atomically, the first-run setup endpoint returns `409` from the very first start, and there is no window for someone else to claim the account.

```bash
OPENHUB_ADMIN_USERNAME=alice \
OPENHUB_ADMIN_PASSWORD='change-me-to-something-strong' \
openhub-web --host 0.0.0.0 --port 8787 \
  --data-dir /var/libopenhub--dist /opt/openhub/web
```
...
2. Compiles `openhub-web` from the workspace.
...
```bash
docker compose logs -f openhub
# look for: "openhub-web: embedded backend + SPA on one port"
```

The compose file mounts a named volume `openhub-data:/data` which holds the SQLite DB, logs, the Bun runtime cache, and per-agent state. Back this up with the same care as any other database.
...
environment:
  ...
  OPENHUB_ADMIN_PASSWORD: "change-me-to-something-strong"
  OPENHUB_HTTPS: "true"        # when fronted by Caddy / nginx with TLS
```
...
$ docker compose up -d
[+] Running 2/2
 ✔ Network openhub_default  Created
 ✔ Container openhub-web    Started

$ docker compose logs -f web
openhub-web  | listening on 0.0.0.0:8787 (auth: enabled)
```
...
2. Set `OPENHUB_HTTPS=true` in the openhub service env (so cookies get the `Secure` flag).
...
The repo includes `packaging/linux/openhub-web.service` and a long-form Linux deployment guide at `packaging/linux/README.md`.
...
cargo build --release -p openhub-web  # → target/release/openhub-web
```

### Layout

```
/opt/openhub/openhub-web    # the binary
/opt/openhub/web/           # contents of ui/dist
/var/lib/openhub/           # data dir (created by systemd's StateDirectory)
```

```bash
sudo useradd --system --home /var/lib/openhub --shell /usr/sbin/nologin openhub
sudo mkdir -p /opt/openhub/web
...
- **Embed in the binary**: build with `OPENHUB_EMBED_BUN=1 cargo build --release -p openhub-web`. Bun is bundled into the binary and self-extracts into the data dir on first run.

Verify: `sudo -uopenhub-s -- which bun` must return a path. Otherwise the first agent spawn will fail with an opaque error.
...
sudo systemctl status openhub-web
```

The shipped unit:

- Binds `127.0.0.1:8787` by default. Change `OPENHUB_WEB_HOST` to
  `0.0.0.0` only after first-run setup is complete or
  `OPENHUB_ADMIN_PASSWORD` is configured.
- Sets `OPENHUB_DATA_DIR=/var/libopenhub to match the systemd-managed `StateDirectoryopenhub. **Keep these two in sync** — if you drop the env line, the data dir silently falls back to the service user's per-user directory (`$XDG_DATA_HOME/OpenHub/OpenHub`, typically `~openhub/.local/share/OpenHub/OpenHub`), decoupled from systemd state.
- Runs as a dedicated openhub user (`Useropenhub, `Groupopenhub).
...
```ini
Environment=OPENHUB_HTTPS=true
```

To pre-seed the admin instead of interactive setup:

```ini
Environment=OPENHUB_ADMIN_USERNAME=admin
Environment=OPENHUB_ADMIN_PASSWORD=change-me-to-something-strong
```

```text
$ sudo systemctl status openhub-web
● openhub-web.service - OpenHub web host
     Loaded: loaded (/etc/systemd/system/openhub-web.service; enabled; preset: enabled)
     Active: active (running) since Tue 2026-06-25 09:12:03 UTC
   Main PID: 12345 (openhub-web)
     CGroup: /system.slice/openhub-web.service
             └─12345 /usr/local/bin/openhub-web --host 127.0.0.1 --port 8787 …
openhub-web[12345]: listening on 127.0.0.1:8787 (auth: enabled)
```
...
| `DISPLAY` / X11 / WebView | **No** | `openhub-web` is fully headless. |

## Security checklist

- **Use TLS for any public deployment.** Cookies and login credentials over plain HTTP can be sniffed. Behind a TLS proxy, set `OPENHUB_HTTPS=true` so the session cookie is flagged `Secure`.
...
- **Close the first-run window** with `OPENHUB_ADMIN_PASSWORD` for any host that becomes reachable before you are ready to interactively complete setup. Alternatively keep the service on `127.0.0.1` until setup is finished, then intentionally bind `0.0.0.0`.
...
**Cookies don't stick over HTTPS.** Set `OPENHUB_HTTPS=true` so the `Secure` flag is added. Without it, browsers reject the cookie on HTTPS responses.

**Agent commands fail with `bun: command not found` under systemd.** Install bun system-wide (see the bun-on-PATH section above) or rebuild with `OPENHUB_EMBED_BUN=1`.
