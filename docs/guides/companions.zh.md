...
从单伙伴版本升级后，首次启动会自动检测旧布局 `{data_dir}/companion/openhub/`：若存在且尚无 `companion/shared/`，自动迁移为共享记忆中枢 + 第一个伙伴（默认名 **"Nomi"**，继承原有 XP / 人格 / 形象 / 模型 / 桌面伙伴位置 / 陪伴会话线程）。迁移幂等可重入，完成后在旧目录写入 `.migrated` 标记并保留原目录（一个版本周期后清理）。无需任何手工操作。
