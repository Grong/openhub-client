...
## OpenHub 与openhub有什么区别？

**OpenHub** 是这个开源项目与面向用户的产品名：桌面应用、WebUI 界面、
代码库、工作区、GitHub 仓库和品牌都使用这个写法。

在本代码库里，小写形式 openhub 仅作为字面意义上的技术标识符出现
——包名（`openhub-app`、`openhub-web` 等）、桌面 bundle id
`com.openhub.desktop`、以 `OPENHUB_` 为前缀的环境变量、仓库目录。
任何作为应用或项目品牌展示给人看的地方，都使用 "OpenHub"。

## 有托管版本吗？

没有。OpenHub 是一个自托管项目。不存在 SaaS 实例、不存在
`openhub.dev` 上的托管登录、不存在你可以注册的中心账户系统。使用它的
两种方式是：

- 安装桌面应用并在本地运行——`openhub-desktop`。
- 在你自己控制的服务器上部署 `openhub-web`。参见
...
- **桌面端**：默认在**按用户的应用数据目录**——Windows 上是
  ...
  设置 `OPENHUB_DATA_DIR=<absolute path>` 后目录会变成
  `$OPENHUB_DATA_DIR/OpenHub`（覆盖语义不变）。旧版构建把数据存在
  `<system temp>/openhub-data/OpenHub` 下；若存在这样的安装，启动时会
  自动搬迁到新位置，旧目录保留为备份。
- **Web（`openhub-web`）**：你传给 `--data-dir`（或
  `OPENHUB_DATA_DIR`）的任何位置，按字面值生效——不附加 `/OpenHub`
  后缀。两者都未设置时，默认与桌面应用是**同一个按用户目录**，因此
  开发中的 `bun run serve:web` 与已安装的应用看到的是同一份状态。
- **Docker**：compose 文件中定义的命名卷（`openhub-data`，挂载到
  `/data`）。

数据目录里有 SQLite 数据库（`openhub-backend.db*`）、各智能体状态、
...
OpenHub 作为 ACP（Agent Client Protocol）后端运行的"智能体 CLI"
包括 `claude`、`codex`、`gemini`、`openhub`、`codebuddy`、`qwen` 与
`opencode`。每一个都是一个独立的 CLI，需要你自己在系统上安装；
OpenHub 会从 `PATH` 上发现它们，并据此填充注册表。运行
`openhub-core doctor` 即可看到你的安装能识别到哪些。

至于裸模型访问（例如提供商密钥、自定义的 OpenAI 兼容端点），系统
通过 `/api/providers/*` 与应用内的设置 UI 支持可配置的提供商。API
密钥由你提供；OpenHub 会以静态加密的方式把它们存到数据目录。

并不存在一个会调用某个托管 OpenHub 端点的内置智能体——根本就没有
...
扩展（主题、助手、频道插件、设置标签页）由 `openhub-extension` 从
数据目录加载。技能则是一组提示词/指令，按会话被解析进智能体的上下文。
两者都是数据目录下的本地文件；市场流程只是把它们下载到该目录。

智能体 CLI 二进制不属于扩展——它们是 OpenHub 通过 ACP 协议作为子
进程派生的外部 CLI。

## 我能在与 UI 不同的机器上跑智能体吗？

可以——这正是 `openhub-web` 的用途。把 Web 宿主部署到你希望智能体
...
- **桌面端**：`bun install && bun run build:ui && cargo run -p
  openhub-desktop`（或 `cargo build --release -p openhub-desktop`）。
- **服务端**：从源码构建（`cargo build --release -p openhub-web`）
...
在 `openhub-web` 上，带内的恢复流程是仅本地的 WebUI 路由
...
- [作为桌面应用运行 OpenHub](../guides/desktop-app.zh.md)
