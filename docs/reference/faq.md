...
- **Desktop**: defaults to the **per-user application-data dir** — `%LOCALAPPDATA%\OpenHub\Nomi` on Windows, `~/Library/Application Support/OpenHub/Nomi` on macOS, `$XDG_DATA_HOME/OpenHub/Nomi` (usually `~/.local/share/OpenHub/Nomi`) on Linux. Set `OPENHUB_DATA_DIR=<absolute path>` and the dir becomes `$OPENHUB_DATA_DIR/Nomi` (unchanged override semantics). Older builds stored data under `<system temp>/openhub-data/Nomi`; if such an install exists it is relocated to the new location automatically on launch, and the old dir is kept as a backup.
- **Web (`openhub-web`)**: whatever you pass to `--data-dir` (or `OPENHUB_DATA_DIR`), taken literally — no `/Nomi` suffix. With neither set it defaults to the **same per-user dir as the desktop app**, so a dev `bun run serve:web` and the installed app see one shared state.
