...
## What is the difference between OpenHub andopenhub

**OpenHub** is the open-source project and the user-facing product name: the desktop app, WebUI surface, codebase, workspace, GitHub repository, and brand all use this spelling.

In this codebase the lowercase form openhub shows up only as a literal technical identifier — package names (`openhub-app`, `openhub-web`, …), the desktop bundle id `com.openhub.desktop`, environment variables prefixed `OPENHUB_`, repository directories. Anywhere it is shown to a human as the app or project brand, use "OpenHub".

## Is there a hosted version?

No. OpenHub is a self-host project. There is no SaaS instance, no managed login at a `openhub.dev` URL, no central account system to sign up for. The two ways to use it are:

- Install the desktop app and run it locally — `openhub-desktop`.
- Deploy `openhub-web` on a server you control. See [Web Server Deployment](../guides/web-server-deployment.md).
...
- **Desktop**: defaults to the **per-user application-data dir** — `%LOCALAPPDATA%\OpenHub\OpenHub` on Windows, `~/Library/Application Support/OpenHub/OpenHub` on macOS, `$XDG_DATA_HOME/OpenHub/OpenHub` (usually `~/.local/share/OpenHub/OpenHub`) on Linux. Set `OPENHUB_DATA_DIR=<absolute path>` and the dir becomes `$OPENHUB_DATA_DIR/OpenHub` (unchanged override semantics). Older builds stored data under `<system temp>/openhub-data/OpenHub`; if such an install exists it is relocated to the new location automatically on launch, and the old dir is kept as a backup.
- **Web (`openhub-web`)**: whatever you pass to `--data-dir` (or `OPENHUB_DATA_DIR`), taken literally — no `/OpenHub` suffix. With neither set it defaults to the **same per-user dir as the desktop app**, so a dev `bun run serve:web` and the installed app see one shared state.
- **Docker**: the named volume defined in the compose file (`openhub-data` mounted at `/data`).

The data directory contains the SQLite database (`openhub-backend.db*`), per-agent state, the Bun cache, log files, and any embedded extension data. Back it up like a database. Because every host defaults to the same directory, the backend guards it with an exclusive `server.lock` — a second backend instance on the same data dir fails fast instead of corrupting state.
...
There is no built-in agent that calls out to a hosted OpenHub endpoint — there is no such endpoint. Every agent / provider you configure is something you control.

## Is OpenHub really local-only?
...
What OpenHub itself does over the network:
...
Extensions (themes, assistants, channel plugins, settings tabs) are loaded by `openhub-extension` from the data directory. Skills are bundles of prompts/instructions resolved into the agent's context per-conversation. Both are local files under your data dir; the marketplace flow simply downloads them into that directory.

The agent CLI binaries are not extensions — they are external CLIs that OpenHub launches as child processes via the ACP protocol.

## Can I run agents on a different machine from the UI?

Yes — that is exactly what `openhub-web` is for. Deploy the web host on the machine where you want the agents (and their CLIs, and their network access) to live, and access the SPA from any browser. See [Web Server Deployment](../guides/web-server-deployment.md).
...
- **Desktop**: `bun install && bun run build:ui && cargo run -p openhub-desktop` (or `cargo build --release -p openhub-desktop`).
- **Server**: build from source (`cargo build --release -p openhub-web`) or `docker compose up -d --build`.
...
On `openhub-web`, the in-band recovery flow is the local-only WebUI route (`POST /api/webui/reset-password`) — you can hit it from the same machine the server runs on, and it generates a fresh random password and prints it. From a remote machine you cannot recover the password through the API.
...
Yes for the server: `openhub-web` is one statically-linked Rust binary plus the `ui/dist/` directory it serves. SQLite is statically linked, TLS uses rustls — there is no `libsqlite`, no `openssl` dependency at runtime. Build with `cargo build --release -p openhub-web`.

The agent runtime (bun) is *not* embedded by default; install it system-wide or use the `OPENHUB_EMBED_BUN=1` build flag to bundle it into the binary. See the [bun-on-PATH](../guides/web-server-deployment.md#bun-must-be-on-the-system-path) section.

The desktop shell also produces a single binary (`openhub-desktop`), but for distribution you typically want the platform-native packaging via `bun run build` once installer signing is set up.
...
- [Running OpenHub as a Desktop App](../guides/desktop-app.md)
