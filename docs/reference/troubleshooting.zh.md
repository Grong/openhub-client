...
运行 OpenHub 时可能遇到的症状，以及它们背后真实的成因。如果你遇到的
...
网卡的 IP）。`localhost` 这样的主机名不会被解析——`openhub-web` 会
...
（`OPENHUB_WEB_PORT`）。桌面外壳没有这个问题——它启动时会向 OS 索要
...
`--port` / `OPENHUB_WEB_PORT`。

### 浏览器无法在非 loopback 地址上访问服务

`openhub-web` 默认绑定 `127.0.0.1`。要接受 LAN/VPN 流量，需要传入
`--host 0.0.0.0` 或设置 `OPENHUB_WEB_HOST=0.0.0.0`。如果该主机可被较大范围访问，
...
这是全新安装且没有设置 `OPENHUB_ADMIN_PASSWORD` 时的预期状态。第一个
访问浏览器的人输入的用户名 + 密码会通过 `POST /api/auth/setup` 成为
管理员。打开 URL，填写表单，登录即可完成。

如果希望在服务对外可达之前关闭这个时间窗口，请在首次启动前设置
`OPENHUB_ADMIN_PASSWORD`（可选地再设置 `OPENHUB_ADMIN_USERNAME`）。
...
`OPENHUB_HTTPS=true` 时才会被加到 cookie 上。在没有该标记的 HTTPS
响应中，浏览器会直接拒收 cookie，下一个请求于是没有会话。设置
`OPENHUB_HTTPS=true` 然后重新加载即可。
...
`openhub-csrf-token` cookie 的值必须与 `x-csrf-token` 请求头匹配。
中间件会在第一个没带它的响应中自动设置该 cookie，因此一个新加载的
SPA 会在它的第一个 GET 请求中拿到。这通常因为以下几种原因失败：

- 客户端假设服务处于无鉴权本地模式，但服务实际运行在已鉴权模式下
  （或反过来）。`openhub-core --local` 与 `openhub-web --insecure-no-auth`
  会跳过 CSRF；普通 `openhub-web` 需要它。桌面外壳使用 `TrustLocalToken`，
...
`openhub`、`codebuddy` 等），它们必须出现在**进程**的 `PATH` 里。
进程 PATH 在启动时被增强（`openhub_runtime::enhance_process_path`），
但若二进制位置不寻常，仍可能被错过。
...
- 在 systemd 下以 `Useropenhub 运行，但数据目录的所属者是另一个
  用户。修复：`chown -R openhubopenhub/var/libopenhub。
...
`%LOCALAPPDATA%\OpenHub\OpenHub`，macOS 上是
`~/Library/Application Support/OpenHub/OpenHub`，Linux 上是
`$XDG_DATA_HOME/OpenHub/OpenHub`），它天然对启动应用的用户可写。设置
`OPENHUB_DATA_DIR=<absolute path>` 后目录会变成
`$OPENHUB_DATA_DIR/OpenHub`。位于 `<system temp>/openhub-data/OpenHub` 的
遗留安装会在启动时被自动搬迁到新默认位置（旧目录保留为备份）；若
搬迁未能完成，应用会继续从遗留目录启动，并在下次启动时重试。

### `data directory ... is already in use by another running OpenHub backend`（数据目录被占用）

所有宿主（桌面外壳、`openhub-web`、`openhub-core` 二进制）默认使用
...
`OPENHUB_DATA_DIR` / `--data-dir` 给新实例指一个独立目录。持有者
退出或崩溃时锁由 OS 自动释放；残留的 `server.lock` 文件无害。
`openhub-core doctor` 与 `mcp-*` stdio 子命令不取这把锁，因此不受影响。

## Docker 专项

### `docker compose up` 完成构建并启动后立刻退出

请阅读日志（`docker compose logsopenhub）。最常见的原因有：

- 数据卷为空*而且*没有设置 `OPENHUB_ADMIN_PASSWORD`——服务运行
  正常，但你必须先通过 HTTP 完成首次启动设置才能进得去。这其实不是
  失败，而是一种状态。
- 镜像内 `--dist` 路径指错。官方 Dockerfile 把 `ui/dist` 复制到
  `/opt/openhub/web`，`CMD` 也据此引用——只在你定制了 Dockerfile
  时才会出问题。
- 一个被 bind 挂载的、容器用户无写权限的数据目录。

### 日志显示 `openhub-web: embedded backend + SPA on one port` 但浏览器无法连接
...
后端**同时**写入 stdout 与 `<log-dir>/openhub-core.log` 上的按日滚动文件
（默认 `<data-dir>/logs`）。出问题时：

- `journalctl -u openhub-web` / `docker compose logsopenhub 视图能
  看到最近几分钟的日志。
- `<log-dir>` 下的滚动文件保存了历史。
- 把受影响模块的级别拉高：MCP 问题用
  `--log-level info,openhub_mcp=trace`，终端用
  `info,openhub_terminal=debug`，智能体会话用
  `info,openhub_conversation=debug`。

## 当一切都不奏效

读源码。每个路由 handler 都在其归属 crate 的 `routes.rs`（或
`routes/`）文件里；装配在
`crates/backend/openhub-app/src/router/routes.rs`。handler 抛出的错误
