...
`%LOCALAPPDATA%\OpenHub\Nomi`，macOS 上是
`~/Library/Application Support/OpenHub/Nomi`，Linux 上是
`$XDG_DATA_HOME/OpenHub/Nomi`），它天然对启动应用的用户可写。设置
`OPENHUB_DATA_DIR=<absolute path>` 后目录会变成
`$OPENHUB_DATA_DIR/Nomi`。位于 `<system temp>/openhub-data/Nomi` 的
