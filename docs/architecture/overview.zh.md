...
OpenHub 围绕一个核心原则构建：**一份 Rust 后端、两种宿主形态、一份前端**。无论你启动桌面产品 **OpenHub**，还是自托管 Web 服务器，同一个 `axum` HTTP/WS 服务器（`openhub-app`，二进制 `openhub-core`）都在宿主进程中执行。`ui/` 下的 React 19 SPA 是唯一客户端，它始终通过普通的 HTTP 与 WebSocket 通信 —— 没有 Electron preload，也没有 Tauri 自定义协议。

本文档是这张地图的总图。配套的四篇文档分别深入介绍各个部分：

- [`backend-crates.md`](backend-crates.zh.md) —— 29 个 `openhub-*` crate。
- [`agent-engine.md`](agent-engine.zh.md) —— 15 个 `openhub-*` crate（AI 引擎）。
...
5. Agent seam
   crates/backend/openhub-ai-agent  — the only backend crate that sees openhub-*
...
| 目录 | 用途 | Crate 前缀 | 数量 |
| --- | --- | --- | --- |
| `crates/agent/` | AI 引擎 —— providers、tools、sessions、MCP、skills、browser/computer-use | `openhub-*` | 15 |
| `crates/backend/` | HTTP/WS 服务器、数据、认证、各项功能 | `openhub-*` | 29 |
| `crates/shared/` | 真正跨层共享工具 | mixed | 2 |

agent 分组是**基本自包含的** —— `openhub-*` crate 不引用 `openhub-*` crate、工作区根目录或 Tauri / sqlx / axum 等后端框架。反向依赖默认通过 `openhub-ai-agent` 这条接缝汇集，它再导出 `openhub_config`、`openhub_types` 和 `RequirementSink`。当前 `openhub-app` 与 `openhub-gateway` 为 browser/computer-use bridge 存在 feature-gated 直接依赖例外；新增例外必须有明确 feature gate 和文档说明。
...
- **OpenHub** —— 桌面产品和项目 / 品牌字标（驼峰式书写，N 与 F 大写）。在散文中使用此写法。
- 小写的 openhub 仅保留给技术标识符 —— npm/JS 包 id、Rust crate 前缀 `openhub-*`、Tauri bundle 标识符 `com.openhub.desktop`、环境变量 `OPENHUB_*`，以及仓库 / 目录名。

## 宿主一览

| 维度 | 桌面（`openhub-desktop`） | Web（`openhub-web`） |
| --- | --- | --- |
| 二进制 | `openhub-desktop`（Tauri 外壳） | `openhub-web`（axum 服务器） |
...
桌面二进制的 `main.rs`（[`apps/desktop/src/main.rs`](../../apps/desktop/src/main.rs)）有意保持精简 —— 大部分逻辑都在 `openhub_app::run_embedded_server` 中。Web 二进制（[`apps/web/src/main.rs`](../../apps/web/src/main.rs)）复用同样的引导辅助函数（`init_environment`、`init_data_layer`、`AppServices::from_config`、`create_router`），并补充了 SPA 回退以及首次运行管理员预置（`ensure_admin_credentials`）。
