...
OpenHub 的各个进程 —— SPA、嵌入式后端、agent CLI 与 MCP 服务器 —— 通过五条彼此独立的通道相互对话。它们的职责互不重叠，挑选合适通道的规则在客户端的适配层（`ui/src/common/adapter/`）以及服务端的路由与服务 crate 中得到了固化。
...
- 请求体上限 —— `openhub_common::constants::BODY_LIMIT`。
- CSRF cookie 名 —— `openhub-csrf-token`。
- CSRF header 名 —— `x-csrf-token`。
- 默认端口（`openhub-web`） —— `8787`。
- 默认 host —— `127.0.0.1`。
...
| --- | --- | --- | --- |
| 会话 JWT | 登录时由 `openhub-auth` 设置 | 每次认证请求由 `auth_middleware` 读取 | 是 |
| CSRF token（`openhub-csrf-token`） | 由 `csrf_middleware` 设置（首次缺失时签发） | 浏览器的 `document.cookie`，再由 SPA 回显到 `x-csrf-token` | 否 —— SPA 必须能读到 |

CSRF 中间件（[`crates/backend/openhub-auth/src/csrf.rs`](../../crates/backend/openhub-auth/src/csrf.rs)）守护 POST / PUT / PATCH / DELETE 请求；安全方法绕过校验。三个豁免路径 —— `/login`、`/api/auth/qr-login`、`/api/auth/setup` —— 会跳过检查，因为它们正用于引导会话本身。桌面外壳使用 `TrustLocalToken`：WebView 呈递本地信任 secret，远程/其他本机客户端仍需正常认证或走 WebUI 登录。`--local` 仅是独立 `nomicore`/开发 Web host 的无鉴权模式。
...
| 事件类别 | 何时发送 | 来源 crate |
| --- | --- | --- |
| `message.stream` | 模型按块发出 token 时 | `openhub-conversation::stream_relay` |
| `conversation.artifact` | 工具产生了产物（文件 / 图像 / 预览） | `openhub-conversation::routes_aux` |
| `terminal.output` | PTY 产生输出 | `openhub-terminal` |
| 审批请求 / 响应 | 工具调用需要用户批准 | `openhub-conversation`（经接缝） |
| 团队 / agent 事件 | 多 agent 团队状态变化 | `openhub-team` |
| `auth-expired` / 关闭 1008 | 会话 JWT 中途失效 | `openhub-realtime` |
| 心跳（`ping` / `pong`） | 连接保活 | `openhub-realtime` |

升级由 `openhub_realtime::ws_upgrade_handler`（[`crates/backend/openhub-realtime/src/handler.rs`](../../crates/backend/openhub-realtime/src/handler.rs)）处理，它校验通过 cookie 或 `Sec-WebSocket-Protocol` header 携带的 JWT（header 的取值会被原样回显以使握手正确完成）。认证失败时它会发送 `auth-expired` 消息并以 1008 关闭；SPA 同时监听这两个信号（参见 [`browser.ts`](../../ui/src/common/adapter/browser.ts)），并在任一路径上重定向到 `/login`。
...
心跳常量定义在 `openhub_realtime::types::{HEARTBEAT_INTERVAL, HEARTBEAT_TIMEOUT, PER_CONNECTION_BUFFER}`。
...
若干 CLI agent —— Claude Code、Codex、Gemini CLI、Qwen、OpenCode —— 都实现了 **Agent Connection Protocol（ACP）**：在子进程的 stdin/stdout 上的 JSON 消息流。OpenHub 通过 PATH 上预置的 `bun` 运行时把它们当作子进程派生。接缝 crate `openhub-ai-agent` 持有这些进程的工厂、注册表与 worker-task 管理器；按 agent 划分的元数据（握手响应、可用模型、取消路径）通过 `IAgentMetadataRepository` 存储于 SQLite。

进程内的流量如下：

```
SPA ──HTTP/WS──▶ openhub-conversation ──▶ openhub-ai-agent::AgentService
                                                      │
                                                      ▼
                                              spawn child CLI
                                                stdio = piped
                                                      │
                                                      ▼
                                          openhub-protocol on stdin/stdout
                                                      │
                                                      ▼
                                          stream tokens / tool calls
                                                      │
                              broadcast through openhub-realtime to /ws
```

`openhub-protocol` crate 定义了分帧与工具审批状态机；`openhub-ai-agent::protocol::events::AgentStreamEvent` 把协议事件翻译成 SPA 能理解的 `WebSocketMessage`。

## MCP —— Model Context Protocol

MCP 服务器对外暴露引擎可调用的工具与资源。当前 `openhub-app`
...
- 用户配置的 MCP 行与 OAuth HTTP MCP 服务器由 `openhub-mcp` 管理；
- Requirement 与 Knowledge 服务器是有作用域的内部 MCP 服务器；
- Desktop Gateway 工具通过 `openhub-gateway` 暴露；
- Browser / Computer 桥按 feature gate 启用；
- 公开 `/mcp` 与 `/mcp-agent` 由 `openhub-public` 提供，并使用 companion token 认证。

针对 HTTP MCP 服务器的 OAuth 流程由 `openhub-mcp::oauth_service` 处理（PKCE、回调 URI、token 存储）。加密后的 token 通过 AES-GCM 落入 SQLite 的 `oauth_tokens` 仓库（参见 `openhub-common::crypto::{encrypt_string, decrypt_string}`）。
...
| 远程 REST 能力调用 | `/v1` |
| Agent CLI 会话 | `openhub-ai-agent` 管理的子进程 stdio |
