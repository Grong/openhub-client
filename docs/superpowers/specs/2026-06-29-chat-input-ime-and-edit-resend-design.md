...
- 日期：2026-06-29
- 范围：UI（`ui/src`）+ Rust 后端（`crates/`，仅 OpenHub 原生引擎）
- 背景：解决两个会话页面交互问题
...
- **设置 UI**：在 `SettingsModal/contents/SystemModalContent/index.tsx` 的 `preferenceItems` 增一行，复用 `PreferenceRow` + `OpenHubSelect`（两项）。仿 `language`/`keepAwake` 的 `useState` + 启动 `configService.get(...) ?? 'enter'` + change 乐观写入/失败 `setLocal` 回滚样板。
...
## 问题二：暂停后编辑重发（仅 OpenHub、仅最近一条、回填输入框）
...
- OpenHub 引擎的模型上下文是内存 `AgentEngine.messages: Vec<Message>`，**与 DB `messages` 表解耦**（DB 仅供 UI 展示/持久化）。引擎自持文件型 session 持久化，从不回读 DB 构建上下文。
...
```
用户在某条最近的用户消息（position='right', type='text'）上点「编辑」(仅 OpenHub、仅最近一条、仅 idle)
  ...
  → 用户改完点提交
  → OpenHubSendBox.handleEditResubmit(msgId, input, files):
...
- **Manager**（`openhub-ai-agent` 的 `OpenHubAgentManager`）
...
- **Service**（`crates/backend/openhub-conversation/src/service.rs`）
  ...
  - 仅当会话 agent 类型为 OpenHub 时可用，其它类型返回 4xx（UI 不会触发）。
...
- **SendBox/index.tsx**（通用、平台无关）：
  ...
  - 仅当宿主提供 `onEditResubmit` 时进入编辑模式（即仅 OpenHub）。
- **OpenHubSendBox.tsx**：提供 `onEditResubmit` → `handleEditResubmit`：先本地移除 ≥ 该消息的行（新增 `useRemoveMessagesFrom(createdAt)` 助手，仿 `useRemoveMessageByMsgId`），调用 `ipcBridge.conversation.editResubmit`，再 emit `'chat.history.refresh'`；进入编辑态时还原附件。
...
前端：`utils/emitter.ts`、`Messages/components/MessageText.tsx`、`components/chat/SendBox/index.tsx`、`platforms/openhub/OpenHubSendBox.tsx`、`pages/conversation/Messages/hooks.ts`（`useRemoveMessagesFrom`）、`common/adapter/ipcBridge.ts`、`locales/{en-US,zh-CN}/conversation.json`。
...
- **问题二**：
  - Rust：`delete_messages_from` 删除区间正确（含/不含边界）；`rewind_last_turn` 截断到锚点且 sanity 失败返回 false；`edit_and_resubmit` 非最近一条/非 OpenHub 返回 4xx；端到端"暂停→编辑→重发"上下文不含旧消息但含更早历史。
