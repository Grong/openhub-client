...
UI 列表请求走 HTTP（`httpGet` → `fetch(baseUrl + path)`，桌面/网页均透传 query 到 axum）。gateway 的 `caps_requirement.rs` 仅服务 AI 工具 `openhub_requirement_list`，不在 UI 列表路径上。
...
| 文件 | 改动 |
|---|---|
| `openhub-api-types/src/requirement.rs` | `ListRequirementsQuery` 增 `order_by: Option<String>`、`order: Option<String>`（`#[serde(default)]`） |
| `openhub-db/src/repository/requirement.rs` | `ListRequirementsParams` 增 `order_by`、`order` 字段（结构体已 `#[derive(Default)]`） |
| `openhub-db/src/repository/sqlite_requirement.rs` | 新增 `build_order_clause(order_by, order)` 白名单构造；`list()` 用它替换写死的 ORDER BY；新增单测 |
| `openhub-requirement/src/service.rs` | `list()` 把 `query.order_by/order` 映射进 `ListRequirementsParams` |
| `openhub-gateway/src/caps_requirement.rs` | 两处 `ListRequirementsQuery { .. }` 字面量补 `order_by: None, order: None`（仅编译兼容） |
