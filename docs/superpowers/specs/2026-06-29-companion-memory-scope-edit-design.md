...
| 系统 | 存储 | 类别 | 用途 |
|---|---|---|---|
| **伙伴记忆**（本设计目标） | SQLite `companion_memories`（`crates/backend/openhub-companion`） | profile/preference/knowledge/episode/task/affective 六类 | 注入伙伴人格 prompt + 对话中 `recall_memories` |
| Agent 编码记忆 | 文件 `MEMORY.md`（`crates/agent/openhub-memory`） | user/feedback/project/reference | Claude-Code 式长期记忆，不在本设计范围 |
...
附带发现的代码设计问题（本设计一并修复）：
- **脱敏不对称**：`insert_memory` 跑 `openhub_redact::redact_secrets`，`update_memory` 不跑。
...
### A. 数据层 `crates/backend/openhub-companion/src/store.rs`
...
- `update_memory`：除 content/pinned/status，接受可选 `scope: Option<MemoryScope>`；**对 content 重跑 `openhub_redact::redact_secrets`**；**trim 拒空**。
...
- **预存且无关的破损**：集成测试 `openhub-ai-agent/tests/factory_provider_integration.rs` 因更早提交给 `BuildTaskOptions` 增了 `conversation_created_at` 字段而未同步，本就编译失败（已 stash 验证与本改动无关），不在本次范围内修复。
