...
**目标**
1. **内置**：把 [obra/superpowers](https://github.com/obra/superpowers)（一套 14 个方法论 `SKILL.md` 的技能库，MIT）随openhub出厂，开箱可用、离线可用。
...
### 4.5 定期热更新
- **来源**：superpowers GitHub Release。复用 `VersionCheckService`（`version.rs`，`new_dynamic` + GitHub releases API，代理感知 http client，无鉴权 60 req/hr）查询最新 tag；下载 release 的 source zip（`zipball_url` 或 `…/archive/refs/tags/<tag>.zip`）。仓库常量指向 `obra/superpowers`（独立于应用自更新的 `OPENHUB_GITHUB_REPO`）。
- **调度**：仿 `state.rs:1481 spawn_idmm_record_janitor` 的 `tokio::spawn + interval` janitor，在 `create_router`（`routes.rs`，事件总线→WS 桥接后）启动 `spawn_superpowers_updater`。首 tick 立即触发（启动即检查）。默认周期 **每 6 小时**（远低于 GitHub 限流），带超时（`tokio::time::timeout` 包裹，因 `openhub_net::http_client` 无超时）。
...
- **开关**：默认开启；环境变量 `OPENHUB_SUPERPOWERS_AUTOUPDATE=0` 关闭，`OPENHUB_SUPERPOWERS_UPDATE_INTERVAL_SECS` 调周期。（settings-UI 开关列为后续。）

### 4.6 依赖与错误类型
- 把 superpowers 的"嵌入 + 物化 + 下载 + 校验 + 解压 + 原子替换 + 有效目录解析"做成一个内聚模块，**放在 `openhub-extension`**——因为 zip 安全解压(`extract_zip_archive`)、物化(`startup_materialize`)、链接(`link_workspace_skills`)、指纹全在此 crate，复用面最大。
- `openhub-extension/Cargo.toml` 当前无 `reqwest`/`openhub-net`；**给它新增 `openhub-net` 依赖**（shared crate，无循环风险），下载统一走 `openhub_net::http_client()`（代理感知），并按 openhub-providers 模式补 connect/read timeout 或用 `tokio::time::timeout` 包裹。
- 周期 janitor 任务本身留在 `openhub-app`（`create_router`），调用 extension 暴露的更新入口；`VersionCheckService`（`openhub-system`）用于查询 GitHub Release（`openhub-app` 已能构造它）。
...
- 引擎主循环/工具分发：`openhub-agent/src/engine.rs:652,693,1004-1052`；`orchestration.rs:24`。
- system prompt/缓存/技能段：`openhub-agent/src/context.rs:17,42,146,247-267`。
