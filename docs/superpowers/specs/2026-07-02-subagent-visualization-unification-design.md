...
- **Task 4** `openhub-ai-agent`：`NomiBuildExtra.allowed_tools` → `NomiResolvedConfig{in_process_spawn, allowed_tools}` → manager 灌 config；纯函数 `engine_spawn_enabled(desktop_gateway, channel_platform)`（本地桌面网关会话禁进程内 Spawn，IM 渠道保留）。
