...
- **编排 run（现有）**：`openhub_run_create(goal)` → planner LLM 拆 DAG → 有依赖/聚合节点。
...
调用方（伙伴会话 / 普通桌面会话 / 未来任何 agent）
   ...
   ├── openhub_run_create(goal)   → planner LLM 拆 DAG（复杂任务，已存在）
   └── openhub_spawn(tasks[])     → 扁平 fan-out，跳过 planner（简单并行，新增）★
        │
        ▼
...
### 3.1 `plan_flat`（跳过 planner 的扁平落库）— `openhub-orchestrator/src/run_service.rs`
...
### 3.2 `openhub_spawn` 网关能力 — `openhub-gateway/src/caps_orchestrator.rs`

以 `openhub_run_create` 为模板，新增第 4 个能力：
...
### 3.3 引擎内 Spawn 门控 — `openhub-agent/src/bootstrap.rs` + config + factory
...
- `OpenHubResolvedConfig`（types.rs）+ `factory/openhub.rs`：桌面**网关**会话（`desktop_gateway` 且非 companion/remote）置 `false` → 改走 `openhub_spawn`；`manager/openhub/agent.rs` 紧邻 browser/computer 开关灌入。**须同步补齐所有 `make_test_config`/`OpenHubAgentManager::new` 调用点**（漏改编译失败）。
...
**v1 纳入**：
- `plan_flat` + `openhub_spawn` 网关能力（supervised 自主度、deny Remote）；
- 引擎 Spawn 门控（桌面网关会话改走 `openhub_spawn`，CLI 保留）；
- per-node 工具白名单（堵住只读角色静默升权——安全必须）；
- prompt 引导；
- 前端：验证可视化原样点亮（预计零改动）；可选打磨扁平 run 空态文案。

**v1 暂不做（记录，非阻塞）**：
- **worktree 隔离**（Spawn 的 `isolate`：每子 agent 独立 git worktree + diff 回收）。编排 worker 共享单一 workspace，多个并行 implementer 改同文件会互相覆盖。v1 **显式约束**：`openhub_spawn` 用于研究/分析/只读扇出，或写入**互不重叠**文件的独立任务；并行改同一文件的隔离场景暂留进程内 Spawn（CLI）或后续 v2 给编排补 per-node worktree。UI/prompt 明示此约束。
...
- 硬前提：被链接会话 `type==='openhub'`（`useConversationRun.ts:46`）——桌面会话满足。
...
- 上一提交的 `coerce_input_to_schema`（Spawn 入参字符串化容错）对 `openhub_spawn` 同样生效。
- 回归验证：`openhub-agent` / `openhub-tools` / `openhub-orchestrator` / `openhub-gateway` / `openhub-ai-agent` cargo test；`cargo check --workspace`；前端 typecheck/i18n/build。
...
2. `openhub_spawn` 走 supervised 自主度即时执行、不停在批准门。
3. 只读角色（searcher/reviewer）的 worker 确实只有只读工具（无静默升权）。
4. CLI / 独立模式的进程内 Spawn 不受影响。
5. Remote 会话拒绝 `openhub_spawn`。
6. 既有 `openhub_run_create` 编排、AutoWork、idmm、知识库全部照常。
7. 全部回归命令绿。
...
- **Task 1** `openhub-tools`：`ToolRegistry::retain_named`（空=不限制）。
- **Task 2** `openhub-config`：`ToolsConfig.in_process_spawn`（默认 true）+ `builtin_allowlist`（默认空）；global/project 两条合并分支同步。
- **Task 3** `openhub-agent` bootstrap：SpawnTool 注册门控 + 全部注册后 `retain_named` 收口（含 MCP 代理、ToolSearch 快照之前）。
- **Task 4** `openhub-ai-agent`：`OpenHubBuildExtra.allowed_tools` → `OpenHubResolvedConfig{in_process_spawn, allowed_tools}` → manager 灌 config；纯函数 `engine_spawn_enabled(desktop_gateway, channel_platform)`（本地桌面网关会话禁进程内 Spawn，IM 渠道保留）。
- **Task 5** `openhub-orchestrator`：`plan()` 拆出 `persist_dag_and_activate`（行为逐字节等价）；`plan_flat`（空拒绝、synthesis 依赖边/role 持久化）；`spawn_plan_flat_and_start`。
- **Task 6** `openhub-orchestrator` worker：`role_allowed_tools`（searcher/reviewer=只读、verifier=+Bash、中文角色/无角色=不限制）；受限角色 `desktopGateway:false` + `allowed_tools`；trait 默认方法 `run_restricted`（17 处 mock 零翻修）；引擎 dispatch 传 `task.role`。
- **Task 7** `openhub-gateway`：`openhub_spawn` 能力（tasks 1-8、role、synthesize→只读 synthesis 节点、**显式 supervised**、link 回调用会话、后台 plan_flat→start、`.deny_on(Remote)`）。
- **Task 8** prompts：`LEAD_ORCHESTRATOR_PROMPT` + 伙伴智能编排 nudge 增 `openhub_spawn` 引导。

**回归（全绿）**：openhub-tools 168 / openhub-config 135 / openhub-agent 444+bootstrap 11 / openhub-ai-agent 582 / companion 207 / conversation 254 / gateway 78 / orchestrator 281（另有 1 项 `cap_2_total_elapsed_reflects_overlap_not_serial_sum` 为**既有**时间敏感 flaky——已用 git stash 在未改动基线上复现同样失败，与本次无关）；`cargo check --workspace`、前端 `typecheck`/`check:i18n` 通过（前端零改动）。

**真机冒烟清单（待用户回来验证运行时画布）**：
1. 桌面普通会话让模型调 `openhub_spawn`（如「用 openhub_spawn 并行跑两个子任务：各自输出 hello world」）→ 工具立即返回 run_id，**主会话不卡**。
2. 右栏「编排」tab / 会话头 pill 点亮 → DAG 画布出现 2 个节点，状态从 pending→running→done 实时变化。
3. 点节点 → 内容区投射该 worker 的实时转录；点 main 返回主会话。
4. `openhub_run_result` 有各任务产出；带 `synthesize:true` 时多一个「综合汇总」节点。
5. 伙伴会话开启智能编排后同样可用；受限角色任务（role=searcher）的 worker 会话无 Write/Bash 工具。
6. CLI（openhub-cli）里 `Spawn` 工具照常注册可用。
