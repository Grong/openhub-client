...
lead agent 通过 `openhub_run_create`（规划 DAG）/`openhub_spawn`（扁平扇出）铸造 run，每节点 = 一条真实
openhub worker 会话，前端经 `conversation.extra.orchestrator_run_id` 被动感知并展示右侧画布。
...
- **C（采用）. 意图标记 + 提示升级**：按钮 = 写 `conversation.extra.agent_cluster_mode: true`；
  后端 `factory/openhub.rs` 检测该标记，在 `SUBAGENT_STANDARD_HINT` 之上追加更强的
  `CLUSTER_MODE_HINT`：**必须刻意评估**是否开集群（openhub_run_create/openhub_spawn），若判定太简单，
...
- 设置真源：`conversation.extra.orchestrator_approval_mode`（会话页集群 pill popover 切换）；
  `openhub_run_create`/`openhub_spawn` 建 run 时读取写入 run 行（与 model_range 同法）。
- 复用既有 task 状态字面量 `needs_review`（前端 taskStatusMeta/MiniMap 色已备好）作为"节点挂起
  待人判"状态。

提问链路（仅 approval_mode='manual' 时启用）：
1. worker brief 追加指令：遇显著决策分歧时调用 `openhub_task_question(question)` 后结束本轮等待；
   auto 模式则明确"自行选择最合理方案并在产出中说明"。
2. 新网关工具 `openhub_task_question`：仅 worker 会话（extra 携 orchestrator_task_id）可调；写
...
不变量守护：回执 best-effort；per-run 锁不跨 LLM await；`openhub_task_question` 不持终态锁；
Remote 面继续硬拒。

## 7. 交付物与验证

- 前端：`bun run build` + `check:i18n`（zh-CN/en-US 全 key）+ ComposerEntryStrip 单测更新。
- 后端：`cargo test -p openhub-orchestrator -p openhub-gateway -p openhub-app`；新增/更新单测：
