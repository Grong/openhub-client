...
1. **会话与伙伴是同一套机制**：同一 `conversations`/`messages` 表、同一 `ConversationService`、同一 openhub 引擎。伙伴 = 一条 `type='openhub'` 会话 + `extra.companionSession/companionId` 标记 + `companion_threads` 绑定 + 独立 `companion/shared/memory.db` + factory 注入的人格/记忆 hook。伙伴聊天已并入「会话」侧栏「桌面伙伴」分组（`2026-07-01-companion-chat-into-sessions`）。
2. **伙伴只有恰好一条无限增长的会话线程**（`companion_threads` 唯一索引保证每伙伴仅一条活跃线程，所有 IM 渠道汇入）——**这正是上下文/成本压力的根因**。openhub 引擎从**自身持久化的 `Session`**（引擎 transcript）恢复上下文，`messages` 表只是 UI 可见记录；引擎自带 micro→auto(167k)→emergency(197k) 压缩，但**跨会话无重置**，故长期只增不减。
...
4. **多 Agent 编排底层已成熟**：`openhub-orchestrator`（lead 无状态一次性 LLM + 隔离 worker 会话，主上下文天然干净）、`openhub_run_create` MCP 工具（仅桌面）、会话原生 UI（右栏 tab + 悬浮画布 + 内容投射）、`RunDecisionFeed`。会话原生编排 v2 已批准并落地。**缺"智能编排"持久化开关 + 伙伴作为 lead 的明确接线。**
...
- **需求 1 是净新增核心**：自包含在 `openhub-companion` crate + 伙伴 UI，风险面小；唯一触及引擎的点是复用**已有且经测试**的 `clear_context`。
...
新模块 `openhub-companion/src/archiver.rs`，**复用 `learner` 已有的 LLM seam（`CompanionCompleter` trait）与 60s tick 基础设施**（不新建 provider 抽象）。
...
- archiver 需一个到 `ConversationService`（或其 `clear_context` + 预热能力）的 seam（依赖注入 `Arc<dyn ...>`，避免 crate 环——参照 `openhub-requirement` 的 `IdmmHandle` trait 模式）。
...
- 任意 `openhub` 会话可成 run 宿主（lead）：`extra.orchestrator_role=="lead"` → factory 注入 `LEAD_ORCHESTRATOR_PROMPT` + `openhub_run_create/status/result`；`caps_orchestrator`（仅桌面）扇出隔离 worker；lead 无状态、只读压缩投影 → 主上下文干净。
- 伙伴会话是桌面 openhub 会话 + `desktopGateway:true` → **技术上已能调 `openhub_run_create`**。
...
- 全局默认：新增 config key `openhub.autoOrchestration: boolean`（默认 `false`，opt-in）。
- 每会话覆盖：会话行能力图标里的「智能编排」开关，写 `conversation.extra.autoOrchestration`。
- **开启效果**：该会话被视为 lead-capable → factory 注入 lead 能力（`openhub_run_create` + 精简 lead 指引：复杂任务可扇出、简单任务直接答）。**关闭**：不自动扇出；但用户仍可显式发起（会话内 composer / 直接命令 agent），保留 Path B。
...
- 伙伴 factory（`factory/openhub.rs` companion 分支）在 `autoOrchestration`（伙伴级或全局）开启时，同样追加 lead 能力段落到人格 system prompt。
- 伙伴的"主 agent 干净上下文"诉求与归档天然协同：重活走 worker、主线程只留 `openhub_run_result` 总结 → 窗口 digest 更小更干净。
...
2. **智能编排开关**（需求 2）：
   - 全局默认：`configKeys.ts` 加 `openhub.autoOrchestration`；`SystemModalContent` 按 `autoPreviewOfficeFiles` 范本加 `<Switch>`。
...
**必须持续正常**（用户点名）：
- **AutoWork**：`openhub-requirement` 循环认领需求→注入会话→等回合。归档只作用于伙伴会话的 idle 窗口；AutoWork 目标会话若空闲被归档 `clear_context`，需保证 AutoWork 下一轮注入仍工作（clear_context 幂等、保留会话行）。**验证点**：AutoWork 会话不应被伙伴归档器扫描（归档器只遍历 `companion_threads` 伙伴，AutoWork 普通会话不在其列）。
- **IDMM**：对伙伴/路由会话让路不变（本设计不碰 idmm）。
- **知识库**：绑定/检索/注入不变（不碰 `openhub-knowledge`）。
...
**需求 2 · 智能编排 — 伙伴 + 普通会话均完整**
- 伙伴：`SharedCompanionConfig.smart_orchestration`（默认 OFF）：开启后本地伙伴人格提示追加「调度子 agent」能力，复杂大任务用 `openhub_run_create` 派隔离子 agent、伙伴只调度+汇总（工具随既有 desktopGateway；远程 IM 不注入，caps_orchestrator 对 Remote 硬拒）。前端学习页开关。
- 普通会话：全局 `openhub.autoOrchestration`（默认 OFF）→ 工厂纯函数 `is_orchestration_lead` 对非伙伴、非远程的桌面会话注入 `LEAD_ORCHESTRATOR_PROMPT`（仅 prompt；工具本就随后端自动授予的 desktopGateway 提供，不改能力/审批模型）。前端系统设置开关。**零改编排引擎/调度/锁不变式。**

**回归（全绿）**：`openhub-companion` 206、`openhub-conversation` 254、`openhub-requirement`/`openhub-idmm`/`openhub-knowledge`/`openhub-orchestrator` 全过；`cargo check --workspace` 通过；前端 `typecheck`/`check:i18n`/`build:ui` 通过。AutoWork/idmm/知识库/编排未受影响（归档器只遍历伙伴的活跃线程，不碰普通会话/AutoWork 目标；clear_context 幂等、保留会话行）。

### 明确的剩余项 / 后续增强

**普通会话的「智能编排（默认驱动）」自动扇出开关 — 已交付。** 探查修正了此前顾虑：
- `openhub_run_create` 随 desktop gateway 提供，而**后端已给每个本地可信桌面会话自动授予 `desktopGateway`**（`openhub-conversation/routes.rs` 的 `grant_desktop_gateway`：strip 客户端伪造值后由后端重授，产品目标「任意会话皆可全平台操作」）。故普通桌面会话**本就有** `openhub_run_create`。
- 澄清：yolo 免批准是**所有 openhub 会话的既有产品默认**（`factory/openhub.rs` session_mode 默认 yolo，显式 `session_mode` 仍胜出），与 gateway 无关——故本功能**不触及审批模型、不引入 yolo 回退**。
- 实现：新增全局客户端偏好 `openhub.autoOrchestration`（默认 OFF）。工厂纯函数 `is_orchestration_lead(role, auto_pref, is_companion, is_remote)` = 显式 `orchestrator_role=="lead"` 或（全局开 且 非伙伴 且 非远程）→ 注入 `LEAD_ORCHESTRATOR_PROMPT`（**仅 prompt，不改能力/审批**）。伙伴走各自 `smart_orchestration` 人设提示，远程不参与。工厂单测 `is_orchestration_lead_policy` 覆盖全部分支。
- 前端：系统设置新增「智能编排（普通会话）」开关（`openhub.autoOrchestration`）；zh/en `settings.autoOrchestration(Desc)` 文案。

**可选增强（记录，非阻塞）**：普通会话的**每会话**智能编排开关（当前是全局默认；如需 per-session 覆盖，前端写 `extra.orchestrator_role="lead"` 即可，后端已支持、无需改动）；把编排能力从「全量总管 gateway」解耦出受限档（当前普通会话开启即拥有全平台总管工具，是既有产品默认，非本功能引入）；`recall_days` 工具（伙伴主动检索历史 digest；当前靠注入最近 digest + 回顾 Tab 覆盖）；digest 保留/TTL 清理（当前永久保留）；per-turn digest contributor（当前建会话快照已足够）。
