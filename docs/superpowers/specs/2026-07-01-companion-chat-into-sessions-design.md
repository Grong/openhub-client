...
用户反馈：「桌面伙伴」的聊天页面藏在 `/openhub`（桌面伙伴配置中心）的一个"聊天"Tab 里，而不是在「会话」体系中，进入路径割裂。
...
- **伙伴聊天本就是真实会话**：每个伙伴背后是一条真实的 `type:'openhub'` conversation，与普通会话同表、复用同一聊天引擎（`ChatLayout` + `OpenHubChat`）。它们只是被前端过滤器 `useConversationListSync.ts:123`（`isCompanionConversation`）**刻意隐藏**在会话列表之外。
...
1. **`/openhub` 保留为管理中心，移除聊天 Tab**。配置（形象/记忆/技能/知识/设置等）仍归宿于此。
...
- **仅交互式**：无 `SessionKindGroup`（无终端会话子组），无"＋新建"（创建留在 `/openhub`）。可有一个小"管理/齿轮"跳到 `/openhub`。
...
- 把"去聊天"导航从 `/openhub?...&tab=chat` 改为 `/conversation/{conversation_id}`，使用桌宠已解析的 thread id。`memories`/`settings`/`suggestions` 深链仍留在 `/openhub`。
...
3. `/openhub` 不再有聊天 Tab，仍可管理形象/记忆/技能/知识/设置；创建伙伴后可一键进入聊天。
4. 桌宠"去聊天"打开会话视图而非失效的 `/openhub?tab=chat`。
