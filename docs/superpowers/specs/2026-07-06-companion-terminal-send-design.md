...
- **权限层已通**：伙伴跑在 Desktop surface + WORK profile（`team_mcp.rs` 的 `WORK_DOMAINS` 含 `"terminal"`），terminal 域全部工具对伙伴可见且放行（`openhub_terminal_write_input` 仅 `deny_on(Channel)`）。
- **缺口一（提示词）**：伙伴系统提示词（`openhub-companion/src/companion.rs` `build_companion_system_prompt`）逐一教了会话/编排/知识库工具，但从未提及终端工具——伙伴不知道自己能操控终端。
- **缺口二（执行语义，用户所报症状的根因）**：`openhub_terminal_write_input` 是裸 base64 写 PTY。`文本+\r` 一次写入时，claude/codex/gemini 等 agent TUI 的 paste-burst 检测会吞掉回车，文字滞留输入框不执行。正确序列（bracketed-paste 包裹 → 间隔 120ms → 单独写 `\r`）已存在于 AutoWork 编排器（`openhub-requirement/src/orchestrator.rs` `terminal_submit_chunks`/`submit_terminal_prompt`），但是 crate 私有函数。base64 编码本身对模型也是易错门槛。
- **缺口三（读输出）**：网关无任何读取终端输出的工具。`openhub_terminal_get` 丢弃了后端 `terminal_service.get()` 本已返回的 scrollback。终端侧没有 `openhub_conversation_status` 的对应品。
...
把「向 TUI 提交文字并回车」下沉为 `openhub-terminal` 的公共原语，网关新增「发送并执行」「读取输出」两个高层工具，伙伴提示词补终端指引，并把既有两份后端提交实现收编到共享原语上。
...
### 1. 共享提交原语（`crates/backend/openhub-terminal`，新增 `submit.rs`）
...
- **`openhub_terminal_send`**（DangerTier::Write，`deny_on(Channel)`，与 write_input 同门禁）：
  - 参数 `{ id, text（纯文本，无 base64）, wait?=false, timeout_secs?=300 }`。
  - 行为：会话须 running（exited 报错并提示先 `openhub_terminal_relaunch`）→ `submit_text` → `wait=true` 时 `await_turn_settle` 后附带输出尾巴。
...
- **`openhub_terminal_read_output`**（DangerTier::Read）：
  - 参数 `{ id, max_bytes?（默认尾部 16KiB）}`。
  - 行为：透传后端已有的 scrollback，解码 + 剥离 ANSI 转义，返回 `{ text, truncated, status }`——终端版的 `openhub_conversation_status`。
- 现有 `openhub_terminal_write_input` 保留（控制键如 Ctrl-C 仍需要它），描述文案追加「发送文本/命令请优先用 openhub_terminal_send」。
...
- `openhub-requirement/orchestrator.rs` 的 `terminal_submit_chunks`/`submit_terminal_prompt` → 改调共享原语（agent TUI 路径就是从它照搬，行为不变）。
- `openhub-idmm/probe.rs` 的 `encode_terminal_input` → 改调共享编码器（对 agent 终端更稳：其现状单行裸写在快速注入下仍有被当 paste-burst 的风险）。
- 前端 `TerminalSendBox` 不动（xterm 运行时 bracketed-paste 探测是不同且合理的机制）。

### 5. 保持不动（不可破坏）

- `openhub_terminal_write_input` 的裸字节语义与现有调用方。
...
- 编排层（openhub-orchestrator）的终端任务节点。
- 前端「发送到终端」的用户 UI 入口。
...
新增：
- `crates/backend/openhub-terminal/src/submit.rs`

修改：
- `crates/backend/openhub-terminal/src/service.rs`（`submit_text`/`await_turn_settle`）与 `lib.rs`（导出）
- `crates/backend/openhub-gateway/src/caps_terminal_ext.rs`（两个新能力 + write_input 描述）
- `crates/backend/openhub-companion/src/companion.rs`（提示词）
- `crates/backend/openhub-requirement/src/orchestrator.rs`（改调共享原语）
- `crates/backend/openhub-idmm/src/probe.rs`（改调共享编码器）

## 验收标准

1. 伙伴聊天里说「在 xx 终端里跑 `git status` 并告诉我结果」，伙伴完成发送 → 执行 → 读回结果全程，无需用户碰终端；对 shell 与 claude/codex 终端均可靠（回车不被吞）。
2. `openhub_terminal_send` 对 exited 会话给出可行动的报错；`wait=true` 时按 `turn_end`/`idle`/`timeout` 如实返回。
3. `openhub_terminal_read_output` 返回剥离 ANSI 后的 scrollback 尾部，超长截断标注。
