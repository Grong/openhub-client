...
**Tech Stack:** Rust（`openhub-api-types` 新纯函数模块；`openhub-orchestrator` run_service；`openhub-gateway` caps_orchestrator），cargo-nextest。无 FE、无迁移。

## Global Constraints

- **Rust 测试**：`cargo nextest run -p openhub-api-types -p openhub-orchestrator -p openhub-gateway`。无 `| tail`；`cargo check` 不编译 test（用 `--tests` 或直接 nextest）。
- **跨 crate 编译（Phase 1 教训）**：本相**不加** `CapabilityProfile`/`FleetMember` 字段（只填既有 `modalities` 字段），故跨 crate 构造点**不破**。但改动涉及 api-types + orchestrator + gateway 三 crate，收尾必跑 `cargo check -p openhub-app`（下游消费者）。若最终仍决定加字段，则 api-types/orchestrator/gateway 所有 `CapabilityProfile{...}`/`FleetMember{...}` 字面量构造点都要补（勘察已列全 ~14 处）——**本计划刻意避免加字段以免这个面**。
...
### Task 1: 移植「模型名→modalities」启发式到 openhub-api-types

**Files:**
- Create: `crates/backend/openhub-api-types/src/model_capability.rs`
- Modify: `crates/backend/openhub-api-types/src/lib.rs`（`mod model_capability;` + re-export）
...
Run: `cargo nextest run -p openhub-api-types infer_model_modalities`
...
Run: `cargo nextest run -p openhub-api-types`
Expected: PASS（3 新测试绿 + 既有不回归）。

- [ ] **Step 5: 提交**
```bash
git add crates/backend/openhub-api-types/src/model_capability.rs crates/backend/openhub-api-types/src/lib.rs
...
**Files:**
- Modify: `crates/backend/openhub-orchestrator/src/run_service.rs`（`infer_model_capability` 填 modalities）
- Modify: `crates/backend/openhub-api-types/src/orchestrator.rs`（**不改** `derive_capability` 签名；见下——modality 在调用点合并）
- Modify: `crates/backend/openhub-gateway/src/caps_orchestrator.rs`（`derive_role_member` 合并 modality；`build_assistant_members` 裸装饰成员填 profile）
- Test: `run_service.rs` mod tests（端到端视觉路由/veto）+ 各填充点单测

**Interfaces:**
- Consumes: `openhub_api_types::infer_model_modalities`（Task 1）。
- 三填充点：
...
Run: `cargo nextest run -p openhub-orchestrator infer_model_capability_sets_vision_modality`
...
fn infer_model_capability(model: &str) -> Option<CapabilityProfile> {
    ...
    let modalities = openhub_api_types::infer_model_modalities(model);
...
```rust
    for md in openhub_api_types::infer_model_modalities(&model) {
...
```rust
    capability_profile: Some(CapabilityProfile {
        strengths: Vec::new(),
        modalities: openhub_api_types::infer_model_modalities(model),
        tools: false,
...
```
（`CapabilityProfile` 已在 caps_orchestrator import；若未 import 则加 `use openhub_api_types::CapabilityProfile;`。）

- [ ] **Step 5: 运行测试 + 跨 crate 校验**

Run: `cargo nextest run -p openhub-orchestrator -p openhub-gateway -p openhub-api-types`
...
- [ ] **Step 6: 提交**
```bash
git add crates/backend/openhub-orchestrator/src/run_service.rs crates/backend/openhub-gateway/src/caps_orchestrator.rs
...
**4. 跨 crate：** 涉 api-types + orchestrator + gateway；因不加字段，构造点不破；收尾 `cargo check -p openhub-app` 兜底。测试跑 `-p openhub-api-types -p openhub-orchestrator -p openhub-gateway`。
