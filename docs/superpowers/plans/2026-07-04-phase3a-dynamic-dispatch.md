...
**Tech Stack:** Rust（`openhub-orchestrator` run_service/engine、`openhub-gateway` caps_orchestrator、`openhub-app` state.rs），cargo-nextest。无 FE、无迁移。

## Global Constraints

- **测试**：`cargo nextest run -p openhub-orchestrator -p openhub-gateway`；改 `RunOutcome` 后**必 `cargo check -p openhub-app`**（`compose_lead_receipt` 穷尽 match 跨 crate——Phase 1a 教训：`RunOutcome` 加变体致 openhub-app 不编译）。无 `| tail`。
...
Task 1 (add_tasks 原语; run_service.rs + engine.rs) ── 基础，串行
   ...
   └──> Task 3 (中途批次观测; engine.rs + openhub-app)     ──┘ → worktree 并行后合并
```
...
**Files:**
- Modify: `crates/backend/openhub-orchestrator/src/run_service.rs`（新增 `add_tasks`；置于 `plan_flat`(498)/`apply_adjusted_plan`(1516) 旁）
- Modify: `crates/backend/openhub-orchestrator/src/engine.rs`（新增 `RunEngine::add_tasks` 包装，仿 `adjust` 锁块 866-871 / `rerun_task` 762）
...
```
> `planned_task(title, spec)`：构 `PlannedTask`（`openhub_api_types`）最小字段（kind 默认 agent、depends_on 空）。若无 helper 则内联构造。`single_task_harness`/`drive_to_completion` Phase1a 已有。

- [ ] **Step 2: 运行确认失败**

Run: `cargo nextest run -p openhub-orchestrator add_tasks`
...
pub async fn add_tasks(&self, run_service: &crate::run_service::RunService,
    run_id: &str, tasks: Vec<openhub_api_types::PlannedTask>)
    -> Result<openhub_api_types::Run, AppError> {
...
Run: `cargo nextest run -p openhub-orchestrator`
Expected: PASS（2 新测试绿 + 既有 adjust/rerun/终态测试不回归）。
Run: `cargo check -p openhub-gateway -p openhub-app`
Expected: clean（本任务未改签名/枚举，下游不破——纯新增方法）。

- [ ] **Step 6: 提交**
```bash
git add crates/backend/openhub-orchestrator/src/run_service.rs crates/backend/openhub-orchestrator/src/engine.rs
git commit -m "feat(orch): add_tasks 运行时追加原语(reconcile 空删除+RunLocks 原子重臂)"
```

---

### Task 2: 网关单 run 收敛 + `openhub_run_add_tasks` 工具（依赖 Task 1；∥ Task 3）

**Files:**
- Modify: `crates/backend/openhub-gateway/src/caps_orchestrator.rs`（`read_conversation_run_id` 助手；`create`(194)/`spawn`(775) 收敛分支；`add_tasks` handler；`register`(1044) 加工具；测试固定名 fixups 1399/1430）
...
- Produces: `read_conversation_run_id`（读 `conv.extra["orchestrator_run_id"]`）；`run_is_appendable`（run 非 `cancelled` 且存在→可追加，终态 completed/failed 亦可追加）；工具 `openhub_run_add_tasks`（`AddTasksParams{tasks: Vec<SpawnTask>}`，run_id 由会话链接 run 解析非入参；`.deny_on(ORCHESTRATOR_DENY_SURFACES)`）。
...
- [ ] **Step 4: `create`/`spawn` 收敛分支 + `openhub_run_add_tasks` 工具注册 + handler。** 更新两处硬编码工具名测试（`orchestrator_tools_registered_and_visible_on_desktop` 1399、`orchestrator_tools_absent_on_remote_surface` 1430）加新名。
- [ ] **Step 5: 跑测试**：`cargo nextest run -p openhub-gateway -p openhub-orchestrator` 全绿；`cargo check -p openhub-app` clean。
- [ ] **Step 6: 提交** `feat(gateway): 单 run 收敛(已有 run 追加不重绑) + openhub_run_add_tasks 工具`。

---

### Task 3: 中途批次观测回执（∥ Task 2；engine + openhub-app）

**Files:**
- Modify: `crates/backend/openhub-orchestrator/src/engine.rs`（`RunOutcome::BatchProgress` 变体+as_str 120/149；run_loop 分支(d) 钩子 1479-1484；loop 局部 1011）
- Modify: `crates/backend/openhub-app/src/router/state.rs`（`compose_lead_receipt` 加 `BatchProgress` 臂 999——**必须，否则不编译**）
- Test: `engine.rs` mod tests
...
- [ ] **Step 5: `compose_lead_receipt` 加 BatchProgress 臂**（openhub-app state.rs:999，见 build sheet 文案：「[编排回执·进展] …刚完成一批节点…可 openhub_run_add_tasks 追加…不新建编排」）。
- [ ] **Step 6: 跑测试 + 跨 crate**：`cargo nextest run -p openhub-orchestrator` 全绿（既有 lead 测试不回归——批次回执是新增，注意计数型断言）；**`cargo check -p openhub-app` 必过**（BatchProgress 臂补齐）。
...
- **Task 2 ∥ Task 3 用 worktree 隔离并行**：二者文件不相交（Task2=gateway caps_orchestrator.rs；Task3=engine.rs 不同函数+openhub-app state.rs）。各在自己 worktree/分支实现+评审，再依次合回主分支。合并后跑一次复合 `cargo nextest -p openhub-orchestrator -p openhub-gateway` + `cargo check -p openhub-app` 确认交叉无碍。
...
**4. 跨 crate（Phase1a 教训）：** Task3 加 `RunOutcome` 变体 → openhub-app `compose_lead_receipt` 穷尽 match 必补臂，Step6 强制 `cargo check -p openhub-app`。Task1/2 无枚举/签名破坏。
