...
**Tech Stack:** Rust（workspace crates：openhub-common、openhub-config、openhub-providers、openhub-api-types、openhub-ai-agent、openhub-conversation）。

## Global Constraints

- 提交作者用 openhub：`git -c user.nameopenhubcommit ...`（复用配置邮箱）。**不加** `Co-Authored-By` 或 "Generated with Claude Code"。
- 仅内存：**不新增 DB 迁移、不写回 provider capabilities**。
- 本次剔除逻辑只覆盖 OpenAI 兼容 provider（`openhub-providers/src/openai.rs`，即报错来源）；Anthropic-family（`anthropic_shared::build_messages`）不在范围。
...
- 新错误码 `UserLlmProviderImageUnsupported` **不得**加入两处 `is_provider_fault`（`model_failover.rs` 与 `openhub-idmm/config.rs`），以免误触发换模型。
...
**Files:**
- Create: `crates/backend/openhub-common/src/vision_registry.rs`
- Modify: `crates/backend/openhub-common/src/lib.rs`（加 `pub mod` + re-export）
- Test: 同文件 `#[cfg(test)]`
...
创建 `crates/backend/openhub-common/src/vision_registry.rs`，先只放测试：
...
Run: `cargo test -p openhub-common vision_registry`
...
在 `crates/backend/openhub-common/src/lib.rs` 按其现有 `pub mod` 风格加：
...
```bash
git add crates/backend/openhub-common/src/vision_registry.rs crates/backend/openhub-common/src/lib.rs
git -c user.nameopenhubcommit -m "feat(common): 新增进程级 VisionUnsupportedRegistry(内存记忆模型不支持图片)"
```
...
**Files:**
- Modify: `crates/agent/openhub-config/src/compat.rs`（struct 字段 + merge + 默认 true 的 accessor）
- Test: 同文件 `#[cfg(test)]`
...
在 `crates/agent/openhub-config/src/compat.rs` 的 `#[cfg(test)]` 模块内加（若无测试模块则新建）：
...
Run: `cargo test -p openhub-config supports_image`
...
Run: `cargo test -p openhub-config supports_image`
Expected: 3 个测试 PASS。再 `cargo build -p openhub-config` 确认无破坏。

- [ ] **Step 5: 提交**

```bash
git add crates/agent/openhub-config/src/compat.rs
git -c user.nameopenhubcommit -m "feat(config): ProviderCompat 增加 supports_image 开关(默认 true)"
```
...
**Files:**
- Modify: `crates/agent/openhub-providers/src/openai.rs`（`build_messages` 用户图分支 line 102-137；`tool_images_user_message` 签名 line 358-376 及其 2 处调用 line 95、260）
- Test: 同文件 `#[cfg(test)]`
...
在 `crates/agent/openhub-providers/src/openai.rs` 的 `#[cfg(test)]` 内加。用现有测试的构造习惯（参照现有 `build_messages` 测试；`ProviderCompat::default()` 即 supports_image=true）：
...
Run: `cargo test -p openhub-providers strips_user_image_when_supports_image_false keeps_user_image_when_supports_image_true`
...
fn tool_images_user_message(
    tool_use_id: &str,
    images: &[openhub_types::tool::ToolImage],
    supports_image: bool,
) -> Option<Value> {
...
Run: `cargo test -p openhub-providers`
Expected: 新增 2 测试 PASS，原有测试全绿（`build_messages` 签名未变，7 处测试调用点无需改）。

- [ ] **Step 5: 提交**

```bash
git add crates/agent/openhub-providers/src/openai.rs
git -c user.nameopenhubcommit -m "feat(providers): OpenAI build_messages 依 supports_image 剔除图片(用户图+工具截图)"
```
...
**Files:**
- Modify: `crates/backend/openhub-api-types/src/agent_error.rs`（enum，line 12-51）
...
在 `crates/backend/openhub-api-types/src/agent_error.rs` 的 `#[cfg(test)]`（若无则新建）加：
...
Run: `cargo test -p openhub-api-types image_unsupported_serde_roundtrip`
Expected: 编译失败（变体不存在）。
...
Run: `cargo test -p openhub-api-types image_unsupported_serde_roundtrip`
Expected: PASS。再 `cargo build -p openhub-api-types`。

- [ ] **Step 5: 提交**

```bash
git add crates/backend/openhub-api-types/src/agent_error.rs
git -c user.nameopenhubcommit -m "feat(api-types): AgentErrorCode 新增 UserLlmProviderImageUnsupported"
```
...
**Files:**
- Modify: `crates/backend/openhub-ai-agent/src/protocol/send_error.rs`（在通用 invalid_request 分支 line 620 **之前**插入 image 分支）
- Test: `send_error.rs` 的 `#[cfg(test)]` + `crates/backend/openhub-conversation/src/model_failover.rs` 的 `#[cfg(test)]`
...
Run: `cargo test -p openhub-ai-agent classifies_image_unsupported_from_serde_variant_error`
Expected: FAIL（当前 image 报错命中通用 invalid_request 分支 → `UserLlmProviderInvalidRequest`）。
Run: `cargo test -p openhub-conversation image_unsupported_is_not_provider_fault`
...
Run: `cargo test -p openhub-ai-agent classifies_image_unsupported_from_serde_variant_error plain_invalid_request_still_classifies_as_invalid_request`
Expected: 均 PASS。
Run: `cargo test -p openhub-conversation image_unsupported_is_not_provider_fault`
Expected: PASS。
...
```bash
git add crates/backend/openhub-ai-agent/src/protocol/send_error.rs crates/backend/openhub-conversation/src/model_failover.rs
git -c user.nameopenhubcommit -m "feat(ai-agent): 分类 image 类 400 为 UserLlmProviderImageUnsupported(排除于 provider fault)"
```
...
**Files:**
- Modify: `crates/backend/openhub-ai-agent/src/types.rs`（`OpenHubCompatOverrides` line 51-55 加字段）
- Modify: `crates/backend/openhub-ai-agent/src/factory/provider_config.rs`（`resolve_provider_fields` 查 registry 设 override）
- Modify: `crates/backend/openhub-ai-agent/src/factory/openhub.rs`（`resolve_openhub_url_and_compat` 构造 `OpenHubCompatOverrides` 的字面量补 `supports_image`）
...
- Produces: `OpenHubCompatOverrides.supports_image: Option<bool>`
...
pub(crate) fn image_support_override(provider_id: &str, model: &str) -> Option<bool> {
    if openhub_common::VisionUnsupportedRegistry::global().is_unsupported(provider_id, model) {
        Some(false)
    } else {
        None
    }
...
Run: `cargo test -p openhub-ai-agent image_override_tests`
Expected: 编译失败（`image_support_override` 未定义）。

- [ ] **Step 3: 写最小实现**

`types.rs` 的 `OpenHubCompatOverrides`（line 51-55）加字段：
...
`openhub.rs` 里 `resolve_openhub_url_and_compat` 构造 `OpenHubCompatOverrides { ... }` 的字面量补一行 `supports_image: None,`（grep `OpenHubCompatOverrides {` 找全部字面量，逐个补；因 `Default` 已派生，也可改用 `..Default::default()`）。
...
```rust
    let (base_url, mut compat_overrides) =
        resolve_openhub_url_and_compat(&row.platform, &row.base_url, &provider, row.is_full_url);
...
Run: `cargo test -p openhub-ai-agent image_override_tests`
Expected: PASS。
Run: `cargo build -p openhub-ai-agent`
Expected: 编译通过；若 `OpenHubCompatOverrides { ... }` 字面量报"missing field supports_image"，逐个补 `supports_image: None`。
...
**Files:**
- Modify: `crates/backend/openhub-conversation/src/message_persistence.rs`（新增 `persist_images_stripped_tip`）
- Modify: `crates/backend/openhub-conversation/src/failover_seam.rs`（新增 `strip_images_and_rebuild`）
...
Run: `cargo test -p openhub-conversation images_stripped_tip_has_warning_type_and_source`
...
```rust
    ...
    pub(crate) async fn strip_images_and_rebuild(
        ...
        openhub_common::VisionUnsupportedRegistry::global().mark_unsupported(&pm.provider_id, &pm.model);
...
Run: `cargo test -p openhub-conversation images_stripped_tip_has_warning_type_and_source`
Expected: PASS。
Run: `cargo build -p openhub-conversation`
Expected: 通过（若 `MessageRow` 字段名/`status` 类型不符，按该文件 `persist_send_failure_tip` 的字段填写对齐）。

- [ ] **Step 5: 提交**

```bash
git add crates/backend/openhub-conversation/src/message_persistence.rs crates/backend/openhub-conversation/src/failover_seam.rs
git -c user.nameopenhubcommit -m "feat(conversation): 图片剔除会话提示 + 同模型剔图重建 helper"
```
...
**Files:**
- Modify: `crates/backend/openhub-conversation/src/service.rs`（send-loop：line 1807 附近加计数器；line 1859-1866 泛化抑制器；line 1928 后加 image 重跑分支）
                        ...
                        relay = relay.with_failover_suppressor(Arc::new(move |code| {
                            (failover_within_bound
                                ...
                                || (image_retry_available
                                    && code
                                        == openhub_api_types::AgentErrorCode::UserLlmProviderImageUnsupported)
                        }));
                    }
                }
```
...
```rust
                ...
                if image_strip_retries_done == 0
                    ...
                    && outcome.terminal.code()
                        == Some(openhub_api_types::AgentErrorCode::UserLlmProviderImageUnsupported)
                {
...
若 `service.rs` 未 `use openhub_api_types::AgentErrorCode;`，则用上面写的全路径即可（无需新增 use）。`info!` 已在文件在用。

- [ ] **Step 4: 编译 + 回归**

Run: `cargo build -p openhub-conversation`
Expected: 通过。
Run: `cargo test -p openhub-conversation`
Expected: 现有 failover/send-loop 测试全绿（抑制器泛化对非 image 场景等价于原 `is_provider_fault` 行为）。

- [ ] **Step 5: 提交**

```bash
git add crates/backend/openhub-conversation/src/service.rs
git -c user.nameopenhubcommit -m "feat(conversation): 发送环抑制 image 错误并同模型剔图重跑,会话不中断"
```
...
Expected: 通过。若某处 `OpenHubCompatOverrides` / `ProviderCompat` 字面量报缺字段，补 `supports_image: None` / `..Default::default()`。

- [ ] **Step 2: 相关 crate 测试**

Run: `cargo test -p openhub-common -p openhub-config -p openhub-providers -p openhub-api-types -p openhub-ai-agent -p openhub-conversation`
Expected: 全绿。
...
```bash
git add -A
git -c user.nameopenhubcommit -m "chore: 图片降级端到端编译收尾"
```
...
- **`is_provider_fault` 两处副本**（`model_failover.rs` + `openhub-idmm/config.rs`）本次**均不改**——新码不入表，故不触发换模型;Task 5 加了断言锁定该不变量。
