...
**Architecture:** 复用引擎+级联 FK+`openhub_file::list_workspace_level`；新增受保护路由(delete/rename/replan/browseWorkspace)；replan=clear(新)+plan(旧,不改)；per-task 时间戳=DTO 补列(列已存在)。
...
- 既有 `orchestrator_run_e2e` 4/4 + 触碰 crate 测试不回归。禁 `cargo fmt`;只跑触碰 crate;`openhub-app` 必编过。
- **禁合并 main**。分支 feat/multi-agent-orchestrator,HEAD 起点 base。

## File Structure
- `openhub-orchestrator/src/repository/orch_run.rs` + `sqlite_orch_run.rs`(delete_run / clear_run_tasks / UpdateRunParams.goal / task_row_to_dto 时间戳)
- `openhub-orchestrator/src/run_service.rs`(delete / rename / replan)
- `openhub-orchestrator/src/routes.rs`(DELETE /runs/{id}、PATCH /runs/{id}、POST /runs/{id}/replan、GET /runs/{id}/workspace)
- `openhub-api-types/src/orchestrator.rs`(RunTask +created_at/updated_at;ReplanRequest;RenameRequest)
...
- [ ] **Step 4: GREEN** nextest + `cargo build -p openhub-app` + e2e 4/4 + 前端 `cd ui && npm run typecheck`(0)。
...
- [ ] **Step 4: GREEN** nextest + `cargo build -p openhub-app` + e2e 4/4。
...
- [ ] **Step 4: GREEN** nextest + `cargo build -p openhub-app` + e2e 4/4 + 前端 typecheck0。
