...
**Architecture:** 把「向 PTY 提交文字并回车」下沉为 `openhub-terminal` 的共享原语（纯编码函数 + 服务方法），网关新增 `openhub_terminal_send`（发送并执行，可选等待+回执）与 `openhub_terminal_read_output`（读输出），伙伴系统提示词补终端操作指引，最后把 AutoWork 与 IDMM 各自的提交实现收编到共享原语。伙伴的权限面本就放行 terminal 域（Desktop surface + WORK profile），无需改门禁。
...
- **提交序列的两条铁律**（编码器必须同时满足，否则回归 bug）：
  - 单行文本（无内部 `\n`）绝不能用 bracketed-paste 包裹——包裹会「插入但不提交」，文本滞留输入框（见 `openhub-idmm/src/probe.rs:1126-1138` 注释）。单行一律 `raw + \r` 一次写。
  - 多行文本发给 agent TUI（claude/codex/gemini）时，提交 CR 必须与 paste-end 标记分成两次写、间隔 120ms——同批写的 CR 会被 paste-burst 检测吞掉（见 `openhub-requirement/src/orchestrator.rs:1053-1060` 注释）。
- **agent TUI 判定**用 `openhub_terminal::resolve_agent_family(program, args, backend).is_some()`（含 gemini）；**lifecycle 完成信号判定**用 `openhub_terminal::terminal_autowork_capable(command, args, backend)`（仅 claude/codex，gemini 无 hook）。二者不同，勿混用。
...
- **不放宽门禁**：新 send 能力 `DangerTier::Write` + `.deny_on(&[Surface::Channel])`（与 `openhub_terminal_write_input` 一致）；read 能力 `DangerTier::Read`。不新增审批门。
- **提交作者**：所有 commit 用 `--author=openhub<rika00@qq.com>"`，不加 Co-Authored-By。
- **前端不改**（伙伴写入经 `terminal.output` 全局广播已实时可见）。

---

### Task 1: 共享提交编码器（`openhub-terminal/src/submit.rs`）

**Files:**
- Create: `crates/backend/openhub-terminal/src/submit.rs`
- Modify: `crates/backend/openhub-terminal/src/lib.rs`（加 `pub mod submit;` 与 re-export）
...
在 `crates/backend/openhub-terminal/src/submit.rs` 写入（含实现骨架先留空以便测试先失败——本步只写测试，实现放 Step 3）：
...
Run: `cargo test -p openhub-terminal submit::tests`
...
在 `crates/backend/openhub-terminal/src/lib.rs` 的 `pub mod` 块加一行（紧跟 `pub mod state;` 之后按字母序）：
...
```bash
git add crates/backend/openhub-terminal/src/submit.rs crates/backend/openhub-terminal/src/lib.rs
git commit --author=openhub<rika00@qq.com>" -m "feat(terminal): 共享 PTY 提交编码器（bracketed-paste + 分离CR）"
```
...
**Files:**
- Modify: `crates/backend/openhub-terminal/src/service.rs`（新增 inherent method；在 tests 模块加测试）
...
Run: `cargo test -p openhub-terminal submit_text_`
...
```bash
git add crates/backend/openhub-terminal/src/service.rs
git commit --author=openhub<rika00@qq.com>" -m "feat(terminal): TerminalService::submit_text 按 agent 家族路由提交序列"
```
...
**Files:**
- Modify: `crates/backend/openhub-terminal/src/submit.rs`（加 `SettleReason` 枚举 + re-export）
- Modify: `crates/backend/openhub-terminal/src/lib.rs`（re-export `SettleReason`）
- Modify: `crates/backend/openhub-terminal/src/service.rs`（新增 `await_turn_settle` + 测试）
...
```rust
    ...
    async fn await_turn_settle_turn_end_via_lifecycle_for_agent_backend() {
        ...
        svc.with_terminal_lifecycle(srv.clone(), "openhub-core".into());

        // 进程用 cat，但声明 backend=claude → 被判为 lifecycle-capable，走 TurnEnd 路径。
        let request = openhub_api_types::CreateTerminalRequest {
            name: None,
...
Run: `cargo test -p openhub-terminal await_turn_settle_`
...
```bash
git add crates/backend/openhub-terminal/src/submit.rs crates/backend/openhub-terminal/src/lib.rs crates/backend/openhub-terminal/src/service.rs
git commit --author=openhub<rika00@qq.com>" -m "feat(terminal): await_turn_settle（lifecycle TurnEnd / idle 静默 双策略）"
```
...
**Files:**
- Modify: `crates/backend/openhub-terminal/src/service.rs`（新增 struct + method + 私有 tail helper + 测试）
- Modify: `crates/backend/openhub-terminal/src/lib.rs`（re-export `TerminalOutputTail`）
...
Run: `cargo test -p openhub-terminal read_output_tail_`
...
```bash
git add crates/backend/openhub-terminal/src/service.rs crates/backend/openhub-terminal/src/lib.rs
git commit --author=openhub<rika00@qq.com>" -m "feat(terminal): read_output_tail（去 ANSI 的 scrollback 尾部读回）"
```

---

### Task 5: 网关能力 `openhub_terminal_send`

**Files:**
- Modify: `crates/backend/openhub-gateway/src/caps_terminal_ext.rs`（params + handler + register + `openhub_terminal_write_input` 描述补注 + 测试）

**Interfaces:**
- Consumes: `deps.terminal_service.submit_text` / `await_turn_settle` / `read_output_tail`（Task 2/3/4）；`openhub_terminal::SettleReason`（Serialize）。
- Produces: 已注册的 `openhub_terminal_send` 能力（domain `terminal`, `DangerTier::Write`, `.deny_on(&[Surface::Channel])`）。
...
Run: `cargo test -p openhub-gateway submit_params_plain_text_no_base64`
...
struct SubmitTerminalParams {
    /// The terminal session id (from openhub_list_terminals).
    id: i64,
...
async fn submit_terminal(deps: Arc<GatewayDeps>, ctx: CallerCtx, p: SubmitTerminalParams) -> Value {
    ...
    if let Err(e) = deps.terminal_service.submit_text(p.id, &p.text).await {
        ...
        return json!({
            "error": e.to_string(),
            "hint": "if the session has exited, call openhub_terminal_relaunch first, then retry"
        });
    }
    if !p.wait {
        return ok(json!({"submitted": true, "id": p.id, "note": "text submitted; use openhub_terminal_read_output to see the result"}));
    }
...
在 `register` 函数内 `out.push(...)` 区加（放在 `openhub_terminal_write_input` 之后）：

```rust
    out.push(Capability::new::<SubmitTerminalParams, _, _>(
        CapabilityMeta::new(
            "openhub_terminal_send",
            "terminal",
            "Type text/a command into a terminal and RUN it (plain text, no base64, no manual newline — Enter and the agent-CLI paste sequence are handled). Optional wait=true returns settle_reason + output_tail. Preferred over openhub_terminal_write_input for sending commands.",
            DangerTier::Write,
        )
...
把现有 `openhub_terminal_write_input` 的描述串结尾追加一句（Modify 其 `CapabilityMeta::new` 的 summary）：

```
"...Powerful: can execute arbitrary commands in the running shell. For sending a command/prompt to run, prefer openhub_terminal_send (handles Enter + agent-CLI paste); use this for raw control bytes like Ctrl-C."
```
...
```bash
git add crates/backend/openhub-gateway/src/caps_terminal_ext.rs
git commit --author=openhub<rika00@qq.com>" -m "feat(gateway): openhub_terminal_send 能力（发送并执行，可选等待回执）"
```

---

### Task 6: 网关能力 `openhub_terminal_read_output` + 可见性守卫测试

**Files:**
- Modify: `crates/backend/openhub-gateway/src/caps_terminal_ext.rs`（params + handler + register + 测试）

**Interfaces:**
- Consumes: `deps.terminal_service.read_output_tail`（Task 4）。
- Produces: 已注册的 `openhub_terminal_read_output`（domain `terminal`, `DangerTier::Read`）。
...
```rust
    ...
    fn send_and_read_are_registered_and_desktop_visible_but_channel_denied() {
        ...
        for name in ["openhub_terminal_send", "openhub_terminal_read_output"] {
        ...
        assert!(
            !reg.tool_visible(crate::registry::Surface::Channel, "openhub_terminal_send"),
            "openhub_terminal_send must be denied on Channel"
        );
    }
```
...
Run: `cargo test -p openhub-gateway read_output_params_defaults`
...
在 `register` 区加（放在 `openhub_terminal_send` 之后）：

```rust
    out.push(Capability::new::<ReadTerminalOutputParams, _, _>(
        CapabilityMeta::new(
            "openhub_terminal_read_output",
            "terminal",
            "Read a terminal's recent output (ANSI-stripped scrollback tail) to see a command's result or diagnose. The terminal analogue of openhub_conversation_status.",
            DangerTier::Read,
        ),
...
Run: `cargo test -p openhub-gateway -- read_output_params_defaults send_and_read_are_registered`
Expected: 2 个测试 PASS。
...
```bash
git add crates/backend/openhub-gateway/src/caps_terminal_ext.rs
git commit --author=openhub<rika00@qq.com>" -m "feat(gateway): openhub_terminal_read_output 能力 + 伙伴可见性守卫测试"
```
...
**Files:**
- Modify: `crates/backend/openhub-companion/src/companion.rs`（`build_companion_system_prompt`，本地分支追加一段 + 测试）
...
在 `companion.rs` 的 `#[cfg(test)] mod tests`（或新建）加。注意 `build_companion_system_prompt` 需 `store`/`profile`——参照该文件已有 prompt 测试的构造方式；若无现成 helper，用最小 `CompanionProfileConfig`/`CompanionStore`（内存）。断言核心是「本地含 openhub_terminal_send，远程不含」：
...
```rust
    ...
    async fn local_prompt_teaches_terminal_tools_remote_does_not() {
        ...
        assert!(local.contains("openhub_terminal_send"), "local prompt must teach terminal send");
        assert!(local.contains("openhub_terminal_read_output"));

        let remote = build_companion_system_prompt(&store, &profile, Some("wecom"), false).await;
        assert!(!remote.contains("openhub_terminal_send"), "remote (IM) prompt must not teach terminal tools");
    }
```
...
Run: `cargo test -p openhub-companion local_prompt_teaches_terminal_tools`
Expected: FAIL（提示词不含 `openhub_terminal_send`）。
    ...
    if !remote {
        system.push_str(
            "\n\n## 操作终端会话\n\
             ...
             - openhub_list_terminals 看有哪些终端及状态(running/exited)；openhub_create_terminal 新建(preset: shell|claude|codex|gemini)。\n\
             - openhub_terminal_send(id, text) 把命令或一段话发进去并【直接执行】——不用自己补回车、不用 base64，agent CLI 的粘贴提交也已处理好；\
             要等它跑完并拿结果时带 wait=true(可选 timeout_secs)，会回执 settle_reason 与输出尾巴。\n\
             - openhub_terminal_read_output(id) 读终端最近输出(已去除控制符)，用来查看命令结果或排查。\n\
             - 目标终端已退出(exited)时先 openhub_terminal_relaunch 再发送；kill/delete 这类破坏性操作会要你确认后再做。\n\
...
```bash
git add crates/backend/openhub-companion/src/companion.rs
git commit --author=openhub<rika00@qq.com>" -m "feat(companion): 伙伴提示词补终端操作指引（本地会话）"
```
...
**Files:**
- Modify: `crates/backend/openhub-requirement/src/orchestrator.rs`（`terminal_submit_chunks` 删除、`submit_terminal_prompt` 改调共享编码器、`TERMINAL_SUBMIT_DELAY` 常量迁移引用 + 特征测试）

**Interfaces:**
- Consumes: `openhub_terminal::{encode_submit_chunks, SubmitChunks, TERMINAL_SUBMIT_DELAY}`（Task 1）。
...
```rust
    ...
    fn autowork_multiline_prompt_uses_paste_then_separate_cr() {
        use openhub_terminal::{encode_submit_chunks, SubmitChunks};
...
Run: `cargo test -p openhub-requirement autowork_multiline_prompt_uses_paste`
Expected: 依赖 `openhub-terminal` 的 re-export 存在则 PASS（本测试即基线）。若 `openhub-requirement` 未 re-export 到位报编译错，说明 Task 1 的 re-export 未生效——回查 lib.rs。

- [ ] **Step 3: 改调共享原语**

在 `orchestrator.rs`：
1. 删除私有 `fn terminal_submit_chunks(...)`（约 :1045-1051）。
2. 把常量 `const TERMINAL_SUBMIT_DELAY: Duration = Duration::from_millis(120);`（:34）改为引用共享常量：删除本地定义，改在使用处用 `openhub_terminal::TERMINAL_SUBMIT_DELAY`（或文件顶部 `use openhub_terminal::TERMINAL_SUBMIT_DELAY;`）。
...
async fn submit_terminal_prompt(
    ...
    match openhub_terminal::encode_submit_chunks(prompt, true) {
        openhub_terminal::SubmitChunks::PasteThenCr { paste, cr } => {
            driver.write_input(terminal_id, &paste).await?;
            sleep(openhub_terminal::TERMINAL_SUBMIT_DELAY).await;
            driver.write_input(terminal_id, &cr).await?;
        }
        openhub_terminal::SubmitChunks::Single(bytes) => {
...
Run: `cargo test -p openhub-requirement`
Expected: 全绿（含既有 AutoWork 测试）。

- [ ] **Step 5: 提交**

```bash
git add crates/backend/openhub-requirement/src/orchestrator.rs
git commit --author=openhub<rika00@qq.com>" -m "refactor(requirement): AutoWork 终端提交改用共享编码器（行为不变）"
```
...
**Files:**
- Modify: `crates/backend/openhub-idmm/src/probe.rs`（`encode_terminal_input` 删除、`inject` 改用共享编码器并处理 `PasteThenCr` + 测试）

**Interfaces:**
- Consumes: `openhub_terminal::{encode_submit_chunks, SubmitChunks, TERMINAL_SUBMIT_DELAY}`；`self.driver.describe`（拿 backend）。
...
Run: `cargo test -p openhub-idmm idmm_single_line_stays_raw_plus_cr`
...
```rust
    async fn inject(&self, action: &WakeAction) -> Result<(), AppError> {
        ...
        let is_agent = self
            ...
            .map(|b| openhub_terminal::enhance::resolve_agent_family(&b, &[], Some(&b)).is_some())
            .unwrap_or(false);
        // Track the payload's lines so the CLI's echo of them isn't re-detected.
        self.note_injection(&text);
        match openhub_terminal::encode_submit_chunks(&text, is_agent) {
            openhub_terminal::SubmitChunks::Single(bytes) => self
                .driver
            ...
            openhub_terminal::SubmitChunks::PasteThenCr { paste, cr } => {
                self.driver
                    ...
                    .await
                    .map_err(|e| AppError::Internal(format!("terminal inject failed: {e}")))?;
                tokio::time::sleep(openhub_terminal::TERMINAL_SUBMIT_DELAY).await;
                self.driver
...
> `resolve_agent_family` 需从 `openhub_terminal::enhance` 可达——它已在 `enhance.rs` `pub fn` 且 lib.rs `pub mod enhance`（`pub use enhance::{...resolve_agent_family...}`）已 re-export，故 `openhub_terminal::resolve_agent_family` 亦可用；二者取其一，编译不过则改用 re-export 路径 `openhub_terminal::resolve_agent_family`。

- [ ] **Step 4: 运行测试确认通过**

Run: `cargo test -p openhub-idmm`
Expected: 全绿（含既有 probe 测试；`recent_injections` 回显抑制按 text 行匹配，两次写不影响）。

- [ ] **Step 5: 提交**

```bash
git add crates/backend/openhub-idmm/src/probe.rs
git commit --author=openhub<rika00@qq.com>" -m "refactor(idmm): 终端注入改用共享编码器（单行不变，多行→agent 修正为分离CR）"
```
...
Run: `cargo test -p openhub-terminal -p openhub-gateway -p openhub-requirement -p openhub-idmm -p openhub-companion`
Expected: 全绿。若某 crate 名不符，用 `cargo test --workspace` 兜底（较慢）。

- [ ] **Step 2: 注册表不变量测试**

Run: `cargo test -p openhub-gateway registry`
Expected: `registry_builds_and_names_fit_mcp_limit`（新工具名 `openhub_terminal_send` 26 字、`openhub_terminal_read_output` 25 字，均 ≤ 42 预算且 ≤ 64 硬顶）、`registry_capability_count_floor`（新增 2 个能力，仍 ≥135）、`all_caps_modules_are_mod_declared_and_registered` 全 PASS。
...
启动应用，在伙伴聊天里说「在某终端里跑 `git status` 并把结果告诉我」，确认：伙伴 `openhub_list_terminals` → `openhub_terminal_send(wait=true)` → 回读结果，全程无需用户碰终端；shell 与 claude 终端回车均不被吞；exited 终端伙伴会先提示 relaunch。

- [ ] **Step 5: 收尾提交（若有 CHANGELOG/release note）**

```bash
git add -A
git commit --author=openhub<rika00@qq.com>" -m "chore: 伙伴终端操作闭环 验证收尾"
```
