...
**Architecture（关键裁定）：自主循环是 EMERGENT，不加后端循环、不加新状态、不加迁移。** Phase 3a 已把 run 终态 → `LeadReporter.report` → `steer_message` 无活跃回合时回退 `send_message` = **一个带完整网关工具的全新 lead agent 回合** → 它调 `openhub_run_add_tasks` → `add_tasks` 重臂 run → 又到终态 → 又回报……这个 `finish_run→report→master回合追加→re-arm→run_loop→finish_run` 循环已闭合且 race-safe。W7c 只需：(a) 让回执**感知 autonomy+目标+预算**（自评达成则收尾、未达成且有预算则追加、预算尽则收尾交用户）；(b) **硬预算闸**折进 `run_is_appendable`/两个网关追加口（LLM 绕不过的确定性 backstop）；(c) boot-resume 复用现成。W7d：worker 向父 run 追加**已可工作**（worker.extra.orchestrator_run_id = 父 run），唯一缺口是**无界深度**——加 `delegation_depth` 守卫（默认 2）。

**Tech Stack:** Rust（`openhub-app` state.rs、`openhub-orchestrator` engine.rs/worker.rs/run_service.rs、`openhub-gateway` caps_orchestrator.rs），cargo-nextest。无 FE、**无迁移**（复用 `orch_runs.total_tokens` + `COUNT(orch_run_tasks)` + 现有 `orch_run_tasks.pattern_config` JSON + 会话 `extra` JSON）。

## Global Constraints

- **测试**：`cargo nextest run -p openhub-orchestrator -p openhub-gateway`；改 compose_lead_receipt/reporter 后**必 `cargo check -p openhub-app`**（跨 crate）。无 `| tail`。
...
Task 1 (硬预算闸: run_is_appendable/网关追加口 + engine 常量) ── 基础,串行,先落(runaway backstop)
   ├──> Task 2 (自主提示: openhub-app state.rs reporter+compose_lead_receipt) ──┐ 文件不相交
...
**Files:**
- Modify: `crates/backend/openhub-orchestrator/src/engine.rs`（3 常量 AUTONOMOUS_TOKEN_BUDGET/MAX_TASKS/NO_PROGRESS_LIMIT）
- Modify: `crates/backend/openhub-gateway/src/caps_orchestrator.rs`（`run_is_appendable` 折进预算闸；两个追加口 `openhub_run_add_tasks` handler + `spawn` 收敛分支拒绝并返清晰错误）
...
- [ ] **Step 1: 写失败测试**：一个 autonomy=autonomous 的 run，`total_tokens` 或 task 数超上限 → `run_is_appendable` 返 false / `openhub_run_add_tasks` 返预算错误、不追加。用小常量或注入可配上限触发（若常量硬编码，测试构造超限 run 状态）。
- [ ] **Step 2: 运行确认失败。**
...
- [ ] **Step 5: 跑测试 + 跨 crate**：`cargo nextest -p openhub-gateway -p openhub-orchestrator` 绿；`cargo check -p openhub-app` clean。
...
**Files:**
- Modify: `crates/backend/openhub-app/src/router/state.rs`（`OrchestratorLeadReporter` 加 `run_repo: Arc<dyn IRunRepository>` 字段 + 装配 ~1159；`report` best-effort `run_repo.get_run`；`compose_lead_receipt` 加 autonomy/goal/budget-aware 分支 + in-memory 无进展 streak DashMap）
...
- Produces: autonomous run 的 `Completed`/`Failed` 回执改为**自评提示**（见 build sheet §1a）：给目标+本轮 brief+预算余量，指示「达成→汇总收尾不追加；未达成且有预算→`openhub_run_add_tasks` 追加继续；预算尽→汇总交用户不追加」。非 autonomous run 回执**不变**（现有「仅汇总」措辞）。无进展 streak（in-memory `DashMap<run_id,{last_done,streak}>`）达 `NO_PROGRESS_LIMIT` → 发收尾提示。
...
- [ ] **Step 5: `cargo check -p openhub-app` clean + `cargo nextest -p openhub-orchestrator`（若 reporter 测试在此）绿。**
...
**Files:**
- Modify: `crates/backend/openhub-orchestrator/src/worker.rs`（`build_worker_extra` +`delegation_depth: u32` 参数 + json 键 `orchestrator_delegation_depth`）
- Modify: `crates/backend/openhub-orchestrator/src/engine.rs`（dispatch 处读 task 的 `pattern_config.delegation_depth` 默认 0，传给 build_worker_extra）
- Modify: `crates/backend/openhub-gateway/src/caps_orchestrator.rs`（`read_conversation_delegation_depth`；两个追加口 depth 守卫 + 给追加 task 盖 `pattern_config.delegation_depth = caller+1`；`DELEGATION_DEPTH_LIMIT` 常量）
- Test: worker.rs + caps_orchestrator.rs mod tests

**Interfaces:**
- 前提（已工作，勿改）：worker 的 `extra.orchestrator_run_id` = **父 run**（workers 与 lead 共 run，仅 task_id 异）→ worker 调 `openhub_run_add_tasks` 追加同级到父 run。只读受限角色 `desktopGateway=false` 已禁。
...
- [ ] **Step 5: 跑测试 + 跨 crate**：`cargo nextest -p openhub-orchestrator -p openhub-gateway` 绿；`cargo check -p openhub-app` clean。
...
- Task 2 ∥ Task 3 worktree 隔离并行：Task2=openhub-app state.rs；Task3=worker.rs+engine.rs+caps_orchestrator.rs——文件不相交。**从 Task1 合入后的栈顶开 worktree（agent 须 reset 到该 base，勿默认 main）。** 各自评审后依次合并，合并后复合 `cargo nextest -p openhub-orchestrator -p openhub-gateway` + `cargo check -p openhub-app`。
...
**4. 跨 crate（Phase1a 教训）：** Task2 改 reporter(加字段,不改 trait 签名→零 impl churn)；Task1/3 改 caps+engine+worker。每任务 `cargo check -p openhub-app`。
