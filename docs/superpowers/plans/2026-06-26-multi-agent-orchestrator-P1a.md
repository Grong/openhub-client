...
- **仓库模式**：`I*Repository` trait（`repository/{name}.rs`）+ `Sqlite*Repository` impl（`repository/sqlite_{name}.rs`）+ repository/mod.rs 4 行 re-export + openhub-db/src/lib.rs 根 re-export（P0 已建此惯例，见 lib.rs 既有 fleet/workspace 根导出）；手写 sqlx；TEXT PK 显式 `generate_prefixed_id` 写入。
...
- **调度器确定性 + 串行**：P1 一次执行一个就绪任务（并行 P2）；引擎复刻 `openhub-requirement::Orchestrator`（state.rs build_requirement_state + orchestrator.rs 为权威模板）。
- **取消语义**：取消令活跃 worker 以 `Finish(Cancelled)` 收尾（沿用现有协作式取消，勿用 Error）。
- **依赖边表解阻塞**：复用 orch_run_task_deps（completion → check_unblocks）。
- **gateway**：caps_orchestrator 3 步契约（caps_orchestrator.rs register + lib.rs mod + build() 调用）+ GatewayDeps 字段 + routes.rs 装配；工具名 `openhub_` 前缀 ≤42 字符；权限经 DangerTier×Surface 集中 gate（handler 不自查）。
...
- **测试**：只跑触碰 crate（`cargo nextest run -p <crate>`）；引擎核心用 mock trait 单测，端到端用集成测试（mock agent 或 Mock AgentInstance）；**禁 cargo fmt**；app 必编过（`cargo build -p openhub-app`）。
...
**后端 — openhub-orchestrator（扩 crate）**
...
- 创建 `src/worker.rs`：`trait WorkerRunner` + `ConversationWorkerRunner`（openhub_agent_run 配方）+ `WorkerOutcome`。
...
**后端 — openhub-gateway**
- 创建 `src/caps_orchestrator.rs`（openhub_run_create/status/result）+ 修改 `src/lib.rs`（mod+build）+ `src/deps.rs`（orchestrator_run_service 字段）。
...
**Files:** Create `crates/backend/openhub-db/src/repository/orch_run.rs` + `sqlite_orch_run.rs`；Modify `repository/mod.rs`、`src/lib.rs`；Test 内联。
...
```
- Consumes: `OrchRunRow`/`OrchRunTaskRow`/`OrchRunTaskDepRow`/`OrchAssignmentRow`（P0 已建，`openhub_db::*`）、`generate_prefixed_id`、now-ms helper（同 P0 Task 2/3）。
...
- [ ] **Step 2: 跑确认失败** — `cargo nextest run -p openhub-db orch_run`。
- [ ] **Step 3: 写 trait + sqlite impl + mod.rs/lib.rs re-export**（仿 P0 fleet 仓库；list_ready_tasks 用上面 SQL）。
- [ ] **Step 4: 跑确认通过** + `cargo build -p openhub-db`。
...
**Files:** Modify `crates/backend/openhub-api-types/src/orchestrator.rs` + `lib.rs`；Test 内联。
...
// 规划产物(PlanProducer 输出 + openhub_run_plan 入参):
...
**Files:** Create `crates/backend/openhub-orchestrator/src/events.rs`；Modify `src/lib.rs`；Create `ui/src/common/types/orchestrator/orchestratorEvents.ts`（手写镜像）；Test 内联。
...
```
- Consumes: `openhub_realtime::EventBroadcaster`、`WebSocketMessage::new(name, serde_json::Value)`。
- 前端镜像（手写，team 先例 lighter-weight，非 ts-rs）：`orchestratorEvents.ts` 定义各事件 payload 类型 `{ run_id, task_id?, status?, member_id? }`。

参照模板：`crates/backend/openhub-cron/src/events.rs`（CronEventEmitter 结构）；`ui/src/common/types/team/teamTypes.ts`（手写事件类型）。
...
- [ ] **Step 2: 跑确认失败** — `cargo nextest run -p openhub-orchestrator events`。
...
**Files:** Create `crates/backend/openhub-orchestrator/src/plan.rs`；Modify `src/lib.rs`；Test 内联（mock）。
...
```
- Consumes: `FleetMember`/`PlannedDag` DTO；`one_shot_completion`（`openhub-ai-agent` factory/provider_config.rs，P0 agent-model 探查确认存在；签名以该文件为准）；`ProviderWithModel`。
...
- [ ] **Step 2: 跑确认失败** — `cargo nextest run -p openhub-orchestrator plan`。
...
**Files:** Create `crates/backend/openhub-orchestrator/src/worker.rs`；Modify `src/lib.rs`；Test 内联（mock + 真实路径对 Mock AgentInstance）。
...
参照模板：`crates/backend/openhub-gateway/src/caps_conversation.rs`（agent_run/await_turn/read_final_text/latest_assistant_text 全套）。
...
**Files:** Create `crates/backend/openhub-orchestrator/src/run_service.rs` + `src/engine.rs`；Modify `src/lib.rs`、`src/state.rs`（OrchestratorRouterState 加 run_service+engine）；Test 内联（mock PlanProducer + mock WorkerRunner）。
...
- 引擎注册表/start/stop/generation/HandleGuard/resume 复刻 `openhub-requirement::Orchestrator`（orchestrator.rs:142-363）。**P1 串行**：run_loop 一次跑一个 ready task（不 spawn 多 worker）。

参照模板：`crates/backend/openhub-requirement/src/orchestrator.rs`（Orchestrator/start/run_loop/resume）；`team scheduler` check_unblocks。
...
- [ ] **Step 2: 跑确认失败** — `cargo nextest run -p openhub-orchestrator engine`。
...
**Files:** Create `crates/backend/openhub-gateway/src/caps_orchestrator.rs`；Modify `src/lib.rs`（mod+build）、`src/deps.rs`（字段）；Test（注册不变量在既有 registry 自测中自动覆盖；本任务加 1 个 caps 计数/命名断言若有既有测试文件）。

**Interfaces produced:** 工具（读为主，写经 RunService/RunEngine）：
- `openhub_run_create`（Write）：入 {workspace_id, goal, fleet_id, autonomy?} → RunService.create + plan + engine.start → 返 run_id。
- `openhub_run_status`（Read）：入 {run_id} → RunDetail 精简（run.status + 各 task status）。
- `openhub_run_result`（Read）：入 {run_id} → run.summary + 各 task output_summary（未完返 status=running）。
...
- GatewayDeps 加 `pub orchestrator_run_service: Arc<openhub_orchestrator::RunService>` + `pub orchestrator_run_engine: Arc<openhub_orchestrator::RunEngine>`（或一个 `OrchestratorRunFacade`）。

参照模板：`crates/backend/openhub-gateway/src/caps_agent.rs`/`caps_companion.rs`（register fn + Capability::new::<P,_,_> + CapabilityMeta::new）；lib.rs build()。

- [ ] **Step 1: 写注册断言测试（失败优先）** — 若 gateway 有 registry 自测(如 `registry_builds_and_names_fit_mcp_limit`)，确认新工具被收录且名 ≤42；否则加一个最小测试断言 `Registry::global().tool_visible(Surface::Desktop, "openhub_run_status")`。
- [ ] **Step 2: 跑确认失败** — `cargo nextest run -p openhub-gateway orchestrator`（或相应测试名）。
- [ ] **Step 3: 实现 caps_orchestrator.rs + lib.rs mod/build + deps.rs 字段**。注意 caps 计数 floor 自测可能需更新阈值。
- [ ] **Step 4: 跑确认通过** + `cargo build -p openhub-gateway`。
...
**Files:** Modify `crates/backend/openhub-orchestrator/src/routes.rs` + `src/state.rs`（OrchestratorRouterState 已在 Task 6 加 run_service+engine）；Test 内联 router-builds。
...
参照模板：P0 `openhub-orchestrator/src/routes.rs`（fleet/workspace 路由形态）。
...
**Files:** Modify `crates/backend/openhub-app/src/router/state.rs`（`build_orchestrator_run_state` 扩 `build_orchestrator_state`）、`src/router/routes.rs`（GatewayDeps）；Create `crates/backend/openhub-app/tests/orchestrator_run_e2e.rs`。
...
- [ ] **Step 2: 跑确认失败** — `cargo nextest run -p openhub-app orchestrator_run`。
- [ ] **Step 3: 实现 build_orchestrator_run_state 扩展 + GatewayDeps 装配**。
- [ ] **Step 4: `cargo build -p openhub-app`（关键闸）+ 跑集成测试确认通过**。
