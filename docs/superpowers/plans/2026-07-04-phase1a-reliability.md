...
**Tech Stack:** Rust（`openhub-orchestrator`、`openhub-db`），cargo-nextest；少量 FE（`runStatusMeta.ts` + i18n）。

## Global Constraints

- **Rust 测试**：只跑触碰 crate：`cargo nextest run -p openhub-orchestrator`；加迁移后另跑 `cargo nextest run -p openhub-db`。避免 `| tail` 掩盖退出码；`cargo check` 不编译 test。
- **迁移**：下一个号 = **029**（最新为 028）。`db_lifecycle.rs` 自动从 `_sqlx_migrations` 推导版本、`sqlx::migrate!()` 构建期发现目录——**无需手改 db_lifecycle 里的硬编码计数**；加完 029 跑 `cargo nextest run -p openhub-db` 验证。提交前 `git pull --rebase` 防撞号。（[[pull-before-commit]]）
...
**Files:**
- Modify: `crates/backend/openhub-orchestrator/src/worker.rs`（`WorkerRunner` trait 加 `last_error_present`；prod impl；`RetryMockWorkerRunner` 覆写以保留既有语义——见 Interfaces）
- Modify: `crates/backend/openhub-orchestrator/src/engine.rs`（常量 `DEFAULT_MAX_TIMEOUT_RETRIES`；`RunEngineDeps.max_timeout_retries` + 默认；重写 `settle_failed_or_retry`）
...
Run: `cargo nextest run -p openhub-orchestrator timeout_without_marker`
...
Run: `cargo nextest run -p openhub-orchestrator retry ; cargo nextest run -p openhub-orchestrator lead`
Expected: 全绿。
...
**Files:**
- Create: `crates/backend/openhub-db/migrations/029_task_on_fail.sql`
- Modify: `crates/backend/openhub-db/src/models/orchestrator.rs`（`OrchRunTaskRow` 加 `on_fail`）
- Modify: `crates/backend/openhub-db/src/repository/sqlite_orch_run.rs`（task SELECT 补 `on_fail`；`UpdateTaskParams` 若需写入则补——本任务只读 `on_fail`，创建时由 plan 落库，见下）
- Modify: `crates/backend/openhub-orchestrator/src/engine.rs`（永久失败时按策略 `skip_downstream`；终态判定分 `completed_with_failures`）
- Modify: `crates/backend/openhub-orchestrator/src/run_service.rs`（3 处 re-arm 终态门 加 `completed_with_failures`）
- Modify: `crates/backend/openhub-db/src/repository/`（persist_dag 落 `on_fail`——若 plan 提供）
...
Run: `cargo nextest run -p openhub-db`
...
Run: `cargo nextest run -p openhub-orchestrator ; cargo nextest run -p openhub-db`
...
- [ ] **Step 8: 提交**
```bash
git add crates/backend/openhub-db crates/backend/openhub-orchestrator ui/src/renderer/pages/orchestrator ui/src/renderer/services/i18n/locales
...
**Files:**
- Modify: `crates/backend/openhub-orchestrator/src/engine.rs`（`RunOutcome` 加 `NodeFailed`；`settle_failed_or_retry` else 分支中途回执）
- Test: `engine.rs` mod tests
...
Run: `cargo nextest run -p openhub-orchestrator permanent_node_failure_reports_to_lead_midrun`
...
**Files:**
- Modify: `crates/backend/openhub-orchestrator/src/plan.rs`（`PLAN_SYSTEM` 追加收尾验收规则）
...
Run: `cargo nextest run -p openhub-orchestrator plan_system_teaches_closing_acceptance_gate`
...
Run: `cargo nextest run -p openhub-orchestrator plan`
Expected: PASS（新测试绿；既有 `plan_system_teaches_verify_pattern`、`parse_plan_*` 不回归）。

- [ ] **Step 5: 提交**
```bash
git add crates/backend/openhub-orchestrator/src/plan.rs
