...
**Architecture:** 新增受保护 REST `POST /api/orchestrator/runs/adhoc` → 复用 `RunService::create_adhoc`；`caps_orchestrator.openhub_run_create` 改为显式入参（不再读调用会话 extra、不再回写 lead extra）。引擎/Router/worker 逻辑零改。
...
- 后端禁 `cargo fmt`；只跑触碰 crate；`openhub-app` 必编过。
...
## File Structure
- `crates/backend/openhub-orchestrator/src/routes.rs`（+ `POST /runs/adhoc` handler `create_adhoc_run`）
- `crates/backend/openhub-gateway/src/caps_orchestrator.rs`（openhub_run_create 显式参数 + 去 lead 读写）
...
**Files:** Modify `openhub-orchestrator/src/routes.rs`、`ui/src/common/adapter/ipcBridge.ts`、`ui/src/common/types/orchestrator/orchestratorTypes.ts`；测试内联（路由测试仿 `list_my_runs` 的受保护层测试）。
...
- [ ] **Step 4: GREEN** `cargo nextest run -p openhub-orchestrator` + `cargo build -p openhub-app` + `cargo nextest run -p openhub-app -E 'binary(orchestrator_run_e2e)'`(4/4) + 前端 `cd ui && npm run typecheck`(0)。
...
**Files:** Modify `openhub-gateway/src/caps_orchestrator.rs`；测试内联。

**背景(现状)：** 会话融合期 `openhub_run_create` 改为读调用会话 extra(orchestrator_role/lead/model_range/workspace) + 回写 `orchestrator_run_id` 到 lead 会话。lead-conversation 概念现移除。
...
- `openhub_run_status`/`openhub_run_result` 不变(仍 deny_on Remote,保留)。
...
- [ ] **Step 4: GREEN** `cargo nextest run -p openhub-gateway -p openhub-orchestrator` + `cargo build -p openhub-app` + e2e 4/4。
- [ ] **Step 5: 提交** `git commit -m "refactor(orchestrator): caps openhub_run_create 改显式参数(移除 lead-conversation 依赖)"`
