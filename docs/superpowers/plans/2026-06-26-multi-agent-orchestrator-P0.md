...
**Goal:** 落地「智能编排」功能的基础层：新 crate `openhub-orchestrator` + 迁移 018（全部 8 张表）+ 仓库 + 编队(Fleet)/工作间(Workspace) CRUD 后端 + 前端侧栏 tab/路由/页面壳 + 编队管理 UI。交付后用户可在真实 app 内创建/编辑/删除编队与工作间。

**Architecture:** 镜像 `openhub-webhook`（最小域 crate 模板）+ `openhub-requirement`（持久服务）的分层。后端：迁移 → Row 模型 → 仓库(trait+sqlite impl) → api-types DTO → 域 crate(service/routes/state) → app 接线。前端：ipcBridge REST 客户端 → 侧栏 entry+路由+i18n → ContentSider 三段页壳 → 编队管理卡片网格。P0 **不**碰 Run 执行引擎/主管/调度器/画布（那是 P1–P3），但迁移 018 一次性建好全部 8 张表（schema 在 spec 已定，避免迁移churn）。
...
- **域 crate 锚定模板** = `crates/backend/openhub-webhook`（Cargo.toml / lib.rs / routes.rs / state.rs / service.rs / error.rs 的形状照搬）。
- **路由层薄**：handler 只做 extract（`State` + `Extension<CurrentUser>` + `Path`/`Json`）→ 调 service → `ApiResponse` 包装；逻辑全在 service；service **无** axum import，方法返回 `Result<_, AppError>`；auth 中间件在 openhub-app 外层施加。
...
- **品牌**：字样 OpenHub（非 Openhub）；用户可见名「智能编排」；内部标识符 `orchestrator`/`fleet` 不动。
- **提交**：每任务末尾 commit（feature 分支 `feat/multi-agent-orchestrator`，已建）；提交前 `git pull --rebase`（多端并行，注意迁移号撞号）。

## File Structure（P0 创建/修改）

**后端**
- 创建 `crates/backend/openhub-db/migrations/018_orchestrator.sql` — 全 8 表 DDL。
- 创建 `crates/backend/openhub-db/src/models/orchestrator.rs` — 8 个 `*Row` 结构。
- 修改 `crates/backend/openhub-db/src/models/mod.rs` — `mod orchestrator;` + 8 个 `pub use`。
- 创建 `crates/backend/openhub-db/src/repository/orch_fleet.rs` + `sqlite_orch_fleet.rs` — `IFleetRepository`（含 fleet + fleet_members）。
- 创建 `crates/backend/openhub-db/src/repository/orch_workspace.rs` + `sqlite_orch_workspace.rs` — `IOrchWorkspaceRepository`。
- 修改 `crates/backend/openhub-db/src/repository/mod.rs` — 各 4 行 re-export。
- 创建 `crates/backend/openhub-api-types/src/orchestrator.rs` — DTO（Fleet/FleetMember/Workspace + Create/Update + CapabilityProfile/MemberConstraints）。
- 修改 `crates/backend/openhub-api-types/src/lib.rs` — `pub mod orchestrator;` + re-export。
- 创建 crate `crates/backend/openhub-orchestrator/`：`Cargo.toml`、`src/lib.rs`、`src/error.rs`、`src/state.rs`、`src/service.rs`（FleetService + WorkspaceService）、`src/routes.rs`。
- 修改根 `Cargo.toml` — 加 `openhub-orchestrator` workspace member + `[workspace.dependencies]` 条目。
- 修改 `crates/backend/openhub-app/src/router/state.rs` — `use` + `ModuleStates` 字段 + `build_orchestrator_state` + `build_module_states` 调用。
- 修改 `crates/backend/openhub-app/src/router/routes.rs` — `use openhub_orchestrator::orchestrator_routes;` + `let orchestrator_authenticated = ...` + `.merge(orchestrator_authenticated)`。
...
**Files:**
- Create: `crates/backend/openhub-db/migrations/018_orchestrator.sql`
- Create: `crates/backend/openhub-db/src/models/orchestrator.rs`
- Modify: `crates/backend/openhub-db/src/models/mod.rs`
- Test: `crates/backend/openhub-db/src/models/orchestrator.rs`（`#[cfg(test)]` 内联）

**Interfaces:**
- Consumes: `openhub_common::TimestampMs`（= i64）；`sqlx::FromRow`；现有 `init_database_memory()`（跑全部迁移于内存 DB）。
...
参照模板：`crates/backend/openhub-db/migrations/017_cron_job_runs.sql`（迁移格式）、`crates/backend/openhub-db/src/models/webhook.rs`（Row 形状）、`001_baseline.sql` 头部注释（ID 规则）。

- [ ] **Step 1: 写迁移 SQL**

`crates/backend/openhub-db/migrations/018_orchestrator.sql`（照抄 spec §5 DDL，全 8 表——注意 spec §5 列了 7 个 CREATE TABLE，本步全部落地）：
...
`crates/backend/openhub-db/src/models/orchestrator.rs`（每个 Row `#[derive(Debug, Clone, sqlx::FromRow, serde::Serialize, serde::Deserialize)]`，时间戳用 `openhub_common::TimestampMs`，可空列用 `Option<...>`，JSON 列存原始 `Option<String>`）。P0 必须的三个示例（其余 4 个同构照建）：

```rust
use openhub_common::TimestampMs;
...
在 `crates/backend/openhub-db/src/models/mod.rs` 加（仿现有行）：
```rust
mod orchestrator;
...
在 `crates/backend/openhub-db/src/models/orchestrator.rs` 底部加：
```rust
#[cfg(test)]
mod tests {
...
```
> 若 `init_database_memory` 的确切签名/返回不同，对照 `crates/backend/openhub-db/src/database.rs` 调整（它跑 `sqlx::migrate!()`）。

- [ ] **Step 5: 跑测试看它先失败再通过**

Run: `cargo nextest run -p openhub-db migration_018`
Expected：先因迁移未建/Row 未声明失败 → 实现后 PASS。

- [ ] **Step 6: 提交**

```bash
git add crates/backend/openhub-db/migrations/018_orchestrator.sql crates/backend/openhub-db/src/models/orchestrator.rs crates/backend/openhub-db/src/models/mod.rs
...
**Files:**
- Create: `crates/backend/openhub-db/src/repository/orch_fleet.rs`（trait `IFleetRepository`）
- Create: `crates/backend/openhub-db/src/repository/sqlite_orch_fleet.rs`（`SqliteFleetRepository`）
- Modify: `crates/backend/openhub-db/src/repository/mod.rs`
- Test: `sqlite_orch_fleet.rs` `#[cfg(test)]` 内联

**Interfaces:**
- Consumes: `FleetRow`、`FleetMemberRow`（Task 1）；`SqlitePool`；`openhub_common::generate_prefixed_id`、`now_ms()`（对照 `openhub-common`）。
- Produces:
```rust
...
参照模板：`crates/backend/openhub-db/src/repository/webhook.rs` + `sqlite_webhook.rs`（trait+impl 形状、手写 SQL、`generate_prefixed_id`）。
...
- [ ] **Step 2: 跑测试确认失败** — `cargo nextest run -p openhub-db fleet_crud` → FAIL（类型未定义）。
...
- [ ] **Step 6: 跑测试确认通过** — `cargo nextest run -p openhub-db fleet_crud` → PASS。
...
**Files:**
- Create: `crates/backend/openhub-db/src/repository/orch_workspace.rs` + `sqlite_orch_workspace.rs`
- Modify: `crates/backend/openhub-db/src/repository/mod.rs`
...
- [ ] **Step 2: 跑确认失败** — `cargo nextest run -p openhub-db orch_workspace`。
...
**Files:**
- Create: `crates/backend/openhub-api-types/src/orchestrator.rs`
- Modify: `crates/backend/openhub-api-types/src/lib.rs`
...
```
- Consumes: `serde`；`Option<Option<T>>` patch 字段照搬 `openhub-api-types` 既有 `double_option` deserializer（见模板文件，role: "double_option"）。

参照模板：`crates/backend/openhub-api-types/src/webhook.rs`（或 conversation DTO）。
...
- [ ] **Step 2: 跑确认失败** — `cargo nextest run -p openhub-api-types orchestrator`。
...
## Task 5: openhub-orchestrator crate + FleetService

**Files:**
- Create: `crates/backend/openhub-orchestrator/Cargo.toml`、`src/lib.rs`、`src/error.rs`、`src/state.rs`、`src/service.rs`
...
参照模板：`crates/backend/openhub-webhook/src/{service.rs,error.rs,state.rs,lib.rs}` + `Cargo.toml`。

- [ ] **Step 1: 建 Cargo.toml + 根 workspace 接线**

`crates/backend/openhub-orchestrator/Cargo.toml`（仿 webhook，依赖 common/db/api-types/auth + axum/tokio/serde_json/thiserror/tracing/async-trait；dev-dep tokio test-util + sqlx）。根 `Cargo.toml`：`members` 数组加 `"crates/backend/openhub-orchestrator"`（注：成员是 glob `crates/backend/*` 则自动包含，确认后决定是否需手加）；`[workspace.dependencies]` 加 `openhub-orchestrator = { path = "crates/backend/openhub-orchestrator" }`。
...
- [ ] **Step 3: 跑确认失败** — `cargo nextest run -p openhub-orchestrator fleet_service`。
...
- [ ] **Step 5: 跑确认通过 + 全 crate 编译** — `cargo nextest run -p openhub-orchestrator` + `cargo build -p openhub-orchestrator`。
...
**Files:**
- Modify: `crates/backend/openhub-orchestrator/src/service.rs`（加 `WorkspaceService`）
- Test: 内联
...
- [ ] **Step 2: 跑确认失败** — `cargo nextest run -p openhub-orchestrator workspace_service`。
...
**Files:**
- Create: `crates/backend/openhub-orchestrator/src/routes.rs`
- Modify: `crates/backend/openhub-orchestrator/src/state.rs`（`OrchestratorRouterState { fleet: FleetService, workspace: WorkspaceService }`）、`src/lib.rs`（re-export `orchestrator_routes`、`OrchestratorRouterState`）
...
参照模板：`crates/backend/openhub-webhook/src/routes.rs`（handler 形状、JsonRejection→BadRequest、ApiResponse 包装、201 CREATED）。
...
- [ ] **Step 2: 跑确认失败** — `cargo nextest run -p openhub-orchestrator routes`。
...
**Files:**
- Modify: `crates/backend/openhub-app/src/router/state.rs`（`use openhub_orchestrator::OrchestratorRouterState;`；`ModuleStates` 加 `pub orchestrator: OrchestratorRouterState,`；`build_orchestrator_state(services) -> OrchestratorRouterState`；`build_module_states` 加 `orchestrator: build_orchestrator_state(services),`）
- Modify: `crates/backend/openhub-app/src/router/routes.rs`（`use openhub_orchestrator::orchestrator_routes;`；`create_router_with_states` 加 `let orchestrator_authenticated = orchestrator_routes(states.orchestrator.clone()).route_layer(from_fn_with_state(auth_mw_state.clone(), auth_middleware));` + `.merge(orchestrator_authenticated)`）
- Modify: `crates/backend/openhub-app/Cargo.toml`（`openhub-orchestrator.workspace = true`）
- Test: `crates/backend/openhub-app/tests/`（新 `orchestrator_e2e.rs` 或加入既有集成测试）
...
参照模板：`crates/backend/openhub-app/src/router/state.rs` 的 `build_webhook_state`；`routes.rs` 的 webhook merge 行；现有集成测试如何起 router + 注入测试 auth。
...
- [ ] **Step 2: 跑确认失败** — `cargo nextest run -p openhub-app orchestrator`。
- [ ] **Step 3: 写 build_orchestrator_state + ModuleStates 字段 + use（state.rs）**。
- [ ] **Step 4: 写 routes.rs merge + use + Cargo.toml dep**。
- [ ] **Step 5: 全量后端编译** — `cargo build -p openhub-app`（关键闸：app 必须编过）。
- [ ] **Step 6: 跑集成测试确认通过** — `cargo nextest run -p openhub-app orchestrator`。
...
**Interfaces:**
- Consumes: `ContentSider`、`useResizableSplit`（独立 storageKey `openhub:orchestrator-sider-width`）、`useLayoutContext`（isMobile→SegmentedTabs）、`useSearchParams`、`ipcBridge.orchestrator`（Task 9）、`useSWR`。
...
- [ ] **Step 6: typecheck** → 0。
- [ ] **Step 7: 真机验证** — 起 app（`bun run dev:web`，设 `OPENHUB_DATA_DIR` 避开桌面锁；或桌面 dev），进「智能编排」→ 编队 → 新建编队（加 2 个不同 agent+model 成员）→ 保存 → 卡片出现 → 编辑改名 → 删除。截图留证。
