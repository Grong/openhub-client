...
## 文件结构（新增/修改）
- 新增 `crates/backend/openhub-extension/assets/superpowers/`：内置 superpowers 语料库（14 技能 + LICENSE + VERSION）。
- 新增 `crates/backend/openhub-extension/src/superpowers/mod.rs`：嵌入常量、指纹、baseline 物化、有效目录解析、下载/校验/替换入口（公开 API）。
- 新增 `crates/backend/openhub-extension/src/superpowers/update.rs`：GitHub 下载 + host 白名单 + zip 安全解压到 staging + 原子替换 overlay。
- 修改 `crates/backend/openhub-extension/src/startup_materialize.rs`：抽出 `pub(crate)` 复用 helper（`commit_staging_dir`、`retry_startup_file_op`、`write_dir_recursive`、锁）。
...
- [ ] Step2 `cargo test -p openhub-extension superpowers::` → FAIL（模块不存在）。
...
- [ ] Step4 `cargo test -p openhub-extension startup_materialize` 全绿（含既有）。
...
- [ ] Step1 找到既有 `materialize_if_needed` 调用点（`grep`），在其后加 `superpowers::materialize_superpowers_baseline(data_dir)`（同样先查 `OPENHUB_BUILTIN_SKILLS_PATH` 型 env 语义，若有 superpowers 覆盖 env 则尊重）。
- [ ] Step2 `cargo build -p openhub-app`。
- [ ] Step3 提交 `feat(superpowers): materialize baseline at startup`。

**Phase 1 收口：** `cargo build` 工作区相关 crate 通过；`cargo test -p openhub-extension -p openhub-ai-agent` 绿。
...
- [ ] Step4 `cargo test -p openhub-extension` 绿（含 skill_service 既有）。
- [ ] Step5 提交 `refactor(extension): share zip-safety helpers`。

### Task 8: 错误变体 + openhub-net 依赖
...
- [ ] Step2/3 加变体 + `Cargo.toml` 增 `openhub-net`。
- [ ] Step4 `cargo build -p openhub-extension` 绿。
- [ ] Step5 提交 `feat(extension): add download/verify errors + openhub-net dep`。
...
- [ ] Step4 `cargo test -p openhub-extension superpowers::update` 绿。
...
**Interfaces — Produces:** `pub async fn fetch_latest_release(http:&reqwest::Client) -> Result<SuperpowersRelease,ExtensionError>`（GET `api.github.com/repos/obra/superpowers/releases/latest`，headers Accept+User-Agent；解析 `tag_name` + `zipball_url`）。常量 `SUPERPOWERS_REPO="obra/superpowers"`（env `OPENHUB_SUPERPOWERS_REPO` 可覆盖）。
...
### Task 11: 周期 janitor
**Files:** Modify `openhub-app/src/router/routes.rs`（`create_router`，事件总线→WS 桥后）、可能 `state.rs`。
**Interfaces — Consumes:** Task9/10、`openhub_net::http_client`、`event_bus`。
...
- [ ] Step2 FAIL。
- [ ] Step3 实现 `run_superpowers_update_once` + `spawn_superpowers_updater(data_dir, bus)`（`tokio::spawn`+`interval`，env `OPENHUB_SUPERPOWERS_AUTOUPDATE`(默认开) / `OPENHUB_SUPERPOWERS_UPDATE_INTERVAL_SECS`(默认 21600)；比较 latest vs 当前 overlay `.version`/baseline version，semver 或字符串不等即更新）；在 `create_router` 调 spawn；广播 `WebSocketMessage::new("superpowers.updated", json!({version}))`。
- [ ] Step4 `cargo test -p openhub-app`（相关）+ `cargo build`。
...
- [ ] Step4 `cargo test -p openhub-ai-agent`（相关）+ build。
