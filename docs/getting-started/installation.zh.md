...
OpenHub 有两种宿主模式，共享同一个 Rust 后端（参见
[简介](introduction.zh.md)）。本页覆盖目前可行的全部三种安装方式：

- [从源码构建桌面应用](#从源码构建桌面应用) —— `openhub-desktop`
  （Tauri 外壳），桌面本地信任，单用户。
- [从源码构建 Web 服务](#从源码构建-web-服务) —— `openhub-web`，
  带鉴权，自托管。
...
`openhub-desktop`），它在进程内链接后端，并在一个空闲 localhost 端口上以
...
2. `cargo` 编译 `openhub-desktop`（以及它依赖的整个工作区）。
...
![以开发模式运行的 openhub-desktop 与嵌入式后端](../images/gs-02-desktop-dev.png)
...
- 位于 `target/release/openhub-desktop`（Windows 上为 `.exe`）的独立可
  执行文件。
...
`openhub-web` 是一个 axum 服务，**既**在进程内挂载同一个后端，**又**在
...
在同时装有桌面应用的机器上，直接裸跑 `openhub-web` 会打开桌面应用的
...
`openhub-web`（定义在 `apps/web/src/main.rs` 中）同时接受 CLI 参数与
环境变量：
...
| `--insecure-no-auth` | `OPENHUB_WEB_INSECURE_NO_AUTH` | `false` | **危险。** 完全禁用鉴权（桌面式本地模式）。仅限 loopback / 受信任的私有网络。 |
| _（仅环境变量）_ | `OPENHUB_HTTPS` | `false` | 当前面有 TLS 终止时设为 `true`，使 cookie 获得 `Secure` 标记。 |
...
一个**无 GUI**镜像：在 `debian:bookworm-slim` 上的 SPA + `openhub-web` +
`bun`。
...
`caddy` 服务即是推荐做法。届时记得把 openhub 服务的 `OPENHUB_HTTPS=true`
...
如果你在日志中看到 `openhub-web: embedded backend + SPA on one port`，
并且 `/api/auth/status` 返回了 JSON，则后端已启动，并且 SPA 在同一个
端口上提供服务。

## 接下来

- [快速上手](quick-start.zh.md) —— 你在 OpenHub 中的第一段会话。
- [`../guides/web-server-deployment.md`](../guides/web-server-deployment.md)
  —— Web 主机的生产环境加固。
