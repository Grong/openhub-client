...
- **agent 选择**：选择 OpenHub、Claude Code、Codex、Gemini、Qwen、OpenCode 等后端。
...
- 如需无人值守长任务，可配置 **Model Failover Queue**，让 OpenHub 引擎在模型失败、
  限流或不可用时按顺序尝试备用模型。
...
1. 选择 **OpenHub**，它不依赖外部 CLI，最适合首跑验证。
2. 选择一个已配置的模型。
3. 可选：选择一个助手。
4. 输入提示词，例如：
...
- `/openhub`：OpenHub 伙伴、远程频道绑定和 companion 设置。
