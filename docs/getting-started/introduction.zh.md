...
**OpenHub** 是一个面向 AI agent 工作流的本地优先工作台。它把多种
CLI agent、内置 OpenHub 引擎、模型提供商、MCP 服务、技能、终端、计划任务
...
![OpenHub 引导页](../images/gs-01-introduction-hero.png)

## 它解决什么问题

真实的 AI 工作流经常被拆散在多个地方：一个终端跑 Claude Code，一个终端跑
Codex，浏览器里开着自托管页面，旁边还有单独的 MCP 服务和项目脚本。
OpenHub 的目标不是再做一个聊天框，而是把这些运行时接到同一个工作区：

- **一个会话入口，多种 agent。** 会话可以选择内置 OpenHub、Claude Code、
...
- **桌面与 Web 共用后端。** Tauri 桌面端和 `openhub-web` 自托管服务使用
  同一套 `openhub-app` 后端与同一份 React SPA。

OpenHub 更适合已经在用 agent 做真实工作的用户。它要求你理解 API key、
...
| 模式 | 二进制 | 鉴权模型 | 典型用途 |
| --- | --- | --- | --- |
| 桌面应用 | `openhub-desktop` | 桌面外壳使用本地信任 token 访问嵌入式后端；远程浏览器仍需登录 | 单机工作站、日常开发 |
| Web 服务 | `openhub-web` | 默认开启登录；首次访问创建管理员 | LAN/VPN/VPS 自托管 |

桌面模式会在进程内启动 `openhub-app`，监听一个随机 localhost 端口，并通过
每次启动生成的本地信任 token 让 WebView 免登录访问。WebUI 远程访问打开后，
额外的 LAN 监听器仍然要求远程浏览器登录。

`openhub-web` 则在一个端口上同时提供 SPA 与 API，默认端口是 `8787`。Docker
和 systemd 部署都走这条路径。
...
- **桌面伙伴**：`/openhub` 管理伙伴、远程频道绑定和 companion 相关设置。
...
OpenHub 仍在活跃开发中，但已经不是旧的 Electron 多仓迁移状态。当前仓库是
