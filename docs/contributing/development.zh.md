...
本页面向修改 **OpenHub** 仓库本身的人：React SPA、Rust 后端、agent 引擎或
...
| `bun run dev:web` | 浏览器 + 后端联调，关闭登录 | `openhub-web --port 8787 --dist ui/dist --insecure-no-auth` 加 Vite。 |
| `bun run serve:web` | 从源码跑生产形态 Web host | `openhub-web` on `http://127.0.0.1:8787`，服务 `ui/dist`，默认开启登录。 |
...
桌面循环已经不是旧 Electron 模型。Tauri shell 直接链接 `openhub-app`，在进程内
...
`openhub-app` 仍然提供独立 `nomicore` binary。app host 不会 spawn 它，但诊断、
...
`OPENHUB_WORK_DIR` 控制会话工作区位置；未设置时回退到数据目录。
...
也可以把 `--log-level` 直接传给 `nomicore` / `openhub-web`。该值是
