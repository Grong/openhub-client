...
- `apps/desktop/tauri.conf.json` **已删除 `version` 字段** → Tauri 自动继承 workspace 版本（已验证：安装包名随之变 `OpenHub_0.1.10_x64-setup.exe`）。
...
3. **托管：GitHub Releases**（`latest.json` 与各平台安装包都作为 Release 资产）。仓库 = `git@github.com:openhub/openhub-client.git`。
...
- **次要不一致**：后端 `VersionCheckService` 默认仓库是 `openhub/openhub-app`（`openhub-system/src/version.rs:5`），与实际仓库 `openhub/openhub-client` 不一致；本方案走 tauri-plugin-updater，不依赖它，但若以后启用该路径需对齐。
...
```bash
bun x tauri signer generate -w apps/desktop/signing/openhub-updater.key   # 密码可留空
```
...
4. **确认发布仓库** = `openhub/openhub-client`（默认按此）。
...
- Windows 那台已验证可出 `OpenHub_0.1.10_x64-setup.exe`；mac/linux 包还没在对应系统上构建过。
...
**配置 / 密钥：**
- 已在本机生成更新签名密钥：私钥 `apps/desktop/signing/openhub-updater.key`（`*.key` 已 gitignore，**未打印、未入库**），公钥写入 `tauri.conf.json` → `plugins.updater.pubkey`。**⚠️ 请尽快备份私钥到发布密钥库**（口令留空，§5-3 默认）。
- `tauri.conf.json` → `plugins.updater.endpoints` 改为 `https://github.com/Grong/openhub-client/releases/latest/download/latest.json`。
...
4. 仓库：`openhub/openhub-client`（与 `git remote` 一致）。
5. 系统代码签名：仍**独立于自动更新**，未在本次处理（mac 走 `build:signed`/`build:mac --signed`，见 signing/README）。

**剩余手动步骤（需要真机 + 你的发布账号）：**
1. 各平台构建机：`export TAURI_SIGNING_PRIVATE_KEY="$(cat apps/desktop/signing/openhub-updater.key)"` → 出签名更新产物（统一用叠加配置文件：`bun run build:<mac|win|linux> --config apps/desktop/tauri.updater.conf.json`；**不要内联 JSON**，Windows PowerShell 5.1 会剥引号）。
...
> 注：后端 `openhub-system/src/version.rs` 那套旧 `VersionCheckService`（默认仓库 `openhub/openhub-app`）与本方案**无关**（本方案不经它）；前端更新检查已全部走 tauri-plugin-updater。如未来要启用该后端路径再对齐其默认仓库。
...
**私钥（本机）：** 已把 `apps/desktop/signing/openhub-updater.key`（+`.key.pub`、`.env.signing`）放到本机，均被 gitignore（不入库）。已校验本机公钥与 `tauri.conf.json` 内嵌 `pubkey` 完全一致（keyID `F3AA272E60AA7952`）——签出的包客户端能验签通过。
...
bun run build:win --config apps/desktop/tauri.updater.conf.json   # 产 OpenHub_<ver>_x64-setup.exe + .sig
