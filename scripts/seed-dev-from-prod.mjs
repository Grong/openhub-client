 ...
 *
 * SAFETY: close ALL OpenHub instances (the installed app, `bun run serve:web`,
 * `openhub-core`, and any running dev build) before seeding — copying a live SQLite
...
/** Mirror `openhub_app::cli::default_data_dir`'s vendor base, per-OS. */
functionopenhubase() {
  const home = homedir();
  switch (platform()) {
    case 'darwin':
      return join(home, 'Library', 'Application Support', 'OpenHub');
    case 'win32':
      return join(process.env.LOCALAPPDATA ?? join(home, 'AppData', 'Local'), 'OpenHub');
    default:
      return join(process.env.XDG_DATA_HOME ?? join(home, '.local', 'share'), 'OpenHub');
  }
}
...
console.log(`    ${prod}`);
console.log(`  → ${dev}`);
console.log('  Ensure ALL OpenHub instances are closed; lock/runtime files are skipped.');
