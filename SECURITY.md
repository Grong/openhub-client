...
OpenHub can execute local tools, shell commands, browser automation, desktop
automation, and remote capability calls. Treat an authenticated OpenHub instance
...
- operating system and deployment mode (`openhub-desktop`, `openhub-web`, or
  standalone `nomicore`),
- reproduction steps,
- impact assessment,
...
- Do not expose the embedded desktop backend port directly. Use WebUI Remote
  Access or `openhub-web`, both of which provide authenticated surfaces.
- Use TLS when exposing `openhub-web` or remote capability APIs over a network.
