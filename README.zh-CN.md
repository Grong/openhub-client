...
<a href="https://www.openhub.dev">
  <img src="docs/images/readme-00-orchestration-hero.png" alt="OpenHub 编排会话、角色沉淀与 DAG 画布" width="100%">
</a>
...
<p>
  ...
  <a href="https://www.openhub.dev"><img alt="Website" src="https://img.shields.io/badge/官网-openhub.dev-FF6F91?style=for-the-badge"></a>
</p>
...
<p>
  <a href="https://www.openhub.dev">🌐 官网</a>&nbsp;·&nbsp;
  <a href="docs/README.zh.md">📖 文档</a>&nbsp;·&nbsp;
  <a href="#-快速开始">🚀 快速开始</a>&nbsp;·&nbsp;
  <a href="https://github.com/Grong/openhub-client/releases">📦 下载</a>&nbsp;·&nbsp;
...
> [!IMPORTANT]
> **公益开源与数据风险声明**：这是一个公益开源项目，不承担迭代过程中用户数据丢失损坏的责任。OpenHub 仍处于快速迭代阶段，请在升级、迁移、体验实验功能或接入真实工作数据前自行做好备份。

---

**OpenHub** 满足你对 AI 工作站的全部想象 —— 而且一切由你做主。一套 React 前端 + 一套 Rust 后端，为你带来会成长的桌面伙伴、无人值守的自动化平台、统一知识库、原生的 computer / browser use，以及任何智能体都能驱动的开放能力总线。无需云账号、无遥测、无订阅。除了**你自己配置**的大模型调用，你的数据绝不离开本机。

> 产品名是 **OpenHub**；小写 openhub 仅用于代码标识符、crate 名、环境变量与仓库路径。
...
在 OpenHub 里，数据安全不是一个开关，而是架构本身。

- **数据全在本地。** OpenHub 绝不主动向外发送任何数据。**唯一**的出站网络请求，是你自己明确配置、调用所选模型厂商的大模型请求；除此之外，没有任何第三方服务的网络对接。
...
<p>
  <img src="docs/images/readme-01-workbench-overview.png" alt="OpenHub 工作编排会话与 DAG 画布" width="100%">
...
<sub>均为 2026-07-01 从当前运行中的 OpenHub 桌面应用截取，并保留 2560px 宽度版本。完整截图清单与采集方式见 <a href="docs/images/SCREENSHOTS.md">截图 manifest</a>。</sub>
...
从普通会话起步，任务一旦值得拆分，OpenHub 就会把它扩展成一张实时 DAG。
...
OpenHub 的每一项能力都经由单一、强类型的能力注册表对外开放 —— **约 20 个域、150+ 个工具** —— 让你能把 OpenHub 接进任何地方。

- **MCP 前门** 位于 `/mcp`（鉴权，Streamable-HTTP）。把 **Claude Code、Cursor 或你自己的智能体**指向它，它们就能像桌面伙伴一样操作 OpenHub。
...
OpenHub 不绑定任何一家模型厂商。你可以按地区、价格、额度、模型能力和数据政策选择供应商，注册后把 API Key 填到 OpenHub 的 **模型 & Agent** 页面即可使用。以下均为第三方服务，费用、可用地区、速率限制与数据处理规则以各家官方说明为准。
...
> 📦 **下载安装包**：优先使用 [GitHub Releases](https://github.com/Grong/openhub-client/releases)；中国大陆下载可使用 [百度网盘镜像](https://pan.baidu.com/s/5GPonoJNrwJ7GciBSDgXLaA)（分享名：openhub）。也可以从源码安装，或用 Docker 跑服务器。
...
OpenHub 目前处于 **pre-1.0**，且为兼职开发，所以还有很多正在路上：预编译安装包、入站 issue / 需求来源接入、更多知识库连接器（飞书及更多）、官方桌面安装包 —— 以及几个我们非常期待的惊喜。**敬请期待。** ✨

---

## 🤝 贡献与社区

OpenHub 非常需要你的加入来壮大 —— 代码贡献、社区运营、技术布道都热烈欢迎。如果你对这个项目有热情，请[联系我们](#-联系我们--社区)，与我们一起共建 OpenHub 的生态。
...
- 从 [open issues](https://github.com/Grong/openhub-client/issues) 找一个起点。
...
OpenHub **完全开源、毫无保留**。个人与企业都可以在它之上二次开发并商用。

- **欢迎二次开发与商用。** 同时，这些行为风险自担 —— 作者与贡献者不承担后续一切法律责任。Apache-2.0 无需我们另行授权。
- **告知一声，是渴望而非要求。** 如果你二次开发或商用 OpenHub，希望你能留言告知我们 —— 这*不是*授权条件，只是因为「知道项目被认可」这份肯定，正是让它走下去的动力。
...
| 渠道 | 入口 |
|---|---|
| 🌐 **官网** | [www.openhub.dev](https://www.openhub.dev) |
...
| ✉️ **邮箱** | `hello@openhub.dev` <sub>（占位 · 待确认）</sub> |
| 📕 **小红书** | [OpenHub](https://xhslink.com/m/4x6ti8n6cA1) |
| 📺 **哔哩哔哩** | [OpenHub](https://b23.tv/0UhgKDh) · [演示视频](https://www.bilibili.com/video/BV1kwKZ6UE5X/) |
| 🎵 **抖音** | [OpenHub](https://v.douyin.com/MDT5QVdYaJk/) |
| ▶️ **YouTube** | [@OpenHub-o2y](https://www.youtube.com/@OpenHub-o2y) · [演示视频](https://youtu.be/AsEToBDFR9s) |
