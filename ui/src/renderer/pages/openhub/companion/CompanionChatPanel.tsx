...
import { useCompanion } from '../useOpenHub';
...
interface Props {
  /** A desktop-companion's single per-companion openhub session (extra.companionSession). */
 ...
 *
 * 取代旧的 /openhub 配置中心「聊天」Tab（ChatTab）：迁移后伙伴聊天统一从会话列表的
 * 「桌面伙伴」分组进入标准 `/conversation/:id`。ChatConversation 见到
 * `type==='openhub' && extra.companionSession` 即渲染本面板（而非全功能 OpenHubConversationPanel），
  ...
  if (!modelConfigured) {
    return (
      <div className='flex flex-col h-full min-h-0 items-center justify-center gap-14px px-16px text-center'>
        ...
        <div className='text-13px text-t-tertiary'>{t('openhub.chat.modelMissing')}</div>
      </div>
    );
  }
