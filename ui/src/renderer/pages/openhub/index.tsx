...
import { useCompanion, useCompanions, useCompanionShared } from './useOpenHub';
  ...
  const openChat = useCallback(
    async (companionId: string | null) => {
      ...
      try {
        ...
        Message.info(t('openhub.chat.modelMissing'));
      }
    },
    [navigate, t]
  );
  ...
  return (
    <div className='w-full min-h-full box-border overflow-y-auto px-16px py-20px'>
      <div className='mx-auto flex w-full max-w-[95%] box-border flex-col'>
        <h1 className='m-0 mb-4px text-20px font-600 text-t-primary'>{t('openhub.title')}</h1>
        <p className='m-0 mb-12px text-13px text-t-secondary'>{t('openhub.subtitle')}</p>
          ...
          <>
            <div className='flex items-center gap-12px mb-4px'>
              <Radio.Group type='button' size='small' value={domain} onChange={(d: Domain) => setDomain(d)}>
                <Radio value='companion'>{t('openhub.domains.companion')}</Radio>
                <Radio value='shared'>{t('openhub.domains.shared')}</Radio>
                <Radio value='figures'>{t('openhub.customFigure.libraryTitle')}</Radio>
              </Radio.Group>
            </div>
              ...
              >
                ...
                {selectedCompanionId ? (
                  <div className='flex-1 min-w-0 flex flex-col'>
                    <div className='shrink-0 mb-8px flex items-center justify-between gap-8px'>
                      <Radio.Group type='button' size='small' value={activeTab} onChange={setTab}>
                        <Radio value='overview'>{t('openhub.tabs.overview')}</Radio>
                        <Radio value='remote'>{t('openhub.tabs.remote')}</Radio>
                        <Radio value='memories'>{t('openhub.tabs.memories')}</Radio>
                        <Radio value='review'>{t('openhub.tabs.review')}</Radio>
                        <Radio value='knowledge'>{t('openhub.tabs.knowledge')}</Radio>
                        <Radio value='skills'>{t('openhub.tabs.skills', { defaultValue: '技能' })}</Radio>
                        <Radio value='secrets'>{t('openhub.tabs.secrets')}</Radio>
                        <Radio value='settings'>{t('openhub.tabs.settings')}</Radio>
                      </Radio.Group>
                      <Button
                        type='primary'
                        size='small'
                  ...
                  <div className='flex-1 flex items-center justify-center bg-fill-1 rd-12px box-border'>
                    <Empty description={t('openhub.companions.empty')} />
                  </div>
                )}
              </div>
            ) : (
              <Tabs activeTab={activeTab} onChange={setTab} lazyload>
                <Tabs.TabPane key='collect' title={t('openhub.tabs.collect')}>
                  <CollectTab shared={shared} />
                </Tabs.TabPane>
                <Tabs.TabPane key='learn' title={t('openhub.tabs.learn')}>
                  <LearnTab shared={shared} />
                </Tabs.TabPane>
                <Tabs.TabPane key='suggestions' title={t('openhub.tabs.suggestions')}>
                  <SuggestionsTab />
                </Tabs.TabPane>
                <Tabs.TabPane key='migrate' title={t('openhub.tabs.migrate')}>
