...
const CompanionModelControl: React.FC<Props> = ({ companion }) => {
  ...
  const hint = !anyChatModel
    ? t('openhub.chat.modelNoTextModel')
    : providerStale
      ? t('openhub.chat.modelStale', { model: selectedModel })
      : currentProvider && availableModels.length === 0
        ? t('openhub.chat.modelProviderNoChat')
        : showStaleModel
          ? t('openhub.chat.modelStale', { model: selectedModel })
          : '';
  ...
  return (
    <div className='flex flex-col gap-6px'>
      <div className='flex items-center gap-6px flex-wrap'>
        <Tooltip content={t('openhub.chat.modelConfigHint')}>
          <span className='flex items-center gap-4px text-12px text-t-tertiary shrink-0 cursor-help'>
            ...
            />
            {t('openhub.chat.modelConfig')}
          </span>
        </Tooltip>
        <Select
          size='mini'
          style={{ width: 148 }}
          placeholder={t('openhub.chat.modelProvider')}
        ...
        >
          ...
          {providerStale && (
            <Select.Option key={providerId} value={providerId} disabled>
              {t('openhub.chat.modelUnavailableOption', { model: providerId })}
            </Select.Option>
          )}
        ...
        <Select
          size='mini'
          style={{ width: 176 }}
          placeholder={t('openhub.chat.modelName')}
        ...
        >
          ...
          {showStaleModel && (
            <Select.Option key={selectedModel} value={selectedModel} disabled>
              {t('openhub.chat.modelUnavailableOption', { model: selectedModel })}
            </Select.Option>
          )}
