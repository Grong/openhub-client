    ...
    <>
      <div className='grid grid-cols-3 gap-10px max-[720px]:grid-cols-2'>
            ...
            >
              ...
              <div className='flex items-center gap-6px'>
                ...
                <span className={classNames('text-13px font-600', active ? 'text-[var(--color-primary)]' : 'text-t-primary')}>
                  {t(`openhub.characters.${c.nameKey}.name`)}
                </span>
              </div>
              <span className='text-11px text-t-tertiary text-center leading-snug'>
                {t(`openhub.characters.${c.nameKey}.style`)}
              </span>
            </div>
          );
        })}
            ...
            >
              ...
              <FigureActionSurface>
                <FigureActionButton
                  tone='danger'
                  disabled={used}
                  title={used ? t('openhub.customFigure.inUseCannotDelete') : t('openhub.customFigure.delete')}
                  ariaLabel={used ? t('openhub.customFigure.inUseCannotDelete') : t('openhub.customFigure.delete')}
                  onClick={(e) => {
                    e.stopPropagation();
              ...
              <span className='text-11px text-t-tertiary'>{t('openhub.customFigure.cardLabel')}</span>
            </div>
          );
        })}
        ...
        >
          ...
          <span className='text-13px font-600 text-t-primary'>{t('openhub.customFigure.createNew')}</span>
          <span className='text-11px text-t-tertiary text-center leading-snug'>{t('openhub.customFigure.cardHint')}</span>
        </div>
      </div>
