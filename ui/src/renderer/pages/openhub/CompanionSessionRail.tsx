...
const CompanionSessionRail: React.FC<Props> = ({
  ...
  const submitCreate = async () => {
    ...
    try {
      ...
      Message.success(t('openhub.companions.created', { companionName: profile.name }));
      onCreated(profile);
    } catch (e) {
  ...
  const requestDelete = useCallback(
    (p: ICompanionWithStatus, e: React.MouseEvent) => {
      ...
      Modal.confirm({
        ...
        content: t('openhub.settings.deleteConfirmBody', { companionName: p.name }),
        okButtonProps: { status: 'danger' },
        onOk: async () => {
          try {
            await ipcBridge.companion.deleteCompanion.invoke({ companion_id: p.id });
            Message.success(t('openhub.settings.deleted', { companionName: p.name }));
            onDeleted(p.id);
          } catch (err) {
    ...
    <div className={classNames('flex flex-col bg-fill-1 rd-12px box-border overflow-hidden', className)}>
      ...
      >
        ...
        <span className='flex flex-col gap-2px min-w-0'>
          <span className='text-14px leading-16px font-700 text-t-primary truncate'>{t('openhub.companions.create')}</span>
          <span className='text-11px leading-13px font-500 text-t-tertiary truncate'>
            {t('openhub.companions.createHint')}
          </span>
        </span>
      </div>
      ...
      <div className='flex-1 min-h-0 overflow-y-auto px-6px pb-6px pt-0 flex flex-col gap-3px'>
        {companions.map((p) => {
          ...
          return (
            ...
            >
              ...
              <div
                role='button'
                aria-label={t('openhub.settings.deleteCompanion')}
                title={t('openhub.settings.deleteCompanion')}
        ...
        {companions.length === 0 && (
          <div className='flex-1 flex items-center justify-center text-12px text-t-tertiary px-8px text-center py-20px'>
            {t('openhub.companions.empty')}
          </div>
        )}
      </div>

      <Modal
        title={t('openhub.companions.createTitle')}
      ...
      >
        <div className='flex flex-col gap-14px'>
          <div className='flex flex-col gap-6px'>
            <span className='text-13px text-t-secondary'>{t('openhub.companions.nameLabel')}</span>
            <Input
              value={name}
              onChange={setName}
              placeholder={t('openhub.companions.namePlaceholder')}
              maxLength={30}
              onPressEnter={() => void submitCreate()}
            />
          </div>
          <div className='flex flex-col gap-6px'>
            <span className='text-13px text-t-secondary'>{t('openhub.companions.characterLabel')}</span>
            <CharacterPicker
