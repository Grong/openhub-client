...
interface MemoriesTabProps {
  /** The companion currently selected on the openhub page; scopes the default view. */
...
const MemoriesTab: React.FC<MemoriesTabProps> = ({ companionId = null, companions = [] }) => {
  ...
  // openhub can save/edit/delete memories mid-chat or from another surface —
  // reflect them live.
  useEffect(() => {
    const unsubs = [
  ...
  const add = useCallback(async () => {
    ...
    try {
      ...
      setAddContent('');
      void refresh();
      Message.success(t('openhub.memories.added'));
    } catch (e) {
  ...
  const saveEdit = useCallback(async () => {
    ...
    try {
      ...
      setEditTarget(null);
      void refresh();
      Message.success(t('openhub.memories.saved'));
    } catch (e) {
      Message.error(String(e));
    }
  ...
  const scopeSelector = (
    ...
    <div className='flex items-center gap-8px flex-wrap'>
      ...
      >
        <Radio value='user'>{t('openhub.memories.scopeShared')}</Radio>
        <Radio value='companion'>{t('openhub.memories.scopePrivate')}</Radio>
      </Radio.Group>
      {scopeKind === 'companion' && (
        <Select
          size='small'
          ...
          placeholder={t('openhub.memories.scopePickCompanion')}
        >
          {companions.map((c) => (
  ...
  const scopeBadge = (m: ICompanionMemory) =>
    m.scope_kind === 'companion' ? (
      <Tag color='arcoblue' bordered>
        {t('openhub.memories.scopePrivateOf', { name: companionName(m.scope_companion_id) })}
      </Tag>
    ) : (
      <Tag bordered>{t('openhub.memories.scopeShared')}</Tag>
    );

  return (
    <div className='flex flex-col gap-12px py-8px'>
      <div className='flex gap-8px flex-wrap items-center'>
        <Select style={{ width: 140 }} value={kind} onChange={setKind} placeholder={t('openhub.memories.kindAll')}>
          <Select.Option value=''>{t('openhub.memories.kindAll')}</Select.Option>
          {KINDS.map((k) => (
        ...
        <Select style={{ width: 110 }} value={memStatus} onChange={setMemStatus}>
          <Select.Option value='active'>{t('openhub.memories.statusActive')}</Select.Option>
          <Select.Option value='archived'>{t('openhub.memories.statusArchived')}</Select.Option>
        </Select>
        {companionId && (
          <Radio.Group type='button' size='small' value={scopeMode} onChange={(v: 'self' | 'all') => setScopeMode(v)}>
            <Radio value='self'>{t('openhub.memories.scopeFilterSelf')}</Radio>
            <Radio value='all'>{t('openhub.memories.scopeFilterAll')}</Radio>
          </Radio.Group>
        )}
        <Input.Search
          style={{ width: 220 }}
          placeholder={t('openhub.memories.searchPlaceholder')}
          value={q}
          onChange={setQ}
          allowClear
        />
        <Button type='primary' onClick={openAdd}>
          {t('openhub.memories.add')}
        </Button>
      </div>
      {loading ? (
        ...
        <Empty description={t('openhub.memories.empty')} />
      ) : (
        <div className='flex flex-col gap-8px'>
          {memories.map((m) => (
            <div key={m.id} className='flex items-start gap-10px bg-fill-2 rd-10px px-12px py-10px'>
              <Tag color={KIND_COLORS[m.kind]}>{t(`openhub.kinds.${m.kind}`)}</Tag>
              <div className='flex-1 min-w-0'>
                ...
                <div className='mt-4px flex items-center gap-10px text-11px text-t-tertiary'>
                  {scopeBadge(m)}
                  <span>
                    {t('openhub.memories.strength')} {(m.strength * 100).toFixed(0)}%
                  </span>
                  <span>{new Date(m.updated_at).toLocaleString()}</span>
                  {m.source !== 'learn' && <span>{t(`openhub.memories.source_${m.source}`, m.source)}</span>}
                </div>
              </div>
              <div className='flex items-center gap-4px shrink-0'>
                <Tooltip content={m.pinned ? t('openhub.memories.unpin') : t('openhub.memories.pin')}>
                  <Button
                    size='mini'
                ...
                <Button size='mini' onClick={() => openEdit(m)}>
                  {t('openhub.memories.edit')}
                </Button>
                <Button size='mini' onClick={() => void toggleArchive(m)}>
                  {m.status === 'active' ? t('openhub.memories.archive') : t('openhub.memories.restore')}
                </Button>
                <Popconfirm title={t('openhub.memories.deleteConfirm')} onOk={() => void remove(m)}>
                  <Button size='mini' status='danger'>
                    {t('openhub.memories.delete')}
                  </Button>
                </Popconfirm>
              </div>
            </div>
      ...
      >
        <div className='flex flex-col gap-12px'>
          ...
          <Input.TextArea
            ...
            placeholder={t('openhub.memories.addPlaceholder')}
          />
        </div>
      </Modal>

      <Modal
        title={t('openhub.memories.edit')}
        visible={!!editTarget}
        onOk={() => void saveEdit()}
      ...
      >
        <div className='flex flex-col gap-12px'>
          ...
          <div className='text-11px text-t-tertiary'>{t('openhub.memories.editHint')}</div>
        </div>
      </Modal>
    </div>
  );
