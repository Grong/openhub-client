...
import { useCompanions } from '../useOpenHub';
...
const RemoteConnectSection: React.FC<{ companionId: string; companionName: string }> = ({ companionId, companionName }) => {
  ...
  const applyRowBinding = useCallback(
    async (rowId: string, bind: boolean) => {
      ...
      try {
        ...
        Message.success(
          bind ? t('openhub.settings.remoteBindSuccess', { companionName }) : t('openhub.settings.remoteUnbindSuccess')
        );
        await refreshStatuses();
      } catch (error) {
  ...
  const confirmBind = useCallback(
    (row: IChannelPluginStatus) => {
      Modal.confirm({
        title: t('openhub.settings.remoteBindRow'),
        content: t('openhub.settings.remoteBindConfirm', { companionName }),
  ...
  const confirmUnbind = useCallback(
    (row: IChannelPluginStatus) => {
      Modal.confirm({
        ...
        content: t('openhub.settings.remoteUnbindConfirm', { companionName }),
  ...
  const confirmMove = useCallback(
    (row: IChannelPluginStatus) => {
      ...
      Modal.confirm({
        ...
        content: t('openhub.settings.remoteMoveConfirm', { from: fromName, to: companionName }),
        onOk: () => applyRowBinding(row.id, true),
      });
    },
  ...
  return (
    <>
      <div className='mt-8px text-13px font-600 text-t-secondary'>{t('openhub.settings.remoteTitle')}</div>
      <div className='text-12px text-t-tertiary -mt-6px'>{t('openhub.settings.remoteHint', { companionName })}</div>
        ...
        if (myRow) {
          ...
          actions = (
            <>
              ...
              >
                {t('openhub.settings.remoteBindRow')}
              </Button>
          ...
          subtitle = t('openhub.settings.remoteOtherBots', {
          ...
          actions = (
            <>
              ...
              <Button size='small' onClick={() => setConfigTarget({ platform: id })}>
                {t('openhub.settings.remoteCreateBot')}
              </Button>
            </>
          );
        } else {
