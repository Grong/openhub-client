...
const CollectTab: React.FC<Props> = ({ shared }) => {
  ...
  return (
    <div ref={rootRef} className='flex flex-col gap-12px py-8px'>
      <p className='m-0 text-13px text-t-secondary'>{t('openhub.collect.intro')}</p>
            ...
            <div key={key} className='flex items-center gap-12px bg-fill-2 rd-10px px-12px py-10px'>
              ...
              <div className='flex-1 min-w-0'>
                <div className='flex items-center gap-8px'>
                  <span className='text-14px text-t-primary font-500'>{t(`openhub.collect.sources.${key}.name`)}</span>
                  <Tag size='small' color={SENSITIVITY_COLOR[sensitivity]}>
                    {t(`openhub.collect.sensitivity.${sensitivity}`)}
                  </Tag>
                </div>
                <div className='text-12px text-t-tertiary mt-2px'>{t(`openhub.collect.sources.${key}.desc`)}</div>
              </div>
              <div className='text-12px text-t-secondary shrink-0 text-right'>
                <div>
                  {t('openhub.collect.today')}: {stat?.today ?? 0}
                </div>
                <div>
                  {t('openhub.collect.total')}: {stat?.total ?? 0}
                </div>
              </div>
            </div>
          );
      ...
      <div className='flex items-center gap-12px flex-wrap'>
        <Popconfirm
          title={t('openhub.collect.disableAllConfirm', {
          ...
          onOk={() => {
            void ipcBridge.companion.disableAll
              ...
              .then(() => {
                void shared.refresh();
                refreshStats();
                Message.success(t('openhub.collect.disabledAll', { defaultValue: '已全部关闭' }));
              })
              .catch((e) => Message.error(String(e)));
          }}
        >
          <Button status='danger' type='primary'>
            {t('openhub.collect.disableAll', { defaultValue: '一键全关' })}
          </Button>
        </Popconfirm>
        <Popconfirm
          title={t('openhub.collect.clearConfirm')}
          onOk={() => {
            void ipcBridge.companion.clearEvents
              .invoke()
              .then(() => {
                Message.success(t('openhub.collect.cleared'));
                refreshStats();
        ...
        >
          <Button status='danger'>{t('openhub.collect.clear')}</Button>
        </Popconfirm>
        <span className='text-12px text-t-tertiary'>{t('openhub.collect.clearHint')}</span>
      </div>
      ...
      <div className='flex flex-col gap-8px'>
        ...
        >
          {rawEvents
            ? t('openhub.collect.hideRaw', { defaultValue: '收起采集内容' })
            : t('openhub.collect.viewRaw', { defaultValue: '查看采集到的内容' })}
        </Button>
        {rawEvents && (
          <div className='flex flex-col gap-4px max-h-360px overflow-y-auto bg-fill-1 rd-10px p-10px'>
            {rawEvents.length === 0 ? (
              <span className='text-12px text-t-tertiary'>
                {t('openhub.collect.rawEmpty', { defaultValue: '还没有采集到任何内容。' })}
              </span>
            ) : (
              rawEvents.map((ev, i) => (
