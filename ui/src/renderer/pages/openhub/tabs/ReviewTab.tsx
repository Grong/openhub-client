  ...
  return (
    <div className='flex flex-col gap-12px py-8px'>
      <Radio.Group type='button' value={mode} onChange={(v) => setMode(v as 'all' | 'onThisDay')}>
        <Radio value='all'>{t('openhub.archive.reviewTitle')}</Radio>
        <Radio value='onThisDay'>{t('openhub.archive.onThisDay')}</Radio>
      </Radio.Group>
      ...
      {loading ? (
        ...
        <Empty description={t(mode === 'onThisDay' ? 'openhub.archive.onThisDayEmpty' : 'openhub.archive.reviewEmpty')} />
      ) : (
