  ...
  return (
    <div className='flex flex-col gap-12px py-8px'>
      <Radio.Group type='button' value={filter} onChange={setFilter}>
        <Radio value='new'>{t('openhub.suggestions.filterNew')}</Radio>
        <Radio value='accepted'>{t('openhub.suggestions.filterAccepted')}</Radio>
        <Radio value='dismissed'>{t('openhub.suggestions.filterDismissed')}</Radio>
        <Radio value='all'>{t('openhub.suggestions.filterAll')}</Radio>
      </Radio.Group>
      {loading ? (
        ...
        <Empty description={t('openhub.suggestions.empty')} />
      ) : (
                ...
                <div className='flex items-center gap-8px'>
                  ...
                  {s.status !== 'new' && (
                    <Tag size='small' color={s.status === 'accepted' ? 'green' : 'gray'}>
                      {t(`openhub.suggestions.status_${s.status}`)}
                    </Tag>
                  )}
                </div>
              ...
              {s.status === 'new' && (
                <div className='flex items-center gap-6px shrink-0'>
                  <Button size='mini' type='primary' onClick={() => void decide(s, true)}>
                    {t('openhub.suggestions.accept')}
                  </Button>
                  <Button size='mini' onClick={() => void decide(s, false)}>
                    {t('openhub.suggestions.dismiss')}
                  </Button>
                </div>
              )}
            </div>
