...
import type { useCompanion } from '../useOpenHub';
...
const SkillsTab: React.FC<Props> = ({ companion }) => {
    ...
    const subs = [
      ...
      ipcBridge.companion.onSkillLearned.on((e) => {
        if (e.companion_id === companionId) {
          void refresh();
          Message.success(t('openhub.skills.learnedToast', { defaultValue: '伙伴学会了一个新技能' }));
        }
      }),
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      setEditMode(false);
      Message.success(t('openhub.skills.saveOk', { defaultValue: '已保存' }));
      void refresh();
    } catch (e) {
  ...
  const statusLabel = (status: string): string => {
    const key = `openhub.skills.status${status.charAt(0).toUpperCase()}${status.slice(1)}`;
  ...
  const teach = useCallback(async () => {
    ...
    try {
      ...
      Message.success(
        name
          ? t('openhub.skills.taughtOk', { defaultValue: '已根据示范起草技能，去待审里看看' })
          : t('openhub.skills.taughtNone', { defaultValue: '没能从这个会话提炼出技能' })
      );
    } catch (e) {
      Message.error(String(e));
    }
  ...
  const gift = useCallback(async () => {
    ...
    try {
      ...
      setGiftFor(null);
      Message.success(t('openhub.skills.giftedOk', { defaultValue: '已赠送给对方' }));
    } catch (e) {
      Message.error(String(e));
    }
    ...
    <div className='flex flex-col gap-12px py-8px'>
      <div className='flex gap-8px items-center flex-wrap'>
        <Select
          ...
          placeholder={t('openhub.skills.statusAll', { defaultValue: '全部' })}
        >
          <Select.Option value=''>{t('openhub.skills.statusAll', { defaultValue: '全部' })}</Select.Option>
          <Select.Option value='draft'>{t('openhub.skills.statusDraft', { defaultValue: '待审' })}</Select.Option>
          <Select.Option value='active'>{t('openhub.skills.statusActive', { defaultValue: '已启用' })}</Select.Option>
          <Select.Option value='archived'>{t('openhub.skills.statusArchived', { defaultValue: '已归档' })}</Select.Option>
        </Select>
        <div className='text-12px text-t-tertiary'>
          {t('openhub.skills.hint', { defaultValue: '伙伴从你的重复操作里自动沉淀的技能，可查看、编辑、审阅' })}
        </div>
        <Button size='small' className='ml-auto' onClick={() => setTeachOpen(true)}>
          {t('openhub.skills.teach', { defaultValue: '示范教学' })}
        </Button>
      </div>
      {loading ? (
        ...
        <Empty description={t('openhub.skills.empty', { defaultValue: '还没有技能。多用平台，伙伴会自己学会。' })} />
      ) : (
            ...
            >
              ...
              <div className='flex-1 min-w-0'>
                ...
                <div className='mt-4px flex items-center gap-10px text-11px text-t-tertiary flex-wrap'>
                  <span>{t(`openhub.skills.source_${s.source}`, { defaultValue: s.source })}</span>
                  <span>
                    {t('openhub.skills.confidence', { defaultValue: '置信' })} {(s.confidence * 100).toFixed(0)}%
                  </span>
                  <span>
                    {t('openhub.skills.usage', { defaultValue: '已用' })} {s.usage_count}
                  </span>
                  <span>
                    {t('openhub.skills.strength', { defaultValue: '强度' })} {(s.strength * 100).toFixed(0)}%
                  </span>
                  {s.provenance.length > 0 && (
                    <Tooltip content={s.provenance.join(', ')}>
                      <span>
                        {t('openhub.skills.provenance', { defaultValue: '来源' })} {s.provenance.length}
                      </span>
                    </Tooltip>
                  )}
                </div>
              ...
              <div className='flex items-center gap-4px shrink-0'>
                <Button size='mini' onClick={() => void openEditor(s)}>
                  {t('openhub.skills.view', { defaultValue: '查看' })}
                </Button>
                {s.status === 'active' && (
                  <Button size='mini' onClick={() => void openGift(s.skill_name)}>
                    {t('openhub.skills.gift', { defaultValue: '赠送' })}
                  </Button>
                )}
                {s.status === 'draft' && (
                  <>
                    <Button size='mini' type='primary' onClick={() => void decide(s, true)}>
                      {t('openhub.skills.accept', { defaultValue: '采纳' })}
                    </Button>
                    <Popconfirm
                      title={t('openhub.skills.rejectConfirm', { defaultValue: '拒绝这个技能？' })}
                      onOk={() => void decide(s, false)}
                    >
                      <Button size='mini' status='danger'>
                        {t('openhub.skills.reject', { defaultValue: '拒绝' })}
                      </Button>
                    </Popconfirm>
                  </>
                )}
        ...
        footer={
          editMode ? (
            <div className='flex justify-end gap-8px'>
              ...
              >
                {t('openhub.skills.cancel', { defaultValue: '取消' })}
              </Button>
              <Button type='primary' loading={saving} onClick={() => void save()}>
                {t('openhub.skills.save', { defaultValue: '保存' })}
              </Button>
            </div>
          ) : (
            <div className='flex justify-end gap-8px'>
              <Button onClick={() => setEditName(null)}>{t('openhub.skills.close', { defaultValue: '关闭' })}</Button>
              <Button type='primary' onClick={() => setEditMode(true)}>
                {t('openhub.skills.edit', { defaultValue: '编辑' })}
              </Button>
            </div>
          )
        }
      ...
      <Modal
        title={t('openhub.skills.teach', { defaultValue: '示范教学' })}
        visible={teachOpen}
        onOk={() => void teach()}
      ...
      >
        <div className='flex flex-col gap-8px'>
          ...
          <Input
            ...
            placeholder={t('openhub.skills.teachPlaceholder', { defaultValue: '会话 ID' })}
          />
        </div>
      </Modal>

      <Modal
        title={t('openhub.skills.gift', { defaultValue: '赠送' })}
      ...
      >
        <div className='flex flex-col gap-8px'>
          <span className='text-12px text-t-secondary'>
            {t('openhub.skills.giftHint', { defaultValue: '把这个技能赠送给另一个伙伴（对方会获得一份副本）。' })}
          </span>
          <Select
            value={giftTarget || undefined}
            onChange={setGiftTarget}
            placeholder={t('openhub.skills.giftPick', { defaultValue: '选择接收的伙伴' })}
          >
            {others.map((c) => (
