...
const SettingsTab: React.FC<Props> = ({ companion, onDeleted }) => {
  ...
  const confirmDelete = useCallback(() => {
    ...
    Modal.confirm({
      title: t('openhub.settings.deleteConfirmTitle'),
      content: t('openhub.settings.deleteConfirmBody', { companionName }),
      okButtonProps: { status: 'danger' },
      onOk: async () => {
        try {
          await ipcBridge.companion.deleteCompanion.invoke({ companion_id: profile.id });
          Message.success(t('openhub.settings.deleted', { companionName }));
  ...
  return (
    <div className='flex flex-col gap-10px py-8px'>
      {row(
        t('openhub.settings.name'),
        t('openhub.settings.nameHint'),
        <Input style={{ width: 260 }} value={nameDraft} onChange={onNameChange} maxLength={30} />
      )}
      {row(
        t('openhub.settings.character'),
        t('openhub.settings.characterHint'),
        <CharacterPicker
      ...
      {row(
        t('openhub.settings.persona'),
        t('openhub.settings.personaHint', { companionName }),
        <div className='flex flex-col gap-8px'>
          ...
          >
            <Radio value='lively'>{t('openhub.settings.personaLively')}</Radio>
            <Radio value='calm'>{t('openhub.settings.personaCalm')}</Radio>
            <Radio value='sassy'>{t('openhub.settings.personaSassy')}</Radio>
          </Radio.Group>
          <Input.TextArea
            rows={2}
            placeholder={t('openhub.settings.personaCustomPlaceholder')}
            value={customDraft}
      ...
      {row(
        t('openhub.settings.quietHours'),
        t('openhub.settings.quietHoursHint'),
      ...
      <div className='mt-8px text-13px font-600 text-[rgb(var(--danger-6))]'>{t('openhub.settings.danger')}</div>
      {row(
        t('openhub.settings.deleteCompanion'),
        t('openhub.settings.deleteCompanionHint', { companionName }),
