...
const OverviewTab: React.FC<Props> = ({ companion, onGoTab }) => {
  ...
  const acknowledgeDisclosure = () => {
    void ipcBridge.companion.applyConsent
      .invoke()
      .then(() => {
        Message.success(t('openhub.disclosure.enabled', { defaultValue: '已开启，桌面伙伴会从你的使用里学习' }));
    ...
    <div className='flex flex-col gap-16px py-8px'>
      {!disclosureSeen && (
        <Alert
          ...
          title={t('openhub.disclosure.title', { defaultValue: '让桌面伙伴越用越懂你' })}
          content={
            <div className='flex flex-col gap-8px'>
              <span className='text-12px text-t-secondary'>
                {t('openhub.disclosure.body', {
                  defaultValue:
              ...
              <div className='flex gap-8px'>
                <Button size='small' type='primary' onClick={acknowledgeDisclosure}>
                  {t('openhub.disclosure.enable', { defaultValue: '开启自学习' })}
                </Button>
                <Button size='small' onClick={dismissDisclosure}>
                  {t('openhub.disclosure.later', { defaultValue: '暂不开启' })}
                </Button>
              </div>
            </div>
          }
      ...
      <div className='flex items-center gap-16px bg-fill-2 rd-10px px-14px py-12px'>
        <div className='flex-1 min-w-0'>
          <div className='text-14px text-t-primary font-500'>{t('openhub.settings.companionEnabled')}</div>
          <div className='text-12px text-t-tertiary mt-2px'>{t('openhub.settings.companionEnabledHint')}</div>
        </div>
        <span className='text-13px text-t-secondary'>
          {profile.appearance.companion_enabled ? t('openhub.overview.companionOn') : t('openhub.overview.companionOff')}
        </span>
        <Switch
      ...
      >
        <div className='text-12px text-t-secondary'>
          {status.model_configured
            ? t('openhub.chat.modelConfigHint')
            : t('openhub.overview.modelMissing', { companionName })}
        </div>
      ...
      {!status.collect_any_enabled && (
        <Alert
          type='info'
          style={{ background: 'var(--color-fill-2)', border: '1px solid var(--color-border-2)' }}
          content={t('openhub.overview.collectOff', { companionName })}
          action={
            <Button size='mini' onClick={() => onGoTab('collect')}>
              {t('openhub.overview.goCollect')}
            </Button>
          }
        />
      )}
      <div className='flex items-center gap-20px flex-wrap'>
        <div className='flex flex-col items-center gap-8px'>
          <button
            ...
            title={t('openhub.customFigure.adjustFigure')}
          ...
          >
            ...
            <span className='absolute inset-4px flex items-end justify-center rd-16px bg-gradient-to-t from-[rgba(0,0,0,0.45)] to-transparent opacity-0 group-hover:opacity-100 transition-opacity'>
              <span className='mb-8px flex items-center gap-4px text-12px font-600 text-white'>
                <IconEdit /> {t('openhub.customFigure.adjustFigure')}
              </span>
            </span>
          </button>
          <Button size='mini' type='text' icon={<IconEdit />} onClick={() => setAdjustOpen(true)}>
            {t('openhub.customFigure.adjustFigure')}
          </Button>
        </div>
        <div className='flex flex-col gap-8px min-w-220px flex-1'>
          <div className='flex items-center gap-8px'>
            <span className='text-18px font-700 text-t-primary'>
              {companionName} · Lv{status.level} {t(`openhub.levels.l${Math.min(status.level, 5)}`)}
            </span>
            <Tag color='pinkpurple'>
              {MOOD_EMOJI[status.mood] || '😌'} {t(`openhub.moods.${status.mood}`, status.mood)}
            </Tag>
          </div>
          ...
          <div className='flex gap-16px text-13px text-t-secondary flex-wrap'>
            <span>
              {t('openhub.overview.memories')}: <b className='text-t-primary'>{status.memories_active}</b>
            </span>
            <span>
              {t('openhub.overview.newSuggestions')}: <b className='text-t-primary'>{status.suggestions_new}</b>
            </span>
            <span>
              {t('openhub.overview.skillsActive', { defaultValue: '专精技能' })}:{' '}
      ...
      {cf && (
        <div className='flex items-center gap-12px bg-fill-2 rd-10px px-14px py-12px flex-wrap'>
          <span className='text-13px text-t-secondary shrink-0'>{t('openhub.customFigure.sizeLabel')}</span>
          <span className='text-12px text-t-tertiary shrink-0'>{t('openhub.customFigure.sizeS')}</span>
          <Slider
          ...
          />
          <span className='text-12px text-t-tertiary shrink-0'>{t('openhub.customFigure.sizeL')}</span>
          ...
          {cf.sizePx != null && (
            <Button size='mini' type='text' onClick={onFigureSizeReset}>
              {t('openhub.customFigure.sizeReset')}
            </Button>
          )}
        </div>
      )}
      {digest && (digest.skills_learned > 0 || digest.memories_added > 0 || digest.learn_runs > 0) && (
        <div className='bg-fill-2 rd-10px px-14px py-12px'>
          <div className='text-14px text-t-primary font-500 mb-6px'>
            {t('openhub.overview.weeklyTitle', { defaultValue: '我这周学到了什么' })}
          </div>
          <div className='flex gap-16px text-13px text-t-secondary flex-wrap'>
            <span>
              {t('openhub.overview.weeklySkills', { defaultValue: '新技能' })}:{' '}
              <b className='text-t-primary'>{digest.skills_learned}</b>
            </span>
            <span>
              {t('openhub.overview.weeklyMemories', { defaultValue: '新记忆' })}:{' '}
              <b className='text-t-primary'>{digest.memories_added}</b>
            </span>
            <span>
              {t('openhub.overview.weeklyRuns', { defaultValue: '学习次数' })}:{' '}
      ...
      <div>
        <h3 className='m-0 mb-8px text-15px text-t-primary'>{t('openhub.overview.diary', { companionName })}</h3>
        {diaries.length === 0 ? (
          <div className='text-13px text-t-tertiary'>{t('openhub.overview.diaryEmpty', { companionName })}</div>
        ) : (
      ...
      <Modal
        ...
        footer={
          <Button type='primary' onClick={() => setAdjustOpen(false)}>
            {t('openhub.customFigure.done')}
          </Button>
        }
        style={{ width: 600 }}
      >
        <div className='flex flex-col gap-10px'>
          <span className='text-12px text-t-tertiary'>{t('openhub.settings.characterHint')}</span>
          <CharacterPicker
