...
const MigrateTab: React.FC<Props> = ({ companions }) => {
  ...
  const exportMemory = async () => {
    ...
    try {
      ...
      Message.success(t('openhub.migrate.exportMemoryOk', { path: res.dest_path }));
    } catch (e) {
  ...
  const exportCompanion = async () => {
    ...
    try {
      ...
      Message.success(t('openhub.migrate.exportCompanionOk', { path: res.dest_path }));
    } catch (e) {
      Message.error(errText(e));
    } finally {
  ...
  const importBundle = async () => {
    ...
    try {
      ...
      if (outcome.kind === 'memory') {
        Message.success(
          t('openhub.migrate.importMemoryOk', { imported: outcome.imported, skipped: outcome.skipped_duplicates })
        );
      } else {
        // useCompanions refreshes the roster via the companion.created WS event.
        Message.success(t('openhub.migrate.importCompanionOk', { name: outcome.name }));
  ...
  const rebuildKnowledgeBinding = async (outcome: Extract<ImportOutcome, { kind: 'companion' }>) => {
    ...
    try {
      ...
      if (matchedIds.length) {
        ...
        Message.success(t('openhub.migrate.bindingRebuilt', { count: matchedIds.length }));
      }
  ...
  const importKnowledge = async () => {
    ...
    try {
      ...
      Message.success(t('openhub.migrate.importKnowledgeOk', { name: base.name }));
    } catch (e) {
      Message.error(errText(e));
    } finally {
  ...
  if (!isTauriRuntime()) {
    return (
      <div className='py-40px'>
        <Empty description={t('openhub.migrate.desktopOnly')} />
      </div>
    );
  }
  ...
  return (
    <div className='flex flex-col gap-16px py-8px'>
      ...
      <section className='bg-fill-2 rd-10px px-14px py-12px'>
        <div className='text-14px text-t-primary font-500'>{t('openhub.migrate.memoryTitle')}</div>
        <div className='text-12px text-t-tertiary mt-2px'>{t('openhub.migrate.memoryDesc')}</div>
        <div className='flex items-center gap-16px mt-10px flex-wrap'>
          <Checkbox checked={includeEvents} onChange={(checked: boolean) => setIncludeEvents(checked)}>
            <span className='text-13px text-t-secondary'>{t('openhub.migrate.includeEvents')}</span>
          </Checkbox>
          <Button type='primary' loading={exportingMemory} onClick={() => void exportMemory()}>
            {t('openhub.migrate.exportMemory')}
          </Button>
        </div>
      </section>
      ...
      {/* 2. companion bundle */}
      <section className='bg-fill-2 rd-10px px-14px py-12px'>
        <div className='text-14px text-t-primary font-500'>{t('openhub.migrate.companionTitle')}</div>
        <div className='text-12px text-t-tertiary mt-2px'>{t('openhub.migrate.companionDesc')}</div>
        <div className='flex items-center gap-12px mt-10px flex-wrap'>
          <Select
            style={{ width: 220 }}
            placeholder={t('openhub.migrate.companionPlaceholder')}
          ...
          <Button type='primary' disabled={!effectiveCompanionId} loading={exportingCompanion} onClick={() => void exportCompanion()}>
            {t('openhub.migrate.exportCompanion')}
          </Button>
        </div>
      </section>
      ...
      {/* 3. import */}
      <section className='bg-fill-2 rd-10px px-14px py-12px'>
        <div className='text-14px text-t-primary font-500'>{t('openhub.migrate.importTitle')}</div>
        <div className='text-12px text-t-tertiary mt-2px'>{t('openhub.migrate.importDesc')}</div>
        <div className='flex items-center gap-12px mt-10px flex-wrap'>
          <Button loading={importingBundle} onClick={() => void importBundle()}>
            {t('openhub.migrate.importBundle')}
          </Button>
          <Button loading={importingKb} onClick={() => void importKnowledge()}>
            {t('openhub.migrate.importKnowledge')}
          </Button>
        </div>
        {unmatchedNames.length > 0 && (
          <Alert
            ...
            content={
              <div>
                <div>{t('openhub.migrate.unmatchedTitle')}</div>
