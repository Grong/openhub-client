...
import type { useCompanionShared } from '../useOpenHub';
...
const LearnTab: React.FC<Props> = ({ shared }) => {
  ...
  const runNow = useCallback(async () => {
    ...
    try {
      ...
      if (run.status === 'ok') {
        Message.success(t('openhub.learn.runOk', { memories: run.memories_added, suggestions: run.suggestions_added }));
      } else {
        Message.info(t(`openhub.learn.status.${run.status}`, run.status));
      }
      refreshRuns();
    } catch (e) {
    ...
    <div className='flex flex-col gap-16px py-8px'>
      <div className='flex items-center gap-16px flex-wrap'>
        <div className='flex items-center gap-8px'>
          <span className='text-13px text-t-secondary'>{t('openhub.learn.enabled')}</span>
          <Switch
        ...
        <div className='flex items-center gap-8px'>
          <span className='text-13px text-t-secondary'>{t('openhub.learn.interval')}</span>
          <InputNumber
            style={{ width: 120 }}
            min={5}
            max={1440}
      ...
      <div className='flex items-start gap-16px bg-fill-2 rd-10px px-14px py-12px'>
        <div className='w-200px shrink-0'>
          <div className='text-14px text-t-primary font-500'>{t('openhub.learn.model')}</div>
          <div className='text-12px text-t-tertiary mt-2px'>{t('openhub.learn.modelHint')}</div>
        </div>
        <div className='flex-1 min-w-0 flex gap-8px flex-wrap'>
          <Select
            style={{ width: 220 }}
            placeholder={t('openhub.settings.providerPlaceholder')}
          ...
          <Select
            style={{ width: 260 }}
            placeholder={t('openhub.settings.modelPlaceholder')}
      ...
      <div className='flex items-start gap-16px bg-fill-2 rd-10px px-14px py-12px'>
        <div className='w-200px shrink-0'>
          <div className='text-14px text-t-primary font-500'>{t('openhub.evolve.title', { defaultValue: '技能进化' })}</div>
          <div className='text-12px text-t-tertiary mt-2px'>
            {t('openhub.evolve.hint', { defaultValue: '从你重复的多步操作里自动沉淀技能，复用上面的学习模型。' })}
          </div>
        </div>
        <div className='flex-1 min-w-0 flex flex-col gap-10px'>
          <div className='flex items-center gap-8px'>
            <span className='text-13px text-t-secondary'>{t('openhub.evolve.enabled', { defaultValue: '开启技能进化' })}</span>
            <Switch
          ...
          <div className='flex items-center gap-8px'>
            <span className='text-13px text-t-secondary'>{t('openhub.evolve.reflect', { defaultValue: '任务后反思' })}</span>
            <Switch
            ...
            <span className='text-11px text-t-tertiary'>
              {t('openhub.evolve.reflectHint', { defaultValue: '复杂任务完成后也反思是否值得固化' })}
            </span>
          </div>
          <div className='flex items-center gap-8px'>
            <span className='text-13px text-t-secondary'>{t('openhub.evolve.autoActivate', { defaultValue: '高置信自动生效' })}</span>
            <Switch
              checked={sharedConfig.evolve.auto_activate}
              onChange={(checked) => void patchSharedConfig({ evolve: { auto_activate: checked } })}
            />
            <span className='text-11px text-t-tertiary'>
              {t('openhub.evolve.autoActivateHint', { defaultValue: '多次重复的高置信技能直接启用（仍可在技能页撤销）' })}
            </span>
          </div>
          <div className='flex items-center gap-8px flex-wrap'>
            <span className='text-13px text-t-secondary'>{t('openhub.evolve.interval', { defaultValue: '挖掘间隔' })}</span>
            <InputNumber
              style={{ width: 120 }}
              min={5}
              max={1440}
            ...
            <span className='text-13px text-t-secondary ml-8px'>
              {t('openhub.evolve.minSessions', { defaultValue: '重复会话数阈值' })}
            </span>
            <InputNumber
              style={{ width: 90 }}
              min={2}
      ...
      <div className='flex items-start gap-16px bg-fill-2 rd-10px px-14px py-12px'>
        <div className='w-200px shrink-0'>
          <div className='text-14px text-t-primary font-500'>{t('openhub.archive.title')}</div>
          <div className='text-12px text-t-tertiary mt-2px'>{t('openhub.archive.hint')}</div>
        </div>
        <div className='flex-1 min-w-0 flex flex-col gap-10px'>
          <div className='flex items-center gap-8px'>
            <span className='text-13px text-t-secondary'>{t('openhub.archive.enabled')}</span>
            <Switch
          ...
          <div className='flex items-center gap-8px'>
            <span className='text-13px text-t-secondary'>{t('openhub.archive.idleMinutes')}</span>
            <InputNumber
              style={{ width: 120 }}
              min={5}
              max={1440}
      ...
      <div className='flex items-start gap-16px bg-fill-2 rd-10px px-14px py-12px'>
        <div className='w-200px shrink-0'>
          <div className='text-14px text-t-primary font-500'>{t('openhub.orchestration.title')}</div>
          <div className='text-12px text-t-tertiary mt-2px'>{t('openhub.orchestration.hint')}</div>
        </div>
        <div className='flex-1 min-w-0 flex flex-col gap-10px'>
          <div className='flex items-center gap-8px'>
            <span className='text-13px text-t-secondary'>{t('openhub.orchestration.enabled')}</span>
            <Switch
      ...
      <Table
        ...
        columns={[
          {
            title: t('openhub.learn.colTime'),
            dataIndex: 'started_at',
            render: (v: number) => new Date(v).toLocaleString(),
          },
          {
            title: t('openhub.learn.colStatus'),
            dataIndex: 'status',
            render: (s: string) => <Tag color={STATUS_COLOR[s] || 'gray'}>{t(`openhub.learn.status.${s}`, s)}</Tag>,
          },
          { title: t('openhub.learn.colEvents'), dataIndex: 'events_processed' },
          { title: t('openhub.learn.colMemories'), dataIndex: 'memories_added' },
          { title: t('openhub.learn.colSuggestions'), dataIndex: 'suggestions_added' },
          {
            title: t('openhub.learn.colNote'),
            dataIndex: 'summary',
