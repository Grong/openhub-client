...
const SecretsTab: React.FC<Props> = ({ companion }) => {
  ...
  const submitAdd = useCallback(async () => {
    ...
    const allowedOrigins = origins
      ...
      .filter((o) => o.length > 0);
    if (!trimmedName) {
      Message.warning(t('openhub.secrets.nameRequired'));
      return;
    }
    if (!value) {
      Message.warning(t('openhub.secrets.valueRequired'));
      return;
    }
    if (allowedOrigins.length === 0) {
      Message.warning(t('openhub.secrets.originsRequired'));
      return;
    }
    setSaving(true);
    try {
      ...
      Message.success(t('openhub.secrets.registered'));
      setAddVisible(false);
  ...
  const remove = useCallback(
    async (secretName: string) => {
      try {
        ...
        Message.success(t('openhub.secrets.removed'));
        await refresh();
      } catch (e) {
        Message.error(String(e));
      }
    ...
    <div className='flex flex-col gap-10px py-8px'>
      <div className='flex items-start gap-16px bg-fill-2 rd-10px px-14px py-12px'>
        <div className='w-200px shrink-0'>
          <div className='text-14px text-t-primary font-500'>{t('openhub.secrets.title')}</div>
          <div className='text-12px text-t-tertiary mt-2px'>{t('openhub.secrets.hint')}</div>
        </div>
        <div className='flex-1 min-w-0 flex flex-col gap-8px'>
          <div>
            <Button type='primary' size='small' onClick={openAdd}>
              {t('openhub.secrets.add')}
            </Button>
          </div>
          {loading ? (
            ...
            <Empty description={t('openhub.secrets.empty')} />
          ) : (
            <div className='flex flex-col gap-6px'>
              {secrets.map((s) => (
                ...
                >
                  ...
                  <Popconfirm
                    title={t('openhub.secrets.removeConfirm', { name: s.name })}
                    onOk={() => remove(s.name)}
                  >
                    <Button size='mini' status='danger' type='text'>
                      {t('openhub.secrets.delete')}
                    </Button>
                  </Popconfirm>
                </div>
              ))}
      ...
      <Modal
        title={t('openhub.secrets.add')}
        visible={addVisible}
        onOk={submitAdd}
        confirmLoading={saving}
        onCancel={() => setAddVisible(false)}
        okText={t('openhub.secrets.register')}
      >
        <div className='flex flex-col gap-12px'>
          <div>
            <div className='text-12px text-t-secondary mb-4px'>{t('openhub.secrets.nameLabel')}</div>
            <Input value={name} onChange={setName} placeholder={t('openhub.secrets.namePlaceholder')} />
          </div>
          <div>
            <div className='text-12px text-t-secondary mb-4px'>{t('openhub.secrets.valueLabel')}</div>
            <Input.Password
              value={value}
              onChange={setValue}
              placeholder={t('openhub.secrets.valuePlaceholder')}
            />
            <div className='text-11px text-t-tertiary mt-4px'>{t('openhub.secrets.valueWriteOnly')}</div>
          </div>
          <div>
            <div className='text-12px text-t-secondary mb-4px'>{t('openhub.secrets.originsLabel')}</div>
            <Input.TextArea
              value={origins}
              onChange={setOrigins}
              placeholder={t('openhub.secrets.originsPlaceholder')}
              autoSize={{ minRows: 2, maxRows: 4 }}
            />
            <div className='text-11px text-t-tertiary mt-4px'>{t('openhub.secrets.originsHint')}</div>
          </div>
        </div>
      </Modal>
    </div>
