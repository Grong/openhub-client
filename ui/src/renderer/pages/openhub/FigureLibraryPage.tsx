    ...
    <>
      <div className='group relative flex flex-col overflow-hidden rd-16px bg-fill-2 border border-solid border-[var(--color-border-2)] transition-all duration-200 hover:-translate-y-2px hover:shadow-[0_10px_28px_rgba(var(--primary-rgb),0.18)] hover:border-[var(--color-primary)]'>
        ...
        <FigureActionSurface>
          <FigureActionButton
            tone='primary'
            title={t('openhub.customFigure.editFigure')}
            ariaLabel={t('openhub.customFigure.editFigure')}
          ...
          <FigureActionButton
            tone='danger'
            disabled={inUse}
            title={inUse ? t('openhub.customFigure.inUseCannotDelete') : t('openhub.customFigure.delete')}
            ariaLabel={inUse ? t('openhub.customFigure.inUseCannotDelete') : t('openhub.customFigure.delete')}
...
const FigureLibraryPage: React.FC = () => {
  ...
  const confirmDelete = (fig: IFigureMeta): void => {
    Modal.confirm({
      title: t('openhub.customFigure.libraryTitle'),
      content: t('openhub.customFigure.deleteConfirm'),
        ...
        try {
          ...
          const msg =
            isBackendHttpError(e) && e.code === 'CONFLICT'
              ? t('openhub.customFigure.inUseCannotDelete')
              : `${t('openhub.customFigure.deleteFailed')}: ${String(e)}`;
          Message.error(msg);
        }
      },
    });
    ...
    <div className='flex flex-col gap-16px py-8px'>
      ...
      <div className='flex items-end justify-between gap-12px flex-wrap'>
        <div className='flex flex-col gap-2px'>
          <h2 className='m-0 text-17px font-700 text-t-primary'>{t('openhub.customFigure.libraryTitle')}</h2>
          <span className='text-12px text-t-tertiary'>{t('openhub.customFigure.homeEntryHint')}</span>
        </div>
        <Button type='primary' icon={<IconPlus />} onClick={() => setWizardOpen(true)}>
          {t('openhub.customFigure.createNew')}
        </Button>
      </div>
      ...
      {loading && !loaded ? (
        ...
        <div className='flex flex-col items-center justify-center gap-14px py-56px rd-16px bg-fill-1 border border-dashed border-[var(--color-border-2)]'>
          ...
          <div className='flex flex-col items-center gap-4px'>
            <span className='text-14px font-600 text-t-primary'>{t('openhub.customFigure.libraryEmpty')}</span>
            <span className='text-12px text-t-tertiary'>{t('openhub.customFigure.copyrightHint')}</span>
          </div>
          <Button type='primary' icon={<IconPlus />} onClick={() => setWizardOpen(true)}>
            {t('openhub.customFigure.createNew')}
          </Button>
        ...
        <div className='grid gap-14px' style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(148px, 1fr))' }}>
          ...
          >
            ...
            <span className='text-12px font-600'>{t('openhub.customFigure.createNew')}</span>
          </button>
        </div>
      )}
      ...
      <CustomFigureWizard
        ...
        onDone={(figure) => {
          setWizardOpen(false);
          add(figure);
          Message.success(t('openhub.customFigure.savedToLibrary'));
        }}
      />
    </div>
  );
