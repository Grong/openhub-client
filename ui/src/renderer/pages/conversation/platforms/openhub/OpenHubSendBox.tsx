...
const useSendBoxDraft = (conversation_id: number) => {
  const { data, mutate } = useOpenHubSendBoxDraft(String(conversation_id));
...
const OpenHubSendBox: React.FC<{
  ...
  const executeCommand = useCallback(
    async ({ input, files }: Pick<ConversationCommandQueueItem, 'input' | 'files'>) => {
      ...
      try {
        ...
        if (files.length > 0) {
          emitter.emit('openhub.workspace.refresh');
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const processInitialMessage = async () => {
      ...
      try {
        ...
        console.error('[OpenHubSendBox] Failed to send initial message:', error);
  ...
  const handleSheetModeChange = useCallback(
    async (mode: string) => {
      ...
      try {
        ...
        console.error('[OpenHubSendBox] Failed to switch mode via sheet:', error);
  ...
  useAddEventListener('openhub.selected.file', setAtPath);
  useAddEventListener('openhub.selected.file.append', (selectedItems: Array<string | FileOrFolderItem>) => {
  ...
  const handleStop = async (): Promise<void> => {
    ...
    try {
      ...
      console.warn('[OpenHubSendBox] stop request failed', error);
    } finally {
      resetState();
  ...
  const handleClearContext = async (): Promise<void> => {
    try {
      ...
      console.warn('[OpenHubSendBox] clear context failed', error);
      Message.error({
    ...
    <div className='max-w-800px w-full mx-auto flex flex-col mt-auto mb-16px'>
      ...
      <SendBox
        ...
        onSelectedWorkspaceItemsChange={(items) => {
          emitter.emit('openhub.selected.file', items);
          setAtPath(items);
        }}
        loading={isBusy}
                    ...
                    return (
                      <Tag
                        ...
                        onClose={() => {
                          ...
                          emitter.emit('openhub.selected.file', newAtPath);
                          setAtPath(newAtPath);
                        }}
                      >
                        {item.name}
