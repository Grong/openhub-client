...
export const useOpenHubMessage = (
  ...
  useEffect(() => {
    return ipcBridge.conversation.responseStream.on((message) => {
      ...
      switch (message.type) {
        ...
        case 'turn_completed':
          {
            ...
            // usage for openhub turns (the finish event has never carried usage) —
            ...
            if (metrics && typeof metrics === 'object') {
              ...
              emitter.emit('openhub.usage.updated', { conversation_id, tokenUsage: newTokenUsage });
        ...
        case 'acp_permission':
          dispatchTurn({ type: 'activity' });
          // Backend openhub emits wire type 'acp_permission' but the payload is
