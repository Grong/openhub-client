...
struct GetBaseParams {
    /// Knowledge base id (from openhub_knowledge_list_bases).
    kb_id: String,
}
...
struct DeleteTagParams {
    /// Tag key to delete (from openhub_knowledge_list_tags).
    key: String,
}
...
pub(crate) fn register(out: &mut Vec<Capability>) {
    ...
    out.push(Capability::new::<DiscardInboxParams, _, _>(
        CapabilityMeta::new(
            "openhub_knowledge_discard_inbox",
            "knowledge",
