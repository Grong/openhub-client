...
pub(crate) fn summarize_provider(row: &openhub_db::models::Provider) -> ProviderSummary {
...
pub(crate) fn resolve_model_chain(
    ...
    let require_enabled = |pid: &str| -> Result<&ProviderSummary, String> {
        let p = find(pid)
            .ok_or_else(|| format!("provider '{pid}' not found; call openhub_list_providers for valid ids"))?;
        if !p.enabled {
            return Err(format!(
                "provider '{}' ({}) is disabled; pick another via openhub_list_providers",
                p.name, p.id
            ));
        }
        Ok(p)
    ...
    match (explicit_provider, explicit_model) {
        ...
        (None, Some(model)) => {
            let p = providers
                ...
                .ok_or_else(|| {
                    format!("no enabled provider offers model '{model}'; call openhub_list_providers for valid combinations")
                })?;
            return Ok(ResolvedModel {
...
pub(crate) async fn resolve_openhub_model(
    deps: &GatewayDeps,
    ctx: &CallerCtx,
    explicit_provider: Option<&str>,
    explicit_model: Option<&str>,
...
/// `openhub_update_conversation`, where a model change is an explicit owner
...
mod tests {
    ...
    fn nothing_resolvable_returns_guidance_error() {
        ...
        assert!(err.contains("openhub_list_providers"), "{err}");
    }

    #[test]
    fn summarize_filters_per_model_enabled_map() {
        let row = openhub_db::models::Provider {
            id: "p1".into(),
