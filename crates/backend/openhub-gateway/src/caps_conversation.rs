...
use openhub_common::{AgentType, AppError};
...
struct CreateConversationParams {
    ...
    #[serde(default)]
    name: Option<String>,
    /// Agent type: "openhub" (default) or "acp". NOT for terminals — any
    /// terminal/shell intent must go through openhub_create_terminal instead.
    #[serde(default)]
...
struct UpdateConversationParams {
    /// The id of the conversation to update (from openhub_list_conversations).
    conversation_id: i64,
...
struct AgentRunParams {
    ...
    /// handle (default 300, clamped 5..1800). Poll openhub_agent_result afterwards.
    #[serde(default)]
...
struct AgentResultParams {
    /// The conversation id returned by openhub_agent_run.
    conversation_id: i64,
}
...
async fn send(deps: Arc<GatewayDeps>, ctx: CallerCtx, p: SendToConversationParams) -> Value {
    ...
    match deps.conversation_service.send_message(&user_id, &id, req, &deps.task_manager).await {
        Ok(msg_id) => ok(json!({
            "msg_id": msg_id,
            "note": "message accepted; the target session processes it asynchronously — use openhub_conversation_status to follow progress"
        })),
        Err(AppError::Conflict(m)) => json!({
            "error": format!("busy: the target conversation is already running a turn ({m}); check openhub_conversation_status and retry later")
        }),
        Err(e) => error_value(e),
    }
}
...
async fn create(deps: Arc<GatewayDeps>, ctx: CallerCtx, p: CreateConversationParams) -> Value {
    ...
    let agent_type_str = p.agent_type.unwrap_or_else(|| "openhub".to_owned());
    if agent_type_str == "terminal" {
        return json!({
            "error": "terminal sessions are not conversations: use openhub_create_terminal (preset shell | claude | codex | gemini) for any terminal/shell intent"
        });
    }
...
/// finished, false on timeout (the run keeps going; poll openhub_agent_result). An
...
async fn subscribe_turn(
    ...
    conv_id: &str,
    wait: Duration,
) -> Option<tokio::sync::broadcast::Receiver<openhub_ai_agent::AgentStreamEvent>> {
...
async fn drain_stream(
    rx: &mut tokio::sync::broadcast::Receiver<openhub_ai_agent::AgentStreamEvent>,
...
/// timeout returns a `{status:"running"}` handle (poll `openhub_agent_result`).
async fn agent_run(deps: Arc<GatewayDeps>, ctx: CallerCtx, p: AgentRunParams, progress: ProgressSink) -> Value {
    ...
    if finished {
        ...
        // `openhub_agent_result` never hits this because it gates on the runtime turn
        ...
        ok(json!({
            ...
            "status": "running",
            "note": "agent run still in progress after timeout; poll openhub_agent_result with this conversation_id"
        }))
    }
}
...
pub(crate) fn register(out: &mut Vec<Capability>) {
    ...
    out.push(Capability::new::<AgentResultParams, _, _>(
        CapabilityMeta::new(
            "openhub_agent_result",
            "agent",
            "Fetch the result (or running status) of a goal delegated via openhub_agent_run, by conversation id.",
            DangerTier::Read,
        ),
        agent_result,
    ));
