...
//! (`openhub_secret::pet_vault_path` now ignores its key and routes to
...
use openhub_browser::{BrowserSecretSource, BrowserTool, OUT_OF_BAND_CONFIRMED_KEY};
...
impl openhub_browser::BrowserApprovalGate for GatewayAutoApprovalGate {
    async fn request_approval(&self, _ask: openhub_browser::ApprovalAsk) -> openhub_browser::ApprovalDecision {
        openhub_browser::ApprovalDecision::Approve
    }
}
...
pub struct BrowserRegistry {
    ...
    /// the user approves it via `openhub_browser_confirm`. Bounded-ish: capped per the
...
impl BrowserRegistry {
    ...
    /// Convenience constructor for `openhub-app`'s gateway wiring: build the
    /// registry with the default browser config so the app does not need a direct
    /// `openhub-config` dependency (the gateway already has one behind this feature).
    ...
    fn slot(&self, key: &str) -> Arc<CompanionBrowser> {
        ...
        if let Some(secret_key) = self.secret_key {
            let vault_path = openhub_secret::pet_vault_path(&self.data_dir, key);
    ...
    pub fn classify(&self, key: &str, action: &str, input: &Value) -> openhub_browser::ApprovalTier {
    ...
    pub fn stash_pending(&self, key: &str, input: Value) -> Option<String> {
        let call_id = openhub_common::generate_prefixed_id("browser_oob");
...
mod tests {
    ...
    fn all_companions_resolve_the_same_shared_secret_vault() {
        ...
        for key in ["companion_a", "companion_b", "conversation:5", "_default"] {
            let p = openhub_secret::pet_vault_path(&r.data_dir, key);
        ...
        assert_eq!(
            openhub_secret::pet_vault_path(&r.data_dir, "companion_a"),
            openhub_secret::pet_vault_path(&r.data_dir, "companion_b"),
    ...
    fn classify_builds_slot_and_returns_a_tier_without_launching() {
        ...
        let r = registry();
        use openhub_browser::ApprovalTier;
        assert_eq!(
    ...
    // engines / user-data-dirs, distinct slots). Set OPENHUB_CHROME_BINARY then:
    //   set OPENHUB_CHROME_BINARY=C:\Program Files\Google\Chrome\Application\chrome.exe
    //   cargo nextest run -p openhub-gateway --features browser-use --run-ignored all -E 'test(gateway_browser)'
    ...
    #[tokio::test]
    #[ignore = "需本机/打包 chrome：set OPENHUB_CHROME_BINARY 后 --run-ignored all -E 'test(gateway_browser)'"]
    ...
    //   set OPENHUB_CHROME_BINARY=C:\Program Files\Google\Chrome\Application\chrome.exe
    //   cargo nextest run -p openhub-gateway --features browser-use --run-ignored all -E 'test(gw2_confirmed)'
    #[tokio::test]
    #[ignore = "需本机/打包 chrome：set OPENHUB_CHROME_BINARY 后 --run-ignored all -E 'test(gw2_confirmed)'"]
