...
async fn submit_terminal(deps: Arc<GatewayDeps>, ctx: CallerCtx, p: SubmitTerminalParams) -> Value {
    ...
    if !p.wait {
        return ok(json!({"submitted": true, "id": id, "note": "text submitted; use openhub_terminal_read_output to see the result"}));
    }
...
pub(crate) fn register(out: &mut Vec<Capability>) {
    ...
    out.push(Capability::new::<WriteInputParams, _, _>(
        CapabilityMeta::new(
            "openhub_terminal_write_input",
            "terminal",
            "Write base64-encoded bytes/keystrokes to a terminal's PTY stdin. Powerful: can execute arbitrary commands in the running shell. For sending a command/prompt to run, prefer openhub_terminal_send (handles Enter + agent-CLI paste); use this for raw control bytes like Ctrl-C.",
            DangerTier::Write,
        )
...
mod tests {
    ...
    fn send_and_read_are_registered_and_desktop_visible_but_channel_denied() {
        ...
        assert!(
            reg.tool_visible(crate::registry::Surface::Channel, "openhub_terminal_read_output"),
            "openhub_terminal_read_output is read-only and should follow Channel's default Read visibility"
        );
    }
}
