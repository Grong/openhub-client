...
//! - `openhub_system_check_update` — needs `VersionCheckService`
//! - `openhub_system_factory_reset` — needs `data_dir: PathBuf`
...
struct CreateProviderParams {
    ...
    api_key: String,
    /// Initial model list. If omitted, use openhub_system_fetch_models after
...
struct UpdateProviderParams {
    /// Provider id (from openhub_list_providers).
    id: String,
...
async fn create_provider(deps: Arc<GatewayDeps>, p: CreateProviderParams) -> Value {
    ...
    match deps.provider_service.create(req).await {
        Ok(resp) => ok(json!({
            ...
            "note": "provider created; use openhub_system_fetch_models to populate the model list from the remote API if models were not specified",
        })),
        Err(e) => json!({ "error": e.to_string() }),
    }
}
...
async fn get_info(_deps: Arc<GatewayDeps>, _p: GetInfoParams) -> Value {
    let info = openhub_system::sysinfo::get_system_info();
    ok(info)
}
...
pub(crate) fn register(out: &mut Vec<Capability>) {
    ...
    out.push(Capability::new::<UpdateSettingsParams, _, _>(
        CapabilityMeta::new(
            "openhub_system_update_settings",
            "system",
    ...
    out.push(Capability::new::<GetPreferencesParams, _, _>(
        CapabilityMeta::new(
            "openhub_system_get_preferences",
            "system",
    ...
    out.push(Capability::new::<UpdatePreferencesParams, _, _>(
        CapabilityMeta::new(
            "openhub_system_update_preferences",
            "system",
    ...
    out.push(Capability::new::<CreateProviderParams, _, _>(
        CapabilityMeta::new(
            "openhub_system_create_provider",
            "system",
    ...
    out.push(Capability::new::<UpdateProviderParams, _, _>(
        CapabilityMeta::new(
            "openhub_system_update_provider",
            "system",
    ...
    out.push(Capability::new::<DeleteProviderParams, _, _>(
        CapabilityMeta::new(
            "openhub_system_delete_provider",
            "system",
...
// ── SKIPPED tools ────────────────────────────────────────────────────────
//
// 10. `openhub_system_check_update` (Read)
//     Needs: `deps.version_check_service: openhub_system::VersionCheckService`
...
//
// 11. `openhub_system_factory_reset` (Destructive, deny_on Channel+Remote)
//     Needs: `deps.data_dir: PathBuf`
//     Method: `openhub_common::factory_reset::write_marker(&data_dir, &ResetMarker::new(ResetScope::Full))`
