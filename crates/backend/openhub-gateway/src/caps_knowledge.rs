...
use openhub_api_types::{KnowledgeSource, KnowledgeSourceEntry, KnowledgeSourceMode};
use openhub_knowledge::source_url::truncate_to_bytes;
use openhub_knowledge::{
...
/// Response cap for `openhub_knowledge_fetch_url` markdown bodies.
...
struct WriteFileParams {
    /// Target knowledge base id (from openhub_knowledge_list_bases).
    kb_id: String,
...
fn unknown_kb_error(id: &str) -> Value {
    json!({ "error": format!("unknown knowledge base id '{id}'; call openhub_knowledge_list_bases for valid ids") })
}
...
fn create_base_note(snapshot_urls: usize) -> String {
    if snapshot_urls == 0 {
        return "base created; it only takes effect on a target after openhub_knowledge_set_binding binds it there"
            .to_owned();
    }
    format!(
        "库已创建；{snapshot_urls} 条 URL 正在后台抓取快照并自动生成梗概，请勿重复创建。\
         稍后可用 openhub_knowledge_list_bases 查看（description 出现即完成）；\
         若较长时间后 description 仍未出现，说明抓取可能失败——可改用 openhub_knowledge_fetch_url \
         自行抓取内容、openhub_knowledge_write_file 落库，再 openhub_knowledge_autogen 生成梗概。\
         库仍需 openhub_knowledge_set_binding 绑定到目标后才会生效。"
    )
}
...
async fn write_file(deps: Arc<GatewayDeps>, ctx: CallerCtx, p: WriteFileParams) -> Value {
    ...
    match deps.knowledge_service.write_document(req).await {
        Ok(out) => ok(json!({
            ...
            "staged": out.staged,
            "updated": matches!(out.op, openhub_knowledge::WriteOp::Update),
            "note": "written via the unified write path (placement enforced by session policy); after substantial additions refresh the overview via openhub_knowledge_autogen",
        })),
        Err(e) => json!({ "error": e.to_string() }),
    }
}
...
async fn set_binding(deps: Arc<GatewayDeps>, p: SetBindingParams) -> Value {
    ...
    if p.enabled && binding.kb_ids.is_empty() {
        return json!({ "error": "kb_ids must not be empty when enabling a binding; call openhub_knowledge_list_bases for valid ids" });
    }
...
pub(crate) fn register(out: &mut Vec<Capability>) {
    out.push(Capability::new::<ListBasesParams, _, _>(
        CapabilityMeta::new("openhub_knowledge_list_bases", "knowledge", "List knowledge bases (optionally filtered).", DangerTier::Read),
        |deps, _ctx, p| list_bases(deps, p),
    ));
    out.push(Capability::new::<CreateBaseParams, _, _>(
        CapabilityMeta::new("openhub_knowledge_create_base", "knowledge", "Create a new managed knowledge base, optionally seeded with URL sources (fetched in the background).", DangerTier::Write),
        |deps, _ctx, p| create_base(deps, p),
    ));
    out.push(Capability::new::<WriteFileParams, _, _>(
        CapabilityMeta::new("openhub_knowledge_write_file", "knowledge", "Create/update one markdown document in a base (placement enforced by per-surface policy).", DangerTier::Write),
        write_file,
    ));
    out.push(Capability::new::<AutogenParams, _, _>(
        CapabilityMeta::new("openhub_knowledge_autogen", "knowledge", "Generate the AI overview (description + root README) for a base.", DangerTier::Write),
        |deps, _ctx, p| autogen(deps, p),
    ));
    out.push(Capability::new::<FetchUrlParams, _, _>(
        CapabilityMeta::new("openhub_knowledge_fetch_url", "knowledge", "Server-side fetch + HTML→markdown of a URL (SSRF-guarded).", DangerTier::Read),
        |deps, _ctx, p| fetch_url(deps, p),
    ));
    out.push(Capability::new::<GetBindingParams, _, _>(
        CapabilityMeta::new("openhub_knowledge_get_binding", "knowledge", "Read the knowledge binding for one target (conversation/terminal/companion).", DangerTier::Read),
        |deps, _ctx, p| get_binding(deps, p),
    ));
    out.push(Capability::new::<SetBindingParams, _, _>(
        CapabilityMeta::new("openhub_knowledge_set_binding", "knowledge", "Set the bound base list / toggle a target's knowledge binding and write-back knobs.", DangerTier::Write),
...
mod tests {
    ...
    fn create_base_note_signals_background_fetch() {
        ...
        assert!(plain.contains("openhub_knowledge_set_binding") && !plain.contains("后台"));
    }
}
