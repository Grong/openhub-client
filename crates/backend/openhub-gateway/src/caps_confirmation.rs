...
//! (`openhub_conversation_status`) with no way to read the options or answer.

use std::sync::Arc;

use openhub_api_types::ConfirmRequest;
...
struct ResolveConfirmationParams {
    ...
    conversation_id: i64,
    /// The call_id of the specific pending decision to resolve (from openhub_list_confirmations).
    call_id: String,
...
/// double-key payload IDMM already uses in `openhub-idmm` probe `inject`.
