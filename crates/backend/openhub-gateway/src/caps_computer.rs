...
//! in-tree `openhub_computer::ComputerTool`, mirroring the inward
...
#[derive(Deserialize, JsonSchema)]
struct RefParams {
    /// Element number `[ref]` from the most recent `openhub_computer_snapshot`.
    r#ref: u32,
}
...
pub(crate) fn register(out: &mut Vec<Capability>) {
    ...
    out.push(Capability::new::<NoParams, _, _>(
        CapabilityMeta::new(
            "openhub_computer_cursor_position",
            "computer",
