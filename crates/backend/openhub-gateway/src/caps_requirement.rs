...
struct RequirementUpdateParams {
    /// The id of the requirement to update (from openhub_requirement_list).
    id: i64,
        ...
        Ok(page) => {
                ...
                .find(|r| is_open_duplicate(&r.status, &r.title, &title))
            {
                return ok(json!({
                    ...
                    "note": "an open requirement with this tag + title already exists — nothing was created. Use openhub_requirement_update to refine it; only create a second one if the owner explicitly asked for a duplicate this turn."
                }));
            }
        }
