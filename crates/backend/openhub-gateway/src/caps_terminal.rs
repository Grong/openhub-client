...
use openhub_api_types::CreateTerminalRequest;
...
struct CreateTerminalParams {
    ...
    /// (bind-on-create); they are mounted into `.openhub/knowledge/` inside the
    /// cwd when the terminal starts. Use openhub_knowledge_list_bases for ids.
    #[serde(default)]
...
async fn create(deps: Arc<GatewayDeps>, ctx: CallerCtx, p: CreateTerminalParams) -> Value {
    ...
    // terminal's WORKPATH (spec §7) and mounted into `{cwd}/.openhub/knowledge/`
    ...
    match deps.terminal_service.create(&user_id, req).await {
        Ok(resp) => ok(json!({
            ...
            "note": "terminal created, its process is running; use openhub_list_terminals to check status. Agent terminals (claude/codex/gemini) are eligible AutoWork targets via openhub_set_autowork."
        })),
