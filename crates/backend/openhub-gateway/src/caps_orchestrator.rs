...
//!
//! Backed by:
//! - `openhub_orchestrator::RunService` — the run control-plane
...
//! - `openhub_orchestrator::RunEngine` — the serial execution loop; `start`
//!   spawns (or restarts) the loop that drives ready tasks to completion.
//!
//! `openhub_run_create` performs the create → plan → (conditionally) start
...
//! [`create_adhoc`](openhub_orchestrator::RunService::create_adhoc) path.
//!
...
use openhub_common::{ProviderWithModel, generate_prefixed_id};
...
/// `openhub_run_status` / `openhub_run_result` take a bare `run_id` with no ownership
/// predicate, so advertising/dispatching them externally would let one companion's
/// token read ANY run's status/output, and `openhub_run_create` is a write that
...
struct RunStatusParams {
    /// The run id (from openhub_run_create).
    run_id: String,
}
...
/// convergence). Reuses `openhub_spawn`'s task shape — the `run_id` is NOT a param; it
...
struct AddTasksParams {
    ...
    /// openhub_spawn's tasks: each becomes a visible worker on the orchestration canvas.
    tasks: Vec<SpawnTaskParam>,
}
...
struct RunAdjustParams {
    /// The run to adjust (from openhub_run_create / openhub_spawn).
    run_id: String,
...
struct TaskRerunParams {
    run_id: String,
    /// The task/node id (from openhub_run_status).
    task_id: String,
}
...
async fn create(deps: Arc<GatewayDeps>, ctx: crate::deps::CallerCtx, p: RunCreateParams) -> Value {
    ...
    // the 主管 can follow it to completion IN-TURN (poll `openhub_run_status` /
    // `openhub_run_result` and summarize) — the same engaged experience as `openhub_spawn`.
    ...
    // re-invoking `openhub_run_create` every ~60s → multiple orphaned `planning` runs +
    ...
    // the SAME run_id via `openhub_run_status`/`openhub_run_result` to completion and
    ...
    if lead_conv_id.is_some() {
        ...
        openhub_orchestrator::spawn_plan_and_start(
...
/// (`openhub_spawn` / `openhub_run_add_tasks` append here instead of minting a new run per
...
/// Pure decision core shared by BOTH append seams (`openhub_spawn` convergence +
/// `openhub_run_add_tasks`): would a caller at `caller_depth` exceed the limit by
...
/// two append seams (`openhub_spawn` convergence + `openhub_run_add_tasks`) branch on so
...
enum Appendability {
    ...
    /// it). The `openhub_spawn` path falls through to minting a FRESH run;
    /// `openhub_run_add_tasks` returns the "no active run" error.
    NoRun,
...
/// gated: a manual `openhub_spawn` / `openhub_run_add_tasks` on a supervised/interactive
...
/// — mirroring the `openhub_spawn` follow-up contract, so the main chat never goes
...
fn planning_started_message(interactive: bool) -> String {
    if interactive {
        ...
        "编排已创建(run_id 见返回)，正在后台拆解为任务图并会自动开跑(可在右侧编排画布实时查看每个子 agent 的状态与产出)。请告诉用户已在后台并行执行、进度见画布，然后结束本轮——你无需轮询等待：全部完成或失败时系统会自动把结果回执给你，届时你再向用户汇总。不要再次创建编排；用户若中途询问进度，可用 openhub_run_status 查一次。".to_string()
    }
}
...
/// `openhub_spawn` 的任务数上限：对齐进程内 Spawn（5）与其全局并发上限（8）的量级，
/// 超过应改用 openhub_run_create 让 planner 规划分批。
const MAX_SPAWN_TASKS: usize = 8;

/// Map the flat spawn-shaped tasks (`{name, prompt, role?}`) → `PlannedTask` nodes
/// (kind `agent`, no deps). The SHARED mapping used by `openhub_spawn`, the single-run
/// convergence append, and `openhub_run_add_tasks`, so a spawned node and an appended
/// node are built identically. (`openhub_spawn`'s optional read-only synthesis node is
...
/// Flat fan-out (`openhub_spawn`): create a run, link it to the calling
...
async fn spawn(deps: Arc<GatewayDeps>, ctx: crate::deps::CallerCtx, p: SpawnParams) -> Value {
    ...
    if p.tasks.len() > MAX_SPAWN_TASKS {
        return json!({ "error": format!("too many tasks: {} (max {MAX_SPAWN_TASKS}); use openhub_run_create for larger goals", p.tasks.len()) });
    }
                    ...
                    .await
                {
                    Ok(run) => {
                        ...
                        let message = if run.status == "running" {
                            "子任务已追加到本会话当前的编排 run 并在画布并行执行（复用同一个 run，未新建）。请告诉用户已在后台并行执行、进度见右侧画布，然后结束本轮——你无需轮询等待：全部完成或失败时系统会自动把结果回执给你再汇总。不要重复创建编排；用户若中途询问进度，可用 openhub_run_status 查一次。"
                        } else {
    ...
    // openhub_spawn 的任务是调用方显式给出的，park 在批准门会回归进程内 Spawn 的
    ...
    ok(json!({
        ...
        "status": "running",
        "task_count": n,
        "message": "子任务已在编排画布并行执行，用户能实时看到每个子 agent 的状态与产出。请告诉用户已在后台并行执行、进度见右侧画布，然后结束本轮——你无需轮询等待：全部完成或失败时系统会自动把结果回执给你再汇总。不要重复创建编排；用户若中途询问进度，可用 openhub_run_status 查一次。",
    }))
}
...
async fn run_adjust(deps: Arc<GatewayDeps>, ctx: crate::deps::CallerCtx, p: RunAdjustParams) -> Value {
    ...
    // Phase 3b runaway backstop (deterministic, not cooperative): `openhub_run_adjust`
...
/// convergence primitive (`openhub_run_add_tasks`). The `run_id` is NOT a param: it is
...
/// work with openhub_spawn / openhub_run_create first); an AUTONOMOUS run over its hard
...
/// via the SAME `spawn_tasks_to_planned` helper `openhub_spawn` uses, then append via
...
async fn run_add_tasks(deps: Arc<GatewayDeps>, ctx: crate::deps::CallerCtx, p: AddTasksParams) -> Value {
    ...
    if p.tasks.is_empty() {
        return json!({ "error": "openhub_run_add_tasks requires at least one task" });
    }
    if p.tasks.len() > MAX_SPAWN_TASKS {
        return json!({ "error": format!("too many tasks: {} (max {MAX_SPAWN_TASKS}); use openhub_run_create for larger goals", p.tasks.len()) });
    }
    ...
    let run_id = match read_conversation_run_id(&deps, &user, &ctx.conversation_id).await {
        Some(id) => match run_appendability(&deps, &id).await {
            ...
            Appendability::NoRun => {
                return json!({
                    "error": "this conversation has no active run to append to; start work with openhub_spawn or openhub_run_create first"
                });
            }
        },
        None => {
...
/// route. Retry a failed node — optionally after `openhub_task_config` changes it.
async fn task_rerun(deps: Arc<GatewayDeps>, ctx: crate::deps::CallerCtx, p: TaskRerunParams) -> Value {
    let user = match require_user(&ctx) {
        Ok(u) => u.to_owned(),
        Err(e) => return e,
    };
    // Phase 3b runaway backstop: `openhub_task_rerun` re-arms a terminal run (re-running
...
async fn task_config(deps: Arc<GatewayDeps>, ctx: crate::deps::CallerCtx, p: TaskConfigParams) -> Value {
        ...
        .await
    {
        Ok(()) => ok(json!({
            "run_id": p.run_id,
            "task_id": p.task_id,
            "message": "已更新该节点的模型/预置要求。pending 节点下次派发即生效；已结算节点请再调 openhub_task_rerun 使其以新配置重跑。",
        })),
        Err(e) => json!({ "error": e.to_string() }),
    }
}
...
async fn task_question(
    ...
    if ctx.conversation_id.is_empty() {
        return json!({ "error": "openhub_task_question 仅编排 worker 节点会话可用（无会话上下文）" });
    }
    ...
    let (Some(run_id), Some(task_id)) = (run_id, task_id) else {
        return json!({
            "error": "openhub_task_question 仅编排 worker 节点会话可用：本会话不是编排节点。若你是主 agent，请直接向用户提问。"
        });
    };
    match deps
        .orchestrator_run_engine
...
pub(crate) fn register(out: &mut Vec<Capability>) {
    ...
    out.push(Capability::new::<AddTasksParams, _, _>(
        CapabilityMeta::new(
            "openhub_run_add_tasks",
            "orchestrator",
            "Append new task(s) to the CURRENT run of this conversation — no new run, no approval, no planner; each task is a visible canvas worker. Use to extend work already in flight. Params: tasks (1-8 of {name, prompt, role?}; role searcher/reviewer = read-only tools, verifier = read-only+Bash, omit = full tools). Errors if this conversation has no active run — start one with openhub_spawn / openhub_run_create first. Returns run_id and status.",
            DangerTier::Write,
        )
        .deny_on(ORCHESTRATOR_DENY_SURFACES),
    ...
    out.push(Capability::new::<TaskRerunParams, _, _>(
        CapabilityMeta::new(
            "openhub_task_rerun",
            "orchestrator",
            "Re-run one node of a run (and cascade-reset its already-settled downstream) — e.g. retry a failed node, optionally after changing its model/prompt via openhub_task_config. Params: run_id, task_id (from openhub_run_status).",
            DangerTier::Write,
        )
        .deny_on(ORCHESTRATOR_DENY_SURFACES),
    ...
    out.push(Capability::new::<TaskConfigParams, _, _>(
        CapabilityMeta::new(
            "openhub_task_config",
            "orchestrator",
            "Override a node's model (provider_id + model) and/or preset requirement before re-running it — e.g. route a failing node to a stronger model, then openhub_task_rerun. Params: run_id, task_id, provider_id?, model?, preset_prompt?.",
            DangerTier::Write,
        )
        .deny_on(ORCHESTRATOR_DENY_SURFACES),
...
mod tests {
    ...
    // A conversation-native `openhub_run_create` now keeps the 主管 ENGAGED instead of
    ...
    // `openhub_spawn` follow-up contract — and must NOT tell it to just wait. It still
    ...
    fn planning_started_message_defers_to_auto_report() {
        ...
        assert!(auto.contains("openhub_run_status"), "keeps openhub_run_status as an on-demand option: {auto}");
    ...
    fn orchestrator_tools_registered_and_visible_on_desktop() {
        ...
        for name in ["openhub_run_create", "openhub_spawn", "openhub_run_status", "openhub_run_result", "openhub_run_adjust", "openhub_run_add_tasks", "openhub_task_rerun", "openhub_task_config", "openhub_run_cancel", "openhub_task_question"] {
            assert!(
                reg.contains(name),
    ...
    fn autonomous_run_at_or_over_token_budget_is_exhausted() {
        assert!(
            autonomous_budget_exhausted(
                "autonomous",
                openhub_orchestrator::AUTONOMOUS_TOKEN_BUDGET,
                0
            ),
        ...
        assert!(
            autonomous_budget_exhausted(
                "autonomous",
                openhub_orchestrator::AUTONOMOUS_TOKEN_BUDGET + 1,
                0
            ),
    ...
    fn autonomous_run_at_or_over_task_budget_is_exhausted() {
        assert!(
            autonomous_budget_exhausted("autonomous", 0, openhub_orchestrator::AUTONOMOUS_MAX_TASKS),
    ...
    fn autonomous_run_within_both_budgets_is_not_exhausted() {
        assert!(
            !autonomous_budget_exhausted(
                "autonomous",
                openhub_orchestrator::AUTONOMOUS_TOKEN_BUDGET - 1,
                openhub_orchestrator::AUTONOMOUS_MAX_TASKS - 1
            ),
    ...
    fn non_autonomous_run_is_never_budget_gated() {
        for autonomy in ["supervised", "interactive", "manual", ""] {
            assert!(
                !autonomous_budget_exhausted(
                    autonomy,
                    openhub_orchestrator::AUTONOMOUS_TOKEN_BUDGET * 10,
                    openhub_orchestrator::AUTONOMOUS_MAX_TASKS * 10
                ),
    ...
    fn classify_appendability_covers_all_cases() {
        ...
        assert_eq!(
            classify_appendability(
                ...
                "autonomous",
                0,
                openhub_orchestrator::AUTONOMOUS_MAX_TASKS
            ),
            Appendability::BudgetExhausted
        );
        ...
        assert_eq!(
            classify_appendability(
                "running",
                "supervised",
                openhub_orchestrator::AUTONOMOUS_TOKEN_BUDGET * 5,
                999
            ),
            Appendability::Yes
        );
    ...
    // (openhub_spawn 收敛 + openhub_run_add_tasks) 守卫的纯判定核。
    #[test]
