...
use openhub_api_types::{ListCronJobsQuery, UpdateConversationRequest};
use openhub_common::AgentType;
use openhub_conversation::response_middleware::{
    CronCreateParams as SvcCronCreate, CronUpdateParams as SvcCronUpdate, ICronService,
};
use openhub_cron::types::cron_job_to_response;
...
struct CronUpdateParams {
    /// The id of the cron job to update (from openhub_cron_list).
    job_id: String,
        ...
        Ok(jobs) => {
                ...
                .find(|j| j.enabled && is_duplicate_job(&j.name, &j.message, &p.name, &p.message))
            {
                return ok(json!({
                    ...
                    "note": "an ACTIVE cron job with the same name or message already exists in this conversation — nothing was created. Use openhub_cron_update to modify it; only create a second job if the owner explicitly asked for a duplicate this turn."
                }));
            }
        }
...
fn command_result(result: openhub_conversation::response_middleware::CronCommandResult) -> Value {
    if result.success {
