...
//! field built by `build_module_states` in `openhub_app::router::state`).
...
struct TestPluginParams {
    ...
    /// from openhub_channel_list_plugins.
    plugin_id: String,
...
struct ApprovePairingParams {
    /// The pairing code to approve (from openhub_channel_list_pairings).
    code: String,
}

#[derive(Deserialize, JsonSchema)]
struct RejectPairingParams {
    /// The pairing code to reject (from openhub_channel_list_pairings).
    code: String,
}
...
struct RevokeUserParams {
    /// The internal user id to revoke (from openhub_channel_list_users). This
...
async fn enable_plugin(deps: Arc<GatewayDeps>, p: EnablePluginParams) -> Value {
    use openhub_channel::manager::EnableChannelSpec;
        ...
        .await
    {
        Ok(channel_id) => ok(json!({
            ...
            "note": "bot enabled; use openhub_channel_test_plugin to verify credentials connect successfully"
        })),
        Err(e) => json!({ "error": e.to_string() }),
    }
}
...
async fn test_plugin(deps: Arc<GatewayDeps>, p: TestPluginParams) -> Value {
    use openhub_channel::types::{PluginConfig, PluginCredentials};
...
async fn send_file(deps: Arc<GatewayDeps>, p: SendFileParams) -> Value {
    ...
    let origin = json!({ "source": "openhub_channel_send_file", "path": path });
...
pub(crate) fn register(out: &mut Vec<Capability>) {
    ...
    out.push(Capability::new::<DisablePluginParams, _, _>(
        CapabilityMeta::new(
            "openhub_channel_disable_plugin",
            "channel",
    ...
    out.push(Capability::new::<ApprovePairingParams, _, _>(
        CapabilityMeta::new(
            "openhub_channel_approve_pairing",
            "channel",
    ...
    out.push(Capability::new::<RejectPairingParams, _, _>(
        CapabilityMeta::new(
            "openhub_channel_reject_pairing",
            "channel",
...
mod send_file_tests {
    ...
    fn send_file_is_visible_on_channel_and_desktop() {
        ...
        assert!(reg.tool_visible(Surface::Channel, "openhub_channel_send_file"));
        assert!(reg.tool_visible(Surface::Desktop, "openhub_channel_send_file"));
    }
