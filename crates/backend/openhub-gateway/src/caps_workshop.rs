...
//!
//! Write authority: `openhub_workshop_apply_ops` never edits `canvas.json` under an
...
//! frontend is polling. See `openhub_workshop::agent_ops`.

use std::sync::Arc;

use openhub_creation::{CreationInput, NewCreationTask};
use openhub_workshop::AgentOp;
use openhub_workshop::service::AssetQuery;
...
struct GetCanvasParams {
    /// Canvas id (`wsc_…`) from openhub_workshop_list_canvases.
    canvas_id: String,
}
...
struct ApplyOpsParams {
    ...
    /// openhub_workshop_get_canvas first).
    ops: Vec<AgentOp>,
}
...
struct GetTaskParams {
    /// Generation task id (`wst_…`) from openhub_workshop_generate.
    task_id: String,
}
...
fn summarize_canvas(meta: &openhub_workshop::WorkshopCanvasMeta, doc: &Value) -> Value {
...
pub(crate) fn register(out: &mut Vec<Capability>) {
    out.push(Capability::new::<ListCanvasesParams, _, _>(
        CapabilityMeta::new(
            "openhub_workshop_list_canvases",
            "workshop",
...
mod tests {
    ...
    fn meta() -> openhub_workshop::WorkshopCanvasMeta {
        openhub_workshop::WorkshopCanvasMeta {
            id: "wsc_1".into(),
            title: "c".into(),
            thumbnail_url: None,
            node_count: 250,
