...
use openhub_ai_agent::IWorkerTaskManager;
use openhub_assistant::AssistantService;
use openhub_companion::CompanionService;
use openhub_conversation::ConversationService;
use openhub_cron::service::CronService;
use openhub_db::IProviderRepository;
use openhub_idmm::IdmmService;
use openhub_knowledge::KnowledgeService;
use openhub_requirement::{Orchestrator, RequirementService};
use openhub_system::{ClientPrefService, ModelFetchService, ProviderService, SettingsService};
use openhub_terminal::TerminalService;

/// Everything the gateway tools need to operate the desktop.
///
/// Constructed by `openhub-app` AFTER `build_module_states` (the
...
/// `openhub-app/src/router/routes.rs::inject_gateway_deps` (clone from the
...
pub struct GatewayDeps {
    ...
    /// `openhub_workshop_list_canvases` capability. Main-db rows only (the canvas
    /// bodies live on disk in the workshop service); the same table the
    /// `/api/workshop/*` routes read.
    pub workshop_repo: Arc<dyn openhub_db::IWorkshopRepository>,
    ...
    /// `openhub_workshop_get_canvas` / `openhub_workshop_list_assets` /
    /// `openhub_workshop_apply_ops`.
    pub workshop_service: Arc<openhub_workshop::WorkshopService>,
    /// 生成引擎 (creation) media task queue — the SAME singleton the
    /// `/api/creation/*` routes use. Backs `openhub_workshop_generate` /
    /// `openhub_workshop_get_task` (submit + inspect generation tasks).
    ...
    pub channel_state: openhub_channel::ChannelRouterState,
    /// Filesystem service (path-scoped to the configured allowed roots).
    pub file_service: openhub_file::FileServiceRef,
    /// Shell-open service (OS ShellExecute / `open`).
    pub shell_service: std::sync::Arc<openhub_shell::ShellService>,
    /// MCP server CRUD (same instance as the `/api/mcp` routes).
    pub mcp_config_service: openhub_mcp::McpConfigService,
    /// Extension registry + hub + skills (same instances as the extension routes).
    pub extension_registry: openhub_extension::ExtensionRegistry,
    pub hub_index_manager: openhub_extension::HubIndexManager,
    pub hub_installer: openhub_extension::HubInstaller,
    pub skill_paths: openhub_extension::SkillPaths,
    /// Agent catalog + remote agents (same instances as the agent routes).
    pub agent_service: std::sync::Arc<openhub_ai_agent::AgentService>,
    pub remote_agent_service: std::sync::Arc<openhub_ai_agent::RemoteAgentService>,
    /// Client-preference repo backing the global model-failover config.
    pub client_pref_repo: std::sync::Arc<dyn openhub_db::IClientPreferenceRepository>,
    ...
    pub orchestrator_run_service: Arc<openhub_orchestrator::RunService>,
    ...
    pub orchestrator_run_engine: Arc<openhub_orchestrator::RunEngine>,
    ...
    /// into an enriched [`openhub_api_types::FleetMember`] when creating an ad-hoc
    ...
    /// openhub-assistant (openhub-assistant does NOT depend on gateway — no cycle).
    ...
    /// gateway exposes no `openhub_browser_*` tools. Each companion gets its own
    ...
    /// otherwise the gateway exposes no `openhub_computer_*` tools. See
