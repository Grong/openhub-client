...
impl Registry {
    ...
    fn build() -> Registry {
        ...
        //      wire it in openhub-app/src/router/routes.rs::inject_gateway_deps.
...
mod tests {
    ...
    /// (panics in `build` otherwise), `openhub_`-prefixed, a non-empty summary, a
    ...
    fn registry_builds_and_names_fit_mcp_limit() {
        ...
        for (name, cap) in reg.by_name.iter() {
            assert!(
                name.starts_with("openhub_"),
                "gateway tool names are openhub_-prefixed: {name}"
            );
            assert!(
            ...
            assert!(
                ...
                "tool name exceeds the {TOOL_NAME_BUDGET}-char style budget (keep `openhub_<domain>_<verb_object>` concise): {name} ({} chars)",
                name.len()
            );
            assert!(
    ...
    fn gateway_surfaces_do_not_advertise_team_tools() {
        ...
        for surface in [Surface::Desktop, Surface::Remote, Surface::Channel] {
            let team_tools: Vec<&str> = reg
                ...
                .iter()
                .map(|s| s.name)
                .filter(|name| name.starts_with("openhub_team_"))
                .collect();
            assert!(
                team_tools.is_empty(),
    ...
    fn tool_specs_for_filters_to_domains() {
        ...
        assert!(names.contains(&"openhub_agent_run"));
        assert!(
            !names.contains(&"openhub_system_update_settings"),
