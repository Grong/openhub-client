...
use openhub_api_types::{AutoWorkState, AutoWorkTargetKind};
...
fn parse_conv_id(target_id: &str) -> Result<i64, openhub_common::AppError> {
    target_id
        .parse::<i64>()
        .map_err(|_| openhub_common::AppError::NotFound(format!("conversation {target_id}")))
}
