...
//!    Crate: `openhub-mcp`, type: `openhub_mcp::McpConfigService`.
//!
//! - `extension_registry: ExtensionRegistry`
//!    Clone of `states.extension.registry` (from `ExtensionRouterState`).
//!    Crate: `openhub-extension`, type: `openhub_extension::ExtensionRegistry`.
//!
//! - `hub_installer: HubInstaller`
//!    Clone of `states.hub.installer` (from `HubRouterState`).
//!    Crate: `openhub-extension`, type: `openhub_extension::hub::installer::HubInstaller`.
//!
//! - `hub_index_manager: HubIndexManager`
//!    Clone of `states.hub.index_manager` (from `HubRouterState`).
//!    Crate: `openhub-extension`, type: `openhub_extension::hub::index_manager::HubIndexManager`.
//!
//! - `skill_paths: SkillPaths`
//!    Clone of `states.skill.skill_paths` (from `SkillRouterState`).
//!    Crate: `openhub-extension`, type: `openhub_extension::skill_service::SkillPaths`.
//!
//! ## SKIPPED tools (listed at the bottom of this file):
//!
//! - `openhub_mcp_test_connection` — requires building a `McpServerTransport`
...
//!
//! - `openhub_skill_set_tags` — needs `skill_tag_repo` + `builtin_skill_tags`;
...
struct McpEditServerParams {
    /// The MCP server id (from openhub_mcp_list_servers).
    id: String,
...
struct ExtensionEnableParams {
    /// Extension name (from openhub_extension_list).
    name: String,
}
...
struct HubInstallExtensionParams {
    /// Extension name from the Hub index (from openhub_hub_list_extensions).
    name: String,
}
...
async fn mcp_add_server(deps: Arc<GatewayDeps>, _ctx: CallerCtx, p: McpAddServerParams) -> Value {
    use openhub_api_types::McpTransport;
    ...
    let req = openhub_api_types::CreateMcpServerRequest {
        name: p.name,
...
async fn mcp_edit_server(deps: Arc<GatewayDeps>, _ctx: CallerCtx, p: McpEditServerParams) -> Value {
    ...
    let req = openhub_api_types::UpdateMcpServerRequest {
        name: None,
        description: p.description,
        transport,
        original_json: None,
...
async fn skill_list(deps: Arc<GatewayDeps>, _ctx: CallerCtx, _p: SkillListParams) -> Value {
    match openhub_extension::skill_service::list_available_skills(&deps.skill_paths).await {
        Ok(items) => {
...
async fn skill_import(deps: Arc<GatewayDeps>, _ctx: CallerCtx, p: SkillImportParams) -> Value {
    ...
    match openhub_extension::skill_service::import_skill(&deps.skill_paths, path).await {
...
async fn skill_delete(deps: Arc<GatewayDeps>, _ctx: CallerCtx, p: SkillDeleteParams) -> Value {
    match openhub_extension::skill_service::delete_skill(&deps.skill_paths, &p.name).await {
...
// ══════════════════════════════════════════════════════════════════════════════
//
// 1. `openhub_mcp_test_connection` (Read/Write)
...
//
// 2. `openhub_skill_set_tags` (Write)
