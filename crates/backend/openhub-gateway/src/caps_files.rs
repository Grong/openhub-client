...
use openhub_file::PathAuthority;
...
pub(crate) fn register(out: &mut Vec<Capability>) {
    ...
    out.push(Capability::new::<ListWorkspaceFilesParams, _, _>(
        CapabilityMeta::new(
            "openhub_fs_list_workspace_files",
            "files",
...
//
// - `openhub_fs_copy_files_to_workspace` — `copy_files_to_workspace` requires a
//   `source_root` and multiple file paths; more of a bulk UI operation than an
//   agent tool. Can be added later if needed.
//
// - `openhub_fs_create_temp_file` / `openhub_fs_create_upload_file` — temp/upload
//   helpers used by the UI upload flow and conversation attachments. The agent
//   can use `openhub_fs_write_file` for workspace files directly.
//
// - `openhub_fs_get_image_base64` / `openhub_fs_fetch_remote_image` — image
//   processing helpers. Could be added as a separate "media" domain if agents
//   need inline image data.
//
// - `openhub_fs_create_zip` / `openhub_fs_cancel_zip` — ZIP packaging operations.
//   Not a typical agent primitive; can be added if agent workflows need
//   archiving.
//
// - `openhub_shell_open_file` — `ShellService::open_file` opens a local file with
...
//
// - `openhub_shell_show_in_folder` — `ShellService::show_item_in_folder` reveals
//   an item in Finder/Explorer. Pure UI convenience; low agent utility.
//
// - `openhub_shell_launch` — `ShellService::launch` opens arbitrary targets
...
//
// - `openhub_shell_open_folder_with` — `ShellService::open_folder_with` opens a
...
//
// - `openhub_shell_check_tool_installed` — `ShellService::check_tool_installed`
