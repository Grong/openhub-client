//! WebSocket event emission for IDMM. Mirrors `openhub_requirement::events`.
//! Event names follow the `domain.camelCaseAction` convention.

use std::sync::Arc;

use openhub_api_types::{IdmmState, InterventionRecord, WebSocketMessage};
...
mod tests {
    use super::*;
    use openhub_api_types::{IdmmRunState, IdmmTargetKind};
    use std::sync::Mutex;
