...
use dashmap::DashMap;
use openhub_api_types::{AutoWorkTargetKind, IdmmConfig, IdmmState, IdmmTargetKind, InterventionRecord};
use openhub_common::{generate_prefixed_id, now_ms};
use openhub_db::IIdmmInterventionRepository;
use openhub_db::models::IdmmInterventionRow;
...
#[async_trait::async_trait]
impl openhub_requirement::IdmmHandle for IdmmManager {
...
impl openhub_conversation::ConversationSupervisionHook for IdmmManager {
...
impl openhub_terminal::TerminalSupervisionHook for IdmmManager {
...
mod tests {
    ...
    use openhub_api_types::{IdmmConfig, WatchTier};
    use openhub_db::DbError;
    use openhub_db::models::ClientPreference;
    use std::sync::Mutex;
    use tokio::sync::mpsc;
    ...
    impl SessionProbe for MockProbe {
        ...
        async fn inject(&self, action: &WakeAction) -> Result<(), openhub_common::AppError> {
            self.injected.lock().unwrap().push(action.clone());
            Ok(())
        }
        async fn snapshot_context(&self, _max: usize) -> Result<String, openhub_common::AppError> {
            Ok("ctx".into())
        }
        fn is_alive(&self) -> bool {
            true
        }
        async fn describe(&self) -> Result<SessionDescription, openhub_common::AppError> {
    ...
    #[async_trait]
    impl openhub_db::IClientPreferenceRepository for MockPrefs {
    ...
    impl openhub_realtime::EventBroadcaster for NullBroadcaster {
        fn broadcast(&self, _e: openhub_api_types::WebSocketMessage<serde_json::Value>) {}
    }
    ...
    struct RecordingRepo {
        inserted: Mutex<Vec<openhub_db::models::IdmmInterventionRow>>,
        /// When true, `insert` returns Err to exercise the fail-open path.
        fail: bool,
    }
    #[async_trait]
    impl openhub_db::IIdmmInterventionRepository for RecordingRepo {
        async fn insert(&self, row: &openhub_db::models::IdmmInterventionRow) -> Result<(), DbError> {
        ...
        async fn list_for_target(
            ...
            _id: &str,
            _limit: i64,
        ) -> Result<Vec<openhub_db::models::IdmmInterventionRow>, DbError> {
            Ok(self.inserted.lock().unwrap().clone())
        }
        async fn delete_for_target(&self, _kind: &str, _id: &str) -> Result<u64, DbError> {
            Ok(0)
        }
        async fn list_recent(&self, _limit: i64) -> Result<Vec<openhub_db::models::IdmmInterventionRow>, DbError> {
            Ok(self.inserted.lock().unwrap().clone())
        }
    ...
    fn sidecar_cfg() -> IdmmConfig {
        ...
        c.fault_watch.base.max_retries = 1;
        c.fault_watch.base.budget.min_interval_secs = 0;
        c.fault_watch.base.bypass_model = openhub_api_types::BypassModelRef {
        ...
        c.decision_watch.base.max_retries = 1;
        c.decision_watch.base.budget.min_interval_secs = 0;
        c.decision_watch.base.bypass_model = openhub_api_types::BypassModelRef {
            provider_id: Some("prov".into()),
            model: Some("m".into()),
        };
        c.decision_watch.strategy.categories.option_decision.allow_unmarked_pick = false;
    ...
    async fn has_pending_decision_true_when_decision_watch_on_and_text_is_decision() {
        use openhub_requirement::IdmmHandle;
