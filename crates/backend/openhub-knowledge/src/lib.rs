//! `openhub-knowledge` — the Knowledge Base platform domain: user-curated
...
/// under the hidden `.openhub/` folder — the same agent-facing namespace as
/// `.openhub/skills` / `.openhub/plans` — so mounting into a user's own project
