...
use openhub_api_types::{ConnectorCredentialSummary, ConnectorSyncState, KnowledgeMountInfo, KnowledgeSource, KnowledgeSourceEntry, KnowledgeSourceMode, KnowledgeTag, UpdateKnowledgeTagRequest};
use openhub_common::{AppError, TimestampMs, generate_prefixed_id, now_ms};
use openhub_db::models::{CreateKnowledgeTagParams, KnowledgeBaseRow, KnowledgeBindingRow};
use openhub_db::{IConnectorCredentialRepository, IKnowledgeRepository};
...
pub struct KnowledgeService {
    ...
    /// rendering backend (`BrowserFetcher`, late-wired from `openhub-ai-agent`)
    ...
    /// `BrowserFetcher`, late-wired from `openhub-ai-agent` when the `browser-use`
...
impl KnowledgeService {
    ...
    /// Late-wire the production LLM completer (see `openhub-ai-agent`'s
    ...
    /// `BrowserFetcher` from `openhub-ai-agent`, wired by the app layer when the
    ...
    async fn load_credential(&self, id: &str) -> Result<ConnectorCredential, AppError> {
        ...
        let plaintext = openhub_common::decrypt_string(&row.payload_encrypted, &key)?;
    ...
    pub async fn create_credential(
        ...
        let encrypted = openhub_common::encrypt_string(&plaintext, &key)?;
        ...
        Fut: Future<Output = ()>,
    {
        ...
        let redacted_user = openhub_redact::redact_secrets_owned(req.user_text);
        let redacted_assistant = openhub_redact::redact_secrets_owned(req.assistant_text);
        ...
        for candidate in out.candidates {
            ...
            let mut content = openhub_redact::redact_secrets_owned(content.to_owned());
    ...
    async fn merge_direct_turn_writeback(
        ...
        let merged = openhub_redact::redact_secrets_owned(out.content.trim().to_owned());
...
impl openhub_common::OnConversationDelete for KnowledgeService {
...
/// `.openhub/knowledge/{link}/…` but `knowledge_write` expects a base-relative
/// path; without this, `.openhub/knowledge/Finance/terms.md` would create a new
...
impl KnowledgeService {
    ...
    pub async fn update_tag(
        ...
        use openhub_db::models::UpdateKnowledgeTagParams;
...
mod tests {
    ...
    #[async_trait::async_trait]
    impl openhub_db::IKnowledgeRepository for FailingUpdateRepo {
        async fn insert_base(&self, row: &KnowledgeBaseRow) -> Result<(), openhub_db::DbError> {
            self.0.insert_base(row).await
        }
        async fn update_base(&self, row: &KnowledgeBaseRow) -> Result<(), openhub_db::DbError> {
            Err(openhub_db::DbError::NotFound(format!("simulated persist failure for {}", row.id)))
        }
        async fn delete_base(&self, id: &str) -> Result<(), openhub_db::DbError> {
            self.0.delete_base(id).await
        }
        async fn get_base(&self, id: &str) -> Result<Option<KnowledgeBaseRow>, openhub_db::DbError> {
            self.0.get_base(id).await
        }
        async fn list_bases(&self) -> Result<Vec<KnowledgeBaseRow>, openhub_db::DbError> {
        ...
        async fn get_binding(
            ...
            kind: &str,
            id: &str,
        ) -> Result<Option<(KnowledgeBindingRow, Vec<String>)>, openhub_db::DbError> {
        ...
        async fn delete_binding(&self, kind: &str, id: &str) -> Result<(), openhub_db::DbError> {
            self.0.delete_binding(kind, id).await
        }
        async fn list_bindings_using_kb(&self, kb_id: &str) -> Result<Vec<KnowledgeBindingRow>, openhub_db::DbError> {
            self.0.list_bindings_using_kb(kb_id).await
        }
        async fn list_knowledge_tags(&self) -> Result<Vec<openhub_db::models::KnowledgeTagRow>, openhub_db::DbError> {
            self.0.list_knowledge_tags().await
        }
        async fn create_knowledge_tag(&self, params: openhub_db::models::CreateKnowledgeTagParams) -> Result<(), openhub_db::DbError> {
            self.0.create_knowledge_tag(params).await
        }
        async fn update_knowledge_tag(&self, key: &str, params: openhub_db::models::UpdateKnowledgeTagParams) -> Result<(), openhub_db::DbError> {
            self.0.update_knowledge_tag(key, params).await
        }
        async fn delete_knowledge_tag(&self, key: &str) -> Result<(), openhub_db::DbError> {
    ...
    impl openhub_realtime::EventBroadcaster for RecordingBroadcaster {
        fn broadcast(&self, event: openhub_api_types::WebSocketMessage<serde_json::Value>) {
    ...
    fn deconfuse_strips_mount_prefix() {
        assert_eq!(deconfuse_rel_path(".openhub/knowledge/Finance/terms.md"), "terms.md");
        assert_eq!(deconfuse_rel_path("./.openhub/knowledge/运维手册/deploy/rollback.md"), "deploy/rollback.md");
    ...
    async fn resolve_mount_prefixed_path_updates_original() {
        let (svc, kb_id, _dir) = test_service_with_file("terms.md", "# 术语").await;
        let spec = WriteTargetSpec::Path { kb_id: kb_id.clone(), rel_path: ".openhub/knowledge/X/terms.md".into() };
    ...
    async fn mothers_bug_staged_mount_prefixed_path_lands_in_inbox_not_nested() {
        let (svc, kb_id, _dir) = test_service_with_file("terms.md", "ORIGINAL").await;
        let req = WriteRequest {
            spec: WriteTargetSpec::Path { kb_id: kb_id.clone(), rel_path: ".openhub/knowledge/Finance/terms.md".into() },
        ...
        let out = svc.write_document(req).await.unwrap();
        assert_eq!(out.final_rel_path, "_inbox/conv-9/terms.md", "mirror original, not nest under .openhub/...");
        assert_eq!(out.op, WriteOp::Update);
        assert_eq!(svc.read_file(&kb_id, "terms.md").await.unwrap().content, "ORIGINAL");
        let files = svc.list_files(&kb_id).await.unwrap();
        assert!(!files.iter().any(|f| f.rel_path.contains(".openhub/knowledge")), "no nested mount-path file: {files:?}");
    }
        ...
        let report = svc
            .finalize_turn_writeback(TurnWritebackRequest {
                mounts: vec![KnowledgeMountInfo {
                    ...
                    rel_path: ".openhub/knowledge/领域库".into(),
    ...
    async fn connector_sync_writes_snapshots_persists_cursor_and_trashes_deletions() {
        ...
        use openhub_db::models::ConnectorCredentialRow;
