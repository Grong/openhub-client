...
pub struct KnowledgeContextOptions<'a> {
    ...
    /// reliable path for openhub-engine sessions, where the generic `Write` tool
...
pub fn build_knowledge_context(
    ...
    if readme {
        out.push_str(
            "# Knowledge bases\n\n\
             This directory is mounted and managed by the OpenHub platform. It contains the \
    ...
    if mounts.iter().any(|m| !m.live_sources.is_empty()) {
        ...
        // Layered tool guidance: `openhub_knowledge_fetch_url` is a desktop
        ...
        out.push_str(
            "To read one of these URLs at its current state, use a web-fetch tool already \
             ...
             `openhub_knowledge_fetch_url` tool is available, use that. If neither is \
...
mod tests {
    use super::*;
    use openhub_api_types::KnowledgeSourceEntry;
    ...
    fn single_base_prompt_section_contract() {
        let mut m = mount("领域知识", ".openhub/knowledge/领域知识");
        ...
        assert!(out.contains("`./.openhub/knowledge/领域知识/`"), "got: {out}");
        ...
        assert!(!out.contains("openhub_knowledge_fetch_url"), "got: {out}");
    }

    #[test]
    fn empty_description_and_summary_lines_are_omitted() {
        let m = mount("库A", ".openhub/knowledge/库A");
        let out = build_knowledge_context(&[m], &prompt_opts(false, None, "conv-1")).unwrap();
    ...
    fn empty_description_base_surfaces_topic_hints_from_toc() {
        let mut m = mount("库A", ".openhub/knowledge/库A");
        m.toc = vec![
    ...
    fn multi_base_renders_protocol_once() {
        let a = mount("库A", ".openhub/knowledge/库A");
        let b = mount("库B", ".openhub/knowledge/库B");
    ...
    fn live_sources_render_realtime_section() {
        let mut m = mount("接口库", ".openhub/knowledge/接口库");
        m.live_sources = vec![
        ...
        let plain = mount("普通库", ".openhub/knowledge/普通库");
        ...
        assert!(out.contains("if a `openhub_knowledge_fetch_url` tool is available"), "got: {out}");
        ...
        assert!(!out.contains("call the `openhub_knowledge_fetch_url` tool instead"), "got: {out}");
    }
    ...
    /// Options helper pinning has_write_tool = true (the openhub-engine surface).
    ...
    fn terminal_readme_is_a_complete_document() {
        ...
        assert!(out.contains("OpenHub"), "got: {out}");
    ...
    fn mount_info_deserializes_legacy_extra_without_new_fields() {
        ...
        let bare = serde_json::to_value(mount("库A", ".openhub/knowledge/库A")).unwrap();
        assert!(bare.get("summary").is_none(), "got: {bare}");
        assert!(bare.get("live_sources").is_none(), "got: {bare}");

        let mut rich = mount("库B", ".openhub/knowledge/库B");
