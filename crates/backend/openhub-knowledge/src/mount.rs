...
//! workspace at `.openhub/knowledge/{link_name}` using NTFS junctions on
...
//! (or the fallback copy). Sibling `.openhub/` trees (`.openhub/skills`, …) are
...
/// One desired mount: `{workspace}/.openhub/knowledge/{link_name}` → `target`.
#[derive(Debug, Clone)]
...
fn sync_mounts_inner(workspace: &Path, specs: &[MountSpec]) -> Vec<String> {
    ...
    if specs.is_empty() {
        ...
        // try to remove the (now empty) scaffolding. The parent `.openhub/` is
    ...
    // `.openhub/knowledge/` — never at the `.openhub/` root — so committable
    // siblings like `.openhub/skills` stay visible to git.
...
mod tests {
    ...
    #[tokio::test]
    async fn legacy_openhub_mounts_cleaned() {
        let bases = TempDir::new().unwrap();
        let ws = TempDir::new().unwrap();
