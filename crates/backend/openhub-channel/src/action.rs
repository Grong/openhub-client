...
pub struct ActionExecutor {
    ...
    requirement_creator: Option<Arc<dyn openhub_common::RequirementCreator>>,
}
...
impl ActionExecutor {
    ...
    pub fn with_requirement_creator(
        mut self,
        creator: Option<Arc<dyn openhub_common::RequirementCreator>>,
    ) -> Self {
    ...
    async fn resolve_action_session(
        ...
        internal_user_id: &str,
        channel_id: &str,
    ) -> Result<openhub_db::models::AssistantSessionRow, ChannelError> {
        let agent_config = self.settings.get_agent_config(action.context.platform).await?;
        self.session_mgr
...
mod action_tests {
    ...
    use openhub_db::{DbError, IChannelRepository, IClientPreferenceRepository, UpdatePluginStatusParams};
    ...
    async fn session_new_creates_session() {
        ...
        match result {
            MessageResult::Action(resp) => {
                ...
                // With no client_preferences configured, defaults to "openhub"
    ...
    async fn agent_select_persists_agent_type() {
        ...
        // First: send a text to create a session (defaults to "openhub")
        let text_msg = make_text_message("tg_42", "chat_1", "Hello", PluginType::Telegram);
