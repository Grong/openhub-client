...
use openhub_ai_agent::{AgentStreamEvent, IWorkerTaskManager};
...
use openhub_common::{AgentType, ConversationSource, MessagePosition, MessageType};
...
impl ChannelMessageService {
    ...
    async fn dispatch_to_conversation(
        ...
        self.conversation_svc
            ...
            .map_err(|e| match e {
                ...
                openhub_common::AppError::Conflict(_) => ChannelError::ConversationBusy,
    ...
    async fn create_conversation_for_session(
        ...
        // 遗留兜底：伙伴模型缺失（伙伴未配置或非 openhub/无绑定）时，
    ...
    pub fn process_stream_event(event: &AgentStreamEvent) -> Option<StreamAction> {
        match event {
            ...
            AgentStreamEvent::AcpPermission(data) => match data {
                openhub_ai_agent::protocol::events::AcpPermissionEventData::Request(req) => {
                ...
                openhub_ai_agent::protocol::events::AcpPermissionEventData::Confirmation(conf) => {
                    confirmation_to_decision(conf)
                }
            },
            AgentStreamEvent::Permission(value) => serde_json::from_value::<openhub_common::Confirmation>(value.clone())
                .ok()
...
/// Maps a `openhub_common::Confirmation` to a `Decision` action.
///
...
fn confirmation_to_decision(conf: &openhub_common::Confirmation) -> Option<StreamAction> {
...
fn platform_to_source(platform: PluginType) -> ConversationSource {
    match platform {
        ...
        | PluginType::Nostr
        | PluginType::Wecom
        | PluginType::Qqbot => ConversationSource::Openhub,
    }
}
...
mod tests {
    ...
    fn conv_name_non_acp_ignores_backend() {
        let name = channel_conversation_name(PluginType::Telegram, "openhub", Some("claude"), Some("70880480"));
        assert_eq!(name, "tg-openhub-70880480");
    }
}
