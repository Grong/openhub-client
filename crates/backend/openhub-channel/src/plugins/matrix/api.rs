...
mod tests {
    ...
    fn txn_id_is_monotonic() {
        ...
        let b = next_txn_id();
        let c = next_txn_id();
        // Each must start with "openhub_" and parse to increasing numbers.
        let na: u64 = a.strip_prefix("openhub_").unwrap().parse().unwrap();
        let nb: u64 = b.strip_prefix("openhub_").unwrap().parse().unwrap();
        let nc: u64 = c.strip_prefix("openhub_").unwrap().parse().unwrap();
        assert!(na < nb);
        assert!(nb < nc);
    }
