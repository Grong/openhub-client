...
use dashmap::DashMap;
use openhub_api_types::{PluginStatusChangedPayload, PluginStatusResponse, WebSocketMessage};
use openhub_common::{decrypt_string, encrypt_string, generate_prefixed_id, now_ms};
use openhub_db::models::ChannelPluginRow;
use openhub_db::{IChannelRepository, UpdatePluginStatusParams};
...
impl ChannelManager {
    ...
    /// because `EventSource` cannot carry the desktop's `x-openhub-local-trust`
...
mod tests {
    ...
    use openhub_db::models::{AssistantSessionRow, AssistantUserRow, ChannelPluginRow, PairingCodeRow};
    use openhub_db::{DbError, IChannelRepository, UpdatePluginStatusParams};
    use std::sync::Mutex;
