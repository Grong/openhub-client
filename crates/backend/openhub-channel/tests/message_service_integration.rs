...
use openhub_ai_agent::agent_task::{AgentInstance, IAgentTask};
use openhub_ai_agent::protocol::events::FinishEventData;
use openhub_ai_agent::types::{BuildTaskOptions, SendMessageData};
use openhub_ai_agent::{AgentSendError, AgentStreamEvent, IMockAgent, IWorkerTaskManager};
use openhub_api_types::WebSocketMessage;
use openhub_channel::channel_settings::ChannelSettingsService;
use openhub_channel::message_service::ChannelMessageService;
use openhub_channel::types::PluginType;
use openhub_common::{AgentKillReason, AgentType, AppError, ConversationStatus, TimestampMs};
use openhub_conversation::ConversationService;
use openhub_conversation::skill_resolver::{ResolvedAgentSkill, SkillResolver};
use openhub_db::models::AssistantSessionRow;
use openhub_db::{
...
struct TestStack {
    ...
    runtime: Arc<openhub_conversation::runtime_state::ConversationRuntimeStateService>,
    channel_repo: Arc<SqliteChannelRepository>,
}

fn build_stack(pool: openhub_db::SqlitePool) -> TestStack {
    let task_manager: Arc<dyn IWorkerTaskManager> = Arc::new(RecordingTaskManager::new());
    let runtime = Arc::new(openhub_conversation::runtime_state::ConversationRuntimeStateService::default());
...
async fn wait_until_idle(svc: &Arc<ConversationService>, conversation_id: &str) {
    use openhub_api_types::ConversationRuntimeStateKind;
    for _ in 0..500 {
...
#[async_trait]
impl openhub_channel::message_service::MasterAgentProfile for StubProfile {
    async fn companion_model(&self, _companion_id: &str) -> Option<openhub_common::ProviderWithModel> {
        None
    }
...
async fn bind_channel_to_companion(repo: &Arc<SqliteChannelRepository>, channel_id: &str, companion_id: &str) {
    use openhub_db::IChannelRepository;
    let now = openhub_common::now_ms();
    repo.upsert_plugin(&openhub_db::models::ChannelPluginRow {
...
async fn companion_without_model_refuses_turn() {
    use openhub_channel::error::ChannelError;
...
struct PublicStubProfile {
    servable: bool,
    model: Option<openhub_common::ProviderWithModel>,
}

#[async_trait]
impl openhub_channel::message_service::MasterAgentProfile for PublicStubProfile {
    async fn companion_model(&self, _companion_id: &str) -> Option<openhub_common::ProviderWithModel> {
        None
    }
    async fn master_companion_id(&self, _platform: &str) -> Option<String> {
    ...
    async fn public_agent_model(&self, _id: &str) -> Option<openhub_common::ProviderWithModel> {
        self.model.clone()
    }
}
...
async fn public_agent_bound_platform_builds_clamped_session() {
    ...
    let model = openhub_common::ProviderWithModel {
