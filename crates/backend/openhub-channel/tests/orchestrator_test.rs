...
use openhub_api_types::{ConversationRuntimeStateKind, ListMessagesQuery, WebSocketMessage};
use openhub_channel::action::{ActionExecutor, MessageResult};
use openhub_channel::channel_settings::ChannelSettingsService;
use openhub_channel::message_service::ChannelMessageService;
use openhub_channel::orchestrator::ChannelOrchestrator;
use openhub_channel::pairing::PairingService;
use openhub_channel::session::SessionManager;
use openhub_channel::stream_relay::{ChannelSender, MessageRecorder};
use openhub_channel::types::{
...
use openhub_db::models::{AssistantUserRow, ChannelPluginRow};
use openhub_db::{
...
async fn unauthorized_user_gets_pairing_response() {
    ...
    let repo: Arc<dyn openhub_db::IChannelRepository> =
        Arc::new(openhub_db::SqliteChannelRepository::new(pool.clone()));
    let bus = Arc::new(openhub_realtime::BroadcastEventBus::new(64));

    let pref_repo: Arc<dyn openhub_db::IClientPreferenceRepository> =
        Arc::new(openhub_db::SqliteClientPreferenceRepository::new(pool));
...
struct Harness {
    ...
    pending_decisions: Arc<openhub_channel::pending_decision::PendingDecisionStore>,
}
...
async fn chat_continue_sends_continue_prompt_to_agent() {
    ...
    assert_eq!(messages, vec![
        ...
        openhub_channel::action::CONTINUE_PROMPT.to_string()
    ]);
}
...
use openhub_channel::pending_decision::PendingDecision;
use openhub_channel::types::DecisionOption;
