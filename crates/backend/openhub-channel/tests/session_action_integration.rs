...
use openhub_common::{generate_id, now_ms};
use openhub_db::models::{AssistantUserRow, ChannelPluginRow};
use openhub_db::{IChannelRepository, SqliteChannelRepository, init_database_memory};
...
async fn setup() -> (
    ...
    let pref_repo: Arc<dyn openhub_db::IClientPreferenceRepository> =
        Arc::new(openhub_db::SqliteClientPreferenceRepository::new(db.pool().clone()));
...
async fn action_session_new() {
    ...
    match result {
        MessageResult::Action(resp) => {
            ...
            // With no client_preferences, defaults to "openhub"
            assert!(text.contains("openhub"));
        }
        _ => panic!("Expected Action result"),
    }
}
...
// openhub-db sqlite_channel.rs::update_session_conversation_persists.
