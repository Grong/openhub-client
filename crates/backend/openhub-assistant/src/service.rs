...
use openhub_extension::{AssistantClassifier, AssistantRuleDispatcher, ExtensionRegistry, ResolvedAssistant};
use serde_json;
...
pub struct AssistantService {
    ...
    /// Defaults to `~/.openhub/` but can be overridden for tests.
...
impl AssistantService {
    ...
    /// directory.
    ///
    /// There is no implicit `~/openhub fallback on purpose: an earlier
    /// version had one, and dev builds silently wrote rule files to the
    /// release directory while the db lived under `~/.openhub-dev/`,
    ...
    pub async fn list_tags(&self) -> Result<Vec<openhub_api_types::AssistantTagResponse>, AppError> {
        use openhub_api_types::{AssistantTagDimension, AssistantTagResponse};
    ...
    pub async fn create_tag(
        &self,
        req: openhub_api_types::CreateAssistantTagRequest,
    ) -> Result<openhub_api_types::AssistantTagResponse, AppError> {
        use openhub_api_types::{AssistantTagDimension, AssistantTagResponse};
    ...
    pub async fn update_tag(
        &self,
        key: &str,
        req: openhub_api_types::UpdateAssistantTagRequest,
    ) -> Result<openhub_api_types::AssistantTagResponse, AppError> {
        use openhub_api_types::{AssistantTagDimension, AssistantTagResponse};
    ...
    pub async fn import(&self, req: ImportAssistantsRequest) -> Result<ImportAssistantsResult, AppError> {
        ...
        for entry in req.assistants {
            ...
            match self.repo.create(&params).await {
                ...
                Err(openhub_db::DbError::Conflict(_)) => {
    ...
    ///   override when `OPENHUB_BUILTIN_ASSISTANTS_PATH` is set).
...
const DEFAULT_AGENT_TYPE: &str = "openhub";
...
mod tests {
    ...
    use openhub_extension::ExtensionStateStore;
    ...
    struct Fixture {
        ...
        provider_repo: Arc<dyn IProviderRepository>,
        _tmp: TempDir,
        _db: openhub_db::Database,
    }

    /// Default fixture: seeded with a single OpenAI-compatible provider so
    /// `resolve_default_agent_type` returns `"openhub"`. Tests that need to
    ...
    struct FixtureOpts {
        ...
        /// Defaults to `"openai"` so existing tests get an `"openhub"`
        /// default agent type.
    ...
    /// or otherwise — should resolve to `"openhub"`, the only built-in
    ...
    async fn tag_lifecycle_and_strip_on_delete() {
        ...
        let tag = fx
            .service
            .create_tag(openhub_api_types::CreateAssistantTagRequest {
                dimension: openhub_api_types::AssistantTagDimension::Audience,
                label: "营销".into(),
            })
            .await
            .unwrap();
