...
/// serves those via `openhub-asset://`.
...
async fn list_tags(
    State(state): State<AssistantRouterState>,
) -> Result<Json<ApiResponse<Vec<openhub_api_types::AssistantTagResponse>>>, AppError> {
    Ok(Json(ApiResponse::ok(state.service.list_tags().await?)))
}

async fn create_tag(
    State(state): State<AssistantRouterState>,
    body: Result<Json<openhub_api_types::CreateAssistantTagRequest>, JsonRejection>,
) -> Result<(StatusCode, Json<ApiResponse<openhub_api_types::AssistantTagResponse>>), AppError> {
    let Json(req) = body.map_err(|e| AppError::BadRequest(e.to_string()))?;
...
async fn update_tag(
    ...
    body: Result<Json<openhub_api_types::UpdateAssistantTagRequest>, JsonRejection>,
) -> Result<Json<ApiResponse<openhub_api_types::AssistantTagResponse>>, AppError> {
    let Json(req) = body.map_err(|e| AppError::BadRequest(e.to_string()))?;
