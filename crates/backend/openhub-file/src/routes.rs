...
use openhub_common::constants::UPLOAD_MAX_SIZE;
...
pub fn file_routes(state: FileRouterState) -> Router {
    ...
    // We first disable the global `DefaultBodyLimit` that `openhub-app`
...
fn to_zip_entry(e: openhub_api_types::ZipFileEntry) -> ZipEntry {
...
fn to_snapshot_info_response(info: SnapshotInfo) -> SnapshotInfoResponse {
    let (mode, reason) = match info.mode {
        SnapshotMode::GitRepo => (openhub_api_types::SnapshotMode::GitRepo, None),
        SnapshotMode::Snapshot => (openhub_api_types::SnapshotMode::Snapshot, None),
        SnapshotMode::Disabled { reason } => (openhub_api_types::SnapshotMode::Disabled, Some(reason)),
    };
...
mod tests {
    ...
    fn zip_entry_conversion_text() {
        let e = openhub_api_types::ZipFileEntry {
            name: "a.txt".into(),
    ...
    fn snapshot_info_response_git_repo() {
        ...
        assert_eq!(r.mode, openhub_api_types::SnapshotMode::GitRepo);
    ...
    fn snapshot_info_response_snapshot_mode() {
        ...
        let r = to_snapshot_info_response(info);
        assert_eq!(r.mode, openhub_api_types::SnapshotMode::Snapshot);
    ...
    fn snapshot_info_response_disabled_mode_carries_reason() {
        ...
        let r = to_snapshot_info_response(info);
        assert_eq!(r.mode, openhub_api_types::SnapshotMode::Disabled);
        assert!(r.branch.is_none());
    ...
    fn compare_response_conversion() {
        use openhub_common::FileChangeOperation;
