...
impl crate::traits::IFileService for FileService {
    ...
    async fn create_temp_file(&self, file_name: &str) -> Result<String, AppError> {
        ...
        tokio::task::spawn_blocking(move || {
            let tmp_dir = std::env::temp_dir().join("openhub");
    ...
    async fn create_upload_file(
        ...
        tokio::task::spawn_blocking(move || {
            let mut dir = std::env::temp_dir().join("openhub");
...
mod tests {
    ...
    fn list_workspace_files_sync_skips_directory_symlinks() {
        let dir = tempfile::tempdir().unwrap();
        let skill_dir = dir.path().join("builtin-skills/auto-inject/openhub-skills");
        fs::create_dir_all(&skill_dir).unwrap();
        fs::write(skill_dir.join("SKILL.md"), "---\ndescription: test\n---\nbody").unwrap();

        let workspace = dir.path().join("workspace/.claude/skills");
        fs::create_dir_all(&workspace).unwrap();
        std::os::unix::fs::symlink(&skill_dir, workspace.join("openhub-skills")).unwrap();

        let files = list_workspace_files_sync(&dir.path().join("workspace")).unwrap();

        assert!(
            files.iter().all(|f| f.name != "openhub-skills"),
    ...
    impl openhub_realtime::EventBroadcaster for NullBroadcaster {
        fn broadcast(&self, _msg: openhub_api_types::WebSocketMessage<serde_json::Value>) {}
    }
