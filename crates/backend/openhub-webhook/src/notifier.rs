//! Completion notifier: implements `openhub_requirement::CompletionNotifier` by
//! looking up the requirement's tag → bound webhook and sending a notification.
//!
//! Dependency direction: this crate depends on `openhub-requirement` (for the
//! trait); `openhub-requirement` does NOT depend on this crate. Mirrors how
//! `openhub-idmm` implements `openhub_requirement::IdmmHandle`.

use std::sync::Arc;

use async_trait::async_trait;
use openhub_api_types::WebhookPlatform;
use openhub_db::models::RequirementRow;
use openhub_db::{ITagSettingRepository, IWebhookRepository};
use openhub_requirement::CompletionNotifier;
...
impl CompletionNotifierImpl {
    ...
    async fn resolve_webhook(&self, tag: &str) -> Option<(openhub_db::models::WebhookRow, Vec<String>)> {
