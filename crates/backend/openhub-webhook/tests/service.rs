...
use openhub_api_types::{CreateWebhookRequest, UpdateWebhookRequest, UpsertTagSettingRequest, WebhookPlatform};
use openhub_db::{
    ITagSettingRepository, IWebhookRepository, SqliteTagSettingRepository, SqliteWebhookRepository,
    init_database_memory,
};
use openhub_webhook::{WebhookSender, WebhookService};
...
impl WebhookSender for MockSender {
    ...
    ) -> Result<(), openhub_webhook::WebhookError> {
        self.calls.lock().unwrap().push((url.to_string(), title.to_string()));
        if self.fail {
            return Err(openhub_webhook::WebhookError::Remote("boom".into()));
        }
        Ok(())
    }
}
