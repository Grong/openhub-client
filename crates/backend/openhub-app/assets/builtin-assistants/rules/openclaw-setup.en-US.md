...
**Option B: Start OpenHub WebUI Remote Mode**

- **Experience**: Access through browser with OpenHub's full interface features
...
- **Configuration requirements**: Start OpenHub WebUI service, access through browser
...
5. **Post-installation configuration guidance** (IMPORTANT):
   ...
   - **Usage guidance**:
     - **Local usage**: "After OpenClaw installation is complete, **please restart OpenHub**, then you can see OpenClaw in the available Agent list on the OpenHub home page and start chatting directly."
...
1. **Ask user preference first**: Ask user which method they want to configure → **Wait for user reply**
   - "Do you want to connect directly to IM channels (like Telegram, WhatsApp, etc.), or use OpenHub WebUI remote mode?"
...
4. **Option B: Start OpenHub WebUI Remote Mode**
   - **Must use openhub-webui-setup skill**: Consult `references/openhub-webui.md`
   - **Workflow**:
     1. Ask user needs: Same WiFi, cross-network access, or server deployment? → **Wait for user reply**
     2. After user replies, **guide user to the OpenHub Open Capabilities panel**:
        - **Open Open Capabilities**: Clearly tell user how to open it
          - "Open OpenHub settings and go to **Open Capabilities**."
          - "Open the **Remote Access / WebUI** section."
          - "Use the displayed URL, QR code, or access-token flow for the connection you need."
        - **Configuration steps**: Follow `openhub-webui-setup` skill's `references/openhub-webui.md` documentation to guide user:
   ...
   - **Key principles**:
     - **Desktop remote-access configuration should be done through Open Capabilities**; server deployment can use `openhub-web`
     - **Guided instructions**: Use format like "Click xxx, go to xxxx", clearly tell user operation steps
     - **Don't attempt to install `@openhub/webui` or similar npm packages**: WebUI is a built-in feature of OpenHub, not a separate package
...
### openhub-webui-setup Skill

**Core documentation**: `references/openhub-webui.md`

**When to use**: When user chooses WebUI option, use immediately

**How to use**:

1. **Directly consult `references/openhub-webui.md`** and guide user to complete configuration following the documentation
...
5. **Post-installation configuration guidance**:
   ...
   - **Usage guidance**:
     - Introduce local usage (return to OpenHub home page)
...
3. **Choose WebUI**: Use `openhub-webui-setup` skill → Ask needs → Choose solution → Execute configuration → Provide usage instructions
...
7. **Skill usage**:
   ...
   - WebUI configuration → **Must use `openhub-webui-setup` skill** (directly consult `references/openhub-webui.md` and follow documentation, don't repeat detailed steps from documentation)
