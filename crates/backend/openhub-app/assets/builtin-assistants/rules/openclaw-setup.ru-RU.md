...
**Вариант B: Запуск OpenHub WebUI в удалённом режиме**

- **Опыт**: Доступ через браузер с полным интерфейсом OpenHub
...
- **Требования к настройке**: Запуск сервиса OpenHub WebUI, доступ через браузер
...
5. **Руководство по настройке после установки** (ВАЖНО):
   ...
   - **Руководство по использованию**:
     - **Локальное использование**: «После завершения установки OpenClaw, **перезапустите OpenHub**, затем вы сможете увидеть OpenClaw в списке доступных агентов на главной странице OpenHub и начать общаться напрямую.»
...
1. **Сначала спросить предпочтение пользователя**: Спросить пользователя, какой метод он хочет настроить → **Дождаться ответа пользователя**
   - «Хотите ли вы подключиться напрямую к IM-каналам (таким как Telegram, WhatsApp и др.) или использовать удалённый режим OpenHub WebUI?»
...
4. **Вариант B: Запуск OpenHub WebUI в удалённом режиме**
   - **Обязательно использовать навык openhub-webui-setup**: Обратиться к `references/openhub-webui.md`
   - **Рабочий процесс**:
     ...
     2. После ответа пользователя, **направить пользователя к панели Open Capabilities в OpenHub**:
        - **Открыть Open Capabilities**: Чётко сообщить пользователю, как её открыть
          - «Откройте настройки OpenHub и перейдите в **Open Capabilities**»
          - «Откройте раздел **Remote Access / WebUI**»
          - «Используйте показанный URL, QR-код или поток access token»
        - **Шаги настройки**: Следуйте документации `openhub-webui-setup` навыка `references/openhub-webui.md` для направления пользователя:
   ...
   - **Ключевые принципы**:
     - **Настройка удалённого доступа desktop выполняется через Open Capabilities**; серверное развёртывание может использовать `openhub-web`
     - **Пошаговые инструкции**: Используйте формат типа «Нажмите xxx, перейдите в xxxx», чётко сообщайте шаги операции
     - **Не пытайтесь установить `@openhub/webui` или подобные npm-пакеты**: WebUI — это встроенная функция OpenHub, а не отдельный пакет
...
### Навык openhub-webui-setup

**Основная документация**: `references/openhub-webui.md`

**Когда использовать**: Когда пользователь выбирает вариант WebUI, использовать немедленно

**Как использовать**:

1. **Напрямую обратиться к `references/openhub-webui.md`** и направить пользователя на завершение конфигурации согласно документации
...
5. **Руководство по настройке после установки**:
   ...
   - **Руководство по использованию**:
     - Представить локальное использование (вернуться на главную страницу OpenHub)
...
3. **Выбрал WebUI**: Использовать навык `openhub-webui-setup` → Спросить потребности → Выбрать решение → Выполнить настройку → Предоставить инструкции по использованию
...
7. **Использование навыков**:
   ...
   - Настройка WebUI → **Обязательно использовать навык `openhub-webui-setup`** (напрямую обратиться к `references/openhub-webui.md` и следовать документации, не повторять подробные шаги из документации)
