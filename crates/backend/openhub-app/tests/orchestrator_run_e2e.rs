...
//!    openhub-app`. That build only succeeds once `build_orchestrator_state`
...
//!    test crate is part of `openhub-app`, so it does not even compile until that
...
#[async_trait]
impl openhub_conversation::skill_resolver::SkillResolver for NoopSkillResolver {
...
#[async_trait]
impl openhub_ai_agent::IWorkerTaskManager for NoopTaskManager {
    fn get_task(&self, _: &str) -> Option<openhub_ai_agent::AgentInstance> {
        None
    }
    async fn get_or_build_task(
        &self,
        _: &str,
        _: openhub_ai_agent::types::BuildTaskOptions,
    ) -> Result<openhub_ai_agent::AgentInstance, AppError> {
        Err(AppError::Internal("noop".into()))
    }
    fn kill(&self, _: &str, _: Option<openhub_common::AgentKillReason>) -> Result<(), AppError> {
        Ok(())
    }
    fn kill_and_wait(
        &self,
...
fn build_test_conv_service(pool: openhub_db::SqlitePool) -> openhub_conversation::ConversationService {
    let conv_repo = Arc::new(openhub_db::SqliteConversationRepository::new(pool.clone()));
    let agent_metadata_repo: Arc<dyn openhub_db::IAgentMetadataRepository> =
        Arc::new(openhub_db::SqliteAgentMetadataRepository::new(pool.clone()));
    let acp_session_repo: Arc<dyn openhub_db::IAcpSessionRepository> =
        Arc::new(openhub_db::SqliteAcpSessionRepository::new(pool));
    openhub_conversation::ConversationService::new(
        std::env::temp_dir(),
...
impl PlanProducer for ChainPlanProducer {
    async fn produce(&self, _goal: &str, _members: &[FleetMember], _sink: Option<&openhub_orchestrator::LeadThinkingSink>) -> Result<PlannedDag, AppError> {
        Ok(PlannedDag {
            tasks: vec![
                PlannedTask {
...
//
// It mirrors `openhub_run_create`'s exact choreography (caps_orchestrator.rs:126-135):
...
async fn adhoc_run_from_model_range_completes_through_engine() {
    ...
    // The exact choreography `openhub_run_create` performs (caps_orchestrator.rs):
