...
//! at the service integration level in `openhub-cron/tests/service_integration.rs`.
...
use openhub_db::{ICronRepository, SqliteCronRepository};
...
async fn seed_conversation(services: &openhub_app::AppServices, id: i64) {
    sqlx::query(
...
async fn cj5b_run_now_legacy_workspace_uses_runtime_edge_whitespace_code() {
    ...
    cron_repo
        .insert(&openhub_db::models::CronJobRow {
