...
async fn seed_telegram_channel(repo: &std::sync::Arc<dyn openhub_db::IChannelRepository>) {
...
async fn pairing_approve_creates_user() {
    ...
    let repo: std::sync::Arc<dyn openhub_db::IChannelRepository> =
        std::sync::Arc::new(openhub_db::SqliteChannelRepository::new(pool));
    let pairing_svc = openhub_channel::pairing::PairingService::new(repo.clone(), services.event_bus.clone());
