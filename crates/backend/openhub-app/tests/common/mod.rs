//! Shared test helpers for openhub-app E2E tests.
#![allow(dead_code)]
...
use openhub_ai_agent::{AgentInstance, IAgentTask, IMockAgent, WorkerTaskManagerImpl};
use openhub_app::{AppConfig, AppServices, build_module_states, create_router, create_router_with_states};
use openhub_extension::{ExternalPathsManager, SkillPaths, SkillRouterState};
use openhub_file::FileService;
use openhub_system::VersionCheckService;
...
pub async fn build_app_with_skill_paths(root: &std::path::Path) -> (axum::Router, AppServices, SkillPaths) {
    ...
    states.skill = SkillRouterState {
        ...
        skill_tag_repo: std::sync::Arc::new(openhub_db::SqliteSkillTagRepository::new(
...
pub async fn build_app_with_noop_opener() -> (axum::Router, AppServices) {
    ...
    let services = AppServices::from_config(db, &AppConfig::default()).await.unwrap();
    let (mut states, _) = build_module_states(&services).await;
    states.shell.shell_service = std::sync::Arc::new(openhub_shell::ShellService::new(std::sync::Arc::new(
        openhub_shell::NoopSystemOpener,
    )));
    let router = create_router_with_states(&services, states);
    (router, services)
}
...
pub async fn build_app_with_mock_agents() -> (axum::Router, AppServices) {
    ...
    let factory: std::sync::Arc<
        dyn Fn(
                openhub_ai_agent::types::BuildTaskOptions,
            ) -> futures_util::future::BoxFuture<'static, Result<AgentInstance, openhub_common::AppError>>
            + Send
            + Sync,
    ...
    let wtm: std::sync::Arc<dyn openhub_ai_agent::IWorkerTaskManager> =
...
impl IAgentTask for NoopMockAgent {
    fn agent_type(&self) -> openhub_common::AgentType {
        openhub_common::AgentType::Acp
    }
    ...
    fn status(&self) -> Option<openhub_common::ConversationStatus> {
        None
    }
    fn last_activity_at(&self) -> openhub_common::TimestampMs {
        openhub_common::now_ms()
    }
    fn subscribe(&self) -> tokio::sync::broadcast::Receiver<openhub_ai_agent::AgentStreamEvent> {
        let (tx, _) = tokio::sync::broadcast::channel(1);
        tx.subscribe()
    }
    async fn send_message(
        &self,
        _data: openhub_ai_agent::types::SendMessageData,
    ...
    fn kill(&self, _reason: Option<openhub_common::AgentKillReason>) -> Result<(), openhub_common::AppError> {
        Ok(())
    }
}
...
pub fn extract_csrf_token(resp: &axum::response::Response) -> Option<String> {
    resp.headers()
        ...
        .find(|s| s.starts_with("openhub-csrf-token="))
        .map(|s| {
            s.strip_prefix("openhub-csrf-token=")
                .unwrap()
                .split(';')
                .next()
                .unwrap()
...
pub fn json_with_token(method_str: &str, uri: &str, body: serde_json::Value, token: &str, csrf: &str) -> Request<Body> {
    Request::builder()
        ...
        .header("cookie", format!("openhub-csrf-token={csrf}"))
...
pub async fn setup_and_login(
    ...
    let hash = openhub_auth::hash_password(password).unwrap();
    if username == "admin" {
        services
            .user_repo
