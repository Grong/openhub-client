...
//! `openhub_whoami`) and asserts the resolved companion_id appears in the JSON-RPC
...
use openhub_gateway::{Registry, Surface};
...
async fn mcp_endpoint_is_mounted_and_token_gated() {
    ...
    services
        ...
        .insert_token(companion_id.to_string(), openhub_auth::token_sha256_hex(token));
...
/// (initialize → notifications/initialized → tools/call for `openhub_whoami`) and
...
async fn mcp_tools_call_binds_companion() {
    ...
    // 3) tools/call openhub_whoami → the result must echo the bound companion_id.
    let call = serde_json::json!({
        "jsonrpc": "2.0",
        "id": 2,
        "method": "tools/call",
        "params": { "name": "openhub_whoami", "arguments": {} }
    });
    ...
    assert!(
        ...
        "openhub_whoami result over /mcp must echo the bound companion_id '{companion_id}'; \
...
async fn rest_v1_endpoint_is_mounted_and_gated() {
    ...
    let call = Request::builder()
        .method("POST")
        .uri("/v1/tools/openhub_list_conversations")
        .header(header::AUTHORIZATION, format!("Bearer {token}"))
        .header(header::CONTENT_TYPE, "application/json")
        .body(Body::from("{}"))
        .unwrap();
    ...
    // `smoke-companion`, and that companion_id must reach dispatch. `openhub_whoami`
    ...
    let whoami = Request::builder()
        .method("POST")
        .uri("/v1/tools/openhub_whoami")
        .header(header::AUTHORIZATION, format!("Bearer {token}"))
        .header(header::CONTENT_TYPE, "application/json")
        .body(Body::from("{}"))
        .unwrap();
    let resp = app.oneshot(whoami).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK, "openhub_whoami call should succeed (got {})", resp.status());
    let body = axum::body::to_bytes(resp.into_body(), 1 << 20).await.unwrap();
    let text = String::from_utf8_lossy(&body);
    assert!(
        text.contains(companion_id),
        "openhub_whoami response must echo the resolved companion_id '{companion_id}' (got: {text})"
    );
}
...
async fn rest_v1_stream_endpoint_emits_result_frame() {
    ...
    services
        .companion_token_validator
        .insert_token("smoke-companion".to_string(), openhub_auth::token_sha256_hex(token));

    let req = Request::builder()
        .method("POST")
        .uri("/v1/tools/openhub_list_conversations/stream")
        .header(header::AUTHORIZATION, format!("Bearer {token}"))
        .header(header::CONTENT_TYPE, "application/json")
        .body(Body::from("{}"))
        .unwrap();
...
fn remote_surface_projection_is_correct() {
    ...
    assert!(remote.contains(&"openhub_agent_run"), "openhub_agent_run must be on the Remote surface");
    assert!(remote.contains(&"openhub_agent_result"), "openhub_agent_result must be on the Remote surface");
    ...
    assert!(desktop.contains(&"openhub_delete_conversation"));
    assert!(
        remote.contains(&"openhub_delete_conversation"),
        "Destructive tools are Confirm (visible) on the Remote surface"
    );
    assert!(
        !channel.contains(&"openhub_delete_conversation"),
    ...
    // (the lead's openhub_run_create must keep working) but ABSENT from the external
    ...
    for name in ["openhub_run_create", "openhub_run_status", "openhub_run_result"] {
        assert!(
