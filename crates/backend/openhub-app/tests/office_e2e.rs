...
//! integration level in `openhub-office/tests/`.
...
use openhub_app::{AppConfig, AppServices, build_module_states, create_router_with_states};
use openhub_office::{
...
fn build_test_office_state(data_dir: &std::path::Path, allowed_roots: Vec<std::path::PathBuf>) -> OfficeRouterState {
    use openhub_office::error::OfficeError;
    use openhub_office::types::DocType;
    use openhub_office::{ProcessHandle, ProcessSpawner};
    ...
    let bc: Arc<dyn openhub_realtime::EventBroadcaster> = Arc::new(NoopBroadcaster);
