...
async fn insert_message(
    ...
    let repo = openhub_db::SqliteConversationRepository::new(services.database.pool().clone());
    let msg = openhub_db::models::MessageRow {
        id: msg_id.into(),
    ...
    openhub_db::IConversationRepository::insert_message(&repo, &msg)
        .await
        .unwrap();
}

async fn update_conversation_workspace(services: &openhub_app::AppServices, conv_id: i64, workspace: &str) {
    let repo = openhub_db::SqliteConversationRepository::new(services.database.pool().clone());
...
async fn upsert_artifact(services: &openhub_app::AppServices, artifact: openhub_db::ConversationArtifactRow) -> i64 {
    let repo = openhub_db::SqliteConversationRepository::new(services.database.pool().clone());
    openhub_db::IConversationRepository::upsert_artifact(&repo, &artifact)
        .await
        .unwrap()
        .id
}
...
async fn seed_cron_job(services: &openhub_app::AppServices, id: &str) {
    sqlx::query(
