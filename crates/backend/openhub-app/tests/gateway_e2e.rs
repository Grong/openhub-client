//! E2E tests for the Desktop Gateway MCP server (`openhub-gateway`).
//!
...
impl Gateway {
    fn from_services(services: &openhub_app::AppServices) -> Self {
        let cfg = services
            .gateway_mcp_config
            .as_ref()
...
async fn seed_user_and_conversation(services: &openhub_app::AppServices, user_id: &str, conv_id: i64) {
...
async fn seed_user_and_conversation_with_extra(
    services: &openhub_app::AppServices,
    user_id: &str,
    conv_id: i64,
    extra: &str,
) {
...
async fn gw_unauthenticated_tool_call_is_rejected() {
    ...
    let resp = local_http_client()
        ...
        .json(&json!({"tool": "openhub_list_conversations", "args": {}}))
        .send()
        .await
        .unwrap();
...
async fn gw_unknown_tool_returns_error() {
    ...
    let body = gw.call("openhub_explode_desktop", "", "u", json!({})).await;
...
async fn gw_list_conversations_returns_rows_with_runtime_state() {
    ...
    let body = gw.call("openhub_list_conversations", "1", "user_gw", json!({})).await;
...
async fn gw_conversation_tools_require_user_identity() {
    let (_app, services) = build_app().await;
    let gw = Gateway::from_services(&services);
    let body = gw.call("openhub_list_conversations", "1", "", json!({})).await;
...
async fn gw_list_conversations_excludes_companion_sessions() {
    ...
    let body = gw.call("openhub_list_conversations", "", "user_companion", json!({})).await;
    let result = result_of(&body);
...
async fn gw_cron_create_list_update_delete_roundtrip() {
    ...
    let body = gw.call("openhub_cron_list", "1", "user_gw", json!({})).await;
    ...
    let body = gw
        .call("openhub_cron_delete", "1", "user_gw", json!({"job_id": job_id, "confirm": true}))
        .await;
...
async fn gw_memory_save_list_update_delete_roundtrip() {
    ...
    let body = gw
        .call("openhub_memory_list", "", "user_gw", json!({"query": "深色主题"}))
        .await;
    ...
    let body = gw
        .call("openhub_memory_update", "", "user_gw", json!({"id": memory_id, "pinned": true}))
        .await;
    assert!(result_of(&body).as_str().unwrap().contains("updated"));

    let body = gw
        .call("openhub_memory_delete", "", "user_gw", json!({"id": memory_id, "confirm": true}))
        .await;
    assert!(result_of(&body).as_str().unwrap().contains("deleted"));
...
async fn gw_memory_update_with_no_fields_is_rejected() {
    ...
    let gw = Gateway::from_services(&services);
    let body = gw
        .call("openhub_memory_update", "", "user_gw", json!({"id": "mem_x"}))
        .await;
...
async fn gw_requirement_create_list_update_delete_roundtrip() {
    ...
    let body = gw
        .call("openhub_requirement_list", "", "user_gw", json!({"tag": "前端"}))
        .await;
    ...
    let body = gw
        .call("openhub_requirement_delete", "", "user_gw", json!({"id": req_id, "confirm": true}))
        .await;
    assert!(result_of(&body).as_str().unwrap().contains("deleted"));
}
