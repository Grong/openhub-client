...
use openhub_ai_agent::protocol::events::{FinishEventData, TextEventData};
...
use openhub_app::{AppConfig, AppServices, create_router};
use openhub_common::{AgentKillReason, AgentType, AppError, ConversationStatus, TimestampMs, now_ms};
...
async fn autowork_broadcasts_run_state_transitions_for_session_list_sync() {
    ...
    let conv = {
        let body = json!({ "type": "openhub", "name": "rs", "extra": { "workspace": "/project" } });
        let resp = app
            .clone()
            .oneshot(json_with_token("POST", "/api/conversations", body, &token, &csrf))
            .await
