...
async fn webui_lan_start_smoke() {
    ...
    // these tests run in parallel, so a shared set_var("OPENHUB_DATA_DIR") would
    // race and two backends could resolve to the SAME dir → "data directory
    // already in use by another running OpenHub backend".
    ...
    let started = openhub_app::DesktopServer::start(&cli, &merged_path, Some(spa_dir), None).await;
...
async fn webui_lan_spa_deep_link_serves_app_shell() {
    ...
    let (server, _keep) =
        openhub_app::DesktopServer::start(&cli, &merged_path, Some(spa_dir), None)
            .await
...
async fn webui_lan_without_app_shell_fails_instead_of_serving_partial_qr_flow() {
    ...
    let (server, _keep) = openhub_app::DesktopServer::start(&cli, &merged_path, None, None)
        .await
        .expect("DesktopServer::start failed");
...
async fn webui_lan_dev_proxy_serves_live_frontend() {
    ...
    let (server, _keep) =
        openhub_app::DesktopServer::start(&cli, &merged_path, None, Some(dev_url))
            .await
            .expect("DesktopServer::start failed");
...
async fn webui_enable_preserves_user_set_username_and_reports_persisted_state() {
    ...
    let (server, _keep) = openhub_app::DesktopServer::start(&cli, &merged_path, Some(spa_dir), None)
        .await
        .expect("DesktopServer::start failed");
    ...
    let db_path = tmp.path().join("openhub-backend.db");
