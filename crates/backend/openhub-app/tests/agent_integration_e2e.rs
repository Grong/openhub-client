...
use openhub_ai_agent::protocol::events::TextEventData;
use openhub_ai_agent::types::{BuildTaskOptions, SendMessageData};
use openhub_ai_agent::{AgentStreamEvent, IWorkerTaskManager};
use openhub_common::{AgentKillReason, AgentType, AppError, Confirmation, ConversationStatus, TimestampMs, now_ms};
...
impl IAgentTask for MockAgent {
    ...
    async fn send_message(&self, _data: SendMessageData) -> Result<(), openhub_ai_agent::AgentSendError> {
...
async fn build_app_with_mock_tasks() -> (axum::Router, openhub_app::AppServices, Arc<MockTaskManager>) {
    let db = openhub_db::init_database_memory().await.unwrap();
    let services = openhub_app::AppServices::from_config(db, &openhub_app::AppConfig::default())
        .await
        .unwrap();
    ...
    let router = openhub_app::create_router(&services).await;
