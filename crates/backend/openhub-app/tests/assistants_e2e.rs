...
//! against a real `openhub_app::create_router_with_states` instance backed by
...
//! extension) without touching `~/.openhub/`.
...
use openhub_app::{AppConfig, AppServices, ModuleStates, build_module_states, create_router_with_states};
...
async fn fixture() -> Fixture {
    ...
    // `build_module_states` point at `~/.openhub/` for the state store and
    ...
    // `~/.openhub/` for user data — neither is appropriate for tests.
    ...
    // an explicit `preset_agent_type` resolve to `"openhub"` instead of
    // erroring out — mirroring a configured production setup.
    provider_repo
        .create(openhub_db::CreateProviderParams {
            id: None,
            platform: "openai",
            name: "Test OpenAI",
...
async fn list_requires_auth() {
    ...
    // (see `openhub_auth::middleware::auth_middleware`).
