...
//! `openhub_app::create_router_with_states` against an in-memory DB.
...
use openhub_app::{ModuleStates, build_module_states, create_router_with_states};
...
async fn fixture_embedded() -> Fixture {
    ...
    unsafe {
        std::env::remove_var("OPENHUB_BUILTIN_SKILLS_PATH");
    }
    ...
    openhub_extension::materialize_if_needed(&data_dir, openhub_extension::builtin_skills_corpus(), "test-fixture")
        .await
    ...
    // `build_module_states` builds a state pointing at `~/.openhub/`,
