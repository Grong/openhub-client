...
//!
//! `openhub-app` is the only layer that sees the companion, public-agent, IDMM
//! and orchestrator subsystems at once, so the cross-subsystem
//! [`ProviderDeletionCoordinator`](openhub_system::provider_deletion::ProviderDeletionCoordinator)
...
mod tests {
    ...
    #[async_trait::async_trait]
    impl openhub_companion::learner::CompanionCompleter for NoopCompleter {
        async fn complete(
            &self,
            _provider_id: &str,
            _model: &str,
            ...
            _user: &str,
            _max_tokens: u32,
        ) -> Result<String, openhub_common::AppError> {
            Ok("{}".into())
        }
    }
    ...
    async fn coordinator(
        dir: &std::path::Path,
    ) -> (AppProviderDeletionCoordinator, Arc<openhub_db::Database>) {
        let db = Arc::new(init_database_memory().await.unwrap());
        let companion = openhub_companion::CompanionService::start(
            dir,
            Arc::new(openhub_realtime::BroadcastEventBus::new(16)),
            Arc::new(NoopCompleter),
            Arc::new(openhub_extension::skill_service::resolve_skill_paths(dir, dir)),
        )
        .await
        .unwrap();
    ...
    async fn aggregates_idmm_backup_usage() {
        ...
        coord
            .client_prefs
            .upsert_batch(&[(openhub_idmm::sidecar::PREF_BACKUP_PROVIDER, "prov_x")])
            .await
            .unwrap();
    ...
    async fn cleanup_strips_failover_queue_entry() {
        use openhub_conversation::model_failover::{
        ...
        cfg.queue = vec![
            openhub_common::ProviderWithModel {
