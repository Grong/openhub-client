//! `openhub-core mcp-knowledge-stdio` subcommand: MCP stdio server for the
...
use openhub_api_types::KnowledgeMcpConfig;
...
pub async fn run_knowledge_stdio() -> ExitCode {
        ...
        .and_then(|e| e.knowledge)
    {
        ...
        let port: u16 = match std::env::var(KnowledgeMcpConfig::ENV_PORT) {
            ...
            Err(_) => {
                eprintln!(
                    "[mcp-knowledge-stdio] ERROR:openhubdesktop not running or restarted — \
