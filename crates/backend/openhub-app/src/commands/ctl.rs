...
//! `OPENHUB_URL` / `OPENHUB_COMPANION_TOKEN` env vars (the token is a
...
fn resolve_endpoint(url: Option<String>, token: Option<String>) -> Result<(String, String), String> {
    let base = url
        .or_else(|| std::env::var("OPENHUB_URL").ok())
        .map(|u| u.trim_end_matches('/').to_owned())
        .filter(|u| !u.is_empty())
        .unwrap_or_else(|| DEFAULT_URL.to_owned());
    let token = token
        .or_else(|| std::env::var("OPENHUB_COMPANION_TOKEN").ok())
        .filter(|t| !t.trim().is_empty())
        .ok_or_else(|| {
            "no access token: pass --token or set OPENHUB_COMPANION_TOKEN (mint one in the \
...
pub async fn run_call(name: &str, args: Option<&str>, url: Option<String>, token: Option<String>) -> ExitCode {
        ...
        .await
    {
        ...
        Err(e) => {
            eprintln!("request to {base}/v1/tools/{name} failed: {e} (is OpenHub running and OPENHUB_URL correct?)");
            ExitCode::from(1)
        }
    }
}
...
/// `openhub_agent_run` against a running instance.
pub async fn run_agent(goal: &str, timeout_secs: Option<u64>, url: Option<String>, token: Option<String>) -> ExitCode {
    ...
    run_call("openhub_agent_run", Some(&args.to_string()), url, token).await
}
