...
pub async fn run_server(env: ServerEnvironment, services: AppServices) -> Result<ExitCode> {
    ...
    // Port failover (shared with desktop LAN + openhub-web): bind a bounded-scan
    ...
    let idle_scanner_handle =
        openhub_ai_agent::start_idle_scanner(services.worker_task_manager.clone(), shutdown_rx, None, None);
