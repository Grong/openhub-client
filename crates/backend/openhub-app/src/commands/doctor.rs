...
//! `openhub_runtime::init` + `enhance_process_path` for `Doctor` as it
...
use openhub_ai_agent::{AgentRegistry, UnavailableReason};
use openhub_db::{IAgentMetadataRepository, SqliteAgentMetadataRepository, init_database};
...
pub async fn run_doctor(cli: &Cli, merged_path: &str) -> Result<ExitCode> {
    ...
    let db_path = cli.data_dir.join("openhub-backend.db");
...
fn print_environment(merged_path: &str, data_dir: &Path) {
    ...
    if let Some(p) = std::env::var_os("OPENHUB_BUN_PATH") {
        println!("  OPENHUB_BUN_PATH: {}", PathBuf::from(p).display());
    }
    println!();
}

fn print_snapshot(snapshot: &[(openhub_api_types::AgentMetadata, Option<UnavailableReason>)]) {
