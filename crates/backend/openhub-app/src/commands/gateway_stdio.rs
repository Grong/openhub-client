...
//! Gateway capabilities (`openhub_*` — the whole platform control surface).
//!
...
use openhub_api_types::GatewayMcpConfig;
...
impl GatewayStdioServer {
    ...
    /// `openhub_gateway::CallerCtx::surface`): an IM channel platform marks an
    ...
    fn visible_tool_specs(&self) -> Vec<openhub_gateway::ToolSpec> {
...
mod tests {
    ...
    fn profile_filter_hides_domains_outside_allow_list() {
        ...
        let names: Vec<&str> = s
            ...
            .map(|spec| spec.name)
            .collect();
        assert!(names.contains(&"openhub_cron_create"));
        assert!(names.contains(&"openhub_requirement_create"));
        assert!(names.contains(&"openhub_knowledge_list_bases"));
        ...
        assert!(names.contains(&"openhub_run_create"));
        assert!(!names.contains(&"openhub_system_update_settings"));
        assert!(!names.contains(&"openhub_mcp_add_server"));
    }
    ...
    fn profile_filter_blocks_direct_call_to_hidden_tool() {
        ...
        assert!(s.blocked_tool_message("openhub_cron_create").is_none());
        let blocked = s
            .blocked_tool_message("openhub_system_update_settings")
