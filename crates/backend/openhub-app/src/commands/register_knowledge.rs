...
/// Build the openhub-knowledge server JSON value (NO token, NO port).
fn knowledge_server_value(nomicore: &str) -> Value {
    json!({ "command": nomicore, "args": ["mcp-knowledge-stdio"] })
}

/// Merge the openhub-knowledge server into an existing (or new) `.mcp.json`-shaped
...
pub fn merge_mcp_json(existing: Option<&str>, nomicore: &str) -> String {
    ...
    servers
        .as_object_mut()
        .unwrap()
        .insert("openhub-knowledge".to_owned(), knowledge_server_value(nomicore));
...
/// - `Codex` → runs `codex mcp add openhub-knowledge -- <nomicore> mcp-knowledge-stdio`
    ...
    match family {
        ...
        AgentCli::Codex => {
            ...
            let out = std::process::Command::new("codex")
                .args(["mcp", "add", "openhub-knowledge", "--", nomicore, "mcp-knowledge-stdio"])
                .output()
                .map_err(|e| {
                    std::io::Error::new(
...
mod tests {
    ...
    fn merge_mcp_json_new_creates_server() {
        ...
        assert_eq!(
            parsed["mcpServers"]["openhub-knowledge"]["command"],
            "/usr/local/bin/nomicore"
        );
        assert_eq!(
            parsed["mcpServers"]["openhub-knowledge"]["args"],
    ...
    fn merge_mcp_json_tolerates_malformed_existing() {
        ...
        assert_eq!(
            parsed2["mcpServers"]["openhub-knowledge"]["command"],
            "/bin/nomicore"
        );
    }
