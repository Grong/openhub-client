...
pub fn knowledge_register_template(nomicore_path: &str) -> RegisterTemplate {
    ...
    let claude_cmd = format!(
        "claude mcp add openhub-knowledge --scope user -- {} mcp-knowledge-stdio",
        shell_quoted
    );
...
// --- Quoting helpers (mirrored from openhub-terminal/src/enhance.rs) ---
...
mod tests {
    ...
    fn spaced_path_is_correctly_quoted_in_each_format() {
        ...
        assert_eq!(
            parsed["mcpServers"]["openhub-knowledge"]["command"].as_str().unwrap(),
            path
        );
        ...
        assert_eq!(
            parsed_g["mcpServers"]["openhub-knowledge"]["command"].as_str().unwrap(),
            path
        );
    }
    ...
    fn claude_cmd_format() {
        ...
        assert_eq!(
            t.claude_cmd,
            "claude mcp add openhub-knowledge --scope user -- \"/bin/nomicore\" mcp-knowledge-stdio"
        );
    }
}
