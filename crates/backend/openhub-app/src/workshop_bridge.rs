//! Bridge wiring the 生成引擎 (`openhub-creation`) to the 创意工坊 asset store
//! (`openhub-workshop`'s data dir + `openhub-db` index), without either domain
...
use openhub_creation::{AssetSink, AssetSource, CreationError, LoadedAsset, PersistAsset};
use openhub_db::{IWorkshopRepository, WorkshopAssetRow};
use openhub_workshop::WORKSHOP_REL_DIR;
...
mod tests {
    use super::*;
    use openhub_db::{SqliteWorkshopRepository, init_database_memory};
    use serde_json::json;
    ...
    async fn bridge() -> (WorkshopAssetBridge, tempfile::TempDir, openhub_db::Database) {
