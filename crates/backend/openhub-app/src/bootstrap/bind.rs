...
//! OS-assigned ephemeral one. Shared by the desktop LAN listener, `openhub-web`,
//! and the `openhub-core` bin so all three hosts fail over identically.
//!
...
/// injection channel for the fixed-port (web / openhub-core) hosts.
...
pub fn announce_bound_port(data_dir: &Path, host: &str, port: u16) {
    let announcement = PortAnnouncement {
        host: host.to_string(),
        port,
        channel: openhub_common::channel::channel().to_string(),
        pid: std::process::id(),
    };
    println!(
        "openhub: listening on {host}:{port} (channel {})",
