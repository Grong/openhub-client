...
//!
//! The standalone web host (`openhub-web`) runs the backend in authenticated
...
//!   * **Pre-seed (opt-in):** when the operator supplies `OPENHUB_ADMIN_PASSWORD`
//!     (optionally `OPENHUB_ADMIN_USERNAME`), this function provisions the admin
...
pub async fn ensure_admin_credentials(services: &AppServices, opts: AdminBootstrap) -> Result<bool> {
    ...
    if has_users {
        if opts.password.is_some() {
            tracing::info!(
                "admin bootstrap: an admin already exists; ignoring OPENHUB_ADMIN_PASSWORD. \
    ...
    let Some(password) = opts.password else {
        tracing::info!(
            "admin bootstrap: no OPENHUB_ADMIN_PASSWORD set — the first WebUI visitor \
    ...
    openhub_auth::validate_username(&username)
        .map_err(|e| anyhow::anyhow!("admin bootstrap: invalid admin username '{username}': {e}"))?;
    openhub_auth::validate_password(&password)
        .map_err(|e| anyhow::anyhow!("admin bootstrap: OPENHUB_ADMIN_PASSWORD rejected: {e}"))?;

    // bcrypt is CPU-bound — hash off the async runtime.
    let pw_for_hash = password.clone();
    let hash = tokio::task::spawn_blocking(move || openhub_auth::hash_password(&pw_for_hash))
        .await
    ...
    if provisioned {
        tracing::info!(%username, "admin bootstrap: pre-seeded admin from OPENHUB_ADMIN_PASSWORD");
    } else {
