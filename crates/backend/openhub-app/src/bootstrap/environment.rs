...
pub fn init_environment(cli: &Cli, merged_path: &str) -> Result<ServerEnvironment> {
    ...
    // Export the *actual* log dir so `openhub_system::sysinfo::resolve_log_dir`
    ...
    // OPENHUB_WORK_DIR export below.
    // SAFETY: called at the very start of boot, before any service initialization
    // or env reads; the only reader of OPENHUB_LOG_DIR is sysinfo, much later.
    unsafe {
        std::env::set_var("OPENHUB_LOG_DIR", &log_dir);
    }
    ...
    unsafe {
        std::env::set_var("OPENHUB_WORK_DIR", &work_dir);
    }
    ...
    let auth_policy = if cli.local {
        openhub_auth::AuthPolicy::NoAuth
    } else {
        openhub_auth::AuthPolicy::Required
    };
    ...
    // database via the normal migration path. See openhub_common::factory_reset.
    match openhub_common::factory_reset::apply_pending_reset(&config.data_dir, &config.work_dir) {
...
pub async fn init_data_layer(config: &AppConfig) -> Result<Database> {
    ...
    let database = openhub_db::init_database(&db_path).await?;
