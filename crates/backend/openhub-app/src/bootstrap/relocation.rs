...
pub async fn rewrite_path_prefixes(
    ...
    old_root: &str,
    new_root: &str,
) -> Result<u64, openhub_db::sqlx::Error> {
    ...
    for (old, new) in prefix_variants(old_root, new_root) {
        ...
        for (table, column) in TEXT_COLUMN_REWRITES {
            ...
            affected += openhub_db::sqlx::query(&sql)
                .bind(&old)
                .bind(&new)
                .execute(&mut *tx)
                .await?
        ...
        affected += openhub_db::sqlx::query(
...
mod tests {
    ...
    async fn kb_root(pool: &openhub_db::SqlitePool, id: &str) -> String {
    ...
    async fn insert_conversation(pool: &openhub_db::SqlitePool, id: i64, extra: &str) {
        sqlx::query(
    ...
    async fn conversation_extra(pool: &openhub_db::SqlitePool, id: i64) -> serde_json::Value {
    ...
    async fn insert_terminal(pool: &openhub_db::SqlitePool, id: i64, cwd: &str) {
        sqlx::query(
    ...
    async fn terminal_cwd(pool: &openhub_db::SqlitePool, id: i64) -> String {
    ...
    async fn marker_gates_and_finalizes() {
        ...
        let db = openhub_db::init_database(&data_dir.join("openhub-backend.db")).await.unwrap();
