...
/// `data_dir`, see [`openhub_common::dir_config`]) → `OPENHUB_WORK_DIR` env
/// (when non-empty) → `--data-dir` fallback.
///
/// The persisted choice sits **above** the env var on purpose: `relaunch()`
/// restarts the whole process and the child can inherit the `OPENHUB_WORK_DIR`
...
pub(super) fn resolve_work_dir(cli_work_dir: Option<PathBuf>, data_dir: &Path) -> PathBuf {
    ...
    if let Some(persisted) = openhub_common::dir_config::persisted_work_dir(data_dir) {
        return persisted;
    }
    std::env::var("OPENHUB_WORK_DIR")
        .ok()
        .filter(|s| !s.is_empty())
        .map(PathBuf::from)
...
mod tests {
    use super::*;
    use openhub_common::{dir_config, now_ms};

    fn temp_data_dir(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("openhub-wdres-{tag}-{}", now_ms()));
