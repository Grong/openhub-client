...
use openhub_common::OnConversationDelete;
use openhub_conversation::runtime_state::ConversationRuntimeStateService;
use openhub_db::{
...
use openhub_realtime::{BroadcastEventBus, WebSocketManager};
use openhub_terminal::{TerminalEventEmitter, TerminalLifecycleServer, TerminalService};
...
pub struct AppServices {
    ...
    pub skill_paths: Arc<openhub_extension::SkillPaths>,
    ...
    pub(crate) _requirement_mcp_server: Option<openhub_requirement::RequirementMcpServer>,
    ...
    /// simply lack the `openhub_*` tools (graceful degradation).
    ...
    pub(crate) _gateway_mcp_server: Option<openhub_gateway::GatewayMcpServer>,
    ...
    pub public_agent_service: Arc<openhub_public_agent::PublicAgentService>,
    ...
    pub workshop_service: Arc<openhub_workshop::WorkshopService>,
    ...
    pub creation_service: Arc<openhub_creation::CreationService>,
    ...
    pub knowledge_service: Arc<openhub_knowledge::KnowledgeService>,
}
...
impl AppServices {
    ...
    pub(crate) async fn inject_gateway_deps(&self, deps: Arc<openhub_gateway::GatewayDeps>) {
    ...
    pub async fn from_config(database: Database, config: &AppConfig) -> anyhow::Result<Self> {
        ...
        // failures say "grant OpenHub … then quit and reopen OpenHub" instead of a
        ...
        openhub_computer::set_host_app_label("OpenHub");
        ...
        let skill_paths = Arc::new(openhub_extension::resolve_skill_paths(
            &app_resource_dir,
            &data_dir,
        ));
        ...
        // not expose Team design/business flows, so no hidden `openhub_create_team`
        ...
        let requirement_emitter =
            openhub_requirement::RequirementEventEmitter::new(event_bus.clone());
        ...
        let webhook_repo_for_notifier: Arc<dyn openhub_db::IWebhookRepository> = Arc::new(
            openhub_db::SqliteWebhookRepository::new(database.pool().clone()),
        );
        let tag_setting_repo_for_notifier: Arc<dyn openhub_db::ITagSettingRepository> = Arc::new(
            openhub_db::SqliteTagSettingRepository::new(database.pool().clone()),
        );
        let completion_notifier = openhub_webhook::CompletionNotifierImpl::new(
            tag_setting_repo_for_notifier,
            webhook_repo_for_notifier,
            Arc::new(openhub_webhook::DefaultWebhookSender::new()),
        )
        .into_arc();
        let attachment_repo: Arc<dyn openhub_db::IAttachmentRepository> = Arc::new(
            openhub_db::SqliteAttachmentRepository::new(database.pool().clone()),
        );
        let attachment_store = Arc::new(openhub_requirement::AttachmentStore::new(
            data_dir.clone(),
            attachment_repo,
        ));
        let requirement_service = Arc::new(
            openhub_requirement::RequirementService::new(requirement_repo, requirement_emitter)
                .with_completion_notifier(completion_notifier)
                .with_attachment_store(attachment_store),
        );
        let requirement_sink =
            openhub_requirement::RequirementServiceSink::into_arc(requirement_service.clone());
        ...
        let (requirement_mcp_server, requirement_mcp_config) =
            match openhub_requirement::RequirementMcpServer::start().await {
                Ok(srv) => {
        ...
        // (channel master-agent, companion companion) the `openhub_*` desktop tools over
        ...
        let (gateway_mcp_server, gateway_mcp_config) =
            match openhub_gateway::GatewayMcpServer::start().await {
                Ok(srv) => {
        ...
        let open_mcp_config =
            cfg!(target_os = "windows").then(|| openhub_api_types::OpenMcpConfig {
                binary_path: backend_binary_path.to_string_lossy().to_string(),
            });
        ...
        let computer_mcp_config =
            cfg!(feature = "computer-use").then(|| openhub_api_types::ComputerMcpConfig {
                binary_path: backend_binary_path.to_string_lossy().to_string(),
            });
        ...
        let browser_mcp_config =
            cfg!(feature = "browser-use").then(|| openhub_api_types::BrowserMcpConfig {
                binary_path: backend_binary_path.to_string_lossy().to_string(),
            });
        ...
        let knowledge_repo: Arc<dyn openhub_db::IKnowledgeRepository> = Arc::new(
            openhub_db::SqliteKnowledgeRepository::new(database.pool().clone()),
        );
        let knowledge_service = Arc::new(openhub_knowledge::KnowledgeService::new(
            knowledge_repo,
            &data_dir,
            openhub_knowledge::KnowledgeEventEmitter::new(event_bus.clone()),
        ));
        ...
        knowledge_service.set_completer(Arc::new(openhub_ai_agent::LiveKnowledgeCompleter {
            provider_repo: provider_repo.clone() as Arc<dyn openhub_db::IProviderRepository>,
            encryption_key,
        ...
        // boundary (`openhub-knowledge` never depends on the browser engine) holds:
        // `BrowserFetcher` lives in `openhub-ai-agent` and is injected here as a
        ...
        #[cfg(feature = "browser-use")]
        {
            ...
            openhub_browser_engine::profile::gc_stale_profiles(
                &data_dir.join("browser-data").join("profiles"),
                std::time::Duration::from_secs(3600),
            );
            let browser_fetcher =
                openhub_ai_agent::BrowserFetcher::new(data_dir.join("knowledge-browser"));
        ...
        knowledge_service.register_connector(Arc::new(
            openhub_knowledge::connector_feishu::FeishuConnector::new(),
        ));
        let connector_cred_repo: Arc<dyn openhub_db::IConnectorCredentialRepository> = Arc::new(
            openhub_db::SqliteConnectorCredentialRepository::new(database.pool().clone()),
        );
        ...
        let (knowledge_mcp_server, knowledge_mcp_config) =
            match openhub_knowledge::KnowledgeMcpServer::start().await {
                Ok(srv) => {
        ...
        terminal_service.with_title_completer(Arc::new(openhub_ai_agent::LiveTerminalTitleCompleter {
        ...
        let companion_completer: Arc<dyn openhub_companion::learner::CompanionCompleter> =
            Arc::new(openhub_companion::learner::LiveCompanionCompleter {
                provider_repo: provider_repo.clone() as Arc<dyn openhub_db::IProviderRepository>,
                encryption_key,
                workspace: data_dir.clone(),
            });
        let companion_service = openhub_companion::CompanionService::start(
            &data_dir,
            event_bus.clone(),
            companion_completer,
            skill_paths.clone(),
        ...
        let public_agent_service = openhub_public_agent::PublicAgentService::start(&data_dir);
        ...
        let workshop_service = openhub_workshop::WorkshopService::start(
            &data_dir,
            Arc::new(openhub_db::SqliteWorkshopRepository::new(database.pool().clone())),
        );
        ...
        let creation_http = openhub_net::http_client();
        let creation_asset_bridge = Arc::new(crate::workshop_bridge::WorkshopAssetBridge::new(
            data_dir.clone(),
            Arc::new(openhub_db::SqliteWorkshopRepository::new(database.pool().clone())),
        ));
        let creation_service = openhub_creation::CreationService::builder(Arc::new(
            openhub_db::SqliteCreationTaskRepository::new(database.pool().clone()),
        ))
        .with_http(creation_http.clone())
        .with_provider_repo(
            Arc::new(openhub_db::SqliteProviderRepository::new(database.pool().clone())),
            encryption_key,
        )
        .with_asset_source(creation_asset_bridge.clone())
        .with_asset_sink(creation_asset_bridge)
        .with_providers(openhub_creation::default_adapters(creation_http))
        .build();
        ...
        if let Ok(seed) = std::env::var("OPENHUB_COMPANION_TOKEN") {
            let seed = seed.trim();
            if !seed.is_empty() && companion_token_validator.resolve(seed).is_none() {
                match companion_service.default_companion_id().await {
                    Some(default_id) => {
                        let hash = openhub_auth::token_sha256_hex(seed);
                        if let Err(e) = companion_token_repo
                            .upsert_for_companion(&default_id, &hash)
                            .await
                        {
                            tracing::warn!("failed to persist OPENHUB_COMPANION_TOKEN seed: {e}");
                        }
                        companion_token_validator.insert_token(default_id.clone(), hash);
                        tracing::info!(
                            "Remote access token seeded from OPENHUB_COMPANION_TOKEN, bound to default companion {default_id}"
                        );
                    }
                    None => tracing::warn!(
                        "OPENHUB_COMPANION_TOKEN set but no companion exists to bind it to; create a companion first"
                    ),
                }
            }
        }
        ...
        let provider_repo_for_services: Arc<dyn IProviderRepository> =
            provider_repo.clone() as Arc<dyn openhub_db::IProviderRepository>;
        ...
        let factory = build_agent_factory(AgentFactoryDeps {
            ...
            client_prefs: Some(Arc::new(openhub_db::SqliteClientPreferenceRepository::new(
                database.pool().clone(),
            ))
            ...
            settings_repo: Some(Arc::new(openhub_db::SqliteSettingsRepository::new(
                database.pool().clone(),
            )) as Arc<dyn openhub_db::ISettingsRepository>),
            ...
            cron_sink_factory: Some(Arc::new(|conversation_id: &str| {
                openhub_cron::sink::cron_sink_for(conversation_id.to_string())
            })),
            ...
            knowledge_retrieval: Some(Arc::new(openhub_ai_agent::LiveKnowledgeRetrievalSink {
            ...
            knowledge_writeback: Some(Arc::new(openhub_ai_agent::LiveKnowledgeWritebackSink {
                service: knowledge_service.clone(),
            })),
            companion_prompt: Some(
                companion_service.clone() as Arc<dyn openhub_ai_agent::CompanionPromptProvider>
            ),
            public_agent_provider: Some(
                public_agent_service.clone() as Arc<dyn openhub_ai_agent::PublicAgentProvider>
            ),
        });
...
/// Ensure every provider model has an authoritative [`openhub_db::ModelProfileRow`].
...
async fn reconcile_model_profiles(
    ...
    for provider in &providers {
        ...
        for model in models {
            ...
            let (tasks, traits) =
                openhub_api_types::derive_tasks_and_traits(&provider.platform, &model);
