...
use openhub_app::cli::{Cli, Command};
use openhub_app::{AppServices, bootstrap, commands};
...
fn main() -> Result<ExitCode> {
    ...
    // parse a uniform CLI, but bypass `openhub_runtime::init` (which would
    ...
    // detection path exactly. It must hit the same `openhub_runtime::init`
