...
//! `agent.model_failover`; the storage helpers live in `openhub_conversation`
//! (`get_global_failover_config` / `set_global_failover_config`). This module is
//! a thin authenticated router around them — `openhub-app` is the only crate that
//! depends on both `openhub-conversation` and the SQLite client-pref repo, so the
//! route is registered here rather than in the agent-listing router (which lives
//! in `openhub-ai-agent`, below conversation in the dependency graph).
...
use openhub_api_types::{ApiResponse, ModelFailoverConfig};
