...
//!
//! Lives in openhub-app (not openhub-auth) because the mint-time model-availability
//! guard needs `CompanionService`, and openhub-companion already depends on
//! openhub-auth (so auth must not depend back on companion).
...
use openhub_api_types::ApiResponse;
use openhub_auth::require_local_trust_middleware;
use openhub_common::AppError;
use openhub_db::ICompanionTokenRepository;

#[derive(Clone)]
pub struct CompanionTokenRouterState {
    pub companion_service: Arc<openhub_companion::CompanionService>,
    pub provider_repo: Arc<dyn openhub_db::IProviderRepository>,
    pub token_repo: Arc<dyn ICompanionTokenRepository>,
    pub token_validator: Arc<openhub_auth::CompanionTokenValidator>,
}
...
struct AccessTokenMintResponse {
    ...
    /// still mints; model-dependent capabilities like openhub_agent_run will fail
...
/// True if `openhub_agent_run` and friends would be able to resolve a model for this
...
async fn companion_model_resolvable(
    ...
    profile: &openhub_companion::CompanionProfileConfig,
) -> bool {
...
async fn mint(
    ...
    let token = openhub_auth::generate_random_hex_secret();
    let hash = openhub_auth::token_sha256_hex(&token);
    ...
    let warning = if companion_model_resolvable(&state, &profile).await {
        ...
        Some(
            "该伙伴尚未配置可用模型，且本机无启用的 provider；外部调用 openhub_agent_run 等需要模型的能力会失败。请先在桌面应用「模型管理」(/models) 配置 provider 与模型，并为该伙伴指定模型。"
                .to_string(),
        )
    };
