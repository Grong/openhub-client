...
use openhub_ai_agent::{agent_routes, remote_agent_routes};
use openhub_assets::{AssetRouterState, asset_routes};
use openhub_assistant::assistant_routes;
use openhub_auth::{
...
use openhub_channel::channel_routes;
use openhub_companion::{companion_public_routes, companion_routes};
use openhub_public_agent::public_agent_routes;
use openhub_workshop::{workshop_public_routes, workshop_routes};
use openhub_creation::creation_routes;
use openhub_conversation::{conversation_ops_routes, conversation_routes};
use openhub_cron::cron_routes;
use openhub_extension::{extension_routes, hub_routes, skill_routes};
use openhub_file::file_routes;
use openhub_idmm::idmm_routes;
use openhub_knowledge::knowledge_routes;
use openhub_mcp::mcp_routes;
use openhub_office::{office_proxy_routes, office_routes};
use openhub_orchestrator::orchestrator_routes;
use openhub_realtime::{WsHandlerState, ws_upgrade_handler};
use openhub_requirement::requirement_routes;
use openhub_shell::shell_routes;
use openhub_system::{connection_test_routes, system_routes};
use openhub_terminal::terminal_routes;
use openhub_webhook::webhook_routes;

use openhub_secret::secret_routes;
...
pub async fn create_router(services: &AppServices) -> Router {
    ...
    let gateway_deps = Arc::new(openhub_gateway::GatewayDeps {
        ...
        provider_repo: Arc::new(openhub_db::SqliteProviderRepository::new(
        ...
        // routes/service use, backing the read-only openhub_workshop_list_canvases cap.
        workshop_repo: Arc::new(openhub_db::SqliteWorkshopRepository::new(
            services.database.pool().clone(),
        )),
        ...
        client_pref_repo: Arc::new(openhub_db::SqliteClientPreferenceRepository::new(
            services.database.pool().clone(),
        )),
        ...
        browser_registry: Some(
            ...
            openhub_gateway::browser_registry::BrowserRegistry::default_for_browser_use()
        ...
        computer_registry: Some(openhub_gateway::computer_registry::ComputerRegistry::new()),
    });
    ...
    let _channel_watchdog = chan_mgr.spawn_watchdog(
        chan_factory,
        openhub_channel::manager::WatchdogConfig::default(),
    );
    tracing::info!(
        elapsed_ms = boot.elapsed().as_millis(),
    ...
    let router = router.nest(
        "/mcp",
        openhub_public::public_mcp_router(
            gateway_deps.clone(),
    ...
    let router = router.nest(
        ...
        openhub_public::public_mcp_router(
            gateway_deps.clone(),
            services.companion_token_validator.clone(),
            Some(openhub_public::AGENT_PROFILE_DOMAINS),
        ),
    );
    ...
    let router = router.nest(
        "/v1",
        openhub_public::public_rest_router(
            gateway_deps,
            services.companion_token_validator.clone(),
        ),
    );
...
pub fn create_router_with_all_state(
    ...
    let model_failover_authenticated = model_failover_routes(ModelFailoverRouterState {
        client_prefs: Arc::new(openhub_db::SqliteClientPreferenceRepository::new(
            services.database.pool().clone(),
        )),
    })
    .route_layer(from_fn_with_state(auth_mw_state.clone(), auth_middleware));
    ...
    let browser_login_authenticated = {
        let browser_data_dir = openhub_config::config::app_config_dir()
            .map(|d| d.join("browser-data"))
            .unwrap_or_else(|| std::env::temp_dir().join("openhub-browser-data"));
    ...
    let router = router.layer(DefaultBodyLimit::max(openhub_common::constants::BODY_LIMIT));
