...
use openhub_auth::generate_random_hex_secret;
...
use openhub_auth::AuthPolicy;
use openhub_db::IUserRepository;
...
pub struct DesktopServer {
    ...
    terminal_service: Arc<openhub_terminal::TerminalService>,
...
impl DesktopServer {
    ...
    pub async fn start_lan(self: &Arc<Self>) -> WebUiStatus {
        ...
        if !has_password {
            let pw = openhub_auth::generate_password(20);
            let pw_for_hash = pw.clone();
            let hashed = match tokio::task::spawn_blocking(move || {
                openhub_auth::hash_password(&pw_for_hash)
            })
            .await
            {
                Ok(Ok(h)) => h,
...
/// so the desktop LAN listener, `openhub-web`, and the `nomicore` bin fail over
/// identically.
