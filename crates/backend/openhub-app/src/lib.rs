...
pub use openhub_auth::AuthPolicy;
// Re-export the build channel so in-process hosts (the Tauri desktop shell)
// reach it through their existing `openhub-app` dependency without adding a
// direct `openhub-common` dep.
pub use openhub_common::channel;
pub use router::{
...
/// In-process server entry used by embedded hosts (Tauri desktop, `openhub-web`)
