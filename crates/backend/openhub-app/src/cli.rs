...
/// `OPENHUB_DATA_DIR` env / `--data-dir` flag remain the escape hatch for an
...
pub fn default_data_dir() -> PathBuf {
    dirs::data_local_dir()
        .map(|dir| dir.join("OpenHub"))
        .unwrap_or_else(|| std::env::temp_dir().join("openhub-data"))
...
/// before failing cryptically. `OPENHUB_WORK_DIR` already gets the same
/// non-empty filter in `bootstrap::work_dir`.
pub fn parse_non_empty_path(s: &str) -> Result<PathBuf, String> {
    if s.trim().is_empty() {
        return Err(
            "must not be empty (unset OPENHUB_DATA_DIR instead of setting it to an empty string)"
                .into(),
        );
    }
    Ok(PathBuf::from(s))
...
pub struct Cli {
    ...
    #[arg(long, default_value_t = openhub_common::constants::DEFAULT_PORT)]
    pub port: u16,

    /// Data directory for database and file storage.
    #[arg(long, env = "OPENHUB_DATA_DIR", default_value_os_t = default_data_dir(), value_parser = parse_non_empty_path)]
    pub data_dir: PathBuf,

    /// Working directory for conversation workspaces.
    /// Falls back to OPENHUB_WORK_DIR env, then to data-dir.
    #[arg(long)]
    ...
    /// Log level filter (e.g. "info", "debug", "info,openhub_mcp=trace").
    #[arg(long)]
...
pub enum Command {
    ...
    McpKnowledgeStdio,
    /// MCP stdio server for the Desktop Gateway tools (`openhub_*` — conversations,
    ...
    /// instance required.
    Tools,
    /// Invoke a capability on a RUNNING OpenHub instance via its REST `/v1` API.
    /// Endpoint/token from `--url`/`--token` or `OPENHUB_URL` /
    /// `OPENHUB_COMPANION_TOKEN`.
    Call {
        /// Capability name, e.g. `openhub_agent_run` (see `nomicore tools`).
        name: String,
        /// JSON arguments object (default `{}`).
        args: Option<String>,
        /// Instance base URL (default `$OPENHUB_URL` or http://127.0.0.1:25808).
        #[arg(long)]
        url: Option<String>,
        /// Per-companion access token (default `$OPENHUB_COMPANION_TOKEN`).
        #[arg(long)]
        token: Option<String>,
    },
    /// Delegate a goal to an autonomous OpenHub agent on a running instance
    /// (convenience wrapper over the `openhub_agent_run` capability).
    Agent {
...
mod tests {
    ...
    #[test]
    fn default_data_dir_is_per_user_openhub_nomi() {
        // Pure shape check on the unset default — env handling belongs to clap
        // (`env = "OPENHUB_DATA_DIR"`) and is not exercised here to keep the
