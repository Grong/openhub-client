...
//!
//! Hooks defined here let lower-layer crates (e.g. `openhub-ai-agent`,
//! `openhub-cron`) react to events owned by higher-layer crates (e.g.
//! `openhub-conversation`) without forming a dependency cycle.
...
/// crates react to a terminal going away without `openhub-terminal` depending
/// on them (e.g. `openhub-requirement` clears the dual-domain
...
/// IM → requirement pipeline). Lets `openhub-channel` file a message as a
/// requirement without depending on `openhub-requirement`; the concrete
/// implementor (in `openhub-requirement`) delegates to `RequirementService`.
