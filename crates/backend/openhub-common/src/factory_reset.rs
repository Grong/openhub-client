...
/// SQLite DB family (relative to data_dir). Mirrors `openhub-db`'s on-disk
...
const DB_FAMILY: &[&str] = &[
    "openhub-backend.db-wal",
    "openhub-backend.db-shm",
    "openhub-backend.db.migrate.lock",
    "openhub-backend.db",
];

/// Derived data directories (relative to data_dir) wiped on a `Full` reset.
/// Names are kept as literals here so `openhub-common` need not depend on the
/// domain crates; they mirror those crates' constants:
///   `knowledge`   → `openhub_knowledge::KB_MANAGED_REL_DIR`
///   `companion`         → `openhub_companion::PET_*_REL_DIR` (whole tree incl. `memory.db`)
///   `attachments` → `openhub_requirement` `ATTACHMENTS_REL_DIR`
///   `cron`        → parent of `openhub_cron::CRON_SKILLS_REL_DIR`
...
/// (mirrors `openhub-db`'s startup file-op retry for raw OS errors 5/32/33).
...
mod tests {
    ...
    fn no_marker_is_noop() {
        let dir = std::env::temp_dir().join(format!("openhub-fr-noop-{}", now_ms()));
    ...
    fn full_reset_wipes_targets_keeps_logs_and_clears_marker() {
        let dir = std::env::temp_dir().join(format!("openhub-fr-full-{}", now_ms()));
        std::fs::create_dir_all(&dir).unwrap();

        // DB family + sidecars.
        touch(&dir.join("openhub-backend.db"));
        touch(&dir.join("openhub-backend.db-wal"));
        touch(&dir.join("openhub-backend.db-shm"));
        touch(&dir.join("openhub-backend.db.migrate.lock"));
        // Derived data dirs.
    ...
    fn malformed_marker_still_triggers_reset() {
        let dir = std::env::temp_dir().join(format!("openhub-fr-bad-{}", now_ms()));
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(dir.join(RESET_MARKER_FILE), b"not json").unwrap();
        touch(&dir.join("openhub-backend.db"));

        assert_eq!(apply_pending_reset(&dir, &dir).unwrap(), true);
        assert!(!dir.join("openhub-backend.db").exists());
        assert!(!dir.join(RESET_MARKER_FILE).exists());
    ...
    fn full_reset_clears_dir_config_override() {
        let dir = std::env::temp_dir().join(format!("openhub-fr-dircfg-{}", now_ms()));
        std::fs::create_dir_all(&dir).unwrap();
