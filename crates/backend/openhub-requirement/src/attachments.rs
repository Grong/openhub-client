...
use openhub_api_types::{AttachmentDto, NewAttachmentRef};
use openhub_common::{AppError, generate_prefixed_id, now_ms};
use openhub_db::IAttachmentRepository;
use openhub_db::models::AttachmentRow;
use openhub_file::path_safety::{has_traversal, validate_path};
use tracing::warn;
...
/// for the model to read: `.openhub/requirement-attachments/{req_id}/{file_name}`.
const WORKSPACE_STAGE_REL_DIR: &str = ".openhub/requirement-attachments";
...
impl AttachmentStore {
    pub fn new(data_dir: PathBuf, repo: Arc<dyn IAttachmentRepository>) -> Self {
        Self {
            data_dir,
            upload_root: std::env::temp_dir().join("openhub"),
            repo,
        }
    }
    ...
    /// `{workspace}/.openhub/requirement-attachments/{req_id}/{file_name}` when a
...
mod tests {
    use super::*;
    use openhub_db::{SqliteAttachmentRepository, init_database_memory};
    use std::sync::Arc;
    ...
    async fn store() -> (AttachmentStore, tempfile::TempDir, tempfile::TempDir) {
        ...
        let repo: Arc<dyn openhub_db::IAttachmentRepository> =
    ...
    async fn ingest_rejects_non_image_and_traversal_and_outside_root() {
        ...
        let err = store
            ...
            .await
            .unwrap_err();
        assert!(matches!(err, openhub_common::AppError::BadRequest(_) | openhub_common::AppError::Forbidden(_)));
        ...
        let err = store
            ...
            .await
            .unwrap_err();
        assert!(matches!(err, openhub_common::AppError::Forbidden(_)));
    ...
    async fn stage_for_prompt_copies_into_workspace_and_falls_back() {
        ...
        assert_eq!(staged[0].path, "./.openhub/requirement-attachments/1/图.png");
        assert!(ws.path().join(".openhub/requirement-attachments/1/图.png").exists());
        assert!(ws.path().join(".openhub/requirement-attachments/.gitignore").exists());
