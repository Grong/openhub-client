...
use dashmap::DashMap;
use openhub_ai_agent::AgentStreamEvent;
use openhub_ai_agent::TurnStopReason;
use openhub_ai_agent::registry::AgentRegistry;
use openhub_ai_agent::task_manager::IWorkerTaskManager;
use openhub_ai_agent::types::BuildTaskOptions;
use openhub_api_types::{AutoWorkState, AutoWorkTargetKind, Requirement, RequirementStatus, SendMessageRequest};
use openhub_common::AppError;
use openhub_conversation::ConversationService;
use openhub_db::{IConversationRepository, IUserRepository};
use openhub_terminal::{LifecycleKind, TerminalDriver};
    ...
    pub fn start_sweeper(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                match service
                    .repo()
                    .sweep_expired_leases(&active, openhub_common::now_ms())
                    .await
                {
    ...
    loop {
        ...
        let result = match kind {
            AutoWorkTargetKind::Conversation => {
                ...
                let turn_started_ms = openhub_common::now_ms();
...
async fn inject_and_wait(
    ...
    let options = BuildTaskOptions {
        ...
        extra,
        // Stamp/validate the openhub session against this conversation instance so
...
async fn wait_terminal_turn_end(
    ...
    terminal_id: i64,
    req_id: i64,
    lifecycle_rx: Option<broadcast::Receiver<openhub_terminal::TerminalLifecycleEvent>>,
) -> TerminalTurnEnd {
    let mut renew = interval(LEASE_RENEW_INTERVAL);
    renew.tick().await; // consume the immediate first tick
...
async fn parse_agent_type(registry: &AgentRegistry, agent_type_str: &str) -> openhub_common::AgentType {
    if registry.find_builtin_by_backend(agent_type_str).await.is_some() {
        return openhub_common::AgentType::Acp;
    }
    serde_json::from_value::<openhub_common::AgentType>(serde_json::Value::String(agent_type_str.to_owned()))
        .unwrap_or(openhub_common::AgentType::Acp)
}
...
mod tests {
    ...
    fn terminal_submit_chunks_keeps_cr_out_of_the_paste_burst() {
        ...
        // by the shared encoder (`openhub_terminal::encode_submit_chunks`).
        use openhub_terminal::{encode_submit_chunks, SubmitChunks};
    ...
    async fn lifecycle_turn_end_event_resolves_immediately() {
        ...
        use openhub_terminal::TerminalLifecycleEvent;
