...
use openhub_common::{AppError, PaginatedResult, now_ms};
use openhub_db::models::{RequirementRow, RequirementRowUpdate, RequirementTagRow};
use openhub_db::{
...
pub struct RequirementService {
    ...
    conversation_service: Option<openhub_conversation::ConversationService>,
...
impl RequirementService {
    ...
    pub fn with_conversation_service(
        mut self,
        cs: openhub_conversation::ConversationService,
    ...
    pub async fn delete_many(&self, ids: &[i64]) -> Result<u64, AppError> {
        ...
        for &id in ids {
            ...
            match self.repo.delete(id).await {
                ...
                Err(openhub_db::DbError::NotFound(_)) => {}
    ...
    pub async fn user_interrupt(&self, id: i64, conversation_id: i64, tag: &str) -> Result<(), AppError> {
        match self.repo.pause_tag(tag, "user_interrupted", Some(id), now_ms()).await {
            Ok(()) => self.emitter.emit_tag_paused(&openhub_api_types::TagPausedPayload {
                tag: tag.to_string(),
    ...
    pub fn emit_autowork_state(&self, state: &openhub_api_types::AutoWorkState) {
    ...
    /// `openhub_terminal::terminal_autowork_capable`, the SAME logic the launch
    ...
    pub async fn ensure_terminal_autowork_eligible(&self, terminal_id: &str) -> Result<(), AppError> {
        ...
        let desc = driver
            ...
            .ok_or_else(|| AppError::NotFound(format!("terminal {terminal_id}")))?;
        let is_agent =
            openhub_terminal::terminal_autowork_capable(&desc.command, &desc.args, desc.backend.as_deref());
        if !is_agent {
    ...
    /// deletion paths (`openhub-conversation` / `openhub-terminal` delete) with
...
/// so the deletion path drives this explicitly. Wired in `openhub-app` via
/// `ConversationService::with_delete_hook`.
#[async_trait::async_trait]
impl openhub_common::OnConversationDelete for RequirementService {
    async fn on_conversation_deleted(&self, conversation_id: i64) {
            ...
            .await
        {
            warn!(
                conversation_id,
                error = %openhub_common::ErrorChain(&e),
...
/// `term_*` owner domain. Wired in `openhub-app` via
/// `TerminalService::with_delete_hook`.
#[async_trait::async_trait]
impl openhub_common::OnTerminalDelete for RequirementService {
...
mod tests {
    use super::*;
    use openhub_db::SqliteRequirementRepository;
    use openhub_db::init_database_memory;
    ...
    async fn svc() -> RequirementService {
        ...
        sqlx::query(
            "INSERT INTO conversations (id, user_id, name, type, created_at, updated_at) \
             VALUES (1, 'user_1', 'Test Conv', 'openhub', 0, 0)",
        )
        .execute(db.pool())
        .await
        .unwrap();
    ...
    async fn svc_with_attachments() -> (RequirementService, tempfile::TempDir, tempfile::TempDir) {
        let db = init_database_memory().await.unwrap();
        let repo: Arc<dyn IRequirementRepository> = Arc::new(SqliteRequirementRepository::new(db.pool().clone()));
        let att_repo: Arc<dyn openhub_db::IAttachmentRepository> =
            Arc::new(openhub_db::SqliteAttachmentRepository::new(db.pool().clone()));
        let emitter = RequirementEventEmitter::new(Arc::new(NoopBroadcaster));
        Box::leak(Box::new(db));
    ...
    async fn create_with_attachments_binds_and_returns_dtos() {
        ...
        let created = s
            .create(CreateRequirementRequest {
                ...
                status: None,
                created_by: None,
                attachments: vec![openhub_api_types::NewAttachmentRef {
                    source_path: src,
    ...
    async fn update_adds_and_removes_attachments() {
        ...
        let created = s
            .create(CreateRequirementRequest {
                ...
                status: None,
                created_by: None,
                attachments: vec![openhub_api_types::NewAttachmentRef { source_path: a, file_name: "a.png".into() }],
            })
            .await
            .unwrap();
        ...
        let updated = s
            .update(
                ...
                UpdateRequirementRequest {
                    ...
                    status: None,
                    completion_note: None,
                    add_attachments: vec![openhub_api_types::NewAttachmentRef { source_path: b, file_name: "b.png".into() }],
        ...
        let err = s
            .update(
                ...
                UpdateRequirementRequest {
                    ...
                    status: None,
                    completion_note: None,
                    add_attachments: vec![openhub_api_types::NewAttachmentRef { source_path: bad, file_name: "bad.txt".into() }],
                    remove_attachment_ids: vec![created.attachments[0].id.clone()],
                },
            )
            .await
    ...
    use openhub_terminal::TerminalDescription;
    use openhub_terminal::error::TerminalError;
    ...
    impl TerminalDriver for MockDriver {
        ...
        fn subscribe_lifecycle(
            &self,
            _id: i64,
        ) -> Option<tokio::sync::broadcast::Receiver<openhub_terminal::TerminalLifecycleEvent>> {
            None
        }
    }
    ...
    async fn svc_with_conv5_and_term5() -> RequirementService {
        ...
        sqlx::query(
            "INSERT INTO conversations (id, user_id, name, type, created_at, updated_at) \
             VALUES (5, 'user_1', 'Conv Five', 'openhub', 0, 0)",
        )
        .execute(db.pool())
        .await
        .unwrap();
