...
//!
//! AutoWork (this crate) drives turn execution; IDMM (the `openhub-idmm` crate)
//! supervises a session for stalls. To let AutoWork ensure a session is being
//! supervised while a turn runs — WITHOUT this crate depending on `openhub-idmm`
//! (which would be a cycle, since idmm conceptually sits above requirement) —
//! AutoWork defines this trait and holds an optional handle. `openhub-idmm`
//! implements it; `openhub-app` injects the implementation at assembly time.

use openhub_api_types::AutoWorkTargetKind;

/// Implemented by `openhub-idmm::IdmmManager`. AutoWork calls
