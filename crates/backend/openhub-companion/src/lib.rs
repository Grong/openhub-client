//! `openhub-companion` — the desktop-companion domain: a roster of companions sharing one
...
//! API surface; `migrate` lifts a legacy `companion/openhub/` install into the split.
