...
//!
//! Validation mirrors `openhub-requirement`'s attachment ingest: the source
//! must canonicalize to a path inside the upload root (`{temp_dir}openhub,
...
mod tests {
    ...
    /// (`{temp_dir}openhub) — figure sources must live under it.
    ...
    fn ingest_rejects_source_outside_upload_root() {
        // tempdir() lands directly in temp_dir(), NOT under {temp_dir}/openhub.
