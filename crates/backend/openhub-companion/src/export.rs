...
//! the knowledge-base exporter (`openhub-knowledge/src/export.rs`):
...
pub const EXPORT_FORMAT: &str = "openhub-export";
...
fn extract_zip_validated(archive_path: &Path, destination: &Path) -> Result<String, AppError> {
    ...
    let mut archive =
        zip::ZipArchive::new(file).map_err(|_| AppError::BadRequest("不是 OpenHub 导出包".into()))?;
    ...
    for index in 0..archive.len() {
        ...
        if entry.is_dir() {
            if rel != Path::new("events") {
                return Err(AppError::BadRequest(format!(
                    "不是 OpenHub 导出包（包含不支持的条目: {entry_name}）"
                )));
            }
    ...
    let manifest_bytes = std::fs::read(destination.join("manifest.json"))
        .map_err(|_| AppError::BadRequest("不是 OpenHub 导出包".into()))?;
...
fn validate_manifest(manifest: &serde_json::Value) -> Result<String, AppError> {
    ...
    if format != Some(EXPORT_FORMAT) {
        return Err(AppError::BadRequest("不是 OpenHub 导出包".into()));
    }
    ...
    match manifest.get("kind").and_then(|v| v.as_str()) {
        ...
        None => Err(AppError::BadRequest("不是 OpenHub 导出包".into())),
    }
}
...
mod tests {
    ...
    fn manifest_json(version: u32, kind: &str) -> String {
        format!(
            r#"{{"format":"openhub-export","version":{version},"kind":"{kind}","exported_at":0,"app_version":"0.0.0"}}"#
        )
    }
    ...
    async fn memory_bundle_roundtrip_full_fidelity_and_dedup() {
        ...
        let mut originals = vec![
            ...
            raw_memory("mem_ccc", "knowledge", "cargo test -p openhub-companion 是门禁", "active"),
        ];
        for m in &originals {
    ...
    async fn import_rejects_wrong_format_kind_newer_version_and_garbage() {
        ...
        assert!(err.to_string().contains("不是 OpenHub 导出包"), "{err}");
