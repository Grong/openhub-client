//! Companion chat threads: real `type='openhub'` conversations driven by the
...
use openhub_ai_agent::CompanionMemorySink;
use openhub_api_types::CreateConversationRequest;
use openhub_common::{AppError, ProviderWithModel};
...
/// platform and that it can drive the whole desktop via the `openhub_*` tools.
pub async fn build_companion_system_prompt(
    ...
    let name = if profile.name.trim().is_empty() { "openhub" } else { profile.name.trim() };
    ...
    if let Some(platform) = channel_platform.filter(|p| !p.is_empty()) {
        system.push_str(&format!(
            "\n\n主人此刻正通过 {platform} 远程和你说话。此刻你是一个通过 IM 陪主人聊天、答疑、出主意的对话助手：\
             你可以用 openhub_list_conversations / openhub_conversation_status 等只读工具帮主人了解桌面上正在跑的会话状态并转述，\
             也可以用 openhub_memory_* 维护你的长期记忆。\
             当主人主动告诉你某个会话卡在决策上、或你查看到它在等人选择（runtime_state 为 WaitingConfirmation，或 pending_confirmations > 0）时，\
             你可以替主人转达：先用 openhub_list_confirmations(conversation_id) 读出待决项和选项，\
             把问题和选项以编号列表发给主人（如「1. 允许  2. 拒绝」），主人回复编号后，\
             用 openhub_resolve_confirmation(conversation_id, call_id, option) 提交对应选项的 value，别擅自替主人做选择。\
    ...
    system.push_str(
        "\n\n## 知识沉淀技巧\n\
         ...
         - 动作序列：openhub_knowledge_create_base 建库（可直接带 urls，snapshot 模式会在后台抓快照并生成梗概，\
         立即返回不必等待、切勿重复建库）→ \
         openhub_knowledge_write_file 写入你整理好的 markdown → openhub_knowledge_autogen 刷新梗概 → \
         openhub_knowledge_set_binding 把库绑定到目标会话/终端/你自己（kind=\"companion\"）。绑定变更在目标下次任务启动时生效。\n\
         - 分工边界：全局记忆（openhub_memory_*）只放轻量的个人事实与偏好；知识库放成体系、可检索的领域资料。闲聊琐事不要建库。",
    );
    ...
    if smart_orchestration && !remote {
        system.push_str(
            "\n\n## 复杂任务：调度子 agent（智能编排）\n\
             需要并行开几个子 agent 干活时，默认用 openhub_spawn(tasks)：不经规划直接并行开工，\
             主人能在画布上实时看到每个子 agent 的状态与产出。只有当任务真正复杂、需要先拆成\
             有依赖关系的任务图时，才用 openhub_run_create(goal) 交给编排引擎自动拆解并行、随即开跑。\
             派发后直接告诉主人已在后台执行、进度见画布，然后正常继续——不必守着轮询：全部完成\
             或失败时系统会自动把结果回执给你，届时你再向主人汇总产出；若失败，先 openhub_run_status \
             看清哪个节点、原因(last_error)，再用 openhub_run_adjust 增量改编排或 openhub_task_config 换\
             模型后 openhub_task_rerun 重跑失败节点，试过几种仍不成就如实问主人怎么办。这样你全程在场、\
             不会像失联，重活也不会挤占你和主人的对话上下文。主人若中途问进度，才用 openhub_run_status 查一次。\
...
fn compute_desired_workspace_dir(
    ...
    match profile.seq {
        Some(seq) => {
            let seg = openhub_common::sanitize_dir_segment(&profile.name);
...
mod workspace_path_tests {
    ...
    fn plan_foreign_temp_cwd_left_untouched() {
        ...
        let ws = Path::new("/ws");
        assert_eq!(
            plan_workspace_reconcile("/data/conversations/openhub-temp-9", desired, legacy, ws),
            WorkspaceAction::Leave
        );
    }
}
...
pub struct CompanionThreads {
    ...
    pub task_manager: Arc<dyn openhub_ai_agent::IWorkerTaskManager>,
}
    ...
    pub async fn create(&self, companion_id: &str, title: Option<String>) -> Result<CompanionThread, AppError> {
        ...
        let req = CreateConversationRequest {
            ...
            extra: serde_json::json!({
                ...
                // the Desktop Gateway tools (openhub_* — sessions/cron/memory/
    ...
    pub async fn set_model(
        ...
        self.conversations
            .update(
                COMPANION_USER_ID,
                conversation_id,
                openhub_api_types::UpdateConversationRequest {
                    name: None,
                    pinned: None,
                    model: Some(ProviderWithModel {
...
fn mirror_memory_to_openhub(dir: &std::path::Path, kind: &str, content: &str) -> std::io::Result<()> {
    ...
    let path = openhub_memory::store::write_memory(dir, &entry)
    ...
    let _ = openhub_memory::index::append_index_entry(&dir.join("MEMORY.md"), &title, filename, &title);
    Ok(())
}
...
mod tests {
    ...
    use openhub_realtime::BroadcastEventBus;
    ...
    async fn companion_system_prompt_uses_profile_name_persona_and_memories() {
        ...
        let today = format_date(openhub_common::now_ms());
    ...
    async fn companion_system_prompt_smart_orchestration_nudge_local_only() {
        ...
        // On + local → nudge present, teaching openhub_run_create delegation.
        ...
        assert!(on.contains("openhub_run_create"));
        assert!(on.contains("openhub_spawn"), "must teach the flat fan-out verb for independent small tasks");
    ...
    async fn local_prompt_teaches_terminal_tools_remote_does_not() {
        // Local companion threads carry the terminal domain (openhub_terminal_*),
    ...
    async fn companion_system_prompt_teaches_knowledge_curation() {
        ...
        for platform in [None, Some("telegram")] {
            ...
            let seq = ["openhub_knowledge_create_base", "openhub_knowledge_write_file", "openhub_knowledge_autogen", "openhub_knowledge_set_binding"];
            let mut last = 0;
            for tool in seq {
    ...
    async fn remote_prompt_forbids_proactive_dispatch_and_filters_task_memories() {
        ...
        assert!(!remote.contains("openhub_send_to_conversation"), "remote must not advertise task dispatch");
