...
use openhub_extension::skill_service::{self, SkillPaths, SkillScope};
use openhub_extension::constants::SKILL_MANIFEST_FILE;
...
impl CompanionService {
    ...
    pub async fn start(
        ...
        bus: Arc<openhub_realtime::BroadcastEventBus>,
    ...
    pub fn attach_companion(
        &self,
        conversations: Arc<openhub_conversation::ConversationService>,
        task_manager: Arc<dyn openhub_ai_agent::IWorkerTaskManager>,
    ) {
    ...
    pub fn memory_sink(&self) -> Arc<dyn openhub_ai_agent::CompanionMemorySink> {
    ...
    pub fn skill_sink(&self) -> Arc<dyn openhub_ai_agent::CompanionSkillSink> {
    ...
    pub async fn add_memory(&self, kind: &str, content: &str, tags: &[String], scope: MemoryScope) -> Result<CompanionMemory, AppError> {
        ...
        // `openhub_memory_save` path, which previously inserted blindly.
    ...
    pub async fn decide_companion_skill(
        ...
        if accept {
            ...
            let fid = openhub_common::generate_prefixed_id("evf");
            let sig = if skill.signature.is_empty() { None } else { Some(skill.signature.as_str()) };
            self.store
                .record_feedback(&fid, name, sig, "reject", reason, openhub_common::now_ms())
                .await?;
...
#[async_trait::async_trait]
impl openhub_ai_agent::CompanionPromptProvider for CompanionService {
...
mod tests {
    ...
    async fn service(data_dir: &std::path::Path) -> Arc<CompanionService> {
        CompanionService::start(
            ...
            Arc::new(openhub_extension::skill_service::resolve_skill_paths(data_dir, data_dir)),
        )
        .await
        .unwrap()
    }
    ...
    async fn accepting_create_skill_promotes_draft_and_is_idempotent() {
        ...
        let input = openhub_extension::skill_service::SkillDraftInput {
            name: "demo".into(),
    ...
    async fn recent_events_returns_newest_window() {
        let dir = tempfile::tempdir().unwrap();
        let svc = service(dir.path()).await;
        let base = openhub_common::now_ms();
        for i in 0..5 {
    ...
    async fn companion_prompt_provider_builds_only_for_bound_companion() {
        use openhub_ai_agent::CompanionPromptProvider;
        let dir = tempfile::tempdir().unwrap();
        let svc = service(dir.path()).await;
