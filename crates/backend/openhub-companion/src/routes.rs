...
/// cannot present the `x-openhub-local-trust` header — the authenticated router
...
async fn weekly_digest(
    ...
    let since_ms = openhub_common::now_ms() - days * 86_400_000;
...
async fn list_day_digests(
    ...
    let digests = if let Some(mmdd) = q.on_day.filter(|s| s.len() == 4) {
        let today = crate::store::local_day(openhub_common::now_ms());
...
/// Binary serve of one companion's figure (the openhub-assets Response template,
