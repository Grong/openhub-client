...
/// One registered companion chat thread (a real `type='openhub'` conversation
...
impl CompanionStore {
    ...
    pub async fn insert_memory_scoped(
        ...
        let content = openhub_redact::redact_secrets(content);
        let now = now_ms();
    ...
    pub async fn update_memory(
        ...
        let redacted: Option<String> = match content {
            Some(c) => {
                ...
                Some(openhub_redact::redact_secrets(trimmed).into_owned())
            }
            None => None,
        };
...
// ---------------------------------------------------------------------------
// 自进化：技能注册表 / 挖矿统计 / 反馈回流
// 正文以磁盘 SKILL.md 为事实源（见 openhub-extension::skill_service）；这里只存
