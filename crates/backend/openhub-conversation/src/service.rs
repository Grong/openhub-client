...
use openhub_ai_agent::{AgentInstance, IWorkerTaskManager};
...
use openhub_db::models::{ConversationRow, MessageRow};
use openhub_db::{
...
use openhub_mcp::{AcpMcpCapabilities, parse_acp_mcp_capabilities};
...
pub(crate) fn parse_conv_id(id: &str) -> Result<i64, openhub_common::AppError> {
    id.parse::<i64>()
        .map_err(|_| openhub_common::AppError::NotFound(format!("conversation {id}")))
}
...
/// One-directional seam letting IDMM (the `openhub-idmm` crate) arm supervision
/// for a desktop conversation at turn start — WITHOUT this crate depending on
/// `openhub-idmm` (which sits above it). `openhub-idmm::IdmmManager` implements
/// it; `openhub-app` injects the implementation at assembly time via
...
pub struct ConversationService {
    ...
    knowledge_service: Arc<RwLock<Option<Arc<openhub_knowledge::KnowledgeService>>>>,
    ...
    /// as `cron_service`). Wired by `openhub-app` so a desktop turn arms 智能决策
    ...
    /// 签名:`openhub-app` 在装配处对 send-loop 实例调用
    /// [`Self::with_failover_deps`]。未注册(两槽为 `None`)即视为故障转移关闭
    /// —— fail-safe,所以不跑故障转移的上下文(测试、纯 webui)无需任何改动。
    failover_provider_repo: Arc<RwLock<Option<Arc<dyn openhub_db::IProviderRepository>>>>,
    failover_client_prefs: Arc<RwLock<Option<Arc<dyn openhub_db::IClientPreferenceRepository>>>>,
}
...
impl ConversationService {
    ...
    /// `with_cron_service`). Called by `openhub-app` so each desktop turn arms
    ...
    /// backs the global failover config. Wired by `openhub-app` on the
    ...
    pub fn with_failover_deps(
        &self,
        provider_repo: Arc<dyn openhub_db::IProviderRepository>,
        client_prefs: Arc<dyn openhub_db::IClientPreferenceRepository>,
    ) {
    ...
    /// `delete()`. Used by `openhub-app` to wire up `WorkerTaskManagerImpl`
    ...
    pub(crate) fn failover_deps(
        ...
        Arc<dyn openhub_db::IProviderRepository>,
        Arc<dyn openhub_db::IClientPreferenceRepository>,
    )> {
...
impl ConversationService {
    ...
    /// source to openhub, and broadcasts `conversation.listChanged(created)`.
    ...
    pub async fn create(
        ...
        let now = now_ms();
        let source = req.source.unwrap_or(ConversationSource::Openhub);

        // Type-aware rule: top-level `model` is openhub-only. Other agent types
        ...
        let response_row = openhub_db::models::ConversationRow {
            id: new_id,
            extra: serde_json::to_string(&extra)
                .map_err(|e| AppError::Internal(format!("Failed to serialize extra: {e}")))?,
            ..row
...
impl ConversationService {
    ...
    pub async fn send_message(
        ...
        let user_msg = openhub_db::models::MessageRow {
        ...
        tokio::spawn(async move {
            ...
            let mut failover_tried: Vec<openhub_common::ProviderWithModel> = Vec::new();
            let mut final_turn_writeback: Option<(
                Arc<openhub_knowledge::KnowledgeService>,
                openhub_knowledge::TurnWritebackRequest,
                String,
                String,
            )> = None;
    ...
    fn note_user_cancel(&self, conversation_id: &str) {
        if let Ok(mut stamps) = self.user_cancel_stamps.lock() {
            stamps.insert(conversation_id.to_string(), openhub_common::now_ms());
        }
    }
...
impl ConversationService {
    ...
    pub(crate) fn build_task_options(&self, row: &openhub_db::models::ConversationRow) -> Result<BuildTaskOptions, AppError> {
        ...
        // openhub_create_team without a separate lookup. Harmless for non-ACP types.
        if let Some(obj) = extra.as_object_mut() {
            obj.entry("user_id")
    ...
    /// stay confined to the hidden `.openhub/knowledge/` directory. Never
    ...
    async fn apply_knowledge_mounts(&self, row: &ConversationRow, build_opts: &mut BuildTaskOptions) {
        ...
        let outcome = if target_kind == "conversation" {
            let wp_key = openhub_knowledge::session_workpath_key(&workspace, &self.workspace_root);
            service
    ...
    fn build_turn_writeback_request(
        ...
        let surface = if companion {
            ...
            openhub_knowledge::WriteSurface::TerminalAcp
        } else {
        ...
        let request = openhub_knowledge::TurnWritebackRequest {
            mounts: mounts.clone(),
            binding: openhub_knowledge::KnowledgeBinding {
                enabled: true,
                writeback,
                writeback_mode,
                writeback_eagerness,
...
/// name (e.g. `"openhub"`). Falls back to the agent type's serde name
...
fn classify_repo_mcp_status(
    row: &openhub_db::models::McpServerRow,
...
/// [`openhub_knowledge::build_knowledge_context`] renders) plus the write-back
...
fn knowledge_mounts_signature(outcome: &openhub_knowledge::MountOutcome) -> String {
...
mod tests {
    ...
    fn enum_to_db_source() {
        assert_eq!(enum_to_db(&ConversationSource::Openhub).unwrap(), "openhub");
    ...
    fn response_with_type(agent_type: openhub_common::AgentType) -> ConversationResponse {
        ...
        let status = classify_session_mcp_status(
            &SessionMcpServer {
                ...
                transport: SessionMcpTransport::Stdio {
                    command: "__definitely_missing_openhub_mcp_command__".into(),
                    args: Vec::new(),
                    env: HashMap::new(),
                },
            },
