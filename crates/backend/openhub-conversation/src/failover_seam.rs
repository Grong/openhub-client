...
use openhub_api_types::{HealthStatus, ModelHealthStatus};
use openhub_common::{AgentKillReason, AgentType, ErrorChain, ProviderWithModel, now_ms};
use openhub_db::{ConversationRowUpdate, UpdateProviderParams};
...
impl ConversationService {
    ...
    pub(crate) async fn resolve_failover_config(
        &self,
        extra_json: &str,
    ) -> Option<openhub_api_types::ModelFailoverConfig> {
    ...
    pub async fn perform_model_failover(
        &self,
        conversation_id: &str,
        config: &openhub_api_types::ModelFailoverConfig,
    ...
    /// send-loop 的进入条件闸(pre-response / bounded / openhub-only 都已由 send-loop
    ...
    pub async fn idmm_failover_conversation(
        ...
        conversation_id: &str,
        task_manager: &Arc<dyn IWorkerTaskManager>,
    ) -> Result<bool, openhub_common::AppError> {
        ...
        let extra_json = match self.conversation_repo().get(conv_id).await {
            ...
            Err(e) => return Err(openhub_common::AppError::from(e)),
        };
        ...
        let req = openhub_api_types::SendMessageRequest {
