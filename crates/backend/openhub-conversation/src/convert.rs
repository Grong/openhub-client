...
use openhub_api_types::{ConversationArtifactResponse, ConversationResponse, MessageResponse, MessageSearchItem};
use openhub_common::{
...
use openhub_db::MessageSearchRow;
use openhub_db::models::{ConversationArtifactRow, ConversationRow, MessageRow};
...
///
/// OpenHub stores the full provider object (`TProviderWithModel`) which includes
...
mod tests {
    use super::*;
    use openhub_common::{AgentType, ConversationSource, ConversationStatus};
    use serde_json::json;
    ...
    #[test]
    fn parse_provider_with_model_openhub_format() {
    ...
    fn row_to_response_marks_workspace_inside_data_dir_as_temporary() {
        let row = make_row(
            ...
            Some("openhub"),
            None,
            r#"{"workspace":"/srv/openhub-data/conversations/claude-temp-abc"}"#,
        );
        let resp = row_to_response(row, Path::new("/srv/openhub-data")).unwrap();
    ...
    fn row_to_response_marks_missing_workspace_as_non_temporary() {
        let row = make_row("acp", "pending", Some("openhub"), None, r#"{}"#);
        let resp = row_to_response(row, Path::new("/srv/openhub-data")).unwrap();
        assert_eq!(resp.extra["is_temporary_workspace"], false);
    }
    ...
    fn row_to_response_marks_companion_workspace_as_non_temporary() {
        ...
        let row = make_row(
            ...
            Some("openhub"),
            None,
            r#"{"companionSession":true,"workspace":"/srv/openhub-data/companion/companions/companion_x/workspace"}"#,
        );
        let resp = row_to_response(row, Path::new("/srv/openhub-data")).unwrap();
        assert_eq!(resp.extra["is_temporary_workspace"], false);
    }
    ...
    fn test_search_row_to_item_builds_nested_conversation() {
        let row = MessageSearchRow {
            ...
            conversation_source: Some("openhub".into()),
        ...
        assert_eq!(item.conversation.source, Some(ConversationSource::Openhub));
