...
use openhub_ai_agent::{AgentSendError, IWorkerTaskManager};

use crate::response_middleware::{CronCommandResult, CronCreateParams, CronUpdateParams, ICronService};
use openhub_api_types::{AgentErrorCode, ConversationArtifactKind};
...
use openhub_knowledge::{KnowledgeBinding, KnowledgeCompleter, KnowledgeEventEmitter, KnowledgeService};
...
impl IConversationRepository for MockRepo {
    async fn get(&self, id: i64) -> Result<Option<ConversationRow>, openhub_db::DbError> {
        let rows = self.rows.lock().unwrap();
        Ok(rows.iter().find(|r| r.id == id).cloned())
    }

    async fn create(&self, row: &ConversationRow) -> Result<i64, openhub_db::DbError> {
    ...
    async fn update(&self, id: i64, updates: &ConversationRowUpdate) -> Result<(), openhub_db::DbError> {
        let mut rows = self.rows.lock().unwrap();
        let row = rows
            .iter_mut()
            .find(|r| r.id == id)
            .ok_or_else(|| openhub_db::DbError::NotFound(format!("Conversation {id}")))?;
    ...
    async fn delete(&self, id: i64) -> Result<(), openhub_db::DbError> {
        ...
        if rows.len() == len_before {
            return Err(openhub_db::DbError::NotFound(format!("Conversation {id}")));
        }
        Ok(())
    }
    ...
    ) -> Result<PaginatedResult<ConversationRow>, openhub_db::DbError> {
        let rows = self.rows.lock().unwrap();
    ...
    async fn find_by_source_and_chat(
        ...
        _chat_id: &str,
        _agent_type: &str,
    ) -> Result<Option<ConversationRow>, openhub_db::DbError> {
        Ok(None)
    }
    ...
    async fn list_by_cron_job(
        ...
        _user_id: &str,
        _cron_job_id: &str,
    ) -> Result<Vec<ConversationRow>, openhub_db::DbError> {
        Ok(vec![])
    }
    ...
    async fn get_messages(
        ...
        page_size: u32,
        order: SortOrder,
    ) -> Result<PaginatedResult<MessageRow>, openhub_db::DbError> {
    ...
    async fn get_message(&self, conv_id: i64, message_id: &str) -> Result<Option<MessageRow>, openhub_db::DbError> {
        let messages = self.messages.lock().unwrap();
        Ok(messages
            .iter()
    ...
    async fn insert_message(&self, message: &MessageRow) -> Result<(), openhub_db::DbError> {
        self.messages.lock().unwrap().push(message.clone());
        Ok(())
    }

    async fn update_message(&self, id: &str, updates: &MessageRowUpdate) -> Result<(), openhub_db::DbError> {
        let mut messages = self.messages.lock().unwrap();
        let message = messages
            .iter_mut()
            .find(|message| message.id == id)
            .ok_or_else(|| openhub_db::DbError::NotFound(format!("Message {id}")))?;
    ...
    async fn delete_messages_by_conversation(&self, conv_id: i64) -> Result<(), openhub_db::DbError> {
        self.messages
            .lock()
            .unwrap()
    ...
    async fn get_message_by_msg_id(
        ...
        msg_id: &str,
        msg_type: &str,
    ) -> Result<Option<MessageRow>, openhub_db::DbError> {
        let messages = self.messages.lock().unwrap();
        Ok(messages
            .iter()
            .find(|message| {
    ...
    async fn list_artifacts(&self, conversation_id: i64) -> Result<Vec<ConversationArtifactRow>, openhub_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn get_artifact(
        ...
        conversation_id: i64,
        artifact_id: i64,
    ) -> Result<Option<ConversationArtifactRow>, openhub_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn mark_skill_suggest_artifacts_saved(
        ...
        cron_job_id: &str,
        updated_at: TimestampMs,
    ) -> Result<Vec<ConversationArtifactRow>, openhub_db::DbError> {
        let mut artifacts = self.artifacts.lock().unwrap();
    ...
    async fn delete_artifacts_by_conversation(&self, conversation_id: i64) -> Result<(), openhub_db::DbError> {
        self.artifacts
            .lock()
            .unwrap()
    ...
    async fn list_legacy_cron_trigger_messages(
        &self,
        conversation_id: i64,
    ) -> Result<Vec<MessageRow>, openhub_db::DbError> {
        Ok(self
            .messages
            .lock()
            .unwrap()
...
async fn create_returns_conversation_with_defaults() {
    ...
    assert_eq!(resp.source, Some(ConversationSource::Openhub));
    assert!(!resp.pinned);
    ...
    assert_eq!(events[0].data["source"], "openhub");
}

#[tokio::test]
async fn create_rejects_workspace_with_trailing_whitespace_in_request() {
    let (svc, _broadcaster, _repo, _task_mgr) = make_service();
    let dir = std::env::temp_dir().join(format!("openhub-test-{}", openhub_common::generate_id()));
...
async fn get_reports_idle_runtime_when_only_persisted_status_is_running() {
    ...
    assert_eq!(runtime.state, openhub_api_types::ConversationRuntimeStateKind::Idle);
...
impl IAgentTask for MockAgent {
    ...
    async fn send_message(&self, _data: SendMessageData) -> Result<(), AgentSendError> {
        ...
        let _ = self.event_tx.send(AgentStreamEvent::Finish(
            openhub_ai_agent::protocol::events::FinishEventData::default(),
        ));
        Ok(())
    }
...
fn unique_test_dir(label: &str) -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    std::env::temp_dir().join(format!("openhub-{label}-{}-{nanos}", std::process::id()))
}
...
async fn send_message_stamps_channel_platform_for_channel_master_conversation() {
    ...
    // A channel master-agent conversation (see openhub-channel's
...
async fn send_message_turn_writeback_runs_after_system_continuation_final_answer() {
    ...
    let knowledge_db = openhub_db::init_database_memory().await.unwrap();
    let knowledge_repo: Arc<dyn openhub_db::IKnowledgeRepository> = Arc::new(
        openhub_db::SqliteKnowledgeRepository::new(knowledge_db.pool().clone()),
    );
...
async fn confirm_removes_confirmation_and_broadcasts() {
    ...
    let req = openhub_api_types::ConfirmRequest {
        msg_id: "msg-1".into(),
...
async fn warmup_restores_skill_links_for_recreated_auto_workspace() {
    ...
    assert!(workspace.join(".openhub/skills/cron").is_dir());
    ...
    assert_eq!(calls[0].rel_dirs, vec![".openhub/skills"]);
...
use openhub_common::ProviderWithModel;
use openhub_db::models::{ClientPreference, Provider};
use openhub_db::{
...
async fn failover_pre_response_fault_rebuilds_with_next_model_and_resends() {
    ...
    let conv_id = seed_openhub_failover_conversation(
        &repo,
        pwm("p1", "m1"),
...
async fn perform_model_failover_returns_none_for_acp_conversation() {
    ...
    let config = openhub_api_types::ModelFailoverConfig {
        enabled: true,
        queue: vec![pwm("p2", "m2")],
        ..Default::default()
    };
...
async fn edit_and_resubmit_rejects_when_no_editable_message() {
    let (svc, _broadcaster, _repo, _task_mgr) = make_service();
    let task_mgr: Arc<dyn IWorkerTaskManager> = Arc::new(MockTaskManager::new());
    let openhub_req: CreateConversationRequest =
        serde_json::from_value(json!({ "type": "openhub", "extra": { "workspace": "/project" } })).unwrap();
    let conv = svc.create("user_1", openhub_req).await.unwrap();
    let conv_id = conv.id.to_string();
