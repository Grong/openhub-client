...
//!    形状抄 `openhub-idmm/service.rs` 的多字段 pref 先例),会话级可在
...
use openhub_api_types::{AgentErrorCode, HealthStatus, ModelFailoverConfig};
use openhub_common::{AppError, ErrorChain, ProviderWithModel};
use openhub_db::IClientPreferenceRepository;
...
/// 单厂商失败(限流 / 5xx / 网络 / 配置)。
///
/// 这张 matches 表是 `openhub-idmm/config.rs::is_provider_fault` 的**就地副本**:
/// 故障转移 seam 在 `openhub-conversation`,而 `openhub-idmm` 在其之上,直接依赖
...
impl ProviderAvailability {
    ...
    fn from_provider(provider: &Provider) -> Self {
        ...
        let model_health = provider
            ...
            .as_deref()
            .and_then(|s| {
                serde_json::from_str::<std::collections::HashMap<String, openhub_api_types::ModelHealthStatus>>(s).ok()
            })
...
mod tests {
    ...
    fn provider(
        ...
        let health_map: HashMap<String, openhub_api_types::ModelHealthStatus> = model_health
            .iter()
            .map(|(m, s)| {
                (
                    m.to_string(),
                    openhub_api_types::ModelHealthStatus {
                        status: *s,
                        last_check: None,
                        latency: None,
                        error: None,
    ...
    fn is_provider_fault_matches_known_codes() {
        ...
        assert!(!is_provider_fault(AgentErrorCode::OpenhubConversationBusy));
    }
