...
use openhub_db::models::ConversationRow;
...
mod tests {
    ...
    /// label `"openhub"` when the conversation's model JSON was an older
    /// shape) and `send_message` parsed the row directly, so the cron
    /// path would emit `Provider 'openhub' not found` while the
    ...
    fn interactive_and_cron_paths_agree_on_provider_id() {
        ...
        assert_ne!(canonical_resolved.provider_id, "openhub");
        assert_ne!(legacy_resolved.provider_id, "openhub");
    }
}
