...
use openhub_api_types::{AgentErrorCode, ConversationRuntimeSummary, WebSocketMessage};
use openhub_common::{ErrorChain, normalize_keys_to_snake_case, now_ms};

use crate::service::ConversationService;
use openhub_db::{IConversationRepository, MessageRowUpdate};
...
fn turn_writeback_status_label(status: openhub_knowledge::TurnWritebackStatus) -> &'static str {
    match status {
        openhub_knowledge::TurnWritebackStatus::Disabled => "disabled",
        openhub_knowledge::TurnWritebackStatus::NoCompleter => "no_completer",
        openhub_knowledge::TurnWritebackStatus::NoCandidate => "no_candidate",
        openhub_knowledge::TurnWritebackStatus::Written => "written",
        openhub_knowledge::TurnWritebackStatus::Partial => "partial",
        openhub_knowledge::TurnWritebackStatus::Failed => "failed",
    }
}

fn turn_writeback_phase_label(phase: openhub_knowledge::TurnWritebackPhase) -> &'static str {
    match phase {
        openhub_knowledge::TurnWritebackPhase::Extracting => "extracting",
        openhub_knowledge::TurnWritebackPhase::Writing => "writing",
    }
}

fn turn_writeback_retryable(status: openhub_knowledge::TurnWritebackStatus) -> bool {
    matches!(
        status,
        openhub_knowledge::TurnWritebackStatus::NoCompleter
            | openhub_knowledge::TurnWritebackStatus::Partial
            | openhub_knowledge::TurnWritebackStatus::Failed
    )
}
...
fn turn_writeback_final_state(
    report: &openhub_knowledge::TurnWritebackReport,
    status: &str,
    attempt_id: &str,
    started_at: i64,
    finished_at: i64,
...
pub(crate) async fn run_turn_writeback_report(
    service: Arc<openhub_knowledge::KnowledgeService>,
    mut request: openhub_knowledge::TurnWritebackRequest,
    repo: Arc<dyn IConversationRepository>,
    broadcaster: Arc<dyn EventBroadcaster>,
    ...
    match report.status {
        openhub_knowledge::TurnWritebackStatus::Written
        | openhub_knowledge::TurnWritebackStatus::Partial => {
            info!(
...
impl StreamRelay {
                ...
                Ok(event) => {
                    ...
                    match &event {
                        ...
                        AgentStreamEvent::AgentStatus(data) => {
                            self.forward_to_websocket(&event);
                            if data.backend == "openhub" && (data.status == "preparing" || data.status == "prepared") {
                ...
                Err(broadcast::error::RecvError::Closed) => {
                    ...
                    let outcome = self
                        .finalize(
                            &full_text_buffer,
                            &text_segments,
                            &AgentStreamEvent::Finish(openhub_ai_agent::protocol::events::FinishEventData::default()),
    ...
    async fn flush_text_segment(&self, segment: &mut TextSegmentState) {
        ...
        if segment.record_created {
            let update = openhub_db::MessageRowUpdate {
            ...
            if let Some(primary_segment) = text_segments.first() {
                if processed.message != text || hidden {
                    ...
                    for segment in text_segments.iter().skip(1) {
                        let hide_update = openhub_db::MessageRowUpdate {
                            content: None,
                            status: Some(Some(status.to_owned())),
                            hidden: Some(true),
                        };
                    ...
                    for segment in text_segments {
                        let status_update = openhub_db::MessageRowUpdate {
                            content: None,
                            status: Some(Some(status.to_owned())),
                            hidden: Some(false),
                        };
    ...
    async fn persist_error_tips(&self, data: &openhub_ai_agent::protocol::events::ErrorEventData) {
    ...
    #[tracing::instrument(skip_all)]
    async fn persist_agent_status(&self, data: &openhub_ai_agent::protocol::events::AgentStatusEventData) {
    ...
    #[tracing::instrument(skip_all)]
    async fn persist_tool_call(&self, data: &openhub_ai_agent::protocol::events::tool_call::ToolCallEventData) {
    ...
    fn incomplete_tool_reason(event: &AgentStreamEvent) -> Option<&'static str> {
        match event {
            ...
            AgentStreamEvent::Finish(data) => match data.stop_reason {
                Some(openhub_ai_agent::protocol::events::TurnStopReason::MaxTokens) => Some("max_tokens"),
                Some(openhub_ai_agent::protocol::events::TurnStopReason::MaxTurnRequests) => {
                    Some("max_turn_requests")
                }
                Some(openhub_ai_agent::protocol::events::TurnStopReason::Refusal) => Some("refusal"),
                Some(openhub_ai_agent::protocol::events::TurnStopReason::Cancelled) => Some("cancelled"),
                Some(openhub_ai_agent::protocol::events::TurnStopReason::EndTurn) => Some("end_turn"),
                None => Some("finish"),
            },
            _ => None,
        }
    ...
    #[tracing::instrument(skip_all)]
    async fn persist_acp_tool_call(&self, data: &openhub_ai_agent::protocol::events::tool_call::AcpToolCallEventData) {
    ...
    #[tracing::instrument(skip_all)]
    async fn persist_tool_group(&self, entries: &[openhub_ai_agent::protocol::events::tool_call::ToolGroupEntry]) {
    ...
    pub async fn complete_conversation_with_context(
        ...
        let update = openhub_db::ConversationRowUpdate {
...
mod tests {
    ...
    async fn run_text_then_finish_persists_message() {
        ...
        let bus = Arc::new(openhub_realtime::BroadcastEventBus::new(64));
    ...
    async fn turn_completed_accumulates_tokens_into_wired_runtime_state() {
        use openhub_ai_agent::protocol::events::TurnCompletedEventData;
    ...
    async fn run_text_tool_text_splits_text_segments() {
        use openhub_ai_agent::protocol::events::tool_call::{ToolCallEventData, ToolCallStatus};
    ...
    async fn run_marks_active_tool_call_error_when_turn_hits_max_tokens() {
        use openhub_ai_agent::protocol::events::tool_call::{ToolCallEventData, ToolCallStatus};
        use openhub_ai_agent::protocol::events::TurnStopReason;
    ...
    async fn run_suppresses_pre_response_error_when_failover_will_fire() {
        ...
        tx.send(AgentStreamEvent::Error(ErrorEventData::legacy(
            ...
            Some(openhub_api_types::AgentErrorCode::UserLlmProviderGatewayError),
        )))
        .unwrap();
    ...
    async fn run_send_error_injects_error_and_completes_turn() {
        ...
        send_error_tx
            .send(AgentSendError::from_app_error(openhub_common::AppError::BadGateway(
        ...
        assert_eq!(
            outcome.terminal,
            RelayTerminal::Error {
                code: Some(openhub_api_types::AgentErrorCode::UserLlmProviderAuthFailed),
    ...
    async fn run_send_error_keeps_existing_stream_error_when_it_arrives_first() {
        ...
        let rx = tx.subscribe();
        let send_error = AgentSendError::from_app_error(openhub_common::AppError::BadGateway(
            "provider returned 401 invalid api key".into(),
        ));
        tx.send(AgentStreamEvent::Error(ErrorEventData::legacy(
    ...
    async fn run_acp_tool_call_inserts_then_updates() {
        use openhub_ai_agent::protocol::events::tool_call::{
    ...
    async fn run_tool_group_persists_message() {
        use openhub_ai_agent::protocol::events::tool_call::{ToolCallStatus, ToolGroupEntry};
    ...
    struct RecordingRepo {
        ...
        updates: Mutex<Vec<(String, openhub_db::MessageRowUpdate)>>,
    }
    ...
    impl RecordingRepo {
        ...
        #[allow(dead_code)]
        fn take_updates(&self) -> Vec<(String, openhub_db::MessageRowUpdate)> {
    ...
    impl IConversationRepository for RecordingRepo {
        async fn get(&self, _id: i64) -> Result<Option<openhub_db::models::ConversationRow>, DbError> {
            Ok(None)
        }
        async fn create(&self, _row: &openhub_db::models::ConversationRow) -> Result<i64, DbError> {
            Ok(1)
        }
        async fn update(&self, _id: i64, _updates: &openhub_db::ConversationRowUpdate) -> Result<(), DbError> {
            Ok(())
        }
        ...
        async fn list_paginated(
            &self,
            _user_id: &str,
            _filters: &openhub_db::ConversationFilters,
        ) -> Result<openhub_common::PaginatedResult<openhub_db::models::ConversationRow>, DbError> {
            Ok(openhub_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
            })
        ...
        async fn find_by_source_and_chat(
            ...
            _chat_id: &str,
            _agent_type: &str,
        ) -> Result<Option<openhub_db::models::ConversationRow>, DbError> {
            Ok(None)
        }
        async fn list_by_cron_job(
            ...
            _user_id: &str,
            _cron_job_id: &str,
        ) -> Result<Vec<openhub_db::models::ConversationRow>, DbError> {
            Ok(vec![])
        }
        ...
        async fn get_messages(
            ...
            _page: u32,
            _page_size: u32,
            _order: openhub_db::SortOrder,
        ) -> Result<openhub_common::PaginatedResult<MessageRow>, DbError> {
            Ok(openhub_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
        ...
        async fn update_message(&self, id: &str, updates: &openhub_db::MessageRowUpdate) -> Result<(), DbError> {
        ...
        async fn search_messages(
            ...
            _page: u32,
            _page_size: u32,
        ) -> Result<openhub_common::PaginatedResult<openhub_db::MessageSearchRow>, DbError> {
            Ok(openhub_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
