...
use openhub_common::{AgentKillReason, AppError, ConversationStatus, TimestampMs, generate_prefixed_id, now_ms};
...
use openhub_db::{IConversationRepository, SqliteConversationRepository, init_database_memory};
...
async fn t9_4_empty_keyword() {
    ...
    assert!(matches!(err, openhub_common::AppError::BadRequest(_)));
}
...
async fn t9_6_search_result_includes_conversation_model() {
    ...
    .unwrap();
    let conv = svc.create(USER_ID, openhub_req).await.unwrap();
