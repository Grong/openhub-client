...
use openhub_common::{AgentKillReason, AgentType, AppError, ConversationSource, ConversationStatus, TimestampMs};
use openhub_conversation::ConversationService;
use openhub_conversation::skill_resolver::SkillResolver;
use openhub_db::{SqliteConversationRepository, init_database_memory};
...
impl SkillResolver for EmptySkillResolver {
    ...
    async fn resolve_skills(&self, _names: &[String]) -> Vec<openhub_extension::ResolvedAgentSkill> {
        Vec::new()
    }
    ...
    async fn link_workspace_skills(
        ...
        _rel_dirs: &[&str],
        _skills: &[openhub_extension::ResolvedAgentSkill],
    ) -> usize {
        0
    }
}
...
async fn setup_with_workspace_root(
    ...
    let agent_metadata_repo: Arc<dyn openhub_db::IAgentMetadataRepository> =
        Arc::new(openhub_db::SqliteAgentMetadataRepository::new(db.pool().clone()));
    let acp_session_repo: Arc<dyn openhub_db::IAcpSessionRepository> =
        Arc::new(openhub_db::SqliteAcpSessionRepository::new(db.pool().clone()));
...
async fn auto_workspace_does_not_reuse_path_when_integer_id_restarts() {
    let root = std::env::temp_dir().join(format!(
        "openhub-conv-reset-{}",
        openhub_common::generate_prefixed_id("test")
    ));
...
async fn t3_2_get_not_found() {
    ...
    assert!(matches!(err, openhub_common::AppError::NotFound(_)));
}
...
#[tokio::test]
async fn create_openhub_strips_extra_model_field() {
    let (svc, _, _task_mgr) = setup().await;
...
#[tokio::test]
async fn update_openhub_strips_extra_model_from_patch() {
    let (svc, _, task_mgr) = setup().await;
...
async fn create_acp_seeds_acp_session_runtime_from_extra() {
    use openhub_db::{SqliteAcpSessionRepository, init_database_memory};

    let db = init_database_memory().await.unwrap();
    let repo = Arc::new(openhub_db::SqliteConversationRepository::new(db.pool().clone()));
    let broadcaster = Arc::new(TestBroadcaster::new());
    let agent_metadata_repo: Arc<dyn openhub_db::IAgentMetadataRepository> =
        Arc::new(openhub_db::SqliteAgentMetadataRepository::new(db.pool().clone()));
    let acp_session_repo: Arc<dyn openhub_db::IAcpSessionRepository> =
    ...
    let task_mgr: Arc<dyn IWorkerTaskManager> = Arc::new(NoopTaskManager);
    let svc = openhub_conversation::ConversationService::new(
        std::env::temp_dir(),
        broadcaster.clone(),
        Arc::new(EmptySkillResolver),
        task_mgr,
