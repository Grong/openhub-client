...
use openhub_auth::SYSTEM_USER_ID;
use openhub_gateway::{CallerCtx, GatewayDeps, Registry, Surface};
...
fn remote_specs_for_scope(scope: Option<&[String]>) -> Vec<openhub_gateway::ToolSpec> {
    match scope {
        Some(domains) => {
...
impl ServerHandler for RemoteMcpHandler {
    fn get_info(&self) -> ServerInfo {
        ...
        info.instructions = Some(
            "OpenHub external companion. These tools drive the OpenHub platform \
