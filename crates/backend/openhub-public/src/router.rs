...
use openhub_auth::CompanionTokenValidator;
use openhub_gateway::GatewayDeps;
...
mod tests {
    ...
    use openhub_auth::token_sha256_hex;
    ...
    async fn valid_token_inserts_remote_companion_extension() {
        ...
        let validator = std::sync::Arc::new(
            openhub_auth::CompanionTokenValidator::new(vec![("comp-x".into(), token_sha256_hex("secret-tok"))]),
        );
