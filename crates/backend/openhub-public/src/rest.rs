...
use openhub_auth::{CompanionTokenValidator, SYSTEM_USER_ID};
use openhub_gateway::{CallerCtx, GatewayDeps, Registry, Surface, ToolSpec};
...
/// `openhub_agent_run`) emit incremental `{"type": ..}` deltas as they happen, and
...
async fn openapi(Query(q): Query<ProfileQuery>) -> Json<Value> {
    ...
    Json(json!({
        "openapi": "3.1.0",
        "info": {
            "title": "OpenHub Remote Capability API",
            "version": "v1",
            "description": "External-companion access to OpenHub platform capabilities. All operations require Authorization: Bearer <companion access token>."
        },
        "paths": paths,
        "components": {
