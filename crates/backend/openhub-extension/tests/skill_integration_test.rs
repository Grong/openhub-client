...
//!
//! These tests exercise the public API surface of `openhub_extension::skill_service`
//! and `openhub_extension::external_paths` against the functional requirements in
//! `08-file-workspace.md` §B.

use std::path::Path;

use openhub_extension::external_paths::ExternalPathsManager;
use openhub_extension::skill_service::{
...
///
/// Production mode: no `OPENHUB_BUILTIN_SKILLS_PATH` set — the built-in
...
async fn cp4_enable_skills_market() {
    ...
    let paths = mgr.get_custom_external_paths().await;
    assert_eq!(paths.len(), 1);
    assert_eq!(paths[0].name, "openhub-skills");
}
