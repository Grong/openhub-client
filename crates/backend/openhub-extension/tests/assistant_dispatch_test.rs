...
use openhub_api_types::{ApiResponse, AssistantSource};
use openhub_common::AppError;
use openhub_extension::classifier::{AssistantClassifier, AssistantRuleDispatcher};
use openhub_extension::external_paths::ExternalPathsManager;
use openhub_extension::skill_routes::{SkillRouterState, skill_routes};
use openhub_extension::skill_service::SkillPaths;
...
async fn router_with_dispatcher(dispatcher: Arc<FakeDispatcher>) -> axum::Router {
    ...
    let state = SkillRouterState {
        ...
        skill_tag_repo: {
            ...
            Arc::new(openhub_db::SqliteSkillTagRepository::new(db.pool().clone()))
        },
