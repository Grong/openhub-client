...
pub enum ExtensionError {
    ...
    #[error("Engine incompatible: extension '{name}' requiresopenhub{required}, got {actual}")]
    EngineIncompatible {
        name: String,
        required: String,
        actual: String,
...
mod tests {
    ...
    fn test_reserved_name_prefix_error_display() {
        let err = ExtensionError::ReservedNamePrefix {
            name: "openhub-test".into(),
            prefix: "openhub-".into(),
        };
        assert_eq!(
            err.to_string(),
            "Extension name 'openhub-test' uses reserved prefix 'openhub-'"
        );
    }
