...
//!
//! These traits live in `openhub-extension` (not `openhub-assistant`) so
//! `skill_routes` can depend on them without pulling `openhub-assistant` into
//! the dependency graph; the concrete implementation ships from
//! `openhub-assistant::AssistantService`.

use openhub_api_types::AssistantSource;
...
///
/// Implemented by `openhub_assistant::AssistantService`; depended on by
