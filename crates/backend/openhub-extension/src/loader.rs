...
/// 1. `$OPENHUB_EXTENSIONS_PATH`
/// 2. `~/.openhub/extensions/` — legacy user data directory
/// 3. Platform AppData directory
///
/// In E2E test mode (`OPENHUB_E2E_TEST=1`), only the environment variable
/// paths are returned to ensure test isolation.
pub fn resolve_scan_paths() -> Vec<ScanPath> {
    let env_path = std::env::var("OPENHUB_EXTENSIONS_PATH").ok();
...
fn resolve_scan_paths_inner(
    ...
    // 2. User data directory (desktop data dir or historical ~/openhubfallback).
    if let Some(data_dir) = explicit_data_dir {
        ...
        if let Some(data_dir) = dirs::data_dir() {
            push(
                data_dir.join("openhub").join(EXTENSIONS_DIR_NAME),
...
fn is_e2e_test_mode() -> bool {
    std::env::var("OPENHUB_E2E_TEST").map(|v| v == "1").unwrap_or(false)
}
...
mod tests {
    ...
    #[test]
    fn scan_loads_openhub_main_contract_extension() {
        let tmp = TempDir::new().unwrap();
    ...
    fn make_loaded_ext(
        ...
        LoadedExtension {
            manifest: ExtensionManifest {
                ...
                homepage: None,
                icon: None,
                engine: engine_openhub.map(|v| EngineConfig {
                    openhub: Some(v.to_string()),
                }),
