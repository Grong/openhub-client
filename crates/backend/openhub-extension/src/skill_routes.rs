...
use openhub_db::ISkillTagRepository;
...
/// (intentionally unlike `openhub-assistant`'s `decode_str_list`, which 500s on
...
async fn set_skill_tags(
    ...
    state
        .skill_tag_repo
        .upsert(&openhub_db::UpsertSkillTagParams {
            skill_name: &name,
...
/// `POST /api/skills/market/enable` — enable theopenhubskills market.
...
/// `POST /api/skills/market/disable` — disable theopenhubskills market.
...
mod tests {
    ...
    struct InMemorySkillTagRepo {
        rows: std::sync::Mutex<Vec<openhub_db::SkillTagRow>>,
    }
    #[async_trait::async_trait]
    impl openhub_db::ISkillTagRepository for InMemorySkillTagRepo {
        async fn get_all(&self) -> Result<Vec<openhub_db::SkillTagRow>, openhub_db::DbError> {
            Ok(self.rows.lock().unwrap().clone())
        }
        async fn upsert(
            &self,
            p: &openhub_db::UpsertSkillTagParams<'_>,
        ) -> Result<openhub_db::SkillTagRow, openhub_db::DbError> {
            let row = openhub_db::SkillTagRow {
        ...
        async fn delete(&self, name: &str) -> Result<bool, openhub_db::DbError> {
            let mut g = self.rows.lock().unwrap();
            let before = g.len();
