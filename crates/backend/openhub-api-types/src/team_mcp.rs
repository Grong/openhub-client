...
//! (`openhub-ai-agent` deserializing `AcpBuildExtra`, etc.) can reference the
...
impl RequirementMcpConfig {
    ...
    /// name `mcp__openhub-requirement__requirement_update_status` (51 chars)
...
/// server; that bridge forwards every `openhub_*` desktop tool call back to the
...
impl GatewayMcpConfig {
    /// Wire-level MCP server name. Kept short so the longest wire-level tool
    /// name `mcp__openhub-desktop__openhub_send_to_conversation` (47 chars) stays
    ...
    pub const WORK_DOMAINS: &'static [&'static str] = &[
        ...
        // up & track multi-agent orchestration runs (openhub_run_create/status/result).
        ...
        // generation (openhub_workshop_*). Desktop-side domain.
        "workshop",
    ];
...
impl OpenMcpConfig {
    ...
    /// `mcp__openhub-open__open` (23 chars) stays well within Anthropic's
    /// 64-char tool-name limit.
    pub const SERVER_NAME: &'static str = "openhub-open";
}
...
impl ComputerMcpConfig {
    ...
    /// `mcp__openhub-computer__cursor_position` (39 chars) stays within
    /// Anthropic's 64-char tool-name limit.
    pub const SERVER_NAME: &'static str = "openhub-computer";
}
...
impl BrowserMcpConfig {
    /// Wire-level MCP server name. Kept short so the longest wire-level tool name
    /// `mcp__openhub-browser__get_dropdown_options` (42 chars) stays within
    /// Anthropic's 64-char tool-name limit.
    pub const SERVER_NAME: &'static str = "openhub-browser";
}
...
mod tests {
    ...
    /// name against the real limit — lives in `openhub-gateway`'s registry
