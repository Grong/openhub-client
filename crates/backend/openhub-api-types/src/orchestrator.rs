...
/// DAG. Mirrors [`OrchRunRow`](../../openhub_db) minus `user_id`/`fleet_snapshot`/
...
pub struct Run {
    ...
    /// 或 `"manual"`（审批模式——worker 可经 `openhub_task_question` 挂起提问，由用户
...
pub struct RunTask {
    ...
    /// 节点挂起的决策问题原文（迁移 030）：审批模式下 worker 经 `openhub_task_question`
...
/// and accepted as the `openhub_run_plan` gateway input. Dependencies and member
...
/// The planned task DAG: the PlanProducer output / `openhub_run_plan` input.
#[derive(Debug, Clone, Serialize, Deserialize)]
