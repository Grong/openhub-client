...
pub struct AcpBuildExtra {
    ...
    /// Marks a session entitled to the Desktop Gateway MCP (`openhub_*` tools —
...
pub struct NomiBuildExtra {
    ...
    #[serde(default = "default_openhub_max_tokens")]
    pub max_tokens: u32,
    #[serde(default)]
    ...
    /// session. Falls back to host config / OPENHUB_COMPUTER_USE when None.
    #[serde(default, alias = "computerUse")]
    pub computer_use: Option<bool>,
    /// Opt-in to the Browser tool (CDP automation) for this session.
    /// Falls back to host config / OPENHUB_BROWSER_USE when None.
...
fn default_openhub_max_tokens() -> u32 {
    8192
}
