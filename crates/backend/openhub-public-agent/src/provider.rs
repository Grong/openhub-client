...
//! factory's runtime. The factory (openhub-ai-agent) owns the trait + the small
//! `PublicAgentRuntime` DTO; this crate depends on the factory (never the
//! reverse — no cycle), exactly like `openhub-companion` does for its persona
//! provider.
//!
...
use openhub_ai_agent::{PublicAgentProvider, PublicAgentRuntime};
