...
use openhub_ai_agent::task_manager::IWorkerTaskManager;
use openhub_ai_agent::types::{BuildTaskOptions, SendMessageData};
use openhub_ai_agent::{AgentRegistry, AgentStreamEvent};
use openhub_api_types::{CreateConversationRequest, SendMessageRequest};
use openhub_common::{
...
use openhub_db::models::MessageRow;
use openhub_db::{ConversationRowUpdate, IConversationRepository};
...
impl JobExecutor {
    ...
    pub async fn get_conversation_row(
        &self,
        conversation_id: &str,
    ) -> Result<Option<openhub_db::models::ConversationRow>, CronError> {
    ...
    pub async fn insert_tips_message(
        ...
        let row = MessageRow {
            ...
            hidden: false,
            created_at: openhub_common::now_ms(),
        };
...
impl JobExecutor {
    ...
    async fn execute_inner(
        ...
        let model = match self.get_conversation_row(conversation_id).await {
            Ok(Some(row)) => {
                openhub_conversation::task_options::provider_model_from_conversation_row(&row)
            }
            Ok(None) => openhub_conversation::task_options::empty_provider_model(),
            Err(e) => {
                error!(
                    job_id = %job.id,
                    conversation_id,
    ...
    fn spawn_skill_suggest_flow(
        &self,
        agent: openhub_ai_agent::AgentInstance,
...
mod tests {
    ...
    use openhub_ai_agent::agent_task::{AgentInstance, IAgentTask, IMockAgent};
    use openhub_ai_agent::protocol::events::FinishEventData;
    use openhub_api_types::{AgentModeResponse, WebSocketMessage};
    use openhub_common::{AgentKillReason, ConversationStatus, PaginatedResult, TimestampMs};
    use openhub_db::{
    ...
    async fn hydrated_registry() -> Arc<AgentRegistry> {
        ...
        let repo = Arc::new(openhub_db::SqliteAgentMetadataRepository::new(
            db.pool().clone(),
        ));
    ...
    #[test]
    fn resolve_model_returns_none_for_non_openhub_type() {
        let job = CronJob {
    ...
    #[test]
    fn resolve_model_openhub_with_full_config() {
        let job = CronJob {
    ...
    #[test]
    fn resolve_model_openhub_without_model_id_defaults_to_default() {
        let job = CronJob {
            agent_type: "openhub".into(),
            agent_config: Some(CronAgentConfig {
                backend: "4056cdea".into(),
    ...
    #[test]
    fn resolve_model_openhub_without_config_returns_none() {
    ...
    #[test]
    fn resolve_model_openhub_with_empty_backend_returns_none() {
        let job = CronJob {
            agent_type: "openhub".into(),
            agent_config: Some(CronAgentConfig {
                backend: "   ".into(),
    ...
    fn make_executor_for_busy_tests(guard: Arc<CronBusyGuard>) -> JobExecutor {
        ...
        impl IWorkerTaskManager for StubTaskManager {
            ...
            async fn get_or_build_task(
                ...
                _: &str,
                _: BuildTaskOptions,
            ) -> Result<AgentInstance, openhub_common::AppError> {
                Err(openhub_common::AppError::Internal("stub".into()))
            }
            fn kill(
                &self,
                _: &str,
                _: Option<openhub_common::AgentKillReason>,
            ...
            fn collect_idle(&self, _: openhub_common::TimestampMs) -> Vec<String> {
                vec![]
            }
        }
        ...
        impl IConversationRepository for StubConvRepo {
            async fn get(
                &self,
                _id: i64,
            ) -> Result<Option<openhub_db::models::ConversationRow>, openhub_db::DbError>
            {
                Ok(None)
            }
            async fn create(
                &self,
                _row: &openhub_db::models::ConversationRow,
            ...
            async fn delete(&self, _id: i64) -> Result<(), openhub_db::DbError> {
                Ok(())
            }
            ...
            ) -> Result<PaginatedResult<openhub_db::models::ConversationRow>, openhub_db::DbError>
            {
                Ok(PaginatedResult {
                    items: vec![],
                    total: 0,
            ...
            async fn list_by_cron_job(
                ...
                _user_id: &str,
                _cron_job_id: &str,
            ) -> Result<Vec<openhub_db::models::ConversationRow>, openhub_db::DbError> {
                Ok(vec![])
            }
            ...
            async fn get_messages(
                ...
                _page_size: u32,
                _order: SortOrder,
            ) -> Result<PaginatedResult<openhub_db::models::MessageRow>, openhub_db::DbError>
            {
                Ok(PaginatedResult {
                    items: vec![],
                    total: 0,
            ...
            async fn insert_message(
                &self,
                _message: &openhub_db::models::MessageRow,
            ) -> Result<(), openhub_db::DbError> {
                Ok(())
            }
            ...
            async fn get_message_by_msg_id(
                ...
                _msg_id: &str,
                _msg_type: &str,
            ) -> Result<Option<openhub_db::models::MessageRow>, openhub_db::DbError> {
                Ok(None)
            }
            async fn search_messages(
                ...
                _page: u32,
                _page_size: u32,
            ) -> Result<PaginatedResult<MessageSearchRow>, openhub_db::DbError> {
                Ok(PaginatedResult {
                    items: vec![],
                    total: 0,
                    has_more: false,
        ...
        impl openhub_realtime::EventBroadcaster for StubBroadcaster {
            fn broadcast(&self, _: WebSocketMessage<serde_json::Value>) {}
        }

        struct StubSkillResolver;
        #[async_trait::async_trait]
        impl openhub_conversation::skill_resolver::SkillResolver for StubSkillResolver {
            ...
            async fn resolve_skills(
                &self,
                _names: &[String],
            ) -> Vec<openhub_conversation::skill_resolver::ResolvedAgentSkill> {
                Vec::new()
            }
            ...
            async fn link_workspace_skills(
                ...
                _rel_dirs: &[&str],
                _skills: &[openhub_conversation::skill_resolver::ResolvedAgentSkill],
            ) -> usize {
                0
            }
        }

        let stub_broadcaster: Arc<dyn openhub_realtime::EventBroadcaster> =
            Arc::new(StubBroadcaster);
        let stub_repo: Arc<dyn IConversationRepository> = Arc::new(StubConvRepo);
        let agent_metadata_repo: Arc<dyn openhub_db::IAgentMetadataRepository> =
            Arc::new(StubAgentMetadataRepo);
        let acp_session_repo: Arc<dyn openhub_db::IAcpSessionRepository> =
    ...
    impl IAgentTask for RecordingAgent {
        ...
        ) -> Result<(), openhub_ai_agent::AgentSendError> {
            self.send_calls.fetch_add(1, Ordering::Relaxed);
            self.sent_messages.write().await.push(data);
            Ok(())
        }

        async fn cancel(&self) -> Result<(), openhub_common::AppError> {
            Ok(())
        }

        fn kill(&self, _reason: Option<AgentKillReason>) -> Result<(), openhub_common::AppError> {
            Ok(())
        }
    }
    ...
    impl IMockAgent for RecordingAgent {
        async fn mode(&self) -> Result<AgentModeResponse, openhub_common::AppError> {
        ...
        async fn set_mode(&self, mode: &str) -> Result<(), openhub_common::AppError> {
    ...
    impl IConversationRepository for ExistingConversationRepo {
        async fn get(
            ...
            Ok(Some(openhub_db::models::ConversationRow {
                id,
                user_id: "cron".into(),
    ...
    struct MissingWorkspaceConversationRepo {
        row: openhub_db::models::ConversationRow,
        updates: Mutex<Vec<ConversationRowUpdate>>,
        inserted_messages: Mutex<Vec<openhub_db::models::MessageRow>>,
    ...
    impl MissingWorkspaceConversationRepo {
        ...
        fn inserted_messages(&self) -> Vec<openhub_db::models::MessageRow> {
    ...
    impl openhub_realtime::EventBroadcaster for RecordingBroadcaster {
    ...
    impl IConversationRepository for MissingWorkspaceConversationRepo {
        ...
        async fn insert_message(
            &self,
            message: &openhub_db::models::MessageRow,
        ) -> Result<(), openhub_db::DbError> {
        ...
        ) -> Result<ConversationArtifactRow, openhub_db::DbError> {
    ...
    fn make_executor_with_task_manager_and_repo(
        ...
        let broadcaster: Arc<dyn openhub_realtime::EventBroadcaster> = Arc::new(StubBroadcaster);
    ...
    fn make_executor_with_task_manager_repo_and_broadcaster(
        task_manager: Arc<dyn IWorkerTaskManager>,
        repo: Arc<dyn IConversationRepository>,
        broadcaster: Arc<dyn openhub_realtime::EventBroadcaster>,
    ) -> JobExecutor {
        struct StubSkillResolver;
    ...
    #[async_trait::async_trait]
    impl openhub_db::IAcpSessionRepository for StubAcpSessionRepo {
        async fn get(
            &self,
            _conversation_id: i64,
        ) -> Result<Option<openhub_db::models::AcpSessionRow>, openhub_db::DbError> {
            Ok(None)
        }
        async fn create(
            &self,
            _params: &openhub_db::CreateAcpSessionParams<'_>,
        ) -> Result<openhub_db::models::AcpSessionRow, openhub_db::DbError> {
            Err(openhub_db::DbError::Init("stub".into()))
        }
        async fn update_session_id(
            &self,
            _conversation_id: i64,
            _session_id: &str,
        ) -> Result<bool, openhub_db::DbError> {
            Ok(false)
        }
        ...
        async fn delete(&self, _conversation_id: i64) -> Result<bool, openhub_db::DbError> {
            Ok(false)
        }
        async fn load_runtime_state(
            &self,
            _conversation_id: i64,
        ) -> Result<Option<openhub_db::PersistedSessionState>, openhub_db::DbError> {
            Ok(None)
        }
        async fn save_runtime_state(
            &self,
            _conversation_id: i64,
            _params: &openhub_db::SaveRuntimeStateParams<'_>,
        ) -> Result<bool, openhub_db::DbError> {
            Ok(false)
        }
    }
    ...
    #[async_trait::async_trait]
    impl openhub_db::IAgentMetadataRepository for StubAgentMetadataRepo {
        async fn list_all(
            &self,
        ) -> Result<Vec<openhub_db::models::AgentMetadataRow>, openhub_db::DbError> {
            Ok(Vec::new())
        }
        async fn get(
            &self,
            _id: &str,
        ) -> Result<Option<openhub_db::models::AgentMetadataRow>, openhub_db::DbError> {
            Ok(None)
        }
        ...
        async fn upsert(
            &self,
            _params: &openhub_db::models::UpsertAgentMetadataParams<'_>,
        ) -> Result<openhub_db::models::AgentMetadataRow, openhub_db::DbError> {
            Err(openhub_db::DbError::Init("stub".into()))
        }
        async fn apply_handshake(
            &self,
            _id: &str,
            _params: &openhub_db::models::UpdateAgentHandshakeParams<'_>,
        ) -> Result<Option<openhub_db::models::AgentMetadataRow>, openhub_db::DbError> {
            Ok(None)
        }
        ...
        async fn delete(&self, _id: &str) -> Result<bool, openhub_db::DbError> {
            Ok(false)
        }
    }
}
