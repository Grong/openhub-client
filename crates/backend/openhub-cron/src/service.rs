...
use openhub_db::{CRON_RUN_HISTORY_LIMIT, CronJobRunRow, ICronRepository, UpdateCronJobParams};
...
impl CronService {
    ...
    pub async fn add_job(&self, req: CreateCronJobRequest) -> Result<CronJob, CronError> {
        ...
        self.validate_openhub_job_model(
            &req.agent_type,
            execution_mode,
            &conversation_id,
    ...
    async fn validate_openhub_job_model(
        &self,
        agent_type: &str,
...
impl openhub_common::OnConversationDelete for CronService {
...
#[async_trait::async_trait]
impl openhub_conversation::response_middleware::ICronService for CronService {
    async fn create_job(
        &self,
        _user_id: &str,
        conversation_id: &str,
        params: &openhub_conversation::response_middleware::CronCreateParams,
    ) -> openhub_conversation::response_middleware::CronCommandResult {
        ...
        let Ok(conversation_key) = conversation_id.parse::<i64>() else {
            return openhub_conversation::response_middleware::CronCommandResult {
                success: false,
        ...
        match self.add_job(req).await {
            ...
            Err(e) => openhub_conversation::response_middleware::CronCommandResult {
                success: false,
                message: e.to_string(),
            },
        }
    ...
    async fn update_job(
        ...
        _user_id: &str,
        conversation_id: &str,
        params: &openhub_conversation::response_middleware::CronUpdateParams,
    ) -> openhub_conversation::response_middleware::CronCommandResult {
    ...
    async fn delete_job(
        ...
        match self.remove_job(job_id).await {
            Ok(()) => openhub_conversation::response_middleware::CronCommandResult {
                success: true,
...
fn build_agent_config_from_conversation(
...
) -> (String, Option<openhub_api_types::CronAgentConfigDto>) {
    ...
    // prevents the cached vendor-label fallback (`"openhub"`) from
    ...
    let full_auto_mode = agent_type_enum
        ...
        .to_owned();
    let agent_config = openhub_api_types::CronAgentConfigDto {
        backend,
...
fn validate_openhub_agent_config(
    agent_type: &str,
    agent_config: Option<&openhub_api_types::CronAgentConfigDto>,
) -> Result<(), CronError> {
...
mod tests {
    ...
    // -- validate_openhub_agent_config ----------------------------------------

    fn agent_cfg_dto(backend: &str) -> openhub_api_types::CronAgentConfigDto {
        openhub_api_types::CronAgentConfigDto {
    ...
    #[test]
    fn validate_openhub_accepts_valid_config() {
        let cfg = agent_cfg_dto("4056cdea");
        assert!(validate_openhub_agent_config("openhub", Some(&cfg)).is_ok());
    }

    #[test]
    fn validate_openhub_rejects_missing_config() {
        let err = validate_openhub_agent_config("openhub", None).unwrap_err();
        assert!(matches!(err, CronError::InvalidAgentConfig(_)));
    }

    #[test]
    fn validate_openhub_rejects_empty_backend() {
    ...
    #[test]
    fn validate_openhub_rejects_whitespace_backend() {
        let cfg = agent_cfg_dto("   ");
        let err = validate_openhub_agent_config("openhub", Some(&cfg)).unwrap_err();
        assert!(matches!(err, CronError::InvalidAgentConfig(_)));
    }
    ...
    #[test]
    fn validate_openhub_ignores_non_openhub_type() {
        // ACP / other types may legitimately omit agent_config or leave backend empty.
        assert!(validate_openhub_agent_config("acp", None).is_ok());
        let cfg = agent_cfg_dto("");
        assert!(validate_openhub_agent_config("claude", Some(&cfg)).is_ok());
    }
    ...
    #[test]
    fn openhub_existing_bound_conversation_is_model_source() {
    ...
    #[test]
    fn openhub_new_conversation_requires_agent_config() {
    ...
    #[test]
    fn openhub_existing_without_bound_conversation_requires_agent_config() {
