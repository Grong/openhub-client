...
use openhub_ai_agent::AgentRegistry;
use openhub_ai_agent::agent_task::AgentInstance;
use openhub_ai_agent::types::BuildTaskOptions;
...
use openhub_common::{PaginatedResult, TimestampMs, now_ms};
use openhub_conversation::ConversationService;
use openhub_conversation::response_middleware::{CronCreateParams, CronUpdateParams};
use openhub_db::{
...
use openhub_cron::busy_guard::CronBusyGuard;
use openhub_cron::events::CronEventEmitter;
use openhub_cron::executor::JobExecutor;
use openhub_cron::scheduler::CronScheduler;
use openhub_cron::service::CronService;
use openhub_cron::types::JobStatus;
...
impl openhub_ai_agent::task_manager::IWorkerTaskManager for StubTaskManager {
...
struct StubConvRepo {
    ...
    artifacts: Mutex<Vec<openhub_db::ConversationArtifactRow>>,
    rows: Mutex<HashMap<i64, openhub_db::models::ConversationRow>>,
...
impl StubConvRepo {
    ...
    fn upsert_artifact_row(&self, mut artifact: openhub_db::ConversationArtifactRow) {
    ...
    fn artifacts(&self) -> Vec<openhub_db::ConversationArtifactRow> {
...
impl IConversationRepository for StubConvRepo {
    ...
    async fn update(
        ...
        let row = rows
            .entry(id)
            .or_insert_with(|| openhub_db::models::ConversationRow {
                id,
                user_id: "u1".into(),
                name: "stub".into(),
                r#type: "default".into(),
    ...
    async fn list_artifacts(
        &self,
        conversation_id: i64,
    ) -> Result<Vec<openhub_db::ConversationArtifactRow>, openhub_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn get_artifact(
        ...
        conversation_id: i64,
        artifact_id: i64,
    ) -> Result<Option<openhub_db::ConversationArtifactRow>, openhub_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn upsert_artifact(
        &self,
        artifact: &openhub_db::ConversationArtifactRow,
    ) -> Result<openhub_db::ConversationArtifactRow, openhub_db::DbError> {
        let mut guard = self.artifacts.lock().unwrap();
...
async fn setup_with_conv_repo() -> (
    ...
    // 14=conv_mode_nomi.
    {
        ...
        for _ in 1..=14 {
            real_conv_repo
                .create(&openhub_db::models::ConversationRow {
    ...
    let data_dir = std::env::temp_dir().join(format!("openhub-cron-test-{}", now_ms()));
    ...
    let task_manager: Arc<dyn openhub_ai_agent::task_manager::IWorkerTaskManager> =
...
async fn cj1b_reject_terminal_target_cron_job() {
    ...
    assert!(matches!(
        err,
        openhub_cron::error::CronError::InvalidTargetKind(_)
    ));
}
...
async fn cj5_get_nonexistent_job() {
    ...
    assert!(matches!(
        err,
        openhub_cron::error::CronError::JobNotFound(_)
    ));
}
...
async fn cj9_update_schedule_type() {
    ...
    assert!(matches!(
        updated.schedule,
        openhub_cron::types::CronSchedule::Cron { .. }
    ));
...
async fn cj12_delete_nonexistent() {
    ...
    assert!(matches!(
        err,
        openhub_cron::error::CronError::Database(openhub_db::DbError::NotFound(_))
    ));
}
...
async fn sk1_1_save_skill_marks_related_skill_suggest_artifacts_saved() {
    ...
    conv_repo.upsert_artifact_row(openhub_db::ConversationArtifactRow {
        id: 0,
        conversation_id: 1,
...
async fn sk4_save_empty_skill() {
    ...
    assert!(matches!(
        err,
        openhub_cron::error::CronError::InvalidSkillContent(_)
    ));
}
...
async fn sc5_invalid_cron_expression() {
    ...
    assert!(matches!(
        err,
        openhub_cron::error::CronError::InvalidCronExpression(_)
    ));
}
...
async fn sc7_every_zero_interval() {
    ...
    assert!(matches!(
        err,
        openhub_cron::error::CronError::InvalidSchedule(_)
    ));
}
...
async fn icron_service_create_job() {
    ...
    use openhub_conversation::response_middleware::ICronService;
...
async fn sr1_system_resume_missed_job() {
    ...
    let params = openhub_db::UpdateCronJobParams {
