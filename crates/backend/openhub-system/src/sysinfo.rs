use openhub_api_types::SystemInfoResponse;
...
fn resolve_cache_dir() -> String {
    ...
    dirs::cache_dir()
        .map(|p| p.join("openhub").to_string_lossy().into_owned())
        .unwrap_or_default()
}
...
fn resolve_log_dir() -> String {
    if let Ok(v) = std::env::var("OPENHUB_LOG_DIR")
        && !v.is_empty()
    {
        return v;
    }
    ...
    if let Some(state) = dirs::state_dir() {
        return state.join("openhub/logs").to_string_lossy().into_owned();
    }
    // Fallback: data_dir/openhub/logs
    dirs::data_dir()
        .map(|p| p.join("openhub/logs").to_string_lossy().into_owned())
        .unwrap_or_default()
}
...
mod tests {
    ...
    fn test_env_override_cache_dir() {
        ...
        // the default path contains "openhub".
        let dir = resolve_cache_dir();
        assert!(dir.contains("openhub"), "cache_dir should contain 'openhub': {dir}");
    }

    #[test]
    fn test_env_override_work_dir() {
        let dir = resolve_work_dir();
        assert!(dir.contains("openhub"), "work_dir should contain 'openhub': {dir}");
    }

    #[test]
    fn test_env_override_log_dir() {
        let dir = resolve_log_dir();
        assert!(dir.contains("openhub"), "log_dir should contain 'openhub': {dir}");
    }
}
