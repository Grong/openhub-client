...
use openhub_api_types::{GitHubReleaseAsset, UpdateCheckRequest, UpdateCheckResult, UpdateReleaseInfo};
use openhub_common::AppError;
use serde::Deserialize;

const DEFAULT_REPO: &str = "openhub/openhub-app";
...
fn resolve_repo(from_request: Option<&str>) -> String {
    ...
    if let Ok(v) = std::env::var("OPENHUB_GITHUB_REPO")
        && !v.is_empty()
    {
        return v;
    }
