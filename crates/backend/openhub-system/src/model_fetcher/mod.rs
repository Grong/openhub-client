...
use openhub_common::{AppError, decrypt_string};
...
mod tests {
    use super::*;
    use openhub_common::encrypt_string;
    use openhub_db::{CreateProviderParams, SqliteProviderRepository, init_database_memory};

    const TEST_KEY: [u8; 32] = [0x42; 32];

    async fn setup() -> (ModelFetchService, openhub_db::Database) {
    ...
    async fn fetch_models_minimax_returns_hardcoded() {
        ...
        assert!(
            resp.models
                .contains(&openhub_api_types::ModelInfo::Id("MiniMax-M3".into()))
        );
        assert!(
            resp.models
                .contains(&openhub_api_types::ModelInfo::Id("MiniMax-Text-01".into()))
        );
    }
