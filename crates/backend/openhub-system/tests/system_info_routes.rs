...
fn build_state(db: &openhub_db::Database, version_check_service: VersionCheckService) -> SystemRouterState {
    ...
    SystemRouterState {
        ...
        model_profile_service: openhub_system::ModelProfileService::new(std::sync::Arc::new(
            openhub_db::SqliteModelProfileRepository::new(db.pool().clone()),
        )),
...
fn make_github_release(tag: &str, draft: bool, prerelease: bool, assets: Vec<serde_json::Value>) -> serde_json::Value {
    json!({
        ...
        "body": "Release notes",
        "html_url": format!("https://github.com/openhub/openhub-app/releases/tag/{tag}"),
...
async fn test_check_update_has_new_version() {
    ...
    Mock::given(method("GET"))
        .and(path("/repos/openhub/openhub-app/releases"))
