...
fn unique_data_dir(tag: &str) -> std::path::PathBuf {
    let dir = std::env::temp_dir().join(format!("openhub-wdroute-{tag}-{}", openhub_common::now_ms()));
...
async fn persists_valid_work_dir_and_creates_it() {
    ...
    assert_eq!(
        openhub_common::dir_config::persisted_work_dir(&data_dir).as_deref(),
        Some(target.as_path())
    );
...
async fn rejects_relative_work_dir() {
    ...
    assert!(openhub_common::dir_config::persisted_work_dir(&data_dir).is_none());
