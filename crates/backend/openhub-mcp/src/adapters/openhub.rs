...
mod tests {
    ...
    impl IMcpServerRepository for MockRepo {
        ...
        async fn find_by_id(&self, id: i64) -> Result<Option<McpServerRow>, openhub_db::DbError> {
            Ok(self.servers.iter().find(|s| s.id == id).cloned())
        }

        async fn find_by_name(&self, name: &str) -> Result<Option<McpServerRow>, openhub_db::DbError> {
        ...
        async fn update_tools(&self, _id: i64, _tools: Option<&str>) -> Result<(), openhub_db::DbError> {
            unimplemented!("not needed for adapter tests")
        }
    }
    ...
    fn source_isopenhub) {
        ...
        let adapter = OpenhubAdapter::new(repo);
    ...
    async fn trait_is_object_safe() {
        let repo = Arc::new(MockRepo::new(vec![]));
        let adapter: Arc<dyn McpAgentAdapter> = Arc::new(OpenhubAdapter::new(repo));
        assert_eq!(adapter.source(), McpSource::Openhub);
        assert!(adapter.is_installed().await.unwrap());
    }
}
