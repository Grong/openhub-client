...
use openhub_api_types::{OAuthLoginResponse, OAuthStatusResponse};
use openhub_common::{TimestampMs, now_ms};
use openhub_db::{IOAuthTokenRepository, UpsertOAuthTokenParams};
...
const DEFAULT_CLIENT_ID: &str = "openhub";
...
impl McpOAuthService {
    ...
    pub fn new_dynamic(token_repo: Arc<dyn IOAuthTokenRepository>) -> Self {
        Self {
            token_repo,
            http_client: Arc::new(openhub_net::http_client),
            pending: Arc::new(Mutex::new(None)),
        }
    }
    ...
    async fn handle_callback_connection(
        ...
        let response = "HTTP/1.1 200 OK\r\n\
            ...
            <p>You can close this window and return to OpenHub.</p>\
            </body></html>";
    ...
    fn build_no_redirect_client() -> Result<reqwest::Client, McpError> {
        openhub_net::proxy::apply_detected_proxy(reqwest::ClientBuilder::new())
...
mod tests {
    ...
    impl IOAuthTokenRepository for MockTokenRepo {
        async fn get_by_url(&self, _: &str) -> Result<Option<openhub_db::models::OAuthTokenRow>, openhub_db::DbError> {
            Ok(None)
        }
        ...
        ) -> Result<openhub_db::models::OAuthTokenRow, openhub_db::DbError> {
            unimplemented!()
        }

        async fn delete(&self, _: &str) -> Result<(), openhub_db::DbError> {
            Ok(())
        }

        async fn list_authenticated_urls(&self) -> Result<Vec<String>, openhub_db::DbError> {
            Ok(vec![])
        }
    }
    ...
    impl IOAuthTokenRepository for IdempotentDeleteRepo {
        ...
        async fn delete(&self, url: &str) -> Result<(), openhub_db::DbError> {
            Err(openhub_db::DbError::NotFound(format!(
    ...
    impl IOAuthTokenRepository for ValidTokenRepo {
        async fn get_by_url(&self, _: &str) -> Result<Option<openhub_db::models::OAuthTokenRow>, openhub_db::DbError> {
            Ok(Some(openhub_db::models::OAuthTokenRow {
