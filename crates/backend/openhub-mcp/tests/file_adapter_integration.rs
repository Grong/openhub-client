...
modopenhub{
    ...
    use openhub_db::{CreateMcpServerParams, DbError, IMcpServerRepository, UpdateMcpServerParams};
    ...
    impl IMcpServerRepository for MockRepo {
        ...
        async fn update_status(
            ...
            _id: i64,
            _s: &str,
            _lc: Option<openhub_common::TimestampMs>,
        ) -> Result<(), DbError> {
            unimplemented!()
        }
    ...
    async fn install_is_noop() {
        let repo = Arc::new(MockRepo::new(vec![]));
        let adapter = OpenhubAdapter::new(repo.clone());
...
mod opencode {
    use super::*;
    use openhub_mcp::OpencodeAdapter;
