...
//! is a faithful reduction of `openhub_requirement::Orchestrator` (see
//! `crates/backend/openhub-requirement/src/orchestrator.rs`). The differences
...
//!    [`list_ready_tasks`](openhub_db::IRunRepository::list_ready_tasks)
...
use openhub_api_types::FleetMember;
use openhub_common::{AppError, now_ms};
use openhub_db::models::{OrchRunRow, OrchRunTaskRow};
use openhub_db::{IOrchWorkspaceRepository, IRunRepository};
use openhub_db::{UpdateRunParams, UpdateTaskParams};
...
/// [`ConversationService::cancel`](openhub_conversation::ConversationService::cancel)
...
/// `async` and the orchestrator crate stays free of a `openhub-conversation`
...
pub enum RunOutcome {
    ...
    /// warranted, append more work (`openhub_run_add_tasks`) to the SAME run — without
    ...
    /// (审批模式，迁移 030): the worker called `openhub_task_question` mid-run, so the
...
/// `openhub-conversation` dependency — wired in `build_orchestrator_state`, exactly
...
impl PlanProducer for NoopSummaryProducer {
    ...
    ) -> Result<openhub_api_types::PlannedDag, AppError> {
...
// the lead as a fresh turn, which may call `openhub_run_add_tasks` to append more work
...
// bounded here; a manual `openhub_spawn` / `openhub_run_add_tasks` on a supervised run is
...
impl RunEngine {
    ...
    /// 审批模式（迁移 030）— **节点提问 (`openhub_task_question`)**。worker 会话在自己的
    ...
    pub async fn add_tasks(
        ...
        run_service: &crate::run_service::RunService,
        run_id: &str,
        tasks: Vec<openhub_api_types::PlannedTask>,
    ) -> Result<openhub_api_types::Run, AppError> {
        let lock = self.deps.run_locks.for_run(run_id);
        let _guard = lock.lock().await;
...
async fn settle_task_outcome(
    ...
    // 审批模式（迁移 030）：worker 在本轮中途经 openhub_task_question 把任务置为
...
pub(crate) struct BriefContext {
    ...
    /// 追加「决策策略」段，指示 worker 遇关键决策调用 `openhub_task_question` 挂起等
...
fn compose_brief(
    ...
    // 审批模式（迁移 030）：仅 manual 追加决策策略段——遇关键决策经 openhub_task_question
    // 挂起等人工作答；auto（全授权，缺省）不渲染任何新内容，brief 零回归。
    if ctx.manual_approval {
        out.push_str(
            "\n决策策略(审批模式):\n本编排处于「审批模式」。当你遇到会显著影响方向、取舍或对外效果的关键决策问题\
             (例如互斥方案二选一、需求歧义、可能造成破坏性影响的操作)时,不要自行猜测:调用 `openhub_task_question` 工具\
...
async fn skip_downstream(
    ...
    run_id: &str,
    from_task_id: &str,
    dep_edges: &[openhub_db::models::OrchRunTaskDepRow],
) {
...
mod tests {
    ...
    struct RecordingBroadcaster {
        events: Mutex<Vec<openhub_api_types::WebSocketMessage<serde_json::Value>>>,
    }
    ...
    /// （模拟 openhub_task_question 写入 / 老化 updated_at）的审批模式测试使用。
    ...
    /// OBSERVE progress and append more work (`openhub_run_add_tasks`) to the SAME run.
    ...
    /// openhub_task_question」（直写 repo：pending_question + needs_review，与网关工具
    ...
    /// `raise_task_question`（openhub_task_question 的引擎入口）守卫矩阵：
    ...
    /// openhub_task_question）；false 与既有 brief 逐字节一致（零回归由既有测试锁定）。
    #[test]
    fn compose_brief_manual_approval_appends_decision_policy() {
        ...
        assert!(brief.contains("openhub_task_question"), "点名提问工具");
    ...
    async fn two_independent_nodes_harness(a_on_fail: &str) -> (RunService, RunEngine, String) {
        use openhub_db::CreateTaskParams;
        let db = init_database_memory().await.expect("db init");
        let pool = db.pool().clone();
        let run_repo = Arc::new(SqliteRunRepository::new(pool.clone()));
        let fleet_repo = Arc::new(SqliteFleetRepository::new(pool.clone()));
    ...
    fn body_run_count(
        ...
        detail: &openhub_api_types::RunDetail,
    ) -> usize {
        let body_id = detail
            .tasks
            .iter()
    ...
    async fn adjust_completed_run_keeps_done_adds_new_redrives_no_strand() {
        ...
        impl PlanProducer for AdjustEngineProducer {
            ...
            async fn adjust(
                &self,
                _intent: &str,
                _tasks: &[openhub_api_types::RunTask],
                _deps: &[openhub_api_types::RunTaskDep],
                _members: &[FleetMember],
                _sink: Option<&crate::plan::LeadThinkingSink>,
    ...
    async fn adjust_streams_lead_thinking_phase_adjust() {
        ...
        let adjust_thinking: Vec<&openhub_api_types::WebSocketMessage<serde_json::Value>> = events
            .iter()
            .filter(|e| {
