...
//! Auth is layered externally in openhub-app (mirrors the webhook / requirement
...
mod tests {
    ...
    /// does in openhub-app — so the handler's `Extension<CurrentUser>` requirement
    ...
    async fn seed_conversation(conv: &ConversationService) -> i64 {
        ...
        conv.create(openhub_auth::SYSTEM_USER_ID, req)
            .await
    ...
    async fn create_adhoc_run_links_originating_conversation() {
        ...
        let fetched = conv.get(openhub_auth::SYSTEM_USER_ID, &conv_id.to_string()).await.expect("re-read conversation");
        assert_eq!(
