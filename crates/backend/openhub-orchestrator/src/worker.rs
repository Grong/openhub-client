...
//! the gateway `openhub_agent_run` recipe** verbatim (see
...
use openhub_common::{AgentType, AppError, ProviderWithModel};
...
impl WorkerRunner for ConversationWorkerRunner {
    ...
    async fn run_restricted(
        ...
        // answer LAST). Mirror openhub_agent_result — gate on the runtime turn having
...
fn build_worker_extra(
    ...
    // 委派出的子任务=2。网关两个追加口（openhub_run_add_tasks / openhub_spawn 收敛）据此
    // 钳制深度（DELEGATION_DEPTH_LIMIT），防无界自我繁殖。
    delegation_depth: u32,
) -> Value {
    // 受限角色（searcher/reviewer 只读、verifier 只读+Bash）：收缩引擎工具白名单，
    // 且不授桌面网关 —— 只读 worker 拿全量 openhub_* 桌面控制等于静默升权。
...
/// [`ErrorInfo`](openhub_protocol-style) `{code, message, retryable}` under
...
mod tests {
    ...
    // openhub_* 桌面控制 = 静默升权）。无角色/implementer：现状完全不变。
    #[test]
