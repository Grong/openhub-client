...
use openhub_common::generate_prefixed_id;
...
impl RunService {
    ...
    async fn persist_dag_and_activate(
        &self,
        run_id: &str,
        run: &openhub_db::models::OrchRunRow,
        members: &[FleetMember],
        dag: PlannedDag,
    ) -> Result<(), AppError> {
        #[allow(unused_mut)]
    ...
    /// [`reconcile_run_plan`](openhub_db::IRunRepository::reconcile_run_plan) with
    ...
    /// [`openhub_file::list_workspace_level`]. The run-history counterpart of
    ...
    pub async fn browse_workspace(
        ...
        openhub_file::list_workspace_level(std::path::Path::new(&root), path, search)
    }
...
/// only `modalities` (from [`infer_model_modalities`](openhub_api_types::infer_model_modalities))
...
mod tests {
    ...
    // ----- plan_flat（openhub_spawn 扁平 fan-out，跳过 planner）-----
    ...
    /// supervised 的 adhoc run（openhub_spawn 前门的形状：单模型、无审批门）。
    ...
    // (via `openhub_api_types::infer_model_modalities`), so the Router's
    ...
    fn reconcile_plan_has_cycle_unit() {
        use openhub_db::ReconcileDepRef as DR;
