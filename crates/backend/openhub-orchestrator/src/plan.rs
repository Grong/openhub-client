...
use openhub_api_types::{FleetMember, PlannedDag, PlannedTask, RunTask, RunTaskDep};
...
/// `openhub_ai_agent::DeltaKind` the provider layer emits. `Text` = the visible
...
/// `lead` provider row (mirrors `openhub-idmm`'s `LiveCompleter`).
...
impl PlanProducer for LlmPlanProducer {
    async fn produce(
        ...
        // on the conversation path (a SYNCHRONOUS `openhub_run_create` tool call) that
...
async fn run_lead_completion(
    cfg: &openhub_ai_agent::openhub_config::config::Config,
    system: &str,
    user: String,
    max_tokens: u32,
    sink: Option<&LeadThinkingSink>,
...
mod tests {
    use super::*;
    use openhub_api_types::TaskProfile;
    ...
    struct RecordingBroadcaster {
        events: std::sync::Mutex<Vec<openhub_api_types::WebSocketMessage<serde_json::Value>>>,
    }
    ...
    fn build_plan_user_prompt_includes_goal_and_members() {
        let member = FleetMember {
            ...
            capability_profile: Some(openhub_api_types::CapabilityProfile {
