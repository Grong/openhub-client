...
use openhub_db::{IUserRepository, SqliteUserRepository, init_database_memory};
...
async fn t4_1_login_success() {
    ...
    assert!(set_cookie.contains("openhub-session="));
...
async fn qr_login_page_hands_success_state_to_spa_before_redirecting() {
    ...
    assert!(
        html.contains("openhub:qr-login-resume"),
...
async fn qr_login_page_checks_app_shell_before_redirecting() {
    ...
    assert!(
        html.contains("openhub_spa_shell_check=1"),
