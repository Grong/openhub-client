...
use openhub_common::constants::COOKIE_MAX_AGE_DAYS;
use openhub_db::{IUserRepository, models::User};
...
/// it can pre-seed with `OPENHUB_ADMIN_PASSWORD` (see
/// `openhub_app::bootstrap::ensure_admin_credentials`), which makes this return
...
(function() {
  ...
  function verifyAppShellThenRedirect() {
    fetch('/?openhub_spa_shell_check=1', {
      method: 'GET',
      cache: 'no-store',
  ...
  .then(function(data) {
    if (data.success) {
      ...
      try {
        sessionStorage.setItem('openhub:qr-login-resume', JSON.stringify({
          at: Date.now(),
          user: data.user
        }));
      } catch (e) {}
