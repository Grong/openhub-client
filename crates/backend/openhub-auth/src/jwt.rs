...
const JWT_ISSUER: &str = "openhub";

/// JWT audience claim value.
const JWT_AUDIENCE: &str = "openhub-webui";
...
pub fn generate_random_secret_string() -> String {
    let mut buf = [0u8; 64];
    // getrandom failure is fatal — mirrors openhub-common's UUID generation.
...
/// `x-openhub-local-trust` HTTP header.
