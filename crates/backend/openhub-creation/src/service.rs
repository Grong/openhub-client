...
use openhub_common::{AppError, decrypt_string, generate_prefixed_id, now_ms};
use openhub_db::{
...
/// `openhub-workshop` (registers each result as a `wsa_` asset), so this crate
/// never depends on `openhub-workshop` (no dependency cycle).
#[async_trait]
...
/// implemented by the app over `openhub-workshop`.
#[async_trait]
...
mod tests {
    ...
    use openhub_db::{SqliteCreationTaskRepository, SqliteProviderRepository};
    ...
    async fn seed_provider(pool: &openhub_db::SqlitePool, platform: &str) -> String {
        let repo = SqliteProviderRepository::new(pool.clone());
        let encrypted = openhub_common::encrypt_string("sk-test-key", &TEST_KEY).unwrap();
        let row = repo
...
mod http_e2e_tests {
    ...
    async fn build(base_url: &str) -> (Arc<CreationService>, String, Arc<CountingSink>, openhub_db::Database) {
        let db = openhub_db::init_database_memory().await.unwrap();
        let pool = db.pool().clone();
        // seed a provider row pointed at the mock server
        let prov_repo = SqliteProviderRepository::new(pool.clone());
        let encrypted = openhub_common::encrypt_string("sk-e2e", &TEST_KEY).unwrap();
