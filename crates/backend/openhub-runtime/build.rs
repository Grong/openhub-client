...
//!   and `[package.metadata.openhub-runtime] bun_version` from Cargo.toml.
...
//! - Caches decompressed+compressed artifacts in `$CARGO_HOME/openhub-bun-cache/`.
...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=OPENHUB_EMBED_BUN");
    ...
    // sets OPENHUB_EMBED_BUN=1 to actually embed.
    if std::env::var("OPENHUB_EMBED_BUN").as_deref() != Ok("1") {
...
fn read_bun_version() -> String {
    ...
    parsed
        ...
        .and_then(|m| m.get("openhub-runtime"))
        .and_then(|r| r.get("bun_version"))
        .and_then(|v| v.as_str())
        .expect("[package.metadata.openhub-runtime] bun_version missing")
        .to_string()
}
