...
///
/// Lifecycle: written once by `openhub-app`'s `main()` before
...
pub fn init(data_dir: impl AsRef<Path>) {
        ...
        && existing != path
    {
        tracing::warn!(
            ...
            "openhub_runtime::init called twice with different paths; keeping first"
        );
    }
}

/// Returns the root cache directory used for allopenhubruntime artifacts.
///
/// Priority:
/// 1. Path supplied via [`init`] (`{data_dir}/runtime`) when the backend
///    started with `--data-dir`.
/// 2. Platform cache dir (via `dirs::cache_dir()`):
///    - macOS:   `~/Library/Caches/openhub/runtime`
///    - Linux:   `$XDG_CACHE_HOME/openhub/runtime` (fallback `~/.cache/openhub/runtime`)
///    - Windows: `%LOCALAPPDATA%openhubruntime`
///
...
pub fn runtime_root() -> Option<PathBuf> {
    ...
    dirs::cache_dir().map(|d| d.join("openhub").join("runtime"))
}
...
mod tests {
    ...
    fn runtime_root_ends_with_expected_suffix() {
        ...
        let tail: Vec<_> = root
            ...
            .collect();
        assert_eq!(tail, vec!["runtime".to_string(), "openhub".to_string()]);
    }
