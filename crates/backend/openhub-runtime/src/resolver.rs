...
///
/// Priority: `OPENHUB_BUN_PATH` env override > embedded + extract >
/// `which("bun")`.
...
fn env_override() -> Option<PathBuf> {
    let raw = std::env::var("OPENHUB_BUN_PATH").ok()?;
    ...
    if p.is_file() {
        ...
        tracing::warn!(path = %p.display(), "OPENHUB_BUN_PATH does not point to a file; ignoring");
        None
    }
}
...
///
/// For `bun` / `bunx` we go through `openhub_runtime` so the bundled
...
mod tests {
    ...
    fn no_embed_falls_back_to_which() {
        ...
        // SAFETY: ENV_LOCK serializes tests that mutate OPENHUB_BUN_PATH.
        unsafe {
            std::env::remove_var("OPENHUB_BUN_PATH");
        }
    ...
    fn env_override_wins_over_embed() {
        ...
        // SAFETY: ENV_LOCK serializes tests that mutate OPENHUB_BUN_PATH.
        unsafe {
            std::env::set_var("OPENHUB_BUN_PATH", &path);
        }
    ...
    fn bad_env_override_falls_through_to_embed() {
        ...
        // SAFETY: ENV_LOCK serializes tests that mutate OPENHUB_BUN_PATH.
        unsafe {
            std::env::set_var("OPENHUB_BUN_PATH", "/definitely/does/not/exist");
        }
