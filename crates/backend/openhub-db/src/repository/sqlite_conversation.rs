...
use openhub_common::PaginatedResult;
...
fn append_filter_conditions(filters: &ConversationFilters, where_parts: &mut Vec<String>, binds: &mut Vec<BindValue>) {
    ...
    // `orchestrator_task_id`(而非 run_id):lead 会话(openhub_run_create 写回)只带
...
mod tests {
    ...
    fn sample_message(conv_id: i64) -> MessageRow {
        let now = openhub_common::now_ms();
        MessageRow {
            id: openhub_common::generate_prefixed_id("msg"),
    ...
    async fn update_conversation_pinned() {
        ...
        let pin_time = openhub_common::now_ms();
        repo.update(
            conv.id,
            &ConversationRowUpdate {
                pinned: Some(true),
    ...
    async fn list_filter_by_source() {
        ...
        let mut c1 = sample_conversation(SYSTEM_USER_ID);
        c1.source = Some("openhub".to_string());
        repo.create(&c1).await.unwrap();
    ...
    async fn list_filter_by_pinned() {
        ...
        let mut c1 = sample_conversation(SYSTEM_USER_ID);
        c1.pinned = true;
        c1.pinned_at = Some(openhub_common::now_ms());
        repo.create(&c1).await.unwrap();
    ...
    /// LEAD 会话与 WORKER 会话的区分:lead (P1 `openhub_run_create` 写回) 的 extra
    ...
    async fn list_keeps_lead_and_excludes_only_worker() {
        ...
        // (b) LEAD conversation — only orchestrator_run_id (openhub_run_create write-back),
    ...
    async fn cron_job_id_roundtrips_as_column() {
        ...
        repo.update(
            ...
            &ConversationRowUpdate {
                ...
                updated_at: Some(openhub_common::now_ms()),
                ..Default::default()
            },
        )
        .await
    ...
    async fn get_and_update_artifact_status_by_i64_id() {
        ...
        let updated = repo
            .update_artifact_status(conv.id, inserted.id, "dismissed", openhub_common::now_ms())
            .await
            .unwrap()
            .unwrap();
        ...
        // Missing id → None.
        let missing = repo
            .update_artifact_status(conv.id, 999_999, "dismissed", openhub_common::now_ms())
            .await
            .unwrap();
    ...
    async fn get_messages_pagination() {
        ...
        for i in 0..10 {
            ...
            msg.id = openhub_common::generate_prefixed_id("msg");
    ...
    async fn search_messages_by_keyword() {
        ...
        msg2.id = openhub_common::generate_prefixed_id("msg");
