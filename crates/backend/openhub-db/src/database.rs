...
fn migrate_lock_path(db_path: &Path) -> PathBuf {
    ...
    let new_name = match p.file_name().and_then(|s| s.to_str()) {
        ...
        None => "openhub.migrate.lock".to_string(),
    };
...
async fn run_migrations(pool: &SqlitePool) -> Result<(), DbError> {
    ...
    // still shutting down, or `openhub-core doctor` racing the server) can both
...
/// file name: `openhub-backend.db-wal`, `openhub-backend.db-shm`).
...
async fn recover_and_retry(path: &Path, original_error: DbError) -> Result<Database, DbError> {
    let backup_path = format!("{}.backup.{}", path.display(), openhub_common::now_ms());
...
mod tests {
    ...
    fn pre_baseline_backup_path_appends_numeric_suffix_when_taken() {
        ...
        let db = dir.path().join("openhub-backend.db");

        let first = pre_baseline_backup_path(&db);
        assert_eq!(first.file_name().unwrap(), "openhub-backend.db.pre-baseline.bak");

        std::fs::write(&first, b"taken").unwrap();
        let second = pre_baseline_backup_path(&db);
        assert_eq!(second.file_name().unwrap(), "openhub-backend.db.pre-baseline.bak.1");

        std::fs::write(&second, b"taken").unwrap();
        let third = pre_baseline_backup_path(&db);
        assert_eq!(third.file_name().unwrap(), "openhub-backend.db.pre-baseline.bak.2");
    }
    ...
    fn migrate_lock_path_sits_next_to_db() {
        let db = Path::new("/var/lib/openhub/openhub-backend.db");
        let lock = migrate_lock_path(db);
        assert_eq!(lock.parent(), db.parent());
        assert_eq!(lock.file_name().unwrap(), "openhub-backend.db.migrate.lock");
    }
