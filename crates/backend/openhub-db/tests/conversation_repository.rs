...
async fn setup() -> (SqliteConversationRepository, openhub_db::Database) {
...
fn make_message(conv_id: i64, content: &str) -> MessageRow {
    ...
    MessageRow {
        ...
        msg_id: Some(openhub_common::generate_prefixed_id("cmsg")),
...
fn make_artifact(conv_id: i64) -> openhub_db::ConversationArtifactRow {
    openhub_db::ConversationArtifactRow {
...
async fn filter_by_source_and_pinned_combined() {
    ...
    let mut c1 = make_conversation("openhub-pinned");
    ...
    c2.pinned = true;
    c2.pinned_at = Some(openhub_common::now_ms());
    repo.create(&c2).await.unwrap();

    let mut c3 = make_conversation("openhub-unpinned");
    c3.source = Some("openhub".to_string());
    c3.pinned = false;
    repo.create(&c3).await.unwrap();

    // Filter: sourceopenhubAND pinned=true
    let result = repo
        .list_paginated(
            USER_ID,
            &ConversationFilters {
...
async fn update_nonexistent_conversation_returns_not_found() {
    ...
    let err = repo
        ...
        .await
        .unwrap_err();
    assert!(matches!(err, openhub_db::DbError::NotFound(_)));
}
