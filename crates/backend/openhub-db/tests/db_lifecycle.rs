use openhub_db::{init_database, init_database_memory};
use sqlx::Row;
...
async fn system_user_has_valid_timestamps() {
    let before = openhub_common::now_ms();
    let db = init_database_memory().await.unwrap();
    let after = openhub_common::now_ms();
...
async fn pre_baseline_database_is_renamed_and_rebuilt() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("openhub-backend.db");
    ...
    let backup = dir.path().join("openhub-backend.db.pre-baseline.bak");
...
async fn pre_baseline_rebuild_refused_when_credential_present() {
    ...
    assert!(
        !dir.path().join("openhub-backend.db.pre-baseline.bak").exists(),
...
async fn pre_baseline_rebuild_numbers_subsequent_backups() {
    ...
    std::fs::write(dir.path().join("openhub-backend.db.pre-baseline.bak"), b"earlier backup").unwrap();
    ...
    let db = init_database(&path).await.unwrap();
    let numbered = dir.path().join("openhub-backend.db.pre-baseline.bak.1");
...
fn concurrent_init_database_does_not_panic_on_unique_conflict() {
    ...
    let lock = path.with_file_name("openhub-backend.db.migrate.lock");
