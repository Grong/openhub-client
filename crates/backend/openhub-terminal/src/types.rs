use openhub_api_types::TerminalSessionResponse;
use openhub_db::TerminalSessionRow;
use std::path::Path;
...
/// Mirrors `openhub_runtime::Builder`'s `resolve_program`.
fn resolve_program(program: &str) -> String {
    if !program.is_empty()
        && !program.contains('/')
        && !program.contains('\\')
        && let Some(path) = openhub_runtime::resolve_command_path(program)
    {
...
/// `is_temporary_workspace` (openhub-conversation/src/convert.rs).
...
mod tests {
    ...
    fn resolve_command_passes_through_unresolvable_command() {
        ...
        let name = "openhub-definitely-not-on-path-xyz-987";
    ...
    fn resolve_command_passes_through_path_like_command() {
        ...
        // search, mirroring openhub_runtime::Builder's resolve_program.
    ...
    fn row_to_response_derives_is_default_workpath() {
        ...
        assert!(!row_to_response(&row, None, work_dir).is_default_workpath);
        row.cwd = "/srv/openhub-workspace".into();
        assert!(!row_to_response(&row, None, work_dir).is_default_workpath);
