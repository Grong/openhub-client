...
use dashmap::DashMap;
use openhub_api_types::{CreateTerminalRequest, TerminalSessionResponse};
use openhub_common::OnTerminalDelete;
use openhub_db::{CreateTerminalParams, ITerminalRepository};
...
/// with **no `TERM`** (the same reason `openhub-runtime::shell_env` has to
...
/// so `openhub-terminal` need not depend on `openhub-idmm`; `IdmmManager`
/// implements it and `openhub-app` injects the impl via
/// [`TerminalService::with_terminal_supervision_hook`]. Mirrors
/// `openhub_conversation::ConversationSupervisionHook`.
///
...
pub struct TerminalService {
    ...
    knowledge: Arc<std::sync::RwLock<Option<Arc<openhub_knowledge::KnowledgeService>>>>,
    /// Hooks notified after a terminal row is deleted (registration order),
    /// mirroring `ConversationService::delete_hooks`. Used by `openhub-app` to
    ...
    knowledge_mcp_config: Arc<std::sync::RwLock<Option<openhub_api_types::KnowledgeMcpConfig>>>,
    ...
    requirement_mcp_config: Arc<std::sync::RwLock<Option<openhub_api_types::RequirementMcpConfig>>>,
...
impl TerminalService {
    pub fn new(
        ...
        Self {
            ...
            mcp_config_dir: Arc::new(std::sync::RwLock::new(
                std::env::temp_dir().join("openhub-terminal-mcp"),
            )),
    ...
    /// by `openhub-app` to wire `RequirementService` so a deleted terminal
    ...
    pub fn with_knowledge_service(&self, service: Arc<openhub_knowledge::KnowledgeService>) {
    ...
    /// router/driver handles see it too). Called by `openhub-app` so a
    ...
    pub fn with_knowledge_mcp_config(
        &self,
        config: openhub_api_types::KnowledgeMcpConfig,
    ...
    fn knowledge_mcp_config(&self) -> Option<openhub_api_types::KnowledgeMcpConfig> {
    ...
    pub fn with_requirement_mcp_config(&self, config: openhub_api_types::RequirementMcpConfig) {
        if let Ok(mut g) = self.requirement_mcp_config.write() {
            *g = Some(config);
        }
    }

    fn requirement_mcp_config(&self) -> Option<openhub_api_types::RequirementMcpConfig> {
    ...
    fn build_enhancement(
        ...
        {
            use openhub_api_types::KnowledgeMcpConfig as K;
        ...
        if let Some(cfg) = self.requirement_mcp_config() {
            use openhub_api_types::RequirementMcpConfig as R;
            let env = std::collections::HashMap::from([
    ...
    fn knowledge_service(&self) -> Option<Arc<openhub_knowledge::KnowledgeService>> {
    ...
    async fn bind_knowledge(&self, id: i64, cwd: &str, kb_ids: &[String]) {
        ...
        let wp_key =
            openhub_knowledge::session_workpath_key(std::path::Path::new(cwd), &self.work_dir);
        ...
        let mut binding = ks
            .get_binding(openhub_knowledge::WORKPATH_BINDING_KIND, &wp_key)
            .await
            .unwrap_or_default();
        binding.enabled = true;
        binding.kb_ids = kb_ids.to_vec();
        if let Err(e) = ks
            .set_binding(openhub_knowledge::WORKPATH_BINDING_KIND, &wp_key, binding)
            .await
        {
    ...
    /// Sync this terminal's bound knowledge bases into `{cwd}/.openhub/knowledge/`
    ...
    async fn sync_knowledge_workspace(
        ...
        let wp_key = openhub_knowledge::session_workpath_key(cwd_path, &self.work_dir);
        let outcome = ks
        ...
        if let Some(readme) = openhub_knowledge::build_knowledge_context(
            &outcome.mounts,
            &openhub_knowledge::KnowledgeContextOptions {
                format: openhub_knowledge::KnowledgeContextFormat::TerminalReadme,
            ...
            let dir = std::path::Path::new(cwd).join(openhub_knowledge::KB_MOUNT_REL_DIR);
            if let Err(e) = async {
    ...
    /// `search` filter live in [`openhub_file::list_workspace_level`]. The exact
    ...
    pub async fn browse_workspace(
        ...
        path: &str,
        search: Option<&str>,
    ) -> Result<Vec<openhub_api_types::WorkspaceEntry>, openhub_common::AppError> {
        let row = self
            .repo
            .get_by_id(id)
            .await
            .map_err(|e| {
                openhub_common::AppError::Internal(format!("Failed to load terminal session: {e}"))
            })?
            .ok_or_else(|| {
                openhub_common::AppError::NotFound(format!("Terminal session '{id}' not found"))
            })?;
        if row.cwd.trim().is_empty() {
            return Err(openhub_common::AppError::BadRequest(
                "Terminal session has no working directory".into(),
            ));
        }
        openhub_file::list_workspace_level(std::path::Path::new(&row.cwd), path, search)
    }
...
mod tests {
    use super::*;
    use crate::submit::SettleReason;
    use openhub_api_types::WebSocketMessage;
    use openhub_realtime::EventBroadcaster;
    use std::sync::Mutex;
    use std::time::Duration;
    ...
    #[derive(Default)]
    struct MemRepo {
        rows: Mutex<HashMap<i64, openhub_db::TerminalSessionRow>>,
    ...
    impl ITerminalRepository for MemRepo {
        ...
        ) -> Result<openhub_db::TerminalSessionRow, openhub_db::DbError> {
            // SQLite mints the id; the mock allocates a monotonic i64.
            let id = self
                .next_id
                .fetch_add(1, std::sync::atomic::Ordering::Relaxed)
                + 1;
            let row = openhub_db::TerminalSessionRow {
                id,
                name: p.name.clone(),
                cwd: p.cwd.clone(),
        ...
        async fn get_by_id(
            &self,
            id: i64,
        ) -> Result<Option<openhub_db::TerminalSessionRow>, openhub_db::DbError> {
            Ok(self.rows.lock().unwrap().get(&id).cloned())
        }
        async fn list_by_user(
            &self,
            user_id: &str,
        ) -> Result<Vec<openhub_db::TerminalSessionRow>, openhub_db::DbError> {
            Ok(self
                .rows
                .lock()
                .unwrap()
        ...
        ) -> Result<(), openhub_db::DbError> {
            let mut rows = self.rows.lock().unwrap();
            let row = rows
                .get_mut(&id)
                .ok_or_else(|| openhub_db::DbError::NotFound(id.to_string()))?;
        ...
        async fn delete(&self, id: i64) -> Result<(), openhub_db::DbError> {
            self.rows
                .lock()
                .unwrap()
                .remove(&id)
        ...
        async fn delete_all(&self) -> Result<u64, openhub_db::DbError> {
            let mut rows = self.rows.lock().unwrap();
        ...
        async fn get_idmm(&self, id: i64) -> Result<Option<String>, openhub_db::DbError> {
            Ok(self
                .rows
                .lock()
                .unwrap()
        ...
        async fn mark_all_running_exited(&self) -> Result<u64, openhub_db::DbError> {
            let mut rows = self.rows.lock().unwrap();
            let mut n = 0u64;
        ...
        async fn save_scrollback(&self, id: i64, data: &[u8]) -> Result<(), openhub_db::DbError> {
            self.scrollback.lock().unwrap().insert(id, data.to_vec());
            Ok(())
        }
        async fn load_scrollback(&self, id: i64) -> Result<Option<Vec<u8>>, openhub_db::DbError> {
            Ok(self.scrollback.lock().unwrap().get(&id).cloned())
        }
        async fn clear_scrollback(&self, id: i64) -> Result<(), openhub_db::DbError> {
    ...
    impl crate::title::TerminalTitleCompleter for FakeTitler {
        async fn summarize(&self, _content: &str) -> Result<String, openhub_common::AppError> {
    ...
    #[derive(Default)]
    struct MemKbRepo {
        bases: Mutex<HashMap<String, openhub_db::models::KnowledgeBaseRow>>,
        ...
        bindings: Mutex<
            HashMap<(String, String), (openhub_db::models::KnowledgeBindingRow, Vec<String>)>,
        >,
        next_binding_id: std::sync::atomic::AtomicI64,
    }

    #[async_trait::async_trait]
    impl openhub_db::IKnowledgeRepository for MemKbRepo {
        async fn insert_base(
            &self,
            row: &openhub_db::models::KnowledgeBaseRow,
        ) -> Result<(), openhub_db::DbError> {
            self.bases
                .lock()
                .unwrap()
        ...
        async fn delete_base(&self, id: &str) -> Result<(), openhub_db::DbError> {
        ...
        async fn get_base(
            &self,
            id: &str,
        ) -> Result<Option<openhub_db::models::KnowledgeBaseRow>, openhub_db::DbError> {
            Ok(self.bases.lock().unwrap().get(id).cloned())
        }
        async fn list_bases(
            &self,
        ) -> Result<Vec<openhub_db::models::KnowledgeBaseRow>, openhub_db::DbError> {
        ...
        async fn get_binding(
            ...
            Option<(openhub_db::models::KnowledgeBindingRow, Vec<String>)>,
            openhub_db::DbError,
        > {
            Ok(self
                .bindings
        ...
        async fn set_binding(
            ...
            updated_at: openhub_common::TimestampMs,
        ) -> Result<i64, openhub_db::DbError> {
            ...
            let mut row = openhub_db::models::KnowledgeBindingRow {
                binding_id,
        ...
        async fn list_bindings_using_kb(
            &self,
            _kb_id: &str,
        ) -> Result<Vec<openhub_db::models::KnowledgeBindingRow>, openhub_db::DbError> {
            Ok(vec![])
        }
        async fn list_knowledge_tags(
            &self,
        ) -> Result<Vec<openhub_db::models::KnowledgeTagRow>, openhub_db::DbError> {
            Ok(vec![])
        }
        async fn create_knowledge_tag(
            &self,
            _params: openhub_db::models::CreateKnowledgeTagParams,
        ) -> Result<(), openhub_db::DbError> {
            Ok(())
        }
        async fn update_knowledge_tag(
            &self,
            _key: &str,
            _params: openhub_db::models::UpdateKnowledgeTagParams,
        ) -> Result<(), openhub_db::DbError> {
            Ok(())
        }
        async fn delete_knowledge_tag(&self, _key: &str) -> Result<(), openhub_db::DbError> {
            Ok(())
        }
    }
    ...
    fn knowledge_fixture(
    ...
    ) -> (Arc<openhub_knowledge::KnowledgeService>, tempfile::TempDir) {
        let data_dir = tempfile::TempDir::new().unwrap();
        let repo = MemKbRepo::default();
        repo.bases.lock().unwrap().insert(
            "kb_test".into(),
            openhub_db::models::KnowledgeBaseRow {
                id: "kb_test".into(),
        ...
        let emitter = openhub_knowledge::KnowledgeEventEmitter::new(Arc::new(
            CapturingBroadcaster::default(),
        ));
        (
            Arc::new(openhub_knowledge::KnowledgeService::new(
                Arc::new(repo),
                data_dir.path(),
                emitter,
            )),
    ...
    async fn create_with_kb_ids_binds_and_writes_readme() {
        ...
        let wp_key = openhub_knowledge::session_workpath_key(cwd.path(), &std::env::temp_dir());
    ...
    async fn create_with_kb_ids_binds_under_custom_workpath() {
        ...
        let key = openhub_knowledge::workpath_key(&cwd.path().to_string_lossy());
        assert_ne!(
            key, "__default__",
    ...
    async fn relaunch_resyncs_knowledge_and_rewrites_readme() {
        ...
        ks.set_binding(
            "terminal",
            &resp.id.to_string(),
            openhub_knowledge::KnowledgeBinding::default(),
        )
        .await
        .unwrap();
        svc.relaunch(resp.id).await.unwrap();
    ...
    async fn browse_workspace_unknown_id_is_not_found() {
        ...
        assert!(matches!(
            ...
            openhub_common::AppError::NotFound(_)
        ));
    }
    ...
    async fn browse_workspace_rejects_parent_traversal() {
        ...
        let err = svc
            ...
            .unwrap_err();
        assert!(
            matches!(err, openhub_common::AppError::BadRequest(_)),
    ...
    async fn with_knowledge_mcp_config_is_stored() {
        let (svc, _bc) = service();
        let cfg = openhub_api_types::KnowledgeMcpConfig {
            port: 51123,
            token: "tok".into(),
            binary_path: "/opt/openhub/nomicore".into(),
        };
        svc.with_knowledge_mcp_config(cfg.clone(), std::env::temp_dir().join("openhub-term-mcp"));
    ...
    async fn spawn_injects_knowledge_mcp_for_claude_when_kb_ids_present() {
        ...
        let enh = TerminalLaunchEnhancement {
            mcp_servers: vec![McpServerSpec {
                name: openhub_api_types::KnowledgeMcpConfig::SERVER_NAME.into(),
                command: cfg.binary_path.clone(),
                args: vec!["mcp-knowledge-stdio".into()],
                env: HashMap::from([
                    (
                        openhub_api_types::KnowledgeMcpConfig::ENV_PORT.into(),
                        "9".into(),
                    ),
                    (
                        openhub_api_types::KnowledgeMcpConfig::ENV_TOKEN.into(),
                        "t".into(),
                    ),
                ]),
            }],
    ...
    async fn with_requirement_mcp_config_is_stored() {
        let (svc, _bc) = service();
        let cfg = openhub_api_types::RequirementMcpConfig {
            port: 52222,
            token: "req-tok".into(),
            binary_path: "/opt/openhub/nomicore".into(),
        };
    ...
    async fn build_enhancement_knowledge_and_requirement_coexist() {
        use openhub_api_types::{KnowledgeMcpConfig as K, RequirementMcpConfig as R};
        let (svc, _bc) = service();
        svc.with_knowledge_mcp_config(
            K {
                port: 111,
