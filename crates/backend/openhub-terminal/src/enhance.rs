...
pub struct McpServerSpec {
    /// Wire-level server name (e.g. "openhub-knowledge"). Must be a bare-key-safe
...
impl AgentCli {
    ...
    /// structured turn-end signal (see `openhub-requirement`'s orchestrator —
...
/// `openhub-knowledge`). The file lives in the platform's session-private dir,
...
mod tests {
    ...
    fn codex_renderer_emits_c_overrides_preserving_user_config() {
        ...
        // 形如 -c mcp_servers.openhub-knowledge.command="..." -c ...args=[...] -c ...env.K="V"
    ...
    fn apply_enhancement_dispatches_by_cli_and_noops_unknown() {
        ...
        assert!(out.iter().any(|a| a.starts_with("mcp_servers.openhub-knowledge")));
        assert!(env.is_empty());
    ...
    fn codex_skips_non_bare_key_safe_name_and_emits_safe_ones() {
        ...
        assert!(joined.contains("mcp_servers.openhub-knowledge.command="));
    }
