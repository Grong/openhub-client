...
//! the provider-backed `LiveTerminalTitleCompleter` lives in `openhub-ai-agent`,
//! and `openhub-app` wires it. A terminal with no completer (tests / webui-only)
...
/// `openhub-ai-agent` (`LiveTerminalTitleCompleter`) over the default provider/
