...
async fn speech_to_text(
    ...
    let config: SpeechToTextConfig = prefs
        ...
        .unwrap_or(SpeechToTextConfig {
            enabled: false,
            provider: openhub_api_types::SpeechToTextProvider::Openai,
            auto_send: None,
            openai: None,
            deepgram: None,
        });
...
mod tests {
    ...
    fn make_state() -> ShellRouterState {
        ...
        let repo = Arc::new(openhub_db::SqliteClientPreferenceRepository::new(pool));
        let client_pref_service = openhub_system::ClientPrefService::new(repo);
