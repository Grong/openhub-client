...
//! layered externally in openhub-app (mirrors the knowledge / webhook routes).
//!
...
use openhub_api_types::{ApiResponse, RegisterSecretRequest, SecretListItem};
...
mod tests {
    ...
    fn router_with_user(dir: &std::path::Path) -> Router {
        ...
        secret_routes(SecretRouterState::new(svc))
            ...
            // openhub-app; tests attach the extension so the Extension extractor resolves).
