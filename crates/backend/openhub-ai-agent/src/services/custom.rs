...
use openhub_common::{AppError, generate_prefixed_id};
use openhub_db::UpsertAgentMetadataParams;
use tracing::warn;
...
async fn probe_or_reject(req: &CustomAgentUpsertRequest, data_dir: &Path) -> Result<(), AppError> {
    ...
    if std::env::var("OPENHUB_BYPASS_PROBE").is_ok() {
        tracing::warn!("OPENHUB_BYPASS_PROBE set — skipping custom agent probe. Test-only.");
        return Ok(());
    }
