        ...
        match err {
            AppError::WorkspacePathEdgeWhitespaceRuntimeUnsupported(path) => Self {
                stream_error: AgentStreamErrorData {
                    ...
                    ownership: Some(AgentErrorOwnership::Openhub),
...
fn classify_upstream_detail(detail: &str) -> AgentSendError {
    ...
    let classified = classify_agent_lifecycle(&lower)
        ...
        .or_else(|| classify_openhub_state(&lower))
...
fn classify_openhub_state(lower: &str) -> Option<ClassifiedError> {
    if lower.contains("conversation is already processing") {
        return Some(ClassifiedError {
            ...
            code: AgentErrorCode::OpenhubConversationBusy,
            ownership: AgentErrorOwnership::Openhub,
            retryable: true,
...
mod tests {
    use super::*;
    use openhub_api_types::{AgentErrorResolutionKind, AgentErrorResolutionTarget};
    ...
    #[test]
    fn preserves_runtime_workspace_validation_as_structured_openhub_error() {
        ...
        assert_eq!(err.ownership(), Some(AgentErrorOwnership::Openhub));
        assert_eq!(
    ...
    fn classifies_provider_request_model_and_context_errors() {
        ...
        assert_classification(
            "API error 400: Invalid schema for function 'openhub_list_models': None is not of type 'array'",
            AgentErrorCode::UserLlmProviderInvalidToolSchema,
            AgentErrorOwnership::UserLlmProvider,
            AgentErrorResolutionKind::Retry,
        );
    ...
    #[test]
    fn classifies_openhub_conversation_busy_after_agent_and_provider_checks() {
        assert_classification(
            "Conflict: Conversation is already processing a message",
            AgentErrorCode::OpenhubConversationBusy,
            AgentErrorOwnership::Openhub,
