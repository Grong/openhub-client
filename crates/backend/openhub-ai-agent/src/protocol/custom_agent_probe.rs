...
use openhub_api_types::TryConnectCustomAgentResponse;
use openhub_common::{CommandSpec, EnvVar};
...
async fn acp_initialize(
    ...
    tokio::select! {
        ...
        res = connect => {
            ...
            // `openhub_runtime::Builder` so CPU stays clean.
            drop(protocol);
            Ok(())
        }
...
mod tests {
    ...
    async fn probe_returns_fail_cli_when_command_missing() {
        ...
        let resp = try_connect_custom_agent("openhub-definitely-does-not-exist-xyz", &[], &HashMap::new(), &tmp).await;
        match resp {
