...
fn append_launch_nudge(
    ...
    if open_injected {
        rule.push_str(
            ...
            application on the user's desktop, you MUST call the `open` tool (MCP server `openhub-open`). \
    ...
    if computer_injected {
        ...
        rule.push_str(
            ...
            `openhub-computer` MCP tools: call `snapshot` to get a numbered [ref] tree of windows \
    ...
    if browser_injected {
        ...
        rule.push_str(
            "[Driving the web] You can drive a managed Chromium browser with the `openhub-browser` \
...
/// (`openhub_knowledge::context::build_knowledge_context`, `PromptSection`
...
fn knowledge_mcp_server(
    cfg: &openhub_api_types::KnowledgeMcpConfig,
    kb_ids: &[String],
) -> McpServer {
    let env = vec![
        EnvVariable::new(
            openhub_api_types::KnowledgeMcpConfig::ENV_PORT.to_owned(),
            cfg.port.to_string(),
        ),
        EnvVariable::new(
            openhub_api_types::KnowledgeMcpConfig::ENV_TOKEN.to_owned(),
            cfg.token.clone(),
        ),
        EnvVariable::new(
            openhub_api_types::KnowledgeMcpConfig::ENV_KB_IDS.to_owned(),
            serde_json::to_string(kb_ids).unwrap_or_else(|_| "[]".to_owned()),
        ),
    ];
    let stdio = McpServerStdio::new(
        openhub_api_types::KnowledgeMcpConfig::SERVER_NAME,
        &cfg.binary_path,
    )
...
/// (`nomicore mcp-gateway-stdio`) forwards every `openhub_*` desktop tool call
...
mod tests {
    ...
    fn launch_nudge_only_when_open_or_computer_injected() {
        ...
        assert!(comp.contains("openhub-computer"), "{comp}");
    ...
    fn launch_nudge_emits_browser_rule_when_browser_injected() {
        ...
        assert!(br.contains("openhub-browser"), "{br}");
    ...
    fn knowledge_context_section_renders_mounts_and_writeback() {
        let mut config = AcpBuildExtra {
            knowledge_mounts: vec![openhub_api_types::KnowledgeMountInfo {
                id: "kb_1".into(),
                name: "领域知识".into(),
        ...
        assert!(readonly.contains(".openhub/knowledge/领域知识"));
    ...
    /// the assembler injects the `openhub-browser` stdio bridge spawning
    ...
    /// (`openhub-computer` ⊥ `openhub-browser`) and spawn distinct subcommands,
    ...
    fn knowledge_mount(id: &str) -> openhub_api_types::KnowledgeMountInfo {
        openhub_api_types::KnowledgeMountInfo {
            id: id.to_owned(),
            name: "领域知识".into(),
            description: "团队约定".into(),
            rel_path: format!(".openhub/knowledge/{id}"),
            toc: vec![],
            summary: None,
            live_sources: vec![],
        }
    ...
    fn knowledge_mcp_injected_only_with_config_and_bound_bases() {
        ...
        let kb_server = servers
            .iter()
            .find(|s| match s {
                McpServer::Stdio(s) => s.name == openhub_api_types::KnowledgeMcpConfig::SERVER_NAME,
                _ => false,
            })
        ...
        assert_eq!(
            env_val(openhub_api_types::KnowledgeMcpConfig::ENV_PORT).as_deref(),
            Some("41555")
        );
        assert_eq!(
            env_val(openhub_api_types::KnowledgeMcpConfig::ENV_TOKEN).as_deref(),
            Some("kb-tok")
        );
        ...
        assert!(
            !servers_b.iter().any(|s| matches!(
                s,
                McpServer::Stdio(s) if s.name == openhub_api_types::KnowledgeMcpConfig::SERVER_NAME
            )),
