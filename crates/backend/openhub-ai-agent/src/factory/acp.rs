...
use openhub_api_types::{AcpBuildExtra, SessionMcpServer, SessionMcpTransport};
use openhub_common::{AppError, CommandSpec};
    ...
    if meta.backend.as_deref() == Some("claude") {
        ...
        if !cc_switch_env.is_empty() {
            ...
            for (name, value) in &cc_switch_env {
                env.push(openhub_common::EnvVar {
                    name: name.clone(),
                    value: value.clone(),
                });
            }
...
mod tests {
    ...
    impl IMcpServerRepository for MockRepo {
        async fn list(&self) -> Result<Vec<McpServerRow>, openhub_db::DbError> {
            if self.fail {
                Err(openhub_db::DbError::Init("simulated".into()))
            } else {
                Ok(self.rows.clone())
            }
        }
        async fn find_by_id(&self, _id: i64) -> Result<Option<McpServerRow>, openhub_db::DbError> {
            unimplemented!()
        }
        async fn find_by_name(
            &self,
            _name: &str,
        ) -> Result<Option<McpServerRow>, openhub_db::DbError> {
            unimplemented!()
        }
        async fn list_by_ids_any(
            &self,
            ids: &[i64],
        ) -> Result<Vec<McpServerRow>, openhub_db::DbError> {
            if self.fail {
                return Err(openhub_db::DbError::Init("simulated".into()));
            }
            Ok(ids
                .iter()
        ...
        async fn create(
            &self,
            _params: openhub_db::CreateMcpServerParams<'_>,
        ) -> Result<McpServerRow, openhub_db::DbError> {
            unimplemented!()
        }
        async fn update(
            &self,
            _id: i64,
            _params: openhub_db::UpdateMcpServerParams<'_>,
        ) -> Result<McpServerRow, openhub_db::DbError> {
            unimplemented!()
        }
        ...
        async fn batch_upsert(
            &self,
            _servers: &[openhub_db::CreateMcpServerParams<'_>],
        ) -> Result<Vec<McpServerRow>, openhub_db::DbError> {
            unimplemented!()
        }
        async fn update_status(
            ...
            _id: i64,
            _status: &str,
            _last_connected: Option<openhub_common::TimestampMs>,
