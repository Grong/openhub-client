...
use openhub_db::models::McpServerRow;
use openhub_runtime::resolve_command_path;
...
pub(super) async fn build(
    ...
    if public_agent_id.is_some() {
        overrides.exposure = overrides
            .exposure
            .stricter(openhub_api_types::ExposureMode::PublicService);
    }
    ...
    // in openhub's build_system_prompt). Mirrors the old architecture's
    ...
    // companion companion threads), mirroring acp.rs. Grants the `openhub_*` desktop
    ...
    if overrides.desktop_gateway && overrides.gateway_mcp_config.is_none() {
        ...
        info!(
            ...
            "gateway_mcp: injected into desktopGateway openhub session"
        );
    }
    ...
    let knowledge_write_surface = if overrides.companion {
        openhub_knowledge::WriteSurface::Companion
    } else if overrides.channel_platform.is_some() {
        openhub_knowledge::WriteSurface::ExternalChannel
    } else {
        openhub_knowledge::WriteSurface::RegularChat
    };
    let knowledge_write_policy = openhub_knowledge::resolve_write_policy(
        knowledge_write_surface,
        &openhub_knowledge::KnowledgeBinding {
            enabled: true,
            ...
            // channel write-back permanently disabled on the openhub engine.
    ...
    let knowledge_write_enabled = !matches!(
        ...
        openhub_knowledge::WriteMode::Disabled
    );
    let knowledge_writeback_staged = matches!(
        knowledge_write_policy.mode,
        openhub_knowledge::WriteMode::Staged { .. }
    );

    // Knowledge bases: append the mounted-bases section (per-base TOC +
    // write-back contract) to the system prompt, so openhub-engine sessions
    ...
    let inject_subagent_hint = should_inject_subagent_hint(
        ...
        matches!(
            overrides.exposure,
            openhub_api_types::ExposureMode::PublicService
        ),
    );
    ...
    if !extra_mcp_servers.is_empty() {
        info!(
            ...
            "Injecting MCP servers into openhub session"
        );
    }
    ...
    let session_directory = deps.data_dir.join("openhub-sessions");
    ...
    let accept_owned =
        |mut session: openhub_agent::session::Session| -> Option<openhub_agent::session::Session> {
            if !openhub_agent::session::session_belongs_to(
                ...
                warn!(
                    ...
                    "Discarding stale openhub session (belongs to a prior conversation that reused this id); starting fresh"
                );
                return None;
            }
        ...
        match session_mgr.load(&ctx.conversation_id) {
            Ok(mut session) => {
                ...
                info!(
                    ...
                    "Loaded existing openhub session for resume"
                );
                accept_owned(session)
            }
            Err(_) => {
                // Fallback: old architecture stored sessions inside the workspace
                let legacy_dir = std::path::Path::new(&ctx.workspace).join(".openhub/sessions");
                ...
                match legacy_mgr.load(&ctx.conversation_id) {
                    Ok(mut session) => {
                        ...
                        info!(
                            ...
                            message_count = session.messages.len(),
                            sanitized_dropped = dropped,
                            "Loaded legacy openhub session from workspace"
                        );
                        accept_owned(session)
                    }
                    Err(e) => {
                        debug!(
                            conversation_id = %ctx.conversation_id,
                            error = %e,
                            "No existing openhub session found, starting fresh"
                        );
                        None
                    }
                }
    ...
    let computer_use_default = read_bool_pref(
        &deps,
        PREF_COMPUTER_USE,
        cfg!(feature = "computer-use") || env_flag("OPENHUB_COMPUTER_USE"),
    )
    .await;
    ...
    // tool regardless. `OPENHUB_BROWSER_USE` env forces it on for feature-less parity/testing.
    let browser_use_default = read_bool_pref(
        &deps,
        PREF_BROWSER_USE,
        cfg!(feature = "browser-use") || env_flag("OPENHUB_BROWSER_USE"),
    )
    .await;
    ...
    let browser_full_power_default = read_bool_pref(
        ...
        env_flag("OPENHUB_BROWSER_FULL_POWER"),
    )
    .await;
    ...
    // shared `openhub_secret::pet_vault_path` now ignores its key and routes every
    ...
    let browser_secret_vault = if browser_use_enabled {
        ...
        Some(crate::types::BrowserSecretVault {
            vault_path: openhub_secret::pet_vault_path(&deps.data_dir, &pet_key),
    ...
    let config = NomiResolvedConfig {
        ...
        session_directory,
        // 默认授权模式 = 全自动（yolo）。产品决策：所有 openhub 会话默认自动批准
        ...
        goal: overrides.goal.clone().map(|g| {
            openhub_agent::goal::runtime::GoalSpec::new(
                g.objective,
        ...
        // Owning conversation instance identity — the openhub manager stamps it
        // onto the session after build so a future reused id is rejected.
        owner_token: owner_token.clone(),
        // 进程内 Spawn 门控：本地桌面网关会话关闭（改走 openhub_spawn 可视化扇出）。
...
/// `SystemSettingsResponse::default().language` in `openhub-api-types`.
...
/// `zh-CN` selects Chinese (supported set lives in `openhub-system`).
...
/// (`openhub_knowledge::context::build_knowledge_context`,
/// `PromptSection` format) so openhub-engine sessions (companion companion threads
...
fn append_knowledge_context(
    ...
    let section = build_knowledge_context(
        ...
        &KnowledgeContextOptions {
            ...
            // The openhub engine registers the native knowledge_write tool whenever
...
pub(crate) const SUBAGENT_STANDARD_HINT: &str = "遇到可并行的独立子任务，或需要成体系拆解的复杂多步任务时，可用 `openhub_spawn(tasks)` 立即并行派发子 agent（每个子任务在右侧编排画布实时可见状态与转录），或用 `openhub_run_create(goal)` 让规划器把目标拆成有依赖关系的任务 DAG（可用模型范围与工作目录自动取用、随即开跑）。派发后拿到 run_id，直接告诉用户已在后台执行、进度可在右侧编排画布查看，然后结束本轮——不要自己轮询等待，也不要重复创建：子任务全部完成或失败时系统会自动把结果回执给你，届时再向用户汇总。简单或单步问题直接作答，无需派发。";
...
pub(crate) const CLUSTER_MODE_HINT: &str = "用户已为本会话显式开启「agent 集群」模式：对每一个任务（无论难度），你都必须先刻意评估是否应当用 openhub_spawn / openhub_run_create 开启多 agent 集群协作，并倾向于开启——多个独立 agent 各自拥有更充足的上下文，交付质量更高。只有当任务确实过于简单（单步可答、无可拆分的子任务）时才可不开启；此时必须在回复的开头先用一两句话向用户说明「本次使用简单模式」的原因，然后再直接作答。";

/// 是否给本会话追加常驻 subagent 提示（纯策略，可单测）。提示点名的 `openhub_spawn` /
/// `openhub_run_create` 工具只随桌面网关标配（`desktop_gateway`=true 的本地可信会话），
...
/// IM 渠道）禁用进程内 Spawn —— 子 agent 改走 openhub_spawn 编排扇出（每个子任务
/// 在 DAG 画布上有状态与转录，不再静默）；IM 渠道 master（openhub_spawn 对 Remote
...
pub(crate) fn resolve_native_write_root(
    exposure: openhub_api_types::ExposureMode,
    channel_platform: Option<&str>,
    workspace: &str,
) -> Option<String> {
    let is_channel = channel_platform.map(str::trim).is_some_and(|s| !s.is_empty());
    let is_local_desktop = !is_channel && matches!(exposure, openhub_api_types::ExposureMode::Private);
    if is_local_desktop {
        return None;
    }
...
pub(crate) fn map_openhub_provider(
    platform: &str,
    model_id: &str,
...
/// Resolve base_url and compat overrides for the openhub provider.
///
/// Mirrors the frontend `envBuilder.ts` logic:
/// - Strips trailing `/v1` from base_url (openhub appends its own path)
...
pub(crate) fn resolve_openhub_url_and_compat(
    platform: &str,
    raw_base_url: &str,
    mapped_provider: &str,
    is_full_url: bool,
    ...
    let normalized = normalize_openhub_base_url(raw_base_url);
...
/// openhub can append its own path suffix (`/v1/messages`, `/v1/chat/completions`).
fn normalize_openhub_base_url(url: &str) -> String {
...
pub(crate) fn resolve_bedrock_config(
    json: Option<&str>,
) -> Option<openhub_config::config::BedrockConfig> {
    let bc: openhub_api_types::BedrockConfig = serde_json::from_str(json?).ok()?;
    Some(openhub_config::config::BedrockConfig {
...
/// Desktop Gateway MCP stdio bridge config for the openhub engine, mirroring the
...
mod tests {
    use super::*;
    use openhub_api_types::GuideMcpConfig;
    ...
    /// factory/acp.rs.
    struct MockSettingsRepo(Result<Option<openhub_db::models::SystemSettings>, ()>);

    #[async_trait::async_trait]
    impl ISettingsRepository for MockSettingsRepo {
        async fn get_settings(
            &self,
        ) -> Result<Option<openhub_db::models::SystemSettings>, openhub_db::DbError> {
            self.0
                .clone()
                .map_err(|_| openhub_db::DbError::Init("simulated".into()))
        }
        ...
        ) -> Result<openhub_db::models::SystemSettings, openhub_db::DbError> {
            unimplemented!("not exercised by the language tests")
        }
    }

    fn settings_row(language: &str) -> openhub_db::models::SystemSettings {
        openhub_db::models::SystemSettings {
            id: 1,
    ...
    fn settings_repo(
        result: Result<Option<openhub_db::models::SystemSettings>, ()>,
    ...
    #[test]
    fn normalize_openhub_base_url_strips_v1() {
        assert_eq!(
            normalize_openhub_base_url("https://api.openai.com/v1"),
        ...
        assert_eq!(
            normalize_openhub_base_url("https://api.anthropic.com"),
            "https://api.anthropic.com"
        );
        assert_eq!(
            normalize_openhub_base_url("https://api.deepseek.com/"),
            "https://api.deepseek.com"
        );
        assert_eq!(
            normalize_openhub_base_url("http://localhost:11434"),
            "http://localhost:11434"
        );
        assert_eq!(normalize_openhub_base_url(""), "");
    }

    #[test]
    fn map_openhub_provider_known_platforms() {
        assert_eq!(map_openhub_provider("anthropic", "m", None), "anthropic");
        assert_eq!(map_openhub_provider("bedrock", "m", None), "bedrock");
        assert_eq!(map_openhub_provider("gemini-vertex-ai", "m", None), "vertex");
    }

    #[test]
    fn map_openhub_provider_custom_and_others_default_to_openai() {
        assert_eq!(map_openhub_provider("custom", "gpt-4o", None), "openai");
        assert_eq!(
            map_openhub_provider("gemini", "gemini-2.5-pro", None),
            "openai"
        );
        assert_eq!(map_openhub_provider("new-api", "m", None), "openai");
        assert_eq!(map_openhub_provider("unknown", "m", None), "openai");
    }

    #[test]
    fn map_openhub_provider_new_api_with_anthropic_protocol() {
        let protocols = r#"{"claude-sonnet":"anthropic","gpt-4o":"openai"}"#;
        assert_eq!(
            map_openhub_provider("new-api", "claude-sonnet", Some(protocols)),
            "anthropic"
        );
        assert_eq!(
            map_openhub_provider("new-api", "gpt-4o", Some(protocols)),
            "openai"
        );
        assert_eq!(
            map_openhub_provider("new-api", "unknown-model", Some(protocols)),
            "openai"
        );
    }

    #[test]
    fn map_openhub_provider_new_api_with_invalid_json() {
        assert_eq!(
            map_openhub_provider("new-api", "m", Some("not json")),
            "openai"
        );
    }

    #[test]
    fn map_openhub_provider_non_new_api_ignores_protocols() {
        let protocols = r#"{"m":"anthropic"}"#;
        assert_eq!(map_openhub_provider("custom", "m", Some(protocols)), "openai");
    }
    ...
    fn resolve_openai_official_sets_max_completion_tokens() {
        let (base_url, compat) =
            resolve_openhub_url_and_compat("custom", "https://api.openai.com/v1", "openai", false);
    ...
    fn resolve_non_openai_keeps_default_max_tokens() {
        let (base_url, compat) =
            resolve_openhub_url_and_compat("custom", "https://api.deepseek.com/v1", "openai", false);
    ...
    fn resolve_gemini_prepends_path_and_sets_api_path() {
        let (base_url, compat) = resolve_openhub_url_and_compat(
            "gemini",
    ...
    fn resolve_domestic_openai_compatible_platforms_use_configured_chat_base() {
        for (platform, base) in [
            ...
            let (base_url, compat) = resolve_openhub_url_and_compat(platform, base, "openai", false);
    ...
    fn subagent_hint_injects_for_plain_desktop_session() {
        ...
        assert!(s.contains("openhub_spawn"));
        assert!(s.contains("openhub_run_create"));
        // 未开集群模式 → 不含集群增强段
    ...
    fn subagent_hint_skips_when_gateway_absent() {
        // 无桌面网关（如远程 WebUI 未授信）→ 不注入：提示点名的 openhub_spawn /
        // openhub_run_create 工具此时不可达，注入只会成为空头支票。
    ...
    fn native_write_root_unrestricted_only_for_local_desktop() {
        use openhub_api_types::ExposureMode;
    ...
    fn append_knowledge_context_renders_mounts_and_writeback() {
        ...
        let mut config = NomiBuildExtra {
            knowledge_mounts: vec![KnowledgeMountInfo {
                ...
                rel_path: ".openhub/knowledge/领域知识".into(),
        ...
        // openhub surface has the native tool → write-back contract points at it,
    ...
    fn knowledge_fields_deserialize_from_extra_and_reach_prompt() {
        ...
        // JSON; the openhub build path must surface them in the system prompt.
        let json = serde_json::json!({
            "knowledge_mounts": [{
                "id": "kb1",
                "name": "运维手册",
                "description": "",
                "rel_path": ".openhub/knowledge/运维手册",
                "toc": ["deploy.md — 部署"],
            }],
    ...
    fn channel_write_opt_in_threads_from_extra_into_write_policy() {
        ...
        // build-extra round-trip so the openhub factory can resolve the
        ...
        // channel write-back was permanently Disabled on the openhub engine.
