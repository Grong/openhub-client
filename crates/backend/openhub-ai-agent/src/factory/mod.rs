...
use openhub_agent::companion_tools::{CompanionMemorySink, CompanionSkillSink};
use openhub_agent::requirement_tools::RequirementSink;
...
/// frozen forever. Implemented by `openhub-companion::CompanionService`.
...
/// stays free of the `openhub-public-agent` config types (which depend on this
...
pub struct PublicAgentRuntime {
    ...
    pub model: openhub_common::ProviderWithModel,
}

/// Resolves a public agent's LIVE runtime + records inbound turns for audit.
/// Implemented by `openhub_public_agent::PublicAgentService`. Read at every agent
...
pub struct AgentFactoryDeps {
    ...
    /// `OPENHUB_WORK_DIR` override must not split the two roots.
    ...
    pub knowledge_mcp_config: Option<openhub_api_types::KnowledgeMcpConfig>,
    ...
    /// companion companion threads) so the agent gets the `openhub_*` desktop tools.
    ...
    pub knowledge_writeback: Option<Arc<dyn openhub_agent::knowledge_tools::KnowledgeWritebackSink>>,
