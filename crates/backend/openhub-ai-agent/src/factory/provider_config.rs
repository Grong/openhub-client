...
//! `openhub_config::config::Config`, and a standalone one-shot LLM completion
...
pub(crate) async fn resolve_provider_fields(
    ...
    let api_key = openhub_common::decrypt_string(&row.api_key_encrypted, encryption_key)?;

    let provider = map_openhub_provider(&row.platform, model, row.model_protocols.as_deref());
...
mod tests {
    ...
    async fn drain_text_response_concatenates_deltas() {
        ...
        tx.send(LlmEvent::Done {
            ...
            usage: openhub_types::message::TokenUsage::default(),
        })
        .await
        .unwrap();
...
mod fallback_tests {
    ...
    use openhub_db::{CreateProviderParams, DbError, UpdateProviderParams};
    ...
    fn provider(id: &str, enabled: bool, models: &[&str], model_enabled: Option<&str>) -> Provider {
        Provider {
            ...
            name: id.into(),
            base_url: String::new(),
            api_key_encrypted: openhub_common::encrypt_string("sk-test", &[0u8; 32])
