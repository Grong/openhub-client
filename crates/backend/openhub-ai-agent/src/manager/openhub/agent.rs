...
use openhub_agent::bootstrap::AgentBootstrap;
use openhub_agent::companion_tools::{
...
use openhub_agent::engine::AgentEngine;
use openhub_agent::knowledge_tools::{KnowledgeReadTool, KnowledgeSearchTool, KnowledgeWriteTool};
use openhub_agent::output::OutputSink;
use openhub_agent::requirement_tools::{RequirementCompleteTool, RequirementSink, RequirementUpdateStatusTool};
...
use openhub_api_types::{AgentModeResponse, SlashCommandItem};
use openhub_common::{
...
pub struct NomiAgentManager {
    ...
    distill_cfg: Arc<openhub_config::config::Config>,
...
pub(crate) fn prepend_knowledge_context(
    hits: &[openhub_agent::knowledge_tools::KnowledgeHit],
    content: String,
) -> String {
    if hits.is_empty() {
        return content;
...
/// Normalize the engine's [`StopReason`](openhub_types::message::StopReason) into
...
pub(crate) fn map_engine_stop_reason(
    reason: openhub_types::message::StopReason,
) -> TurnStopReason {
    use openhub_types::message::StopReason;
    match reason {
...
impl NomiAgentManager {
    pub async fn new(
        ...
        knowledge_retrieval_sink: Option<Arc<dyn openhub_agent::knowledge_tools::KnowledgeRetrievalSink>>,
        knowledge_kb_ids: Vec<String>,
        knowledge_prelude: Option<String>,
        knowledge_writeback_sink: Option<Arc<dyn openhub_agent::knowledge_tools::KnowledgeWritebackSink>>,
        ...
        let distill_dir: Option<PathBuf> = if companion_sink.is_some() {
            ...
            openhub_memory::paths::auto_memory_dir(std::path::Path::new(&workspace))
        };
        ...
        config.compact.context_window = openhub_config::compact::resolve_context_window(
        ...
        if let Some(sink) = knowledge_writeback_sink {
            if register_knowledge_write {
                let mode = if knowledge_writeback_staged {
                    openhub_agent::knowledge_tools::WriteMode::Staged { scope: conversation_id.clone() }
                } else {
                    openhub_agent::knowledge_tools::WriteMode::Direct
                };
        ...
        let result = loop {
            ...
            if let Some(Ok(agent_result)) = &r {
                let reason = match agent_result.stop_reason {
                    openhub_types::message::StopReason::MaxTokens => Some("the output token limit"),
                    openhub_types::message::StopReason::MaxTurns => Some("the per-turn request cap"),
                    _ => None,
                };
    ...
    pub fn confirm(&self, _msg_id: &str, call_id: &str, data: Value, always_allow: bool) -> Result<(), AppError> {
        ...
        if is_cancel {
            ...
            let scope = if always_allow {
                openhub_protocol::commands::ApprovalScope::Always
            } else {
                openhub_protocol::commands::ApprovalScope::Once
            };
...
fn openhub_engine_error_to_send_error(error_msg: String) -> AgentSendError {
...
mod tests {
    ...
    fn make_test_config() -> NomiResolvedConfig {
        NomiResolvedConfig {
            ...
            session_directory: std::env::temp_dir().join("openhub-test-sessions"),
            session_mode: None,
    ...
    async fn max_tokens_continuation_prompt_forbids_repeating_large_write() {
        ...
        while let Ok(event) = rx.try_recv() {
            if let AgentStreamEvent::ToolCall(data) = event
                && data.call_id == "openhub-call-large-write"
    ...
    #[async_trait::async_trait]
    impl openhub_agent::companion_tools::CompanionMemorySink for StubCompanionSink {
    ...
    fn openhub_provider_connection_error_is_user_llm_provider_error() {
        ...
        assert_eq!(
            send_error.code(),
            Some(openhub_api_types::AgentErrorCode::UserLlmProviderConfigError)
        );
        assert_eq!(
            send_error.ownership(),
            Some(openhub_api_types::AgentErrorOwnership::UserLlmProvider)
        );
    ...
    fn openhub_api_connection_error_is_user_llm_provider_network_error() {
        ...
        assert_eq!(
            send_error.code(),
            Some(openhub_api_types::AgentErrorCode::UserLlmProviderNetworkError)
        );
        assert_eq!(
            send_error.ownership(),
            Some(openhub_api_types::AgentErrorOwnership::UserLlmProvider)
...
mod knowledge_prelude_tests {
    ...
    fn prepend_knowledge_context_formats_hits_and_passthrough_when_empty() {
        use openhub_agent::knowledge_tools::KnowledgeHit;
