...
/// avoids surprising users with unexpected spend. openhub-config has no memory
/// section today, so an env flag is the lowest-risk gate (same pattern as the
/// `OPENHUB_COMPUTER_USE` / `OPENHUB_BROWSER_USE` host flags).
const DISTILL_ENABLED_ENV: &str = "OPENHUB_MEMORY_DISTILL";
...
pub async fn run_distill(cfg: Arc<Config>, dir: PathBuf, transcript: String) {
    ...
    // Write on a blocking thread (synchronous fs in openhub-memory).
