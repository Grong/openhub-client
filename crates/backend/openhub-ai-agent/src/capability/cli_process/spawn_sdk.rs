use openhub_common::{AppError, CommandSpec, ErrorChain};
use openhub_runtime::Builder as CmdBuilder;
use std::path::Path;
use std::sync::Arc;
...
impl CliAgentProcess {
    ...
    pub async fn spawn_for_sdk(config: CommandSpec, data_dir: &Path) -> Result<Self, AppError> {
        let proxy_env =
            openhub_net::proxy::child_proxy_env(config.env.iter().map(|e| e.name.as_str()));
    ...
    fn agent_spawn_env(data_dir: &Path) -> Vec<(String, String)> {
        ...
        // `openhub_runtime::enhance_process_path` during startup; children
    ...
    /// to `openhub_runtime::resolve_command_in`, which honours `PATHEXT` on
    ...
    fn find_native_claude() -> Option<String> {
        ...
        for dir in std::env::split_paths(&path_var) {
            ...
            if let Some(found) = openhub_runtime::resolve_command_in("claude", &dir) {
