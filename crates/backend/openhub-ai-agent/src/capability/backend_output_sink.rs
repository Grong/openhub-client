...
pub struct BackendOutputSink {
    ...
    /// Accumulates this turn's assistant text so the `<openhub-mem-citation>`
...
impl BackendOutputSink {
    ...
    /// Citation reflow: parse the `<openhub-mem-citation>` block from the turn's
    ...
    fn reflow_citations(&self, full_text: &str) {
        ...
        for fname in openhub_memory::distill::parse_citation_filenames(full_text) {
            if let Err(e) = openhub_memory::store::bump_memory_usage(dir, &fname, now) {
...
mod tests {
    ...
    fn emit_tool_call_delta_sends_running_tool_call_with_preview_input() {
        ...
        match event {
            AgentStreamEvent::ToolCall(data) => {
                assert_eq!(data.call_id, "openhub-call_write_1");
    ...
    fn duplicate_tool_names_use_distinct_internal_call_ids() {
        ...
        assert_eq!(call_ids[0].0, "openhub-call_a");
        assert_eq!(call_ids[1].0, "openhub-call_b");
        assert_eq!(call_ids[2].0, "openhub-call_a");
        assert_eq!(call_ids[3].0, "openhub-call_b");
    ...
    fn citation_reflow_bumps_cited_file_on_stream_end() {
        use openhub_memory::store::{read_memory, write_memory};
        use openhub_memory::types::{MemoryEntry, MemoryType};
    ...
    fn no_distill_dir_means_no_reflow_and_no_accumulation() {
        ...
        sink.emit_stream_start("m1");
        sink.emit_text_delta("<openhub-mem-citation>\nuser_role.md|note=[x]\n</openhub-mem-citation>", "m1");
        sink.emit_stream_end("m1", 1, 10, 5, 0, 0);
