...
//! `LiveKnowledgeCompleter`: the trait lives in `openhub-agent`, this is the
//! backend impl wired in at app startup.

use std::sync::Arc;

use async_trait::async_trait;
use openhub_agent::knowledge_tools::{KnowledgeHit, KnowledgeRetrievalSink};
...
impl KnowledgeRetrievalSink for LiveKnowledgeRetrievalSink {
    async fn search(&self, kb_ids: &[String], query: &str, limit: usize) -> Result<Vec<KnowledgeHit>, String> {
        ...
        Ok(hits
            .into_iter()
            .map(|h| KnowledgeHit {
                handle: openhub_knowledge::encode_doc_handle(&h.kb_id, &h.rel_path),
                kb_id: h.kb_id,
                kb_name: h.kb_name,
                rel_path: h.rel_path,
                heading: h.heading,
    ...
    async fn read_document(&self, kb_ids: &[String], handle: &str) -> Result<String, String> {
        let (kb_id, rel_path) =
            openhub_knowledge::decode_doc_handle(handle).ok_or_else(|| format!("invalid handle: {handle}"))?;
