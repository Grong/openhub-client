...
//! (`openhub-browser-engine`). Same late-wire layering as [`crate::knowledge_completer`]:
//! the trait ([`openhub_knowledge::PageFetcher`]) lives in the knowledge crate,
...
//!
//! **Why a browser backend at all** — the default [`openhub_knowledge::HttpFetcher`]
...
//! discipline as `openhub_browser::BrowserTool`).
//!
...
use openhub_knowledge::PageFetcher;
use openhub_knowledge::source_url::{FETCH_MAX_BYTES, FetchedPage, html_to_markdown, truncate_to_bytes};
use tokio::sync::Mutex;

use openhub_browser_engine::firewall::FirewallConfig;
use openhub_browser_engine::{BrowserEngine, EngineConfig, create_engine};
...
mod tests {
    ...
    //   cargo nextest run -p openhub-ai-agent --features browser-use --run-ignored all \
    ...
    async fn js_rendered_page_is_captured_after_render() {
        ...
        let (_t, static_md) =
            openhub_knowledge::source_url::html_to_markdown(&html);
        assert!(
