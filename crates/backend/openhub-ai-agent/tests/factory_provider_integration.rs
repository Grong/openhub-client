...
use openhub_ai_agent::AcpSessionSyncService;
use openhub_ai_agent::AcpSkillManager;
use openhub_ai_agent::factory::{AgentFactoryDeps, build_agent_factory};
use openhub_ai_agent::registry::AgentRegistry;
use openhub_ai_agent::types::BuildTaskOptions;
use openhub_common::{AgentType, ProviderWithModel, encrypt_string};
use openhub_db::{
...
fn make_factory(
...
) -> openhub_ai_agent::task_manager::AgentFactory {
    let tmp = tempfile::TempDir::new().unwrap();
    let skill_paths = Arc::new(openhub_extension::resolve_skill_paths(tmp.path(), tmp.path()));
    build_agent_factory(AgentFactoryDeps {
        ...
        agent_registry,
        acp_agent_service,
        data_dir: PathBuf::from("/tmp/openhub-test"),
        work_dir: PathBuf::from("/tmp/openhub-test"),
        backend_binary_path: Arc::new(PathBuf::from("/tmp/openhub-test/nomicore")),
...
async fn openhub_factory_returns_unavailable_when_no_providers_configured() {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn openhub_factory_falls_back_to_first_enabled_when_bound_provider_missing() {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn openhub_factory_resolves_provider_from_db() {
    let (provider_repo, remote_agent_repo, agent_registry, acp_agent_service) = setup().await;
    insert_test_provider(&*provider_repo, "prov-001", "openai").await;
    let factory = make_factory(provider_repo, remote_agent_repo, agent_registry, acp_agent_service);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn openhub_factory_respects_use_model_override() {
    let (provider_repo, remote_agent_repo, agent_registry, acp_agent_service) = setup().await;
