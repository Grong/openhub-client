...
use openhub_ai_agent::capability::prompt_pipeline::{PromptCtx, PromptPipeline};
use openhub_ai_agent::factory::acp_assembler::{AcpSessionParams, WorkspaceInfo, assemble_acp_params};
use openhub_ai_agent::manager::acp::{
...
use openhub_ai_agent::shared_kernel::ModelId;
use openhub_ai_agent::{AcpBuildExtra, AcpSkillManager, AgentRuntime};
...
fn fixture_skill_manager() -> Arc<AcpSkillManager> {
    ...
    let paths = Arc::new(openhub_extension::resolve_skill_paths(tmp.path(), tmp.path()));
