...
use openhub_ai_agent::factory::acp_assembler::{WorkspaceInfo, assemble_acp_params};
use openhub_ai_agent::manager::acp::AcpAgentManager;
use openhub_ai_agent::registry::AgentRegistry;
use openhub_ai_agent::{AgentInstance, AgentStreamEvent, IAgentTask};
use openhub_common::ConversationStatus;
use openhub_db::{SqliteAgentMetadataRepository, init_database_memory};
...
async fn make_mock_agent(script: &str, backend: &str) -> (Arc<AcpAgentManager>, broadcast::Receiver<AgentStreamEvent>) {
    ...
    let config = openhub_ai_agent::AcpBuildExtra {
    ...
    let skill_paths = std::sync::Arc::new(openhub_extension::resolve_skill_paths(
        tmp_skills.path(),
        tmp_skills.path(),
    ));
    let skill_manager = openhub_ai_agent::AcpSkillManager::new(skill_paths);
...
fn acp_build_extra_populates_skills_from_extra_json() {
    ...
    let extra: openhub_ai_agent::AcpBuildExtra = serde_json::from_value(json).unwrap();
...
async fn acp_agent_type_is_acp() {
    ...
    assert_eq!(agent.agent_type(), openhub_common::AgentType::Acp);
...
async fn acp_agent_kill_terminates_process() {
    ...
    agent.kill(Some(openhub_common::AgentKillReason::IdleTimeout)).unwrap();
