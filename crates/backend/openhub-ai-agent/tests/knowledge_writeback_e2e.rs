...
use openhub_agent::knowledge_tools::{
...
/// `openhub_realtime` ships no public no-op broadcaster, so define a local one
...
async fn staged_write_tool_with_mount_prefixed_path_lands_in_inbox_not_nested() {
    ...
    let sink: Arc<dyn KnowledgeWritebackSink> =
        Arc::new(openhub_ai_agent::LiveKnowledgeWritebackSink { service: svc.clone() });
    ...
    let res = tool
        .execute(json!({
            "base": "领域库",
            "rel_path": ".openhub/knowledge/领域库/terms.md",
    ...
    assert!(
        !files.iter().any(|f| f.rel_path.contains(".openhub/knowledge")),
...
async fn search_read_write_handle_loop_updates_original_in_direct_mode() {
    ...
    let retrieval: Arc<dyn KnowledgeRetrievalSink> =
        Arc::new(openhub_ai_agent::LiveKnowledgeRetrievalSink { service: svc.clone() });
