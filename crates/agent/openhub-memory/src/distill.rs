...
//! `openhub-ai-agent` (it owns the provider and the tokio runtime). Keeping
...
/// Parse the filenames cited inside a `<openhub-mem-citation>` block in the
...
pub fn parse_citation_filenames(text: &str) -> Vec<String> {
    const OPEN: &str = "<openhub-mem-citation>";
    const CLOSE: &str = "</openhub-mem-citation>";
