...
//! `.openhub/skills/` and `.openhub/commands/` layout, then exercises the full
...
use openhub_agent::context::{SystemPromptCache, build_system_prompt};
use openhub_agent::skill_tool::SkillTool;
use openhub_agent::skills::loader::load_all_skills;
use openhub_agent::skills::permissions::SkillPermissionChecker;
use openhub_agent::skills::types::SkillMetadata;
...
/// Create a project-like temp directory with `.git` marker and `.openhub/skills/` + `.openhub/commands/`.
...
async fn e2_legacy_commands_discovered() {
    ...
    println!("E2 PASS: legacy command 'legacy-cmd' discovered from .openhub/commands/");
}
