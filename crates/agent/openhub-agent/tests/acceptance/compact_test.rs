...
use openhub_agent::compact::auto::{BOUNDARY_PREFIX, autocompact, should_autocompact};
use openhub_agent::compact::emergency::is_at_emergency_limit;
use openhub_agent::compact::micro::{CLEARED_TOOL_RESULT, microcompact};
use openhub_agent::compact::state::CompactState;
...
async fn autocompact_triggers_llm_summary() {
    ...
    let provider = openhub_providers::create_provider(&config);
