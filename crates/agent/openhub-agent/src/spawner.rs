...
use openhub_tools::bash::BashTool;
use openhub_tools::edit::EditTool;
use openhub_tools::glob::GlobTool;
use openhub_tools::grep::GrepTool;
use openhub_tools::read::ReadTool;
use openhub_tools::registry::ToolRegistry;
use openhub_tools::write::WriteTool;
use openhub_types::message::TokenUsage;

use crate::engine::AgentEngine;
use crate::output::OutputSink;
use crate::output::null_sink::NullSink;

// Re-export from openhub-types — single source of truth
pub use openhub_types::spawner::{ForkOverrides, Spawner, SubAgentConfig, SubAgentResult};
...
impl AgentSpawner {
    ...
    async fn spawn_one_with_board(
        ...
        tracing::info!(target: "openhub_agent", cwd = %self.cwd.display(), "sub-agent spawned with workspace cwd");
    ...
    pub async fn spawn_parallel_isolated(&self, sub_configs: Vec<SubAgentConfig>) -> Vec<SubAgentResult> {
        if !openhub_tools::worktree::is_git_repo(&self.cwd) {
            tracing::warn!(target: "openhub_agent", "isolate requested but workspace is not a git repo — running without worktree isolation");
        ...
        let tasks: Vec<_> = sub_configs
            ...
            .map(|config| {
                ...
                async move {
                    match openhub_tools::worktree::Worktree::create(&repo) {
                        Ok(wt) => {
...
fn build_tool_registry(
    ...
    let all_tools: Vec<(&str, Box<dyn openhub_tools::Tool>)> = vec![
...
mod phase7_tests {
    ...
    use openhub_types::message::{StopReason, TokenUsage};
