...
//! bound bases; standalone `openhub-cli` passes `None` and the tool is absent.
//!
//! Mirrors `requirement_tools.rs`: trait here, impl in `openhub-ai-agent`.
...
/// `KnowledgeService`; `openhub-agent` only depends on this trait.
#[async_trait]
...
/// `KnowledgeService::write_file`; `openhub-agent` only depends on this trait.
///
...
/// Backend seam for knowledge write-back. Implemented by the backend over its
/// `KnowledgeService::write_document`; `openhub-agent` only depends on this trait.
