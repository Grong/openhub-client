...
use openhub_config::compact::CompactConfig;
use openhub_config::config::Config;
use openhub_config::hooks::HookEngine;
use openhub_protocol::events::ToolCategory;
use openhub_providers::{LlmProvider, ProviderError, create_provider};
use openhub_tools::registry::ToolRegistry;
use openhub_types::llm::{LlmEvent, LlmRequest};
use openhub_types::message::{ContentBlock, Message, Role, StopReason, TokenUsage};
use openhub_types::skill_types::{ContextModifier, PlanModeTransition, effort_to_string};
...
/// guard. See docs/superpowers/specs/2026-06-21-openhub-agent-overhaul-design.md §5 F0.3.
...
pub struct AgentEngine {
    ...
    thinking: Option<openhub_types::llm::ThinkingConfig>,
    /// Resolved provider compat settings (for capability validation)
    compat: openhub_config::compat::ProviderCompat,
    ...
    protocol_writer: Option<Arc<dyn openhub_protocol::writer::ProtocolEmitter>>,
    ...
    compaction_level: openhub_compact::CompactionLevel,
    toon_enabled: bool,
...
impl AgentEngine {
    ...
    pub fn compaction_level(&self) -> openhub_compact::CompactionLevel {
    ...
    pub fn compat(&self) -> &openhub_config::compat::ProviderCompat {
        &self.compat
    }
    ...
    pub fn init_session(
        ...
        if let Some(mgr) = &self.session_manager {
            ...
            tracing::info!(target: "openhub_agent", session_id = %session.id, provider = %provider_name, model = %self.model, "session started");
    ...
    pub fn set_approval_manager(&mut self, mgr: Arc<openhub_protocol::ToolApprovalManager>) {
        self.approval_manager = Some(mgr);
    }

    pub fn set_protocol_writer(&mut self, writer: Arc<dyn openhub_protocol::writer::ProtocolEmitter>) {
    ...
    pub fn apply_config_update(
        ...
        if let Some(thinking_str) = thinking {
            if !self.compat.supports_thinking() {
                ...
                match thinking_str.as_str() {
                    "enabled" => {
                        ...
                        self.thinking = Some(openhub_types::llm::ThinkingConfig::Enabled {
                            budget_tokens: budget,
                        });
                        changes.push(format!("thinking: enabled (budget: {budget})"));
                    }
                    "disabled" => {
                        self.thinking = Some(openhub_types::llm::ThinkingConfig::Disabled);
            ...
            && let Some(openhub_types::llm::ThinkingConfig::Enabled { budget_tokens }) =
                &mut self.thinking
        {
        ...
        if let Some(ref level_str) = compaction {
            match level_str.parse::<openhub_compact::CompactionLevel>() {
                Ok(new_level) => {
    ...
    async fn run_compaction(&mut self) -> Result<(), AgentError> {
        ...
        if should_compact {
            tracing::info!(target: "openhub_agent", last_input_tokens = self.compact_state.last_input_tokens, "context compaction triggered");
    ...
    pub async fn run_stop_hooks(&self) {
        if let Some(hook_engine) = &self.hooks {
            ...
            for msg in messages {
                tracing::info!(target: "openhub_agent", hook_message = %msg, "stop hook output");
            }
        }
    }
...
mod set_config_tests {
    ...
    use openhub_tools::registry::ToolRegistry;
    use openhub_types::llm::{LlmEvent, LlmRequest};
    use openhub_types::message::{ContentBlock, Role};
    ...
    impl LlmProvider for LoopProvider {
        async fn stream(
            ...
            let _ = tx
                .send(LlmEvent::Done {
                    stop_reason: openhub_types::message::StopReason::ToolUse,
    ...
    impl LlmProvider for SlowProvider {
        async fn stream(
            ...
            tokio::spawn(async move {
                ...
                let _ = tx
                    .send(LlmEvent::Done {
                        stop_reason: openhub_types::message::StopReason::EndTurn,
                        usage: Default::default(),
                    })
                    .await;
            });
    ...
    fn make_engine(model: &str) -> super::AgentEngine {
        super::AgentEngine {
            ...
            thinking: None,
            compat: openhub_config::compat::ProviderCompat::anthropic_defaults(),
            ...
            allow_list: vec![],
            current_reasoning_effort: None,
            compact_config: openhub_config::compact::CompactConfig::default(),
            compact_state: super::CompactState::new(),
            plan_state: Default::default(),
            plan_active_flag: None,
            cache_detector: super::CacheBreakDetector::new(),
            compaction_level: openhub_compact::CompactionLevel::default(),
            toon_enabled: false,
            max_recent_images: 3,
            commands: crate::commands::default_registry(),
            goal: None,
    ...
    async fn safety_net_caps_unbounded_loop() {
        ...
        assert_eq!(result.stop_reason, openhub_types::message::StopReason::MaxTurns);
    ...
    fn set_config_changes_effort() {
        let mut engine =
            make_engine_with_compat("m", openhub_config::compat::ProviderCompat::openai_defaults());
    ...
    fn set_config_enables_thinking() {
        ...
        match &engine.thinking {
            Some(openhub_types::llm::ThinkingConfig::Enabled { budget_tokens }) => {
    ...
    fn set_config_disables_thinking() {
        let mut engine = make_engine("m");
        engine.thinking = Some(openhub_types::llm::ThinkingConfig::Enabled {
            budget_tokens: 8000,
        });
        let changes = engine.apply_config_update(None, Some("disabled".into()), None, None, None);
        match &engine.thinking {
            Some(openhub_types::llm::ThinkingConfig::Disabled) => {}
    ...
    fn set_config_all_fields_at_once() {
        let compat = openhub_config::compat::ProviderCompat {
    ...
    fn set_config_thinking_budget_ignored_when_disabled() {
        let mut engine = make_engine("m");
        engine.thinking = Some(openhub_types::llm::ThinkingConfig::Disabled);
        let changes = engine.apply_config_update(None, None, Some(20000), None, None);
        match &engine.thinking {
            Some(openhub_types::llm::ThinkingConfig::Disabled) => {}
...
mod phase6_tests {
    ...
    use openhub_tools::registry::ToolRegistry;
    use openhub_types::llm::{LlmEvent, LlmRequest};
    use openhub_types::skill_types::{ContextModifier, EffortLevel};
    ...
    fn tool_result_msg_with_image(id: &str) -> Message {
        Message::new(
            ...
            vec![ContentBlock::ToolResult {
                ...
                is_error: false,
                images: vec![openhub_types::tool::ToolImage {
...
mod plan_mode_tests {
    ...
    use openhub_tools::registry::ToolRegistry;
    use openhub_types::llm::{LlmEvent, LlmRequest};
    use openhub_types::skill_types::{ContextModifier, PlanModeTransition};
