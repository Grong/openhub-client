...
use openhub_protocol::events::{OutputType, ProtocolEvent, ToolCategory, ToolInfo, ToolStatus};
use openhub_protocol::writer::ProtocolEmitter;
use openhub_protocol::{ToolApprovalManager, ToolApprovalResult};
use openhub_types::message::ContentBlock;
use openhub_types::skill_types::ContextModifier;
use openhub_types::tool::ToolResult;

use openhub_tools::registry::ToolRegistry;

const RECOVERED_PARTIAL_WRITE_KEY: &str = "__openhub_recovered_partial_write";
...
async fn execute_single(
    ...
    tracing::info!(target: "openhub_agent", tool = %name, call_id = %id, "tool execution started");
    ...
    let (result, modifier) = match registry.get(name) {
        Some(tool) => {
            ...
            let input = &openhub_tools::coerce_input_to_schema(&tool.input_schema(), input.clone());
            ...
            // and terminating the subprocess — openhub-cli awaits `engine.run()`
            ...
            let content = openhub_compact::compact_output(&content, compaction_level);
            let content = if toon_enabled {
                openhub_compact::compact_output_toon(&content)
            } else {
                content
            };
            (
    ...
    if let Some(hook_engine) = hooks {
        let messages = hook_engine
            ...
            .await;
        for msg in messages {
            tracing::info!(target: "openhub_agent", hook_message = %msg, "post-tool-use hook output");
        }
    }

    let duration_ms = start.elapsed().as_millis() as u64;
    tracing::info!(target: "openhub_agent", duration_ms, success = !result.is_error, "tool execution completed");
    ...
    let content = openhub_redact::redact_secrets_owned(result.content);
...
mod tests {
    ...
    impl Tool for MockDeferredTool {
        ...
        async fn execute(&self, input: serde_json::Value) -> openhub_types::tool::ToolResult {
            if input.get("tasks").is_none() {
                return openhub_types::tool::ToolResult {
                    content: "Missing or invalid 'tasks' array".to_string(),
                    is_error: true,
                    images: Vec::new(),
                };
            }
            openhub_types::tool::ToolResult {
                content: "ok".to_string(),
                is_error: false,
                images: Vec::new(),
            }
        }
        fn category(&self) -> openhub_protocol::events::ToolCategory {
            openhub_protocol::events::ToolCategory::Exec
        }
    }
    ...
    impl Tool for BrowserLikeApprovalTool {
        ...
        async fn execute(&self, _input: serde_json::Value) -> openhub_types::tool::ToolResult {
            openhub_types::tool::ToolResult {
                content: "ok".to_string(),
                is_error: false,
                images: Vec::new(),
        ...
        fn category_for(&self, input: &serde_json::Value) -> openhub_protocol::events::ToolCategory {
            if input
                .get("irreversible")
                .and_then(serde_json::Value::as_bool)
                .unwrap_or(false)
            {
                openhub_protocol::events::ToolCategory::Irreversible
            } else {
                openhub_protocol::events::ToolCategory::Exec
            }
        }
        fn auto_approve_invocation(
            ...
            category: openhub_protocol::events::ToolCategory,
        ) -> bool {
            category != openhub_protocol::events::ToolCategory::Irreversible
        }
    }
    ...
    impl openhub_protocol::writer::ProtocolEmitter for CapturingEmitter {
        fn emit(&self, event: &openhub_protocol::events::ProtocolEvent) -> std::io::Result<()> {
    ...
    async fn tool_level_auto_approval_skips_prompt_for_safe_invocation() {
        ...
        let approval_manager = std::sync::Arc::new(openhub_protocol::ToolApprovalManager::new());
        let writer_capture = std::sync::Arc::new(CapturingEmitter::default());
        let writer: std::sync::Arc<dyn openhub_protocol::writer::ProtocolEmitter> = writer_capture.clone();
        ...
        let outcome = execute_tool_calls_with_approval(
            ...
            &[],
            None,
            openhub_compact::CompactionLevel::Off,
            false,
        )
        .await
        .unwrap();
        ...
        tokio::spawn(async move {
            loop {
                if writer_for_task.has_tool_request() {
                    am.resolve("danger-browser-call", openhub_protocol::ToolApprovalResult::Approved);
                    break;
                }
    ...
    impl Tool for MockSecretTool {
        ...
        fn category(&self) -> openhub_protocol::events::ToolCategory {
            openhub_protocol::events::ToolCategory::Info
        }
    }
    ...
    async fn recovered_partial_write_result_tells_model_to_continue() {
        ...
        registry.register(Box::new(
            openhub_tools::write::WriteTool::new(None).with_cwd(Some(dir.path().to_path_buf())),
        ));
        let call = ContentBlock::ToolUse {
    ...
    async fn execute_single_recovers_from_a_panicking_tool() {
        ...
        // loop. openhub-cli awaits engine.run() directly on the #[tokio::main]
