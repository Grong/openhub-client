...
impl openhub_browser::extract::ExtractModel for SessionExtractModel {
    async fn complete(&self, prompt: &str) -> Result<String, String> {
        use openhub_types::llm::{LlmEvent, LlmRequest};
        use openhub_types::message::{ContentBlock, Message, Role};
...
#[cfg(feature = "browser-use")]
#[async_trait::async_trait]
impl openhub_browser::visual_fallback::VisualLocator for SessionVisualLocator {
    async fn locate(
        &self,
        screenshot: &[u8],
        instruction: &str,
    ) -> Result<openhub_browser::visual_fallback::VisualLocateResult, String> {
    ...
    async fn locate_labeled(
        ...
        instruction: &str,
        n_labels: usize,
    ) -> Result<openhub_browser::visual_fallback::SomLabelResult, String> {
...
/// Parse the vision model's reply into a [`VisualLocateResult`](openhub_browser::visual_fallback::VisualLocateResult).
///
...
fn parse_visual_locate_result(
    ...
    use openhub_browser::visual_fallback::{PixelBox, VisualLocateResult};
...
/// Parse the vision model's SoM reply into a [`SomLabelResult`](openhub_browser::visual_fallback::SomLabelResult).
///
...
fn parse_som_locate_result(
    ...
    use openhub_browser::visual_fallback::SomLabelResult;
...
pub struct AgentBootstrap {
    ...
    approval_manager: Option<Arc<openhub_protocol::ToolApprovalManager>>,
    ...
    /// the raw pieces (NOT the `openhub_browser` type) so the field exists regardless of the
    ...
    /// (default) → fail-closed (current behavior). Feature-gated: the trait is in `openhub_browser`.
    #[cfg(feature = "browser-use")]
    approval_gate: Option<Arc<dyn openhub_browser::BrowserApprovalGate>>,
}
...
impl AgentBootstrap {
    ...
    pub fn approval_manager(mut self, mgr: Arc<openhub_protocol::ToolApprovalManager>) -> Self {
    ...
    /// callers (`openhub-ai-agent`) need not depend on `openhub-browser` to wire it. Omit it
    ...
    #[cfg(feature = "browser-use")]
    pub fn approval_gate(mut self, gate: Arc<dyn openhub_browser::BrowserApprovalGate>) -> Self {
    ...
    pub async fn build(mut self) -> anyhow::Result<BootstrapResult> {
        ...
        tracing::info!(target: "openhub_agent", workspace = %cwd, "agent bootstrap: workspace cwd resolved");

        let provider = self
            .provider
            .unwrap_or_else(|| openhub_providers::create_provider(&self.config));

        let memory_dir = openhub_memory::paths::auto_memory_dir(cwd_path);

        let file_cache = if self.config.file_cache.enabled {
            Some(Arc::new(std::sync::RwLock::new(
                openhub_tools::file_cache::FileStateCache::new(&self.config.file_cache),
            )))
        } else {
            None
        };

        let mut registry = openhub_tools::registry::ToolRegistry::new();
        ...
        registry.register(Box::new(openhub_tools::read::ReadTool::new(
            file_cache.clone(),
            Some(cwd_path.to_path_buf()),
        )));
        registry.register(Box::new(
            openhub_tools::write::WriteTool::new(file_cache.clone())
                .with_write_root(write_root.clone())
                .with_cwd(Some(cwd_path.to_path_buf())),
        ));
        registry.register(Box::new(
            openhub_tools::edit::EditTool::new(file_cache.clone())
                .with_write_root(write_root.clone())
                .with_cwd(Some(cwd_path.to_path_buf())),
        ));
        registry.register(Box::new(
            openhub_tools::apply_patch::ApplyPatchTool::new(file_cache)
        ...
        // one language server is configured (default off → no behaviour change).
        {
            ...
            if !lsp_map.is_empty() {
                registry.register(Box::new(openhub_tools::lsp::LspTool::new(
                    lsp_map,
        ...
        if bash_sandbox_on {
            ...
            #[cfg(unix)]
            if self.config.tools.persistent_shell {
                let shell = std::sync::Arc::new(openhub_tools::persistent_shell::PersistentShell::new(
                    cwd_path.to_string_lossy().into_owned(),
                ));
                registry.register(Box::new(openhub_tools::bash::BashTool::with_persistent_shell(
                    cwd_path.to_path_buf(),
                    shell,
                )));
            } else {
                registry.register(Box::new(openhub_tools::bash::BashTool::new(cwd_path.to_path_buf())));
            }
            #[cfg(not(unix))]
            registry.register(Box::new(openhub_tools::bash::BashTool::new(
                cwd_path.to_path_buf(),
            )));
        }
        registry.register(Box::new(openhub_tools::grep::GrepTool::new(
            cwd_path.to_path_buf(),
        )));
        registry.register(Box::new(openhub_tools::glob::GlobTool::new(
            cwd_path.to_path_buf(),
        )));
        ...
        let skills = openhub_skills::loader::load_all_skills(
            cwd_path,
        ...
        let skill_checker = openhub_skills::permissions::SkillPermissionChecker::new(
        ...
        // 改走可视化的 openhub_spawn 编排扇出（DAG 画布/转录）；CLI/独立模式保持
        // 进程内 Spawn（其唯一扇出手段）。
        ...
        if self.config.tools.computer.enabled {
            ...
            registry.register(Box::new(openhub_computer::ComputerTool::new(
        ...
        if self.config.tools.browser.enabled {
            ...
            let mut browser_tool = openhub_browser::BrowserTool::with_policy(
                ...
                self.browser_secret_source
                    .clone()
                    .map(|(vault_path, key)| openhub_browser::BrowserSecretSource { vault_path, key }),
            );
            ...
            if self.config.tools.browser.site_memory {
                let sm_root = openhub_config::config::app_config_dir()
                    .map(|d| d.join("browser-data").join("site-memory"))
                    .unwrap_or_else(|| std::env::temp_dir().join("openhub-browser-data").join("site-memory"));
                let sink = openhub_browser::site_memory::FileSiteMemorySink::new(sm_root);
                let store = std::sync::Arc::new(openhub_browser::site_memory::SiteMemoryStore::new(
                    Box::new(sink),
                ));
        ...
        let process_store = Arc::new(openhub_tools::process_store::ProcessStore::new());
        registry.register(Box::new(openhub_tools::exec_command::ExecCommandTool::new(
            Arc::clone(&process_store),
            cwd_path.to_path_buf(),
        )));
        registry.register(Box::new(openhub_tools::write_stdin::WriteStdinTool::new(
            Arc::clone(&process_store),
        )));
        ...
        registry.register(Box::new(openhub_tools::update_plan::UpdatePlanTool::new()));
        ...
        registry.register(Box::new(openhub_tools::tool_search::ToolSearchTool::new(
            tool_defs_snapshot,
        )));
