...
//! through a `CompanionMemorySink` trait object. The backend (openhub-companion)
...
/// by `openhub-companion` over its `CompanionStore`; `openhub-agent` only depends on this.
#[async_trait]
...
/// `openhub-companion` over its store + `skill_service`; `openhub-agent` only depends
