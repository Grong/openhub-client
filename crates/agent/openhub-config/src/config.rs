...
use openhub_types::llm::ThinkingConfig;
...
pub struct ToolsConfig {
    ...
    /// 由工厂置 false —— 子 agent 改走可视化的 `openhub_spawn` 编排扇出（每个
...
pub fn global_config_path() -> PathBuf {
    app_config_dir()
        .unwrap_or_else(|| PathBuf::from("openhub"))
        .join("config.toml")
}
...
fn load_config_file(path: &Path) -> ConfigFile {
    match std::fs::read_to_string(path) {
        Ok(content) => toml::from_str(&content).unwrap_or_else(|e| {
            tracing::warn!(target: "openhub_config", path = %path.display(), error = %e, "failed to parse config file");
            ConfigFile::default()
        }),
...
pub fn init_config() -> anyhow::Result<()> {
    ...
    if path.exists() {
        tracing::info!(target: "openhub_config", path = %path.display(), "config file already exists");
        return Ok(());
    }
    ...
    tracing::info!(target: "openhub_config", path = %path.display(), "config file created");
    Ok(())
}
...
[session]
enabled = true
directory = ".openhub/sessions"  # relative to project root
...
mod tests {
    ...
    fn browser_config_default_source_is_managed() {
        ...
        // 引擎侧 `ChromeSource::from_source_str("managed")` == Managed（见 openhub-browser-engine::acquire）。
