use openhub_config::logging::{ResolvedLogging, create_file_layer};
use tracing::info;
...
fn create_file_layer_writes_json_to_file() {
    ...
    info!(target: "openhub_test", key = "value", "test message");
    ...
    assert!(content.contains("openhub_test"), "log should contain target");
}
