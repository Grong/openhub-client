...
use openhub_agent::output::protocol_sink::ProtocolSink;
use openhub_agent::output::terminal::TerminalSink;
use openhub_agent::session;
use openhub_config::auth;
use openhub_config::config::{self, CliArgs, Config, McpServerConfig, TransportType};
use openhub_agent_mcp::manager::McpManager;
use openhub_agent_mcp::tool_proxy::register_single_server_tools;
use openhub_protocol::commands::{ApprovalScope, ProtocolCommand};
use openhub_protocol::events::ProtocolEvent;
use openhub_protocol::reader::spawn_stdin_reader;
use openhub_protocol::writer::{ProtocolEmitter, ProtocolWriter};
...
struct Cli {
    ...
    /// Log level filter (e.g. "info", "debug", "info,openhub_providers=debug")
    #[arg(long)]
...
async fn main() -> anyhow::Result<()> {
    ...
    let _log_guard = {
        ...
        if resolved.enabled {
            match openhub_config::logging::create_file_layer(&resolved) {
                Ok((layer, guard)) => {
...
async fn repl_loop(
    engine: &mut openhub_agent::engine::AgentEngine,
    ...
    loop {
        ...
        match engine.run(input, "").await {
            ...
            Err(openhub_agent::engine::AgentError::UserAborted) => break,
            Err(e) => {
    ...
    while let Some(cmd) = cmd_rx.recv().await {
        match cmd {
            ProtocolCommand::AddMcpServer {
                ...
                tracing::info!(target: "openhub_mcp", %name, %transport, ?command, "AddMcpServer received");
                let config =
                ...
                tracing::info!(target: "openhub_mcp", %name, "connecting to mcp server");
                match McpManager::connect_all(&single_configs).await {
                    Ok(mgr) => {
                        let tool_names: Vec<String> = mgr
                            ...
                            .collect();
                        tracing::info!(target: "openhub_mcp", %name, tools = tool_names.len(), "mcp server connected");
                    ...
                    Err(e) => {
                        tracing::warn!(target: "openhub_mcp", %name, error = %e, "mcp server connection failed");
    ...
    loop {
        ...
        match cmd {
                            ...
                            Some(sub_cmd) = cmd_rx.recv() => {
                                match sub_cmd {
                                    ...
                                    ProtocolCommand::SetConfig { model, thinking, thinking_budget, effort, compaction } => {
                                        ...
                                        let _ = writer.emit(&openhub_protocol::events::ProtocolEvent::Info {
                                            msg_id: String::new(),
                                    ...
                                    ProtocolCommand::Ping => {
                                        let _ = writer.emit(&openhub_protocol::events::ProtocolEvent::Pong);
                                    }
                                    _ => {
                                        tracing::debug!(target: "openhub_protocol", "ignoring command during active message processing");
                                    }
                                }
                            }
                        }
            ...
            ProtocolCommand::InitHistory { text } => {
                tracing::debug!(target: "openhub_protocol", chars = text.len(), "InitHistory received");
            }
            ProtocolCommand::SetMode { mode } => {
                ...
                tracing::debug!(target: "openhub_protocol", mode = %mode_str, "SetMode applied");
            }
