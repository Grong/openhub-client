...
/// Manages runtime discovery of `.openhub/skills/` directories found in
...
impl RuntimeDiscovery {
    ...
    /// Discover `.openhub/skills/` directories by walking up from each file path to `cwd`.
    ///
                ...
                if !self.checked_dirs.contains(&skill_dir) {
                    ...
                    if tokio::fs::metadata(&skill_dir).await.is_ok() {
                        ...
                        if is_path_gitignored(containing_dir, resolved_cwd).await {
                            tracing::debug!(target: "openhub_skills", path = %skill_dir.display(), "skipping gitignored skills directory");
                        } else {
    ...
    pub async fn add_skill_directories(&mut self, dirs: &[PathBuf]) -> usize {
        ...
        if added > 0 {
            tracing::info!(target: "openhub_skills", added, directories = dirs.len(), "dynamically discovered new skills");
        }
    ...
    /// already known not to contain a `.openhub/skills/` subdirectory.
