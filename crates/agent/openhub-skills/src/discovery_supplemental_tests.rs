...
mod discovery_supplemental_tests {
    ...
    // TC-22: discovers `.openhub/skills/` inside a direct subdirectory of cwd.
    #[tokio::test]
    async fn tc22_discover_dirs_finds_openhub_skills_in_subdir() {
        let tmp = TempDir::new().unwrap();
        let cwd = tmp.path().to_str().unwrap().to_string();

        // Create /tmp/proj/module/.openhub/skills/
        ...
        assert!(found[0].ends_with(".openhub/skills"));
    }
    ...
    // TC-23: `.openhub/skills/` at cwd level is not returned (loaded at startup).
    #[tokio::test]
    ...
    // TC-25: directories without `.openhub/skills/` are still recorded to avoid
    // repeated stat calls.
    #[tokio::test]
    async fn tc25_discover_dirs_records_miss_dirs_in_checked() {
        ...
        // `b` does NOT have .openhub/skills/
