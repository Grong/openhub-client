...
//!
//! Run with:  cargo run -p openhub-a11y --example winsmoke
//! Optional:  cargo run -p openhub-a11y --example winsmoke -- <pid>
//!
...
fn main() {
    ...
    use openhub_a11y::{ElementAction, ObserveOpts, Snapshot, Target};
    ...
    let tmp = std::env::temp_dir().join("openhub_winsmoke.txt");
    let _ = std::fs::write(&tmp, format!("OpenHub TextPattern 验证 Hello {MARKER}\n第二行 line two\n"));
    ...
    let engine = match openhub_a11y::create_engine() {
        Ok(e) => e,
        Err(e) => {
    ...
    if let Some(e) = doc {
        let text = "OpenHub SetValue 测试".to_string();
    ...
    let stale = engine.invoke(
        &Target::Ref(1),
        openhub_a11y::SnapshotGen(snap.generation.0.wrapping_sub(1)),
        ElementAction::Focus,
    );
