...
```bash
docker build -t openhub-a11y-linux:dev \
  -f crates/agent/openhub-a11y/dev/Dockerfile.linux-a11y .

crates/agent/openhub-a11y/dev/run-linux-a11y.sh test
crates/agent/openhub-a11y/dev/run-linux-a11y.sh smoke
```
...
```bash
CARGO_TARGET_DIR="$HOME/openhub-a11y-target" \
CARGO_BUILD_BUILD_DIR="$HOME/openhub-a11y-build" \
  cargo build -p openhub-a11y --example linux_smoke
```
