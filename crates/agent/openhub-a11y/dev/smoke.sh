...
# accessible GTK app, then run the openhub-a11y `linux_smoke` example against it.
set -uo pipefail
...
dbus-run-session -- bash -u -c '
  ...
  echo "=== running openhub-a11y linux_smoke example ==="
  CARGO_TARGET_DIR=/target cargo run -p openhub-a11y --example linux_smoke
  rc=$?
