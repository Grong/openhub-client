...
//! path. The engine stays LLM-free (no vision-model call in openhub-browser-engine). The
//! orchestration lives here in the facade (`openhub-browser`).
//!
...
///
/// - The real adapter is `SessionVisualLocator` in openhub-agent's bootstrap: it reuses the
///   session `LlmProvider` (NOT openhub-computer — openhub-browser stays free of that dep) and sends
