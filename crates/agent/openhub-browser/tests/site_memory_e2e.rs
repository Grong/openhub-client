//! **P7A: Site-memory real-Chrome smoke test** (`#[ignore]`, requires OPENHUB_CHROME_BINARY).
//!
...
//!
//! Run:
//!   export OPENHUB_CHROME_BINARY="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
//!   cargo nextest run -p openhub-browser --run-ignored all -E 'test(site_memory_real_chrome)'

use std::sync::Arc;

use openhub_browser::site_memory::{InMemorySink, SiteMemoryStore};
use openhub_browser::BrowserTool;
use openhub_tools::Tool;
use serde_json::json;

fn isolated_data_dir() -> std::path::PathBuf {
    std::env::temp_dir().join("openhub-p7a-site-memory-smoke")
}
...
#[tokio::test]
#[ignore = "requires OPENHUB_CHROME_BINARY + network access to example.com"]
