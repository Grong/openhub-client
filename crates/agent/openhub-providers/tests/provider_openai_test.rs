...
use openhub_providers::openai::OpenAIProvider;
use openhub_types::llm::{LlmEvent, LlmRequest};
use openhub_types::message::{ContentBlock, Message, Role, StopReason};
use serde_json::json;
...
async fn test_openai_api_error_non_success_status() {
    ...
    match result.unwrap_err() {
        openhub_providers::ProviderError::Api { status, .. } => {
...
async fn test_openai_rate_limited() {
    ...
    match err {
        openhub_providers::ProviderError::RateLimited { retry_after_ms, .. } => {
