...
use openhub_types::llm::{LlmEvent, LlmRequest, ThinkingConfig};
use openhub_types::message::{StopReason, TokenUsage};

use super::anthropic_shared;
use crate::{LlmProvider, ProviderError};
use openhub_config::compat::{self, ProviderCompat};
        ...
        while let Some((event_data, consumed)) = parse_aws_event(&buffer) {
            ...
            if let Some(payload) = event_data {
                ...
                if let Ok(wrapper) = serde_json::from_slice::<Value>(&payload) {
                    ...
                    {
                        tracing::debug!(target: "openhub_providers", chunk = %inner, "bedrock event chunk");
...
/// Build AwsCredentials from openhub-config's BedrockConfig
pub fn credentials_from_config(bc: &openhub_config::config::BedrockConfig) -> AwsCredentials {
