...
use openhub_types::tool::{ToolDef, truncate_deferred_description};
...
impl LlmProvider for OpenAIProvider {
    async fn stream(
        ...
        tracing::debug!(target: "openhub_providers", body = %serde_json::to_string_pretty(&body).unwrap_or_default(), "outgoing request");
    ...
    while let Some(chunk) = stream.next().await {
        ...
        while let Some(line_end) = buffer.find('\n') {
            ...
            if let Some(data) = line.strip_prefix("data: ") {
                tracing::debug!(target: "openhub_providers", chunk = %data, "sse chunk received");
                if data == "[DONE]" {
...
mod tests {
    ...
    use openhub_types::llm::LlmEvent;
    ...
    fn recovers_hermes_json_tool_call_from_text() {
        let text = "<tool_call>{\"name\": \"openhub_run_status\", \"arguments\": {\"run_id\": \"run_x\"}}</tool_call>";
        let calls = parse_text_tool_calls(text);
        assert_eq!(calls.len(), 1);
        assert_eq!(calls[0].0, "openhub_run_status");
    ...
    fn tool_images_ride_in_follow_up_user_message() {
        ...
        let compat = openhub_config::compat::ProviderCompat::openai_defaults();
