...
/// web、用户尚未放行（实测系统 Edge/Safari/Chrome 下载即此值），agent 用 `OpenHub`，event-uuid 留空。
///
...
pub fn write_motw(path: &Path) -> std::io::Result<()> {
    ...
    let value = format!("0081;{secs:x};OpenHub;");
...
mod tests {
    ...
    fn write_motw_creates_zone_identifier_ads_on_ntfs() {
        ...
        let dir = std::env::temp_dir().join("openhub-motw-test");
    ...
    fn write_motw_sets_quarantine_xattr_on_macos() {
        ...
        let dir = std::env::temp_dir().join("openhub-motw-test-macos");
        std::fs::create_dir_all(&dir).unwrap();
        let f = dir.join(format!("dl-{}.pdf", std::process::id()));
        std::fs::write(&f, b"%PDF-1.4 fake").unwrap();
        ...
        // 读回 com.apple.quarantine xattr，断言形态 `0081;<hex>;OpenHub;`。
        ...
        assert!(value.contains("OpenHub"), "quarantine agent should be OpenHub: {value:?}");
