...
// ═══════════════════════════════════════════════════════════════════════════════
// Real-Chrome test (requires OPENHUB_CHROME_BINARY)
// ═══════════════════════════════════════════════════════════════════════════════
#[cfg(test)]
mod tests {
    ...
    ///
    /// Run: `OPENHUB_CHROME_BINARY="..." cargo nextest run -p openhub-browser-engine --run-ignored all -E 'test(generate_selector_returns_stable_selector)'`
    #[tokio::test]
    #[ignore = "需 OPENHUB_CHROME_BINARY（真 Chrome）：selectorGenerator 冒烟"]
    async fn generate_selector_returns_stable_selector() {
        ...
        let chrome = crate::acquire::resolve_chrome_path(
            &std::env::temp_dir().join("openhub-selector-test-data"),
            None,
        )
        .await
        ...
        let cfg = LaunchConfig {
            chrome_path: chrome,
            user_data_dir: std::env::temp_dir().join("openhub-selector-test-profile"),
            headful: false,
        };
