...
pub fn redact_debug_text(s: &str) -> String {
    // First pass: openhub_redact known patterns + high-entropy (via redact_yaml which does both).
...
/// More aggressive than openhub_redact's SECRET_ASSIGNMENT_REGEX (which only matches 4 keywords).
