...
/// utility world 名的前缀（随机 hex 拼成 `__openhub_<hex>__`，每会话唯一）。随机化用于
/// ①防与页面自有全局/world 名冲突；②轻度反检测（DESIGN §6 binding 命名同理）。
const WORLD_NAME_PREFIX: &str = "__openhub_";
...
/// 生成 `__openhub_<hex>__` 形态的随机名（utility world / 未来 binding 名共用此构造）。
...
pub struct InjectionManager {
    ...
    session_id: String,
    /// 本会话随机化的 utility world 名（`__openhub_<hex>__`）。
    world_name: String,
...
impl InjectionManager {
    ...
    async fn create_isolated_worlds_for_existing_frames(&self) -> Result<(), InjectError> {
        ...
        for fid in frame_ids {
            ...
            {
                tracing::warn!(
                    target: "openhub_browser_engine::injected",
...
mod tests {
    ...
    fn world_name_is_random_and_prefixed() {
        ...
        assert!(a.starts_with("__openhub_") && a.ends_with("__"), "shape: {a}");
        assert_ne!(a, b, "two names should differ (random)");
        // `__openhub_` + 16 hex + `__` = 25 chars（CSPRNG 路径）。
    ...
    //   cargo nextest run -p openhub-browser-engine -- --ignored inject_aria_snapshot_smoke
    ...
    async fn inject_aria_snapshot_smoke() {
        ...
        let cfg = LaunchConfig {
            chrome_path: chrome,
            user_data_dir: std::env::temp_dir().join("openhub-inject-smoke-profile"),
            headful: false,
        };
