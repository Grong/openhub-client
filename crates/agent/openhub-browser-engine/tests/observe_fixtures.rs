...
//!   cargo nextest run -p openhub-browser-engine --run-ignored all -E 'test(observe_inject_contract)'
...
use openhub_browser_engine::injected::InjectionManager;
use openhub_browser_engine::launch::{launch_chrome, LaunchConfig};
use openhub_browser_engine::transport::Connection;
...
async fn observe_inject_contract_iframe() {
    // 1) resolve chrome（env OPENHUB_CHROME_BINARY > 打包 > 数据目录 > 下载兜底）+ launch headless。
    ...
    let cfg = LaunchConfig {
        chrome_path: chrome,
        user_data_dir: std::env::temp_dir().join("openhub-observe-contract-profile"),
        headful: false,
    };
    ...
    let cr = conn
        .send::<CreateTargetParams>(openhub_browser_engine::transport::ROOT_SESSION, &create)
        .await
        .expect("createTarget");
...
use openhub_browser_engine::{BrowserEngine, ObserveOpts};
