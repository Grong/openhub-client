...
//!   cargo nextest run -p openhub-browser-engine --run-ignored all -E 'test(w4d)'
...
use openhub_browser_engine::storage_state::{SameSite, StorageState, StorageStateCookie};
use openhub_browser_engine::{
    load_storage_state, save_storage_state, shared_storage_state_path, storage_state_path,
};
use openhub_browser_engine::BrowserEngine;
...
fn login_cookie(name: &str, value: &str) -> StorageStateCookie {
    StorageStateCookie {
        ...
        priority: openhub_browser_engine::storage_state::Priority::Medium,
        source_scheme: openhub_browser_engine::storage_state::SourceScheme::NonSecure,
        source_port: -1,
        partition_key: None,
    }
}
...
async fn seed_local_storage(backend: &openhub_browser_engine::backend::CdpBackend, pairs: &[(&str, &str)]) {
...
async fn read_local_storage(backend: &openhub_browser_engine::backend::CdpBackend, key: &str) -> Option<String> {
...
async fn w4d_persistent_login_cross_engine_via_encrypted_vault() {
    ...
    // ── 会话 A：登录 → capture → save 加密 vault ──────────────────────────────────
    {
        ...
        let seed_state = StorageState {
            cookies: vec![login_cookie("openhub_session", "persisted-login-tok-abc")],
            ..Default::default()
        };
        ...
        assert!(find_cookie(&state, "openhub_session").is_some(), "A must have the login cookie before save");
        eprintln!(
    ...
    assert!(find_cookie(&loaded, "openhub_session").is_some(), "vault must carry the login cookie");
    ...
    let c = find_cookie(&after, "openhub_session")
...
async fn w4d_none_storage_state_injects_nothing() {
    ...
    assert!(
        find_cookie(&captured, "openhub_session").is_none(),
...
///
///   set OPENHUB_CHROME_BINARY=...\chrome.exe
///   cargo nextest run -p openhub-browser-engine --run-ignored all -E 'test(shared_identity)'
#[tokio::test]
#[ignore = "需本机/打包 chrome：set OPENHUB_CHROME_BINARY 后 --run-ignored all -E 'test(shared_identity)'"]
async fn shared_identity_login_crosses_companions_via_shared_vault() {
    ...
    // ── 伙伴 A：登录 → capture → save 到共享 vault ──────────────────────────────────
    {
        ...
        let seed_state = StorageState {
            cookies: vec![login_cookie("openhub_session", "shared-login-tok-xyz")],
            ..Default::default()
        };
        backend_a.restore_cookies(&seed_state).await.expect("seed cookie into A");
        backend_a.navigate(ORIGIN, false).await.expect("A navigate example.com");
    ...
    assert!(
        find_cookie(&loaded, "openhub_session").is_some(),
