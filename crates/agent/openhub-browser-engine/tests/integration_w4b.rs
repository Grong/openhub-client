...
//! [`openhub_browser_engine::backend::CdpBackend::capture_cookies`] 文档——本 chromiumoxide_cdp 版本无
...
//!   cargo nextest run -p openhub-browser-engine --run-ignored all -E 'test(w4b)'
...
async fn w4b_cookie_round_trip_and_login_state_loadable() {
    ...
    let state = StorageState {
        cookies: vec![login_cookie("openhub_session", "login-token-abc123")],
        ..Default::default()
    };
    backend
    ...
    let c = find(&captured, "openhub_session")
    ...
    assert!(
        find(&after_nav, "openhub_session").is_some(),
